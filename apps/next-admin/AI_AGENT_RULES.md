...
Do not create internal `/v2` routes, `FooV2` DTOs, versioned GraphQL fields,
components, or old/new package exports. Update the canonical contract and all
callers in one cutover. Internal versions only postpone the same migration while
doubling tests, security fixes, docs, and plausible sources of truth; future
agents then copy whichever version they encounter first. A genuinely published
external protocol may carry a boundary version, but it must map to one
unversioned internal model.
...
or fake transport success. If target functionality is required, connect the
real package, transport, route, and UI flow and test observable behavior. If it
is not required, delete it. If the task lacks authority to complete it, report
the gap instead of presenting a stub as implementation. Audit and remove any
existing suppression in files touched by the task.
