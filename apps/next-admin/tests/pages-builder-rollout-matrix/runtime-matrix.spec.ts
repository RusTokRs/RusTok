...
function withProfile(
  ...
  builder.preview = { ...nested(builder.preview), enabled: flags.preview_enabled };
  builder.properties = { ...nested(builder.properties), enabled: flags.properties_enabled };
  builder.publish = { ...nested(builder.publish), enabled: flags.publish_enabled };
...
async function writePagesSettings(
  ...
  if (canonicalJson(returned) !== canonicalJson(settings)) {
    fail("updateModuleSettings returned settings different from the requested semantic object");
  }
  return result;
}
...
async function settleAdminPage(page: Page, adminRoute: string): Promise<void> {
  ...
  if (response === null || response.status() >= 400) {
    fail("Pages admin rollout fixture route did not return a successful response");
  }
  await page.waitForLoadState("networkidle", { timeout: 15_000 }).catch(() => undefined);
  await page.locator(providerSelector).first().waitFor({ state: "visible", timeout: 15_000 });
}

async function assertUiProfile(page: Page, profile: MatrixProfile): Promise<{
  provider_state: string;
  ...
  if (providerState !== profile.provider_state || providerHealth !== "unobserved") {
  ...
  const capabilityState = async (
    ...
    if ((await fieldset.count()) === 0) {
      if (expectedEnabled) fail(`${label} fieldset is unexpectedly hidden for ${profile.id}`);
      return "hidden";
    }
    const disabled = (await fieldset.getAttribute("disabled")) !== null;
    if (disabled === expectedEnabled) {
      fail(`${label} fieldset capability state does not match profile ${profile.id}`);
    }
...
async function allowedPreview(
...
): Promise<{ observation: PreviewObservation; template: PreviewTemplate | null }> {
  ...
  const requestPromise = page.waitForRequest(
    (request) =>
      request.method() === "POST" && new URL(request.url()).pathname === capabilityPath,
    { timeout: 15_000 },
  );
  await button.click();
...
async function deniedBrowserIntent(
  ...
  if (
    ...
    fail(`${intent} rollout preflight did not return the expected typed capability denial`);
  }
  return {
    status: response.status(),
    body_bytes: body.length,
...
test("Pages persisted rollout profiles agree across owner, UI, SSR, intents and reads", async ({
  ...
  const pageId = requireUuid(requiredEnvironment(contract.fixtures.page_id_environment));
  ...
  if (!restoreVerified || originalSettings === null) {
    fail("matrix completed without verified restoration of original Pages settings");
  }
