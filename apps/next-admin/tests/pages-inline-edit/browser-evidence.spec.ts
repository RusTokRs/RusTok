...
function requireOrigin(value: string, label: string): string {
  ...
  if (
    ...
    fail(`${label} must be an HTTP(S) origin without credentials, path, query, or fragment`);
  }
...
function requireRelativePath(value: string, label: string): string {
  ...
  if (parsed.origin !== "https://evidence.invalid" || parsed.username || parsed.password) {
...
function requireComponentId(value: string, label: string): string {
  if (!/^[A-Za-z0-9_-]{1,128}$/u.test(value)) {
    fail(`${label} must use the bounded browser evidence component-id alphabet`);
  }
  return value;
}
...
function regularFileRecord(value: string, label: string): FileRecord & { path: string } {
...
function validateArtifactHttpInput(
  ...
  if (http?.origin !== origin || http?.deployment_image_digest !== deploymentDigest) {
    fail("artifact/HTTP origin or deployment digest does not match browser target");
  }
  if (
    !Array.isArray(docker?.repo_digests) ||
    !docker.repo_digests.includes(deploymentDigest)
  ) {
    fail("browser deployment digest is absent from artifact/HTTP Docker evidence");
  }
...
async function assertLaunchHidden(context: BrowserContext, route: string): Promise<boolean> {
...
function scanForbiddenDomMarkers(source: string, label: string): void {
  ...
  for (const marker of forbiddenDomMarkers) {
    if (lower.includes(marker.toLowerCase())) {
      fail(`${label} contains forbidden session, grant, proof, token, or signing marker`);
    }
  }
}
...
async function waitForAuthoringRoot(page: Page): Promise<RootFacts> {
  ...
  if (id !== `fly-inline-${domId(pageId)}-${projectHash}`) {
    fail("hydrated Page Builder root id is not bound to page identity and project hash");
  }
...
async function assertEligibility(
  ...
  for (const componentId of blockedIds) {
    ...
    if (
      ...
      fail("a provider, composite, templated, interactive, or runtime-owned component became editable");
    }
  }
}
...
async function captureRejectedCommit(
  ...
  if (response.status() < 400) fail("expected inline commit rejection returned a success status");
...
async function textForComponent(page: Page, componentId: string): Promise<string> {
  const value = await componentLocator(page, componentId).innerText();
  return value.replace(/\r\n/gu, "\n").replace(/\r/gu, "\n");
}

test("retains bounded Pages inline edit browser evidence", async ({ browser }) => {
  if (
  ...
  const paths = {
    ...
    published: requireRelativePath(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_PUBLISHED_ADMIN_PATH"),
      "published admin path",
    ),
    localeLess: requireRelativePath(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_LOCALELESS_ADMIN_PATH"),
      "locale-less admin path",
    ),
    missing: requireRelativePath(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_MISSING_ADMIN_PATH"),
      "missing admin path",
    ),
    standalone: requireRelativePath(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_STANDALONE_ADMIN_PATH"),
      "standalone admin path",
    ),
  };
  const componentIds = {
    editable: requireComponentId(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_EDITABLE_COMPONENT_ID"),
      "editable component id",
    ),
    provider: requireComponentId(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_PROVIDER_COMPONENT_ID"),
      "provider component id",
    ),
    composite: requireComponentId(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_COMPOSITE_COMPONENT_ID"),
      "composite component id",
    ),
    templated: requireComponentId(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_TEMPLATED_COMPONENT_ID"),
      "templated component id",
    ),
    interactive: requireComponentId(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_INTERACTIVE_COMPONENT_ID"),
      "interactive component id",
    ),
    runtime: requireComponentId(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_RUNTIME_COMPONENT_ID"),
      "runtime-owned component id",
    ),
  };
  if (new Set(Object.values(componentIds)).size !== Object.keys(componentIds).length) {
  ...
  const storageInputs = {
    editor: regularFileRecord(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_EDITOR_STORAGE_STATE"),
      "editor storage state",
    ),
    unauthorized: regularFileRecord(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_UNAUTHORIZED_STORAGE_STATE"),
      "unauthorized storage state",
    ),
    standalone: regularFileRecord(
      requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_STANDALONE_STORAGE_STATE"),
      "standalone storage state",
    ),
  };
  const artifactInput = readJsonInput(
    requiredEnvironment("RUSTOK_PAGES_INLINE_EDIT_BROWSER_ARTIFACT_HTTP_EVIDENCE"),
  ...
  try {
    ...
    if (new URL(popup.url()).toString() !== launchTarget.toString()) {
      fail("admin launch did not navigate to the exact same-origin authoring target");
    }
    await popup.close();
    ...
    const ssrResponse = await editorContext.request.get(launchTarget.toString(), {
      failOnStatusCode: false,
    });
    if (ssrResponse.status() !== 200) fail("authenticated authoring SSR request did not return 200");
    ...
    if (
      ...
      fail("authoring SSR HTML does not bind the expected root, page, and locale");
    }
    ...
    await mainPage.goto(launchTarget.toString(), { waitUntil: "domcontentloaded" });
    ...
    for (const [id] of authoringAssetPaths) {
      if (assetStatuses.get(id) !== 200) fail(`authoring ${id} asset did not return 200`);
    }
    if (
    ...
    for (const key of new URL(mainPage.url()).searchParams.keys()) {
      if (forbiddenUrlKeys.has(key.toLowerCase())) {
        fail("hydrated authoring URL contains a forbidden credential or proof key");
      }
    }
    ...
    await stalePage.goto(launchTarget.toString(), { waitUntil: "domcontentloaded" });
    ...
    if (
      ...
      fail("preloaded stale tab did not start from the same document revision and hash");
    }
    ...
    const successful = await captureCommit(mainPage, componentIds.editable, savedText);
    ...
    if (replay.status() < 400) fail("exact successful inline request replay was accepted");
    ...
    if (
      (await textForComponent(stalePage, componentIds.editable)) !== savedText ||
      staleReloadRoot.revision !== currentRoot.revision
    ) {
      fail("stale rejection produced a partial document write or wrong reload revision");
    }
    if (
    ...
    if (
      ...
      fail("successful inline text or replacement revision did not survive reload");
    }

    const expiryPage = await editorContext.newPage();
    const expiryFailures = observeFailures(expiryPage);
    await expiryPage.goto(launchTarget.toString(), { waitUntil: "domcontentloaded" });
    ...
    if (
      (await textForComponent(expiryPage, componentIds.editable)) !== savedText ||
      expiryReloadRoot.revision !== currentRoot.revision
    ) {
      fail("expiry rejection produced a partial document write or wrong reload revision");
    }
    if (
    ...
    const outputDocument: Record<string, unknown> = {
      ...
      identity_sha256: {
        ...
        components: Object.fromEntries(
          Object.entries(componentIds).map(([role, value]) => [role, sha256(value)]),
        ),
        edited_values: {
