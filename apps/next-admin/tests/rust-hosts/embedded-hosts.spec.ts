...
function expectNonceBackedElements(
  ...
  expect(
    tags.length,
    `expected at least ${minimum} <${tagName}> element(s)`
...
async function observeCriticalFailures(page: Page): Promise<string[]> {
  ...
  page.on('requestfailed', (request) => {
    ...
    if (critical) {
      failures.push(
        `${resourceType} ${url}: ${request.failure()?.errorText || 'unknown failure'}`
      );
    }
  });
  return failures;
...
test('server-hosted storefront renders under strict CSP', async ({
  request,
  page
}) => {
...
test('embedded admin assets hydrate from the admin mount under strict CSP', async ({
  request,
  page
}) => {
  ...
  expect(html).not.toMatch(
    /(?:src|href)="\/(?:rustok-admin|snippets|output\.css)/i
  );
