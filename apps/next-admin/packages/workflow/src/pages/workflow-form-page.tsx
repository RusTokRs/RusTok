...
export default function WorkflowFormPage({
  ...
  const defaultTriggerType = workflow
    ? (((workflow.triggerConfig as { type?: string })
        .type as FormValues['triggerType']) ?? 'manual')
    : 'manual';
  ...
  const {
    ...
    defaultValues: {
      ...
      triggerEventType:
        (workflow?.triggerConfig as { event_type?: string })?.event_type ?? '',
      triggerCron:
        (workflow?.triggerConfig as { expression?: string })?.expression ?? ''
    }
  });
  ...
  function buildTriggerConfig(values: FormValues): Record<string, unknown> {
    switch (values.triggerType) {
      ...
      case 'cron':
        return {
          type: 'cron',
          expression: values.triggerCron ?? '0 * * * * *'
        };
      case 'webhook':
        return {
          type: 'webhook',
          path: `/hooks/${values.name.toLowerCase().replace(/\s+/g, '-')}`
        };
      default:
        return { type: 'manual' };
    }
  ...
  async function onSubmit(values: FormValues) {
    try {
      ...
      toast.error(
        isEdit ? 'Failed to update workflow' : 'Failed to create workflow'
      );
    }
  }
