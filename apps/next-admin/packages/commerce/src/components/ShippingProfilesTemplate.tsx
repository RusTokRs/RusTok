...
export function ShippingProfilesTemplate({ opts }: { opts: GqlOpts }) {
  ...
  const [editingProfile, setEditingProfile] =
    React.useState<ShippingProfile | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to save shipping profile.'
      );
    }
  };
          ...
          <Card className='border-primary/20 bg-primary/5'>
            <CardHeader>
              <CardTitle className='text-base'>
                {editingProfile
                  ? `Edit Profile: ${editingProfile.slug}`
                ...
                <div className='border-t pt-4'>
                  ...
                  <div className='grid gap-4 md:grid-cols-2'>
                    <div className='bg-background space-y-3 rounded-lg border p-3'>
                      <h5 className='text-muted-foreground text-xs font-bold'>
                        Russian (RU)
                      </h5>
                      <div className='space-y-2'>
                  ...
                  <TableBody>
                    {profiles.length === 0 ? (
                      ...
                      profiles.map((profile) => {
                        const ruTrans = profile.translations.find(
                          (t) => t.locale === 'ru'
                        );
                        const enTrans = profile.translations.find(
                          (t) => t.locale === 'en'
                        );
                        return (
                          <TableRow key={profile.id}>
                            ...
                            <TableCell>
                              <Badge
                                variant={
                                  profile.active ? 'default' : 'secondary'
                                }
                              >
                            ...
                            <TableCell className='flex items-center justify-end gap-2'>
                              ...
                              <Button
                                size='sm'
                                variant={
                                  profile.active ? 'destructive' : 'default'
                                }
