...
- Лёгкая проверка без компиляции запускается командой `npm run verify:seo-runtime-fixtures` из `apps/next-frontend`; она дополнительно проверяет существование docs rows и ключевые static symbols для Next SEO adapter, Rust renderer, Next Admin transport и Leptos storefront SEO runtime wiring.
