...
const stagingRoot = process.env.TRUNK_STAGING_DIR ? resolve(process.env.TRUNK_STAGING_DIR) : null;
const distRoot = resolve(adminRoot, 'dist');
const targetRoots = new Set([
  resolve(distRoot, 'richtext/frame'),
  ...(stagingRoot ? [resolve(stagingRoot, 'richtext/frame')] : [])
]);
...
for (const targetRoot of targetRoots) {
  ...
  await cp(
    resolve(packageDist, manifest.leptos_adapter),
    resolve(targetRoot, manifest.leptos_adapter)
  );
}
