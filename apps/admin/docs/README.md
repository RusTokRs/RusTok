...
- Recovery для failed module lifecycle post-hook операций остаётся host/control-plane сценарием: Leptos admin читает `moduleOperationRecoveryPlan` / `failedModuleOperationRecoveryPlans` и вызывает `retryFailedModuleOperationPostHook` / `compensateFailedModuleOperation` через canonical GraphQL helpers в `features/modules/api.rs`; локальный SQL, локальный rollback и собственная lifecycle taxonomy запрещены.
