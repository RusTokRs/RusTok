...
const RECOVERY_AUDIT_TABLE: &str = "blog_comments_tcp_delegation_schedule_audit_recovery_audits";
...
async fn stale_and_non_terminal_requeue_fail_closed() -> Result<()> {
    ...
    let outcome = async {
        ...
        .await?;
        ensure!(stale_epoch == CommentsTcpDelegationScheduleAuditRecoveryOutcome::StaleInspection);
        ...
        .await?;
        ensure!(non_terminal == CommentsTcpDelegationScheduleAuditRecoveryOutcome::NotDeadLetter);
...
async fn concurrent_requeue_admits_one_epoch_and_next_worker_claim_starts_at_one() -> Result<()> {
    let Some(context) = PostgresRecoveryEvidenceDb::setup("concurrent_epoch", true).await? else {
        return Ok(());
    };
...
async fn recovery_audit(db: &DatabaseConnection, audit_id: Uuid) -> Result<StoredRecoveryAudit> {
    let row = query_one(
        db,
        format!(
