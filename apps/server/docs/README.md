...
- Auth/password reset: GraphQL `forgot_password` dispatches reset emails via `tokio::spawn` (non-blocking) — SMTP handshake no longer holds the HTTP request. Delivery errors are logged via `tracing::warn` and do not affect the response. Email transport uses `rustok.email` settings; safe no-send fallback when email delivery is disabled.
...
- Ecommerce transport: live REST surface для commerce теперь ограничен `/store/*` и `/admin/*`; compatibility-слой `/api/commerce/*` удалён из runtime router и OpenAPI. Текущий admin commerce surface уже включает `products`, paginated `orders`, `orders/{id}`, explicit order lifecycle endpoints и list/detail/lifecycle endpoints для `payment-collections` и `fulfillments`; GraphQL schema держит parity-read/lifecycle surface над теми же commerce services, включая admin list queries для payment/fulfillment, а storefront GraphQL уже выровнен по discovery (`regions`/`shipping-options`), cart lifecycle, tri-state context patch, customer-owned `me/order` read path и checkout/payment mutations.
...
- Marketplace registry V1: `apps/server` now exposes a reference read-only catalog contract at `GET /v1/catalog` with a backward-compatible legacy alias `GET /catalog`; payload is schema-versioned, normalized for `source="registry"`, carries module `name`/`description` metadata for registry-only UX, now also preserves manifest-driven `category`/`tags` plus optional `icon`/`banner`/`screenshots` metadata, and `ManifestManager` now enforces a basic metadata policy for catalog entries (`description >= 20`, `icon` must be absolute `http(s)` `.svg`, `banner`/`screenshots` must be absolute `http(s)` image URLs). The endpoint is included in generated OpenAPI as part of the host API surface and intentionally publishes only first-party modules so `RegistryMarketplaceProvider` can be exercised end-to-end before a dedicated `modules.rustok.dev` deployment exists.
- Module settings schema: `ManifestManager` now supports declarative scalar `options` in `[settings]`; schema validation rejects mismatched/default-outside-options cases, GraphQL exposes `settingsSchema.options`, and admin UI can render these fields as typed `select` controls instead of free-text inputs.
