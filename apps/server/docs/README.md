...
- channel resolution middleware теперь только собирает request facts и применяет domain-owned resolver из `rustok-channel`; current precedence остаётся `header -> query -> built-in host slice -> default`, при этом `Policy` уже зарезервирован как следующий typed resolution layer, а `default` означает explicit default channel tenant'а;
