...
use rustok_product::{
    ...
    services::{
        ...
        BindCategoryAttributeInput, CatalogCategoryKind, CategoryAttributeBindingKind,
        CategoryTranslationInput, CreateCatalogCategoryInput, CreateCategoryAttributeGroupInput,
        CreateProductAttributeInput, ProductCatalogSchemaService,
    },
};
use rustok_server::{
    auth::AuthConfig,
...
const RESOURCE_KIND: &str = "category_form";
...
async fn product_category_form_registered_translation_provider_multi_replica_evidence_postgres(
    ...
    let database_name = unique_postgres_database_name("rustok_category_form_translation_evidence");
...
async fn run_contract(database_url: &str) -> TestResult<()> {
    ...
    seed_tenant(&seed_connection, tenant_id, "category-form-main").await?;
    seed_tenant(&seed_connection, other_tenant_id, "category-form-isolated").await?;

    let owner = owner_service(seed_connection.clone());
    let actor_id = Uuid::new_v4();
    let category = owner
        .create_category(tenant_id, actor_id, source_category("catalog-details"))
        .await?;
    let first_group = owner
        .create_category_group(
            tenant_id,
            actor_id,
            source_group(category.id, "core", 0, "Core details"),
        )
        .await?;
    let other_category = owner
        .create_category(
            other_tenant_id,
            actor_id,
            source_category("isolated-details"),
        )
        .await?;
    owner
        .create_category_group(
            other_tenant_id,
            actor_id,
            source_group(other_category.id, "isolated", 0, "Isolated details"),
        )
        .await?;
    ...
    assert_eq!(identity.resource_id.as_str(), category.id.to_string());
    ...
    let initial = provider
        ...
        .await?;
    assert_eq!(initial.summary.display_label, "catalog-details");
    assert_eq!(initial.fields.len(), 1);
    let first_group_key = format!("group:{}", first_group.id);
    let group = field(&initial, &first_group_key);
    ...
    assert!(group.exact_target_value.is_none());
    assert!(initial.fields.iter().all(|field| {
        field.descriptor.key.as_str().starts_with("group:")
    ...
    let owner_changes = provider
        ...
        .await?;
    assert_eq!(owner_changes.changes.len(), 1);
    assert_eq!(owner_changes.changes[0].identity, identity);
    assert_eq!(
        owner_changes.changes[0].resource_revision,
    ...
    let first_receipt = first_provider
        .apply_patch(
            apply_context(tenant_id, "first-apply", "category-form-first-apply"),
            first_patch.clone(),
        )
        .await?;
    let replay = second_provider
        .apply_patch(
            apply_context(tenant_id, "first-replay", "category-form-first-apply"),
            first_patch,
        )
        .await?;
    ...
    let completed = provider
        ...
        .await?;
    assert_eq!(completed.required_units, 1);
    assert_eq!(completed.exact_required_units, 1);
    ...
    let first_race = first_provider.apply_patch(
        apply_context(tenant_id, "race-one", "category-form-race-one"),
        full_patch(&replica_one, "race-a"),
    );
    let second_race = second_provider.apply_patch(
        apply_context(tenant_id, "race-two", "category-form-race-two"),
    ...
    let second_group = owner
        .create_category_group(
            tenant_id,
            actor_id,
            source_group(category.id, "advanced", 1, "Advanced details"),
        )
        .await?;
    ...
    let stale_error = provider
        .apply_patch(
            apply_context(tenant_id, "stale", "category-form-stale-after-group"),
            stale_patch,
        )
        .await
    ...
    assert_eq!(after_late_group.fields.len(), 2);
    ...
    let incomplete = provider
        ...
        .await?;
    assert_eq!(incomplete.required_units, 2);
    assert_eq!(incomplete.exact_required_units, 1);
    assert_eq!(incomplete.complete_resources, 0);

    let final_receipt = provider
        .apply_patch(
            apply_context(tenant_id, "final", "category-form-final-apply"),
    ...
    let final_progress = provider
        ...
        .await?;
    assert_eq!(final_progress.required_units, 2);
    assert_eq!(final_progress.exact_required_units, 2);
...
fn registered_provider(db: DatabaseConnection) -> Arc<dyn TranslationTargetProvider> {
    ...
    let extensions = build_shared_runtime_extensions_with_host_providers(
        ...
        &settings,
        runtime,
        AuthConfig::new("test-secret-key-for-category-form-32bytes!".to_string()),
    )
    ...
    targets
        ...
        .expect("product/category_form provider")
}

fn source_category(slug: &str) -> CreateCatalogCategoryInput {
    CreateCatalogCategoryInput {
        ...
        position: 0,
        rule_config: json!({}),
        metadata: json!({"evidence": "category-form-translation"}),
        translations: vec![CategoryTranslationInput {
            locale: "en".into(),
            name: "Catalog details".into(),
            description: Some("Canonical Taxonomy-owned category copy".into()),
            meta_title: None,
...
fn bind_only_attribute() -> CreateProductAttributeInput {
    CreateProductAttributeInput {
        ...
        validation: json!({}),
        default_value: None,
        metadata: json!({"evidence": "category-form-binding"}),
        ...
        fields: snapshot
            ...
            .map(|field| TranslationFieldPatch {
                ...
                value: match field.descriptor.key.as_str() {
                    ...
                    key => panic!("unexpected Product Category Form field: {key}"),
                },
                expected_source_hash: field.source_hash.clone(),
            })
            .collect(),
        proposal_id: format!("category-form-proposal-{suffix}"),
        approval_receipt_id: format!("category-form-approval-{suffix}"),
    }
}
...
fn read_context(tenant_id: Uuid, suffix: &str) -> PortContext {
    PortContext::new(
        ...
        PortActor::system(),
        "en",
        format!("category-form-postgres-{suffix}"),
    )
...
async fn seed_tenant(
    ...
    database
        .execute_raw(Statement::from_sql_and_values(
            ...
            vec![
                tenant_id.into(),
                "Product Category Form translation evidence".into(),
