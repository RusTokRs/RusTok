...
async fn run_contract(database_url: &str) -> TestResult<()> {
    ...
    seed_tenant(&seed_connection, tenant_id, "commerce-main").await?;
    seed_tenant(&seed_connection, other_tenant_id, "commerce-isolated").await?;
    ...
    let collection_id = owner
        .create_collection_owner(
            ...
            "manual".into(),
            None,
            json!({"evidence": "collection-copy"}),
            vec![CollectionTranslationExactLocaleRecord {
                locale: "en".into(),
                title: "Autumn Collection".into(),
                handle: "autumn-collection".into(),
                description: Some("Seasonal collection".into()),
            }],
        )
        .await?;
    ...
    assert_eq!(identity.resource_id.as_str(), collection_id.to_string());
    ...
    assert!(
        !owner
            .update_collection_owner(
                ...
                None,
                None,
                Some(owner_translations(&before_noop)),
            )
            .await?
    );
    ...
    source.title = "Autumn Collection Revised".into();
    source.description = Some("Revised seasonal collection".into());
    assert!(
        owner
            .update_collection_owner(
                tenant_id,
...
fn owner_translations(
    ...
    vec![
        CollectionTranslationExactLocaleRecord {
