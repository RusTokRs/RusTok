...
const RESOURCE_KIND: &str = "product";
...
async fn product_registered_translation_provider_multi_replica_evidence_postgres() -> TestResult<()> {
    ...
    let database_name = unique_postgres_database_name("rustok_product_translation_evidence");
...
async fn run_contract(database_url: &str) -> TestResult<()> {
    ...
    seed_tenant(&seed_connection, tenant_id, "product-main").await?;
    seed_tenant(&seed_connection, other_tenant_id, "product-isolated").await?;
    ...
    assert_eq!(identity.resource_id.as_str(), product_id.to_string());
    ...
    let initial = provider
        ...
        .await?;
    assert_eq!(field(&initial, "title").source_value, "Autumn Product");
    ...
    let first_patch = patch(
        ...
        "produit-automne",
        "Produit saisonnier",
        "Produit d'automne meta",
        "Meta description saisonniere",
        "first",
    );
    let first_receipt = first_provider
        .apply_patch(
            apply_context(tenant_id, "first-apply", "product-first-apply"),
            first_patch.clone(),
        )
        .await?;
    let replay = second_provider
        .apply_patch(
            apply_context(tenant_id, "first-replay", "product-first-apply"),
            first_patch,
        )
        .await?;
    ...
    let first_apply = first_provider.apply_patch(
        apply_context(tenant_id, "race-one", "product-race-one"),
        patch(
            &first_snapshot,
            "Produit A",
            "produit-a",
    ...
    let second_apply = second_provider.apply_patch(
        apply_context(tenant_id, "race-two", "product-race-two"),
        patch(
            &second_snapshot,
            "Produit B",
            "produit-b",
    ...
    let mut owner_update = owner_translations(&before_noop);
    let source = owner_update
        .iter_mut()
    ...
    let after_owner_update = provider
        ...
        .await?;
    assert_eq!(
        field(&after_owner_update, "title").source_value,
        "Autumn Product Revised"
    );
    assert_ne!(
        before_noop.summary.resource_revision,
        after_owner_update.summary.resource_revision
    );
    let owner_update_revision = after_owner_update.summary.resource_revision.clone();

    let late_receipt = provider
        .apply_patch(
            apply_context(tenant_id, "late-apply", "product-late-apply"),
            patch(
                &after_owner_update,
                "Produit final",
                "produit-final",
    ...
    let next_window = provider
        ...
        .await?;
    assert_eq!(next_window.changes.len(), 2);
    assert_eq!(
        next_window.changes[0].resource_revision,
        owner_update_revision
    );
    assert_eq!(
    ...
    assert!(
        deleted.changes[0]
            .resource_revision
            .as_str()
            .starts_with("product-deleted-v1:")
    );
    ...
    let read_error = provider
        ...
        .await
        .expect_err("deleted Product must not remain readable");
...
fn registered_provider(db: DatabaseConnection) -> Arc<dyn TranslationTargetProvider> {
    ...
    let extensions = build_shared_runtime_extensions_with_host_providers(
        ...
        &settings,
        runtime,
        AuthConfig::new("test-secret-key-for-product-translation-32bytes!".to_string()),
    )
    ...
    targets
        ...
        .expect("product/product provider")
}
...
fn initial_product() -> CreateProductInput {
    CreateProductInput {
        translations: vec![ProductTranslationInput {
            locale: "en".into(),
            title: "Autumn Product".into(),
            handle: Some("autumn-product".into()),
            description: Some("Seasonal product".into()),
            meta_title: Some("Autumn Product Meta".into()),
            meta_description: Some("Seasonal product meta description".into()),
        }],
        options: Vec::new(),
        variants: vec![CreateVariantInput {
            sku: Some("PRODUCT-EVIDENCE-SKU".into()),
            barcode: None,
        ...
        tags: Vec::new(),
        metadata: json!({"evidence": "product-translation"}),
        publish: false,
    }
}
        ...
        fields: snapshot
            ...
            .map(|field| TranslationFieldPatch {
                ...
                value: match field.descriptor.key.as_str() {
                    ...
                    other => panic!("unexpected Product field: {other}"),
                },
                expected_source_hash: field.source_hash.clone(),
            })
            .collect(),
        proposal_id: format!("product-proposal-{suffix}"),
        approval_receipt_id: format!("product-approval-{suffix}"),
    }
}

fn owner_translations(snapshot: &TranslationResourceSnapshot) -> Vec<ProductTranslationInput> {
    vec![
        ProductTranslationInput {
            locale: snapshot.source_locale.as_str().into(),
            title: field(snapshot, "title").source_value.clone(),
            handle: Some(field(snapshot, "handle").source_value.clone()),
            description: optional_source(snapshot, "description"),
            meta_title: optional_source(snapshot, "meta_title"),
            meta_description: optional_source(snapshot, "meta_description"),
        },
        ProductTranslationInput {
            locale: snapshot.target_locale.as_str().into(),
            title: exact_target(snapshot, "title"),
            handle: Some(exact_target(snapshot, "handle")),
            description: optional_target(snapshot, "description"),
            meta_title: optional_target(snapshot, "meta_title"),
            meta_description: optional_target(snapshot, "meta_description"),
        },
    ]
}
...
fn exact_target(snapshot: &TranslationResourceSnapshot, key: &str) -> String {
    field(snapshot, key)
        .exact_target_value
        .clone()
        .unwrap_or_else(|| panic!("target {key}"))
}

fn optional_source(snapshot: &TranslationResourceSnapshot, key: &str) -> Option<String> {
    let value = field(snapshot, key).source_value.clone();
    (!value.is_empty()).then_some(value)
}

fn optional_target(snapshot: &TranslationResourceSnapshot, key: &str) -> Option<String> {
    field(snapshot, key)
        .exact_target_value
        .clone()
...
fn read_context(tenant_id: Uuid, suffix: &str) -> PortContext {
    PortContext::new(
        ...
        PortActor::system(),
        "en",
        format!("product-postgres-{suffix}"),
    )
...
async fn seed_tenant(
    ...
    database
        .execute_raw(Statement::from_sql_and_values(
            ...
            vec![
                tenant_id.into(),
                "Product translation evidence".into(),
