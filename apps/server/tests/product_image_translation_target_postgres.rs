...
async fn product_image_registered_translation_provider_multi_replica_evidence_postgres(
    ...
    let database_name = unique_postgres_database_name("rustok_image_translation_evidence");
...
async fn run_contract(database_url: &str) -> TestResult<()> {
    ...
    seed_tenant(&seed_connection, tenant_id, "image-main").await?;
    seed_tenant(&seed_connection, other_tenant_id, "image-isolated").await?;
    ...
    // Product currently has no public Image-create command. Seed only the canonical
    // owner rows needed to establish the fixture; every observed mutation, replay,
    ...
    assert_eq!(identity.resource_id.as_str(), image_id.to_string());
    ...
    assert_eq!(initial_progress.complete_resources, 1);
    ...
    let first_patch = patch(&initial, "Vue avant du produit", "first");
    let first_receipt = first_provider
        .apply_patch(
            apply_context(tenant_id, "first-apply", "image-first-apply"),
            first_patch.clone(),
        )
        .await?;
    let replay = second_provider
        .apply_patch(
            apply_context(tenant_id, "first-replay", "image-first-apply"),
            first_patch,
        )
        .await?;
    ...
    let first_apply = first_provider.apply_patch(
        apply_context(tenant_id, "race-one", "image-race-one"),
        patch(&first_snapshot, "Vue avant A", "race-one"),
    );
    let second_apply = second_provider.apply_patch(
        apply_context(tenant_id, "race-two", "image-race-two"),
        patch(&second_snapshot, "Vue avant B", "race-two"),
    );
    ...
    let late_receipt = provider
        .apply_patch(
            apply_context(tenant_id, "late-apply", "image-late-apply"),
            patch(&restored, "Vue avant finale", "late"),
        )
        .await?;
    ...
    let read_error = provider
        ...
        .await
        .expect_err("deleted Product Image must not remain readable");
...
fn registered_provider(db: DatabaseConnection) -> Arc<dyn TranslationTargetProvider> {
    ...
    let extensions = build_shared_runtime_extensions_with_host_providers(
        ...
        &settings,
        runtime,
        AuthConfig::new("test-secret-key-for-image-translation-32bytes!!".to_string()),
    )
...
fn initial_product() -> CreateProductInput {
    CreateProductInput {
        translations: vec![ProductTranslationInput {
            locale: "en".into(),
            title: "Image Evidence Product".into(),
            handle: Some("image-evidence-product".into()),
            description: Some("Product carrying Image translation evidence".into()),
            meta_title: None,
            meta_description: None,
        }],
        options: Vec::new(),
        variants: vec![CreateVariantInput {
            sku: Some("IMAGE-EVIDENCE-SKU".into()),
            barcode: None,
        ...
        product_type: Some("image-evidence".into()),
        shipping_profile_slug: None,
        primary_category_id: None,
        tags: Vec::new(),
        metadata: json!({"evidence": "image-translation"}),
        publish: false,
    }
}
...
fn patch(
    ...
    TranslationPatchRequest {
        ...
        fields: snapshot
            ...
            .iter()
            .map(|field| {
                assert_eq!(field.descriptor.key.as_str(), "alt_text");
                TranslationFieldPatch {
                    key: field.descriptor.key.clone(),
                    value: alt_text.to_string(),
                    expected_source_hash: field.source_hash.clone(),
                }
            })
            .collect(),
        proposal_id: format!("image-proposal-{suffix}"),
        approval_receipt_id: format!("image-approval-{suffix}"),
    }
}
...
async fn seed_tenant(
    ...
    database
        .execute_raw(Statement::from_sql_and_values(
            ...
            vec![
                tenant_id.into(),
                "Product Image translation evidence".into(),
...
async fn seed_image_source(
    ...
    database
        .execute_raw(Statement::from_sql_and_values(
            DbBackend::Postgres,
            "INSERT INTO product_images (id, product_id, media_id, position) VALUES ($1, $2, $3, $4)",
            vec![
                image_id.into(),
                product_id.into(),
                Uuid::new_v4().into(),
    ...
    database
        .execute_raw(Statement::from_sql_and_values(
            DbBackend::Postgres,
            "INSERT INTO product_image_translations (id, image_id, locale, alt_text) VALUES ($1, $2, $3, $4)",
            vec![
                Uuid::new_v4().into(),
                image_id.into(),
                "en".into(),
                "Front product view".into(),
            ],
        ))
        .await?;
    Ok(())
