...
async fn run_contract(database_url: &str) -> TestResult<()> {
    ...
    seed_tenant(&seed_connection, tenant_id, "pricing-main").await?;
    seed_tenant(&seed_connection, other_tenant_id, "pricing-isolated").await?;
        ...
        .create_price_list(
            ...
            CreatePriceListOwnerInput {
                translations: vec![PriceListOwnerTranslationInput {
                    ...
                    description: Some("Seasonal pricing".into()),
                }],
                list_type: "sale".into(),
                status: "active".into(),
                channel_id: None,
    ...
    let initial = provider
        ...
        .await?;
    assert_eq!(field(&initial, "name").source_value, "Autumn Sale");
    assert!(field(&initial, "name").exact_target_value.is_none());
    ...
    let replay = second_provider
        .apply_patch(
            apply_context(tenant_id, "first-replay", "pricing-first-apply"),
            first_patch,
        )
        .await?;
    ...
    let first_apply = first_provider.apply_patch(
        apply_context(tenant_id, "race-one", "pricing-race-one"),
        patch(&first_snapshot, "Vente A", "Description A", "race-one"),
    );
    let second_apply = second_provider.apply_patch(
        apply_context(tenant_id, "race-two", "pricing-race-two"),
        patch(&second_snapshot, "Vente B", "Description B", "race-two"),
    );
    ...
    assert_eq!(
        ...
        after_operational.summary.resource_revision
    );
    let progress_after_operational = provider
        .read_progress(
            read_context(tenant_id, "progress-operational"),
            TranslationTargetProgressRequest {
                source_locale: TenantLocale::new("en")?,
                target_locale: TenantLocale::new("fr")?,
            },
    ...
    owner
        .update_price_list(
            ...
            price_list.id,
            UpdatePriceListOwnerInput {
                translations: Some(owner_translations(&after_operational)),
                ..Default::default()
            },
        )
        .await?;
    ...
    let late_receipt = provider
        .apply_patch(
            apply_context(tenant_id, "late-apply", "pricing-late-apply"),
            patch(&late_snapshot, "Vente finale", "Description finale", "late"),
        )
        .await?;
    ...
    let read_error = provider
        ...
        .await
        .expect_err("deleted Price List must not remain readable");
...
fn registered_provider(db: DatabaseConnection) -> Arc<dyn TranslationTargetProvider> {
    ...
    targets
        ...
        .expect("pricing/price_list_copy provider")
}
        ...
        fields: snapshot
            ...
            .map(|field| TranslationFieldPatch {
                ...
                value: match field.descriptor.key.as_str() {
                    ...
                    other => panic!("unexpected Pricing field: {other}"),
                },
                expected_source_hash: field.source_hash.clone(),
            })
            .collect(),
        proposal_id: format!("pricing-proposal-{suffix}"),
        approval_receipt_id: format!("pricing-approval-{suffix}"),
    }
}
...
fn owner_translations(snapshot: &TranslationResourceSnapshot) -> Vec<PriceListOwnerTranslationInput> {
    ...
    vec![
        PriceListOwnerTranslationInput {
            ...
            name: field(snapshot, "name").source_value.clone(),
...
fn read_context(tenant_id: Uuid, suffix: &str) -> PortContext {
    PortContext::new(
        ...
        PortActor::system(),
        "en",
        format!("pricing-postgres-{suffix}"),
    )
...
async fn seed_tenant(
    ...
    database
        .execute_raw(Statement::from_sql_and_values(
            ...
            vec![
                tenant_id.into(),
                "Pricing translation evidence".into(),
