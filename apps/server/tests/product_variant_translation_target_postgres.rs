...
const RESOURCE_KIND: &str = "variant";
...
async fn product_variant_registered_translation_provider_multi_replica_evidence_postgres(
    ...
    let database_name = unique_postgres_database_name("rustok_variant_translation_evidence");
...
async fn run_contract(database_url: &str) -> TestResult<()> {
    ...
    seed_tenant(&seed_connection, tenant_id, "variant-main").await?;
    seed_tenant(&seed_connection, other_tenant_id, "variant-isolated").await?;
    ...
    let first_patch = patch(&initial, "Variante d'automne", "first");
    let first_receipt = first_provider
        .apply_patch(
            apply_context(tenant_id, "first-apply", "variant-first-apply"),
            first_patch.clone(),
        )
        .await?;
    let replay = second_provider
        .apply_patch(
            apply_context(tenant_id, "first-replay", "variant-first-apply"),
            first_patch,
        )
        .await?;
    ...
    let first_apply = first_provider.apply_patch(
        apply_context(tenant_id, "race-one", "variant-race-one"),
        patch(&first_snapshot, "Variante A", "race-one"),
    );
    let second_apply = second_provider.apply_patch(
        apply_context(tenant_id, "race-two", "variant-race-two"),
        patch(&second_snapshot, "Variante B", "race-two"),
    );
    ...
    owner
        .update_product(
            ...
            product_id,
            UpdateProductInput {
                vendor: Some("Vendor outside Variant translation revision".into()),
                ..Default::default()
            },
        )
        .await?;
    ...
    let late_receipt = provider
        .apply_patch(
            apply_context(tenant_id, "late-apply", "variant-late-apply"),
            patch(&restored, "Variante finale", "late"),
        )
        .await?;
    ...
    assert!(
        deleted.changes[0]
            .resource_revision
            .as_str()
            .starts_with("variant-deleted-v1:")
    );
    ...
    let read_error = provider
        ...
        .await
        .expect_err("deleted Product Variant must not remain readable");
...
fn registered_provider(db: DatabaseConnection) -> Arc<dyn TranslationTargetProvider> {
    ...
    let extensions = build_shared_runtime_extensions_with_host_providers(
        ...
        &settings,
        runtime,
        AuthConfig::new("test-secret-key-for-variant-translation-32bytes!".to_string()),
    )
    ...
    targets
        ...
        .expect("product/variant provider")
}
...
fn initial_product() -> CreateProductInput {
    CreateProductInput {
        translations: vec![ProductTranslationInput {
            locale: "en".into(),
            title: "Variant Evidence Product".into(),
            handle: Some("variant-evidence-product".into()),
            description: Some("Product carrying Variant translation evidence".into()),
            meta_title: None,
            meta_description: None,
        }],
        options: Vec::new(),
        variants: vec![CreateVariantInput {
            sku: Some("VARIANT-EVIDENCE-SKU".into()),
            barcode: None,
        ...
        product_type: Some("variant-evidence".into()),
        shipping_profile_slug: None,
        primary_category_id: None,
        tags: Vec::new(),
        metadata: json!({"evidence": "variant-translation"}),
        publish: false,
    }
}
...
fn patch(
    ...
    TranslationPatchRequest {
        ...
        proposal_id: format!("variant-proposal-{suffix}"),
        approval_receipt_id: format!("variant-approval-{suffix}"),
    }
}
...
fn field(
    ...
    assert_eq!(fields[0].descriptor.key.as_str(), "title");
    &fields[0]
}
...
fn read_context(tenant_id: Uuid, suffix: &str) -> PortContext {
    PortContext::new(
        ...
        PortActor::system(),
        "en",
        format!("variant-postgres-{suffix}"),
    )
...
async fn seed_tenant(
    ...
    database
        .execute_raw(Statement::from_sql_and_values(
            ...
            vec![
                tenant_id.into(),
                "Product Variant translation evidence".into(),
