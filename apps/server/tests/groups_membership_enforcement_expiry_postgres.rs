...
async fn connect(url: &str) -> DatabaseConnection {
    ...
    Database::connect(options)
        .await
        .expect("PostgreSQL Groups expiry evidence connection should open")
}
...
fn user_write_context(
    ...
    PortContext::new(
        ...
        "en",
        format!("groups-postgres-expiry-write-{}", Uuid::new_v4()),
    )
...
fn owner_read_context(tenant_id: Uuid, owner_id: Uuid) -> PortContext {
    PortContext::new(
        ...
        "en",
        format!("groups-postgres-expiry-read-{}", Uuid::new_v4()),
    )
    .with_deadline(Duration::from_secs(10))
...
VALUES ('{group_id}', '{tenant_id}', '{owner_id}', 'enforcement-postgres-expiry', 3);
...
"#,
    ...
    .await
    .expect("Groups PostgreSQL expiry evidence fixture should seed");
}
...
async fn enforcement_projection(
    ...
    tenant_id: Uuid,
    user_id: Uuid,
) -> (i64, bool, bool, String) {
    let row = db
        .query_one(Statement::from_string(
            DatabaseBackend::Postgres,
            format!(
                "SELECT revision, effective_until IS NOT NULL AS has_effective_until, revoked_at IS NOT NULL AS is_revoked, source_kind FROM group_membership_enforcements WHERE tenant_id = '{tenant_id}' AND user_id = '{user_id}'"
            ),
        ))
        .await
    ...
    (
        ...
        row.try_get("", "has_effective_until")
            .expect("effective-until presence should decode"),
        row.try_get("", "is_revoked")
            .expect("revocation presence should decode"),
...
async fn postgres_group_membership_enforcement_expiry_and_revoke_restore_without_cleanup() {
    let base_url = std::env::var(POSTGRES_URL_ENV)
        .expect("RUSTOK_GROUPS_TEST_POSTGRES_URL must be configured");
    let schema = format!("groups_enforcement_expiry_{}", Uuid::new_v4().simple());
    let admin = connect(&base_url).await;
    admin
        .execute_unprepared(&format!("CREATE SCHEMA {schema}"))
        .await
        .expect("isolated Groups expiry evidence schema should create");
    ...
    let expiring_suspension = GroupMembershipEnforcementCommandPort::suspend_membership(
        ...
        user_write_context(
            tenant_id,
            owner_id,
            format!("postgres-expiry-suspend-{}", Uuid::new_v4()),
        ),
    ...
    .await
    .expect("owner should create an expiring direct-local suspension on PostgreSQL");
    assert_eq!(
    ...
    assert_eq!(
        membership_revision(&fixture, tenant_id, expiring_user_id).await,
        expiring_initial_revision + 1,
        "clock expiry must not require cleanup or a synthetic membership revision bump"
    );
    assert_eq!(group_member_count(&fixture, tenant_id, group_id).await, 3);
    let (_, has_persisted_until, persisted_revoked, persisted_source) =
        enforcement_projection(&fixture, tenant_id, expiring_user_id).await;
    assert!(has_persisted_until);
    assert!(!persisted_revoked);
    ...
    let revoke_command = GroupMembershipEnforcementCommandService::new(connect(&scoped_url).await);
    let permanent_suspension = GroupMembershipEnforcementCommandPort::suspend_membership(
        &revoke_command,
        user_write_context(
            tenant_id,
            owner_id,
            format!("postgres-revoke-suspend-{}", Uuid::new_v4()),
        ),
        SuspendGroupMembershipRequest {
            group_id,
    ...
    .await
    .expect("owner should create a direct-local suspension for PostgreSQL revoke evidence");
    assert_eq!(
    ...
    let revoked = GroupMembershipEnforcementCommandPort::revoke_membership_suspension(
        ...
        user_write_context(
            tenant_id,
            owner_id,
            format!("postgres-revoke-direct-{}", Uuid::new_v4()),
        ),
    ...
    assert!(after_revoke_projection.2);
    ...
    assert_eq!(
        ...
        "suspend and revoke should each advance membership revision exactly once"
    );
    assert_eq!(group_member_count(&fixture, tenant_id, group_id).await, 3);
    ...
    admin
        ...
        .await
        .expect("isolated Groups expiry evidence schema should drop");
}
