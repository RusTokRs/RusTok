...
async fn connect(url: &str) -> DatabaseConnection {
    ...
    Database::connect(options)
        .await
        .expect("Groups governance expiry PostgreSQL connection should open")
}
...
VALUES ('{group_id}', '{tenant_id}', '{owner_id}', 'governance-postgres-suspension-expiry', 4);
...
"#,
    ...
    .await
    .expect("Groups PostgreSQL governance expiry fixture should seed");
}
...
fn write_context(tenant_id: Uuid, actor_id: Uuid, operation: &str) -> PortContext {
    PortContext::new(
        ...
        "en",
        format!("groups-governance-postgres-expiry-{}", Uuid::new_v4()),
    )
...
async fn governance_authority_follows_temporary_suspension_and_owner_clock_expiry_postgres() {
    let base_url = std::env::var(POSTGRES_URL_ENV)
        .expect("RUSTOK_GROUPS_TEST_POSTGRES_URL must be configured");
    let schema = format!("groups_governance_expiry_{}", Uuid::new_v4().simple());
    let admin_db = connect(&base_url).await;
    admin_db
        .execute_unprepared(&format!("CREATE SCHEMA {schema}"))
        .await
        .expect("isolated Groups governance expiry schema should create");
    ...
    let baseline = GroupGovernanceCommandPort::change_group_role(
        &governance,
        write_context(tenant_id, admin_id, "postgres-admin-baseline-role-change"),
    ...
    .await
    .expect("active administrator should change a member role before PostgreSQL suspension");
    ...
    let suspended = GroupMembershipEnforcementCommandPort::suspend_membership(
        &enforcement,
        write_context(tenant_id, owner_id, "postgres-owner-suspend-admin"),
        SuspendGroupMembershipRequest {
            group_id,
    ...
    let denied = GroupGovernanceCommandPort::change_group_role(
        &governance,
        write_context(tenant_id, admin_id, "postgres-suspended-admin-role-change"),
        ChangeGroupRoleRequest {
            group_id,
    ...
    .await
    .expect_err("suspended administrator must not retain PostgreSQL governance authority");
    ...
    let restored = GroupGovernanceCommandPort::change_group_role(
        &governance,
        write_context(tenant_id, admin_id, "postgres-restored-admin-role-change"),
        ChangeGroupRoleRequest {
            group_id,
            target_user_id: second_target_id,
            role: GroupRole::Moderator,
    ...
    .await
    .expect("expired suspension should restore PostgreSQL administrator governance authority without cleanup");
    ...
    admin_db
        ...
        .await
        .expect("isolated Groups governance expiry schema should drop");
}
