...
const fail = (message) => {
  throw new CustomerFbaNoCompileVerificationError(message);
};
...
export function verifyCustomerFbaNoCompile() {
  ...
  if (!centralRegistry.includes('| `customer` |') || !centralRegistry.includes(registryPath)) {
    fail('central readiness board must reference customer FBA registry');
  }
  ...
  for (const operation of ['read_customer_projection', 'list_customer_projections']) {
    ...
    if (!staticEvidence.cases?.some((entry) => entry.operation === operation)) {
      fail(`static evidence missing ${operation}`);
    }
  ...
  if (staticEvidence.promotion_gate !== 'does_not_raise_boundary_ready_without_runtime_execution') {
    fail('static evidence must keep promotion gated');
  }
  ...
  for (const expectedCode of ['port.deadline_required', 'customer.tenant_id_invalid', 'customer.customer_not_found']) {
    if (!JSON.stringify(runtimeSmoke.typed_error_matrix).includes(expectedCode)) {
      fail(`runtime smoke missing typed error ${expectedCode}`);
    }
  }

  for (const doc of [plan, readme, localDocs]) {
    if (!doc.includes('node scripts/verify/verify-customer-fba-no-compile.mjs')) {
      fail('customer docs must reference the no-compile customer gate');
    }
  }
  if (!plan.includes('- FBA status: `in_progress`')) fail('plan FBA status drift');
  if (!plan.includes('- [x] Проверить полноту и актуальность `README.md`')) fail('plan docs audit backlog not completed');
  if (!plan.includes('- [x] Зафиксировать/обновить verification gates')) fail('plan verification backlog not completed');
}
