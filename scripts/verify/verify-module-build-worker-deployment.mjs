...
assert.throws(() => parseArguments(['--namespace', 'rustok', '--image', 'worker', '--digest', 'latest', '--runtime', 'gvisor', '--tls-secret', 'tls', '--attestation-config-map', 'attestation', '--config-map', 'config', '--source-pvc', 'source']), /lowercase SHA-256 digest/);
assert.ok(renderDeployment(parseArguments(['--namespace', 'rustok', '--image', 'worker', '--digest', digest, '--runtime', 'kata', '--tls-secret', 'tls', '--attestation-config-map', 'attestation', '--config-map', 'config', '--source-pvc', 'source'])).includes('runtimeClassName: kata'));
