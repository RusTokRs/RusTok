...
for (const [value, label] of [
  ...
  ['fn tax_calculation_local_operation(code: &str)', 'stable-code classifier'],
  ['let Some(local_operation) = tax_calculation_local_operation(error.code.as_str()) else {', 'stable-code mapper use'],
  ['return error;', 'unknown outcome pass-through'],
  ['PortErrorKind::Unavailable | PortErrorKind::Timeout | PortErrorKind::InvariantViolation', 'technical severity classification'],
  ['"tax calculation local technical outcome retained safe delegated context"', 'technical diagnostic event'],
  ['"tax calculation local outcome retained safe delegated context"', 'ordinary diagnostic event'],
...
for (const [value, label] of [
  ...
  ['claim_count = context_facts.claim_count', 'claim count'],
  ['role_count = context_facts.role_count', 'role count'],
  ['channel_present = context_facts.channel_present', 'channel presence'],
  ['channel_length = ?context_facts.channel_length', 'channel length'],
  ['locale_length = context_facts.locale_length', 'locale length'],
  ['causation_id_present = context_facts.causation_id_present', 'causation presence'],
  ['causation_id_length = ?context_facts.causation_id_length', 'causation length'],
  ['traceparent_present = context_facts.traceparent_present', 'trace presence'],
  ['traceparent_length = ?context_facts.traceparent_length', 'trace length'],
  ['idempotency_key_present = context_facts.idempotency_key_present', 'idempotency presence'],
  ['idempotency_key_length = ?context_facts.idempotency_key_length', 'idempotency length'],
  ['deadline_ms = ?context_facts.deadline_ms', 'deadline context'],
  ['currency_code_length = facts.currency_code_length', 'currency shape'],
  ['channel_id_present = facts.channel_id_present', 'channel id presence'],
  ['channel_id_non_nil = ?facts.channel_id_non_nil', 'channel id non-nil fact'],
...
for (const [code, label] of [
  ['tax.currency_code_invalid', 'currency validation'],
  ['tax.negative_policy_rate', 'policy rate validation'],
  ['tax.country_code_invalid', 'country code validation'],
  ['tax.negative_country_rate', 'country rate validation'],
  ['tax.duplicate_country_rule', 'country rule uniqueness'],
  ['tax.negative_taxable_amount', 'taxable amount validation'],
  ['tax.validation', 'owner provider validation'],
  ['tax.negative_total', 'negative result total'],
  ['tax.exempt_customer_charged', 'exempt result invariant'],
  ['tax.total_overflow', 'result total overflow'],
  ['tax.total_mismatch', 'result total mismatch'],
  ['tax.provider_id_invalid', 'provider identity invariant'],
  ['tax.negative_line', 'negative result line'],
  ['tax.currency_mismatch', 'result currency mismatch'],
  ['tax.unknown_taxable_target', 'unknown result target'],
]) {
  requireIn(wrapper, `"${code}" => Some(`, label);
}

for (const [value, label] of [
  ['match (error.code.as_str(), error.message.as_str())', 'message-based routing'],
  ['internal_message = %error.message', 'duplicated public message field'],
  ['tenant_id = %context.tenant_id', 'raw tenant value'],
  ['actor = ?context.actor', 'raw actor value'],
  ['channel = ?context.channel', 'raw channel value'],
  ['locale = %context.locale', 'raw locale value'],
  ['causation_id = ?context.causation_id', 'raw causation value'],
  ['traceparent = ?context.traceparent', 'raw trace value'],
  ['idempotency_key = ?context.idempotency_key', 'raw idempotency value'],
  ['channel_id = ?facts.channel_id', 'raw channel UUID'],
...
for (const [value, label] of [
  ...
  ['stable-code-only local attribution', 'stable-code policy documentation'],
