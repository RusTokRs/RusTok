...
for (const [source, value, label] of [
  [
    ...
    'commerce root mounted storefront runtime',
  ],
  [
    mountedRuntime,
    'include!("storefront_checkout_runtime.rs")',
    'private legacy compatibility include',
  ],
  [
    mountedRuntime,
    'pub async fn complete_storefront_checkout(',
    'mounted checkout completion facade',
  ],
  [
    mountedRuntime,
    'payment_provider_registry: rustok_payment::providers::PaymentProviderRegistry',
    'mounted provider registry input',
  ],
  [
    mountedRuntime,
    'idempotency_key: impl Into<String>',
    'mounted idempotency input',
  ],
  [
    mountedRuntime,
    'storefront_staged_checkout_runtime::complete_storefront_checkout(',
    'mounted staged delegation',
  ],
  [
    stagedRuntime,
    'pub async fn complete_storefront_checkout_input(',
    'full staged storefront entrypoint',
  ],
  [
    stagedRuntime,
    'RecoveringStagedCheckoutService::new(staged, compensation)',
    'staged recovery composition',
  ],
  [
    stagedRuntime,
    'checkout_input.shipping_selections.clone()',
    'full shipping selection preservation',
  ],
  [
    stagedRuntime,
    'with_payment_provider_registry(payment_provider_registry.clone())',
    'staged provider registry composition',
  ],
  [
    graphqlCheckout,
    'complete_storefront_checkout_input(',
    'GraphQL staged checkout entrypoint',
  ],
  [
    graphqlCheckout,
    'payment_provider_registry_from_context(ctx)',
    'GraphQL host provider registry',
  ],
  [
    restCheckout,
    'RecoveringStagedCheckoutService::new(staged, compensation)',
    'REST staged recovery composition',
  ],
  [
    restCheckout,
    'let idempotency_key = required_idempotency_key(&headers)?;',
    'REST explicit idempotency identity',
  ],
  [
    nativeCheckout,
    'services::storefront_staged_checkout_runtime',
    'native staged runtime import',
  ],
  [
    nativeCheckout,
    'payment_provider_registry',
    'native host provider registry',
  ],
  [
    journaledCheckout,
    'Execution is fully delegated to',
    'journaled compatibility-only contract',
  ],
  [
    journaledCheckout,
    'RecoveringStagedCheckoutService::new(staged, compensation)',
    'journaled staged delegation',
  ],
  [
    legacyRuntime,
    'pub async fn complete_storefront_checkout(',
    'retained private legacy completion source',
  ],
]) {
...
for (const [source, label] of [
  ...
  [stagedRuntime, 'staged storefront runtime'],
]) {
  for (const value of [
    'CheckoutService::new',
    'JournaledCheckoutService::new',
  ]) {
    forbidText(source, value, `${label} legacy checkout construction`);
  }
}
...
forbidText(
  mountedRuntime,
  'pub use legacy::*;',
  'mounted wildcard legacy export',
);
forbidText(
  mountedRuntime,
  'legacy::complete_storefront_checkout',
  'mounted legacy completion export',
);
forbidText(
  graphqlCheckout,
  'bind_in_process_atomic_cart_checkout_with_pricing',
  'GraphQL duplicate staged pipeline construction',
);
forbidText(
  graphqlCheckout,
  'PrepareCartCheckoutSnapshotRequest',
  'GraphQL duplicate cart prepare construction',
);
...
console.log(
  '✔ REST, GraphQL, native, mounted, and journaled storefront checkout enter the staged recovery pipeline',
);
