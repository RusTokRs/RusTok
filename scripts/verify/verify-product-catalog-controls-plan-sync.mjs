...
if (plan.includes(umbrellaPending) === plan.includes(umbrellaComplete)) {
  ...
  failures.push(`${planPath}: umbrella task must remain pending until attribute_filters reach both surfaces and owner execution`);
}
...
if (attributeFiltersComplete) requireText(plan, attributeMarker, `${planPath}: completed attribute_filters slice is not recorded`);
...
if (!attributeFiltersComplete && plan.includes(attributeMarker)) failures.push(`${planPath}: attribute_filters marker is complete without typed source parity`);

forbidText(plan, "optional catalog filters/sorts, detached-value marker contract", `${planPath}: catalog controls must not be described as source-locked before end-to-end execution exists`);
for (const marker of [
  "Recheck on 2026-07-29",
  "`attribute_filters` through typed UI state",
...
const productRegistryRow = registry.split("\n").find((line) => line.startsWith("| `product` |"));
if (!productRegistryRow) {
  failures.push(`${registryPath}: product live-plan row is missing`);
} else if (!/catalog read provider|consumer fallback|transport_verified/i.test(productRegistryRow)) {
  failures.push(`${registryPath}: product nearest priority must advance to provider/fallback execution after catalog controls close`);
}
