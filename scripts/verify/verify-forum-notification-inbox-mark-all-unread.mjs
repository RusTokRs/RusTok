...
for (const delivered of [
  ...
  if (contract.composition?.[delivered] !== true) {
    failures.push(`forum notification mark-all-unread contract must record ${delivered} as delivered`);
  }
}
...
for (const residual of [
  ...
  if (!contract.not_delivered?.includes(residual)) {
    failures.push(`forum notification mark-all-unread contract must keep ${residual} explicitly open`);
  }
}
if (contract.not_delivered?.some((item) => item.includes("mark-all-unread"))) {
  failures.push("FORUM-20Z must remove mark-all-unread from current residuals");
}
...
if (planSync.status === "pending") {
  ...
  requireText(plan, "FORUM-20A-Z provide", "synchronized canonical plan must advance through Z");
  for (const slice of deliveredSlices) {
...
const unreadStart = owner.indexOf("pub struct NotificationInboxMarkAllUnreadRequest");
const unreadOwner = unreadStart >= 0 ? owner.slice(unreadStart) : "";
for (const marker of [
  ...
  "notification inbox mark-all-unread identity must not be nil",
]) {
...
for (const marker of [
  ...
  requireText(migration, marker, `mark-all-unread index support is missing ${marker}`);
}
for (const marker of [
  "mod inbox_bulk;",
...
for (const marker of [
  "bounded_page_reopens_seen_and_read_without_touching_terminal_states",
  "mark_all_unread_pages_are_bounded_and_resumable",
...
for (const marker of [
  ...
  "mark-all-archive and arbitrary selected-ID",
]) {
...
if (
  ...
  upstream.task !== "FORUM-20Y" ||
  upstream.upstream_task !== "FORUM-20X" ||
  upstream.composition?.bounded_owner_mark_all_read_service !== true ||
  !upstream.not_delivered?.includes(
    "mark-all-unread mark-all-archive and arbitrary selected-id bulk mutations",
  )
) {
  failures.push("FORUM-20Z must remain linked to the historical FORUM-20Y mark-all-unread residual");
}

if (failures.length > 0) {
  console.error("Forum notification inbox mark-all-unread verification failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log("Forum notification inbox mark-all-unread contract is source-ready.");
