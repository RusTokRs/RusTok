...
verifySupportAdapter({
  module: 'ai-content',
  registryPath: 'crates/modules/rustok-ai-content/contracts/ai-content-fba-registry.json',
  sourceMarkers: [
...
const aiOrder = verifySupportAdapter({
  module: 'ai-order',
  registryPath: 'crates/modules/rustok-ai-order/contracts/ai-order-fba-registry.json',
  sourceMarkers: [
...
const orderProvider = json('crates/modules/rustok-order/contracts/order-fba-registry.json');
...
const authRegistryPath = 'crates/modules/rustok-auth/contracts/auth-fba-registry.json';
...
const authSource = read('crates/modules/rustok-auth/src/lib.rs');
...
const aiRegistryPath = 'crates/modules/rustok-ai/contracts/ai-fba-registry.json';
...
const aiAdminModel = read('crates/modules/rustok-ai/admin/src/model.rs');
hasAll(aiAdminModel, ['agent_model_assignments: Vec<AiAgentModelAssignmentPayload>'], 'ai admin bootstrap model');
const aiAdminNative = read('crates/modules/rustok-ai/admin/src/transport/native_server_adapter.rs');
hasAll(aiAdminNative, ['list_tenant_agent_model_assignments', 'map_agent_model_assignment'], 'ai native assignment bootstrap');
const aiAdminGraphql = read('crates/modules/rustok-ai/admin/src/transport/graphql_adapter.rs');
...
const aiQuery = read('crates/modules/rustok-ai/src/graphql/query.rs');
...
const aiGraphqlTypes = read('crates/modules/rustok-ai/src/graphql/types.rs');
...
const aiProviderNative = read('crates/modules/rustok-ai/admin/src/transport/native_server_adapter.rs');
hasNone(aiProviderNative, ['restricted_role_slugs: Vec<String>'], 'ai native provider input');
const aiProviderUi = read('crates/modules/rustok-ai/admin/src/ui/leptos.rs');
hasNone(aiProviderUi, ['provider_restricted_roles'], 'ai provider UI input');
const aiRuntimeFactory = read('crates/modules/rustok-ai/src/service/types.rs');
...
const aiDeploymentRuntime = read('crates/modules/rustok-ai/src/runtime_extensions.rs');
hasAll(aiDeploymentRuntime, ['RUSTOK_AI_SECRET_RESOLVERS_JSON', 'DeploymentSecretResolverConfig', 'VaultResolver::new', 'KubernetesSecretResolver::in_cluster', 'LazyAwsResolver', 'LazyGcpResolver', 'AzureKeyVaultResolver::from_default_credential', 'secret resolver aliases must be unique and non-empty', 'every_deployment_resolver_kind_deserializes_without_initializing_clients'], 'AI deployment secret resolver composition');
const runtimeExtensions = read('crates/libs/rustok-core/src/module.rs');
...
const runtimeScheduler = read('crates/libs/rustok-runtime/src/lib.rs');
...
const aiReadme = read('crates/modules/rustok-ai/README.md');
...
const aiModule = read('crates/modules/rustok-ai/src/lib.rs');
...
const aiAgents = read('crates/modules/rustok-ai/src/agent.rs');
hasAll(aiAgents, ['Product,', 'Code,', 'Orchestrator,', 'Review,', 'pub const fn slug(self)', 'AgentKind::Product'], 'AI product/code agent taxonomy');
hasNone(aiAgents, ['AgentKind::Domain', '    Domain,'], 'AI legacy domain agent taxonomy');
const aiAgentGraphql = read('crates/modules/rustok-ai/src/graphql/types.rs');
hasAll(aiAgentGraphql, ['pub enum AiAgentKindGql', 'Product,', 'AgentKind::Product => Self::Product'], 'AI product agent GraphQL taxonomy');
hasNone(aiAgentGraphql, ['AiAgentKindGql::Domain', 'AgentKind::Domain'], 'AI legacy GraphQL domain taxonomy');
hasAll(aiAdminNative, ['kind: value.kind.slug().to_string()'], 'AI native canonical agent-kind transport');
hasNone(aiAdminNative, ['format!("{:?}", value.kind)'], 'AI native Debug-derived agent-kind transport');
const aiGraphqlRuntime = read('crates/modules/rustok-ai/src/graphql_runtime.rs');
...
const aiRouter = read('crates/modules/rustok-ai/src/router.rs');
hasAll(aiRouter, ['provider role restriction awaits the platform tenant RBAC catalog'], 'ai router legacy role policy');
const aiSchedulerAdapter = read('crates/modules/rustok-ai/src/scheduler.rs');
...
const moduleScheduler = read('crates/libs/rustok-runtime/src/lib.rs');
...
const tenantRbacContract = read('crates/libs/rustok-api/src/tenant_rbac.rs');
...
const tenantRbacProvider = read('crates/modules/rustok-rbac/src/catalog.rs');
...
const aiMutation = read('crates/modules/rustok-ai/src/graphql/mutation.rs');
...
const aiAgentPanel = read('crates/modules/rustok-ai/admin/src/ui/components/agent_panel.rs');
