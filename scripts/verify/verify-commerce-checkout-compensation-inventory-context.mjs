...
for (const [value, label] of [
  ['.map_err(|error| boundary_error("read_cart", error))?', 'cart read remains out of scope'],
  ['.map_err(|error| boundary_error("release_cart", error))?', 'cart release remains out of scope'],
...
console.log(
  '✔ Checkout inventory compensation retains owner context without changing cart paths or public envelopes',
);
