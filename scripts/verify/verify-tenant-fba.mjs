...
const registryPath = 'crates/modules/rustok-tenant/contracts/tenant-fba-registry.json';
const evidencePath = 'crates/modules/rustok-tenant/contracts/evidence/tenant-contract-test-static-matrix.json';
const registry = json(registryPath);
const evidence = json(evidencePath);
const manifest = read('crates/modules/rustok-tenant/rustok-module.toml');
const plan = read('crates/modules/rustok-tenant/docs/implementation-plan.md');
const central = read('docs/modules/registry.md');
const cargo = read('crates/modules/rustok-tenant/Cargo.toml');
const lib = read('crates/modules/rustok-tenant/src/lib.rs');
const dto = read('crates/modules/rustok-tenant/src/dto/mod.rs');
const ports = read('crates/modules/rustok-tenant/src/ports.rs');
const tenantService = read('crates/modules/rustok-tenant/src/services/tenant_service.rs');
const integrationTests = read('crates/modules/rustok-tenant/tests/integration.rs');
const localeConcurrencyPostgres = read('crates/modules/rustok-tenant/tests/locale_policy_concurrency_postgres.rs');
const ensureConcurrencyPostgres = read('crates/modules/rustok-tenant/tests/tenant_ensure_concurrency_postgres.rs');
const localePolicyMigration = read('crates/modules/rustok-tenant/src/migrations/m20260726_000001_enforce_tenant_locale_policy.rs');
...
const installerPersistence = read('crates/utils/rustok-installer-persistence/src/seaorm_ports.rs');
const authCli = read('crates/modules/rustok-auth/cli/src/lib.rs');
const tenantAdminCargo = read('crates/modules/rustok-tenant/admin/Cargo.toml');
const tenantAdminNative = read('crates/modules/rustok-tenant/admin/src/transport/native_server_adapter.rs');
...
if (port.context !== 'crates/modules/rustok-tenant/src/ports.rs::PortContext' || port.error !== 'crates/modules/rustok-tenant/src/ports.rs::PortError') fail('context/error drift');
