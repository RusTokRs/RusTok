...
if (pkg.scripts?.['verify:outbox:fba'] !== 'node scripts/verify/verify-outbox-fba.mjs') fail('package script verify:outbox:fba drift');
...
if (port.context !== 'crates/rustok-outbox/src/ports.rs::PortContext' || port.error !== 'crates/rustok-outbox/src/ports.rs::PortError') fail('context/error drift');
...
for (const marker of ['trait OutboxRelayPort', 'impl OutboxRelayPort for crate::OutboxRelay', 'context.require_write_semantics()?', 'require_deadline_semantics', 'OutboxRelayRunOnceProjection', 'PortErrorKind::Validation', 'outbox.idempotency_required']) {
...
if (!plan.includes('- FBA status: `in_progress`') || !plan.includes(registryPath) || !plan.includes('OutboxRelayPort') || !plan.includes('outbox-contract-test-static-matrix.json')) fail('local plan FBA evidence drift');
if (!central.includes('| `outbox` |') || !central.includes(registryPath) || !central.includes('`in_progress` | `in_progress`')) fail('central readiness board drift');
