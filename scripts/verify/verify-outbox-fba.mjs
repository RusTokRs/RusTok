...
const registryPath = 'crates/modules/rustok-outbox/contracts/outbox-fba-registry.json';
const evidencePath = 'crates/modules/rustok-outbox/contracts/evidence/outbox-contract-test-static-matrix.json';
...
const manifest = read('crates/modules/rustok-outbox/rustok-module.toml');
const plan = read('crates/modules/rustok-outbox/docs/implementation-plan.md');
const central = read('docs/modules/registry.md');
const pkg = json('package.json');
const lib = read('crates/modules/rustok-outbox/src/lib.rs');
const ports = read('crates/modules/rustok-outbox/src/ports.rs');
...
if (!manifest.includes('[fba.provider]') || !manifest.includes('registry = "contracts/outbox-fba-registry.json"') || !manifest.includes('contract_version = "outbox.relay_control.v1"') || !manifest.includes('context = "rustok_api::ports::PortContext"') || !manifest.includes('error = "rustok_api::ports::PortError"')) fail('manifest FBA provider drift');
