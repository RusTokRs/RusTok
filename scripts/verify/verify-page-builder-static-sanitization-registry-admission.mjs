...
requireValue(
  contract.github_run?.repository === "RusTokRs/RusTok" &&
    ...
    contract.github_run?.run_repository_must_equal_required_repository === true &&
...
requireValue(
  contract.validation?.unit_test_command ===
      "node --test scripts/evidence/admit-page-builder-static-sanitization-registry-update.test.mjs" &&
    contract.validation?.source_verifier_command ===
      "node scripts/verify/verify-page-builder-static-sanitization-registry-admission.mjs" &&
    contract.validation?.cargo_required === false &&
    contract.validation?.database_required === false &&
    contract.validation?.browser_required === false &&
    contract.validation?.network_required === false,
  `${files.contract}: validation boundary drifted`,
);
requireValue(
...
for (const marker of [
  ...
  'run?.repository?.full_name === requirements.repository',
...
for (const marker of [
  ...
  "rejects workflow repository drift",
