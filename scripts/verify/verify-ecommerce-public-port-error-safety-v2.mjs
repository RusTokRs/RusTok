...
for (const value of [
  ...
  forbidText(customer, value, 'customer payload diagnostics');
}
...
for (const value of [
  ...
  forbidText(inventory, value, 'inventory payload diagnostics');
}
...
for (const [source, value, label] of [
  ...
  [customer, 'actor_kind = context_facts.actor_kind', 'customer actor kind logging'],
  [customer, 'claim_count = context_facts.claim_count', 'customer claim count logging'],
  [customer, 'role_count = context_facts.role_count', 'customer role count logging'],
  ...
  [customer, 'error_message_present = !error.message.is_empty()', 'customer admission message shape'],
  [customer, 'search_present = request_facts.search_present', 'customer list request shape'],
  [customer, 'search_length = ?request_facts.search_length', 'customer search length shape'],
  [customer, 'boundary = CUSTOMER_READ_PORT_BOUNDARY', 'customer read boundary'],
  [customer, 'operation = owner_operation', 'customer owner operation logging'],
  [customer, '"customer.context_invalid"', 'customer context stable code'],
  [customer, '"customer.page_invalid"', 'customer page stable code'],
  [customer, '"customer.per_page_invalid"', 'customer page-size stable code'],
  [customer, '"customer.database_unavailable"', 'customer database stable code'],
  [customer, '"customer.validation"', 'customer validation stable code'],
  [customer, '"customer.profile_unavailable"', 'customer profile stable code'],
  ...
  [inventory, 'actor_kind = context_facts.actor_kind', 'inventory actor kind logging'],
  [inventory, 'claim_count = context_facts.claim_count', 'inventory claim count logging'],
  [inventory, 'role_count = context_facts.role_count', 'inventory role count logging'],
  ...
  [inventory, 'text_field_count = error_facts.text_field_count', 'inventory error text shape'],
  [inventory, 'uuid_field_count = error_facts.uuid_field_count', 'inventory error UUID shape'],
  [inventory, 'numeric_field_count = error_facts.numeric_field_count', 'inventory numeric error shape'],
  [inventory, 'opaque_payload_present = error_facts.opaque_payload_present', 'inventory opaque payload shape'],
  ...
  [inventory, '"inventory.context_invalid"', 'inventory context stable code'],
  [inventory, '"inventory.database_unavailable"', 'inventory database stable code'],
  [inventory, '"inventory.variant_not_found"', 'inventory not-found stable code'],
  [inventory, '"inventory.insufficient_inventory"', 'inventory conflict stable code'],
  [inventory, '"inventory.validation"', 'inventory validation stable code'],
  [inventory, '"inventory.invariant_violation"', 'inventory invariant stable code'],
