...
for (const [value, label] of [
  ...
  ['public_retryable = retryable', 'public retryability'],
  ['tracing::error!(', 'technical severity'],
  ['tracing::warn!(', 'ordinary severity'],
  ['BoundaryError::Public {', 'direct stable boundary result'],
]) {
...
if (sourceConstructors.length !== 2) {
  failures.push(`expected exactly two facade constructors in query source, found ${sourceConstructors.length}`);
}
...
for (const value of [
  ...
  forbidText(optionLookup + optionList + portMapper, value, 'shipping-option query port boundary');
}
...
console.log(
  '✔ Commerce GraphQL shipping-option queries use the fulfillment read port with retained context while legacy fulfillment lifecycle reads remain facade-isolated',
);
