...
usage() {
    ...
    echo "  taxonomy-ownership-boundary-self-test  Verify Taxonomy ownership guard fixtures"
    echo "  taxonomy-ownership-boundary  Verify Taxonomy/consumer storage ownership"
    ...
    echo "  marketplace-registry-freshness  Verify per-registry operator freshness transport and UI parity"
    echo "  worker-runtime-policy  Verify bounded admission, graceful shutdown and subprocess cancellation"
...
SCRIPTS=(
    ...
    "verify-taxonomy-ownership-boundary-self-test.mjs:Taxonomy Ownership Boundary Fixtures"
    "verify-taxonomy-ownership-boundary.mjs:Taxonomy Ownership Boundary"
