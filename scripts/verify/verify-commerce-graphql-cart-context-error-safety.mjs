...
const source = read('crates/modules/rustok-commerce/src/graphql/mutations/cart.rs');
const ownerSource = read('crates/modules/rustok-commerce/src/services/context.rs');
const failures = [];
...
for (const [value, label] of [
  ...
  ['StoreContextError::TenantBoundary { .. }', 'tenant boundary mapping'],
  ...
  ['"STORE_CONTEXT_RESOLUTION_FAILED"', 'owner boundary code'],
  ['"STORE_CONTEXT_TEMPORARILY_UNAVAILABLE"', 'temporary code'],
  ['"tenant_not_found"', 'tenant not-found error kind'],
...
for (const [value, label] of [
  ...
  ['TenantBoundary { code: String, message: String }', 'owner tenant boundary variant'],
