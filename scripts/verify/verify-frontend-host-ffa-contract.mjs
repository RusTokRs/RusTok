...
function assertExists(relativePath) {
  if (!existsSync(repoPath(relativePath)))
    fail(`${relativePath}: expected file to exist`);
}
...
const docs = [
  ...
  "crates/modules/rustok-events-module/next-admin/src/index.ts",
...
const storefrontHeaderCore = readRepo(
  "apps/storefront/src/widgets/header/core.rs",
);
const storefrontHeader = readRepo("apps/storefront/src/widgets/header/mod.rs");
const storefrontEnabledModulesContext = readRepo(
  "apps/storefront/src/shared/context/enabled_modules.rs",
);
const storefrontEnabledModulesAdapter = readRepo(
  "apps/storefront/src/shared/context/enabled_modules_native_server_adapter.rs",
);
const storefrontCanonicalRouteContext = readRepo(
  "apps/storefront/src/shared/context/canonical_route.rs",
);
const storefrontCanonicalRouteAdapter = readRepo(
  "apps/storefront/src/shared/context/canonical_route_native_server_adapter.rs",
);
const storefrontSeoPageContext = readRepo(
  "apps/storefront/src/shared/context/seo_page_context.rs",
);
const storefrontSeoPageContextAdapter = readRepo(
  "apps/storefront/src/shared/context/seo_page_context_native_server_adapter.rs",
);
const adminWorkflowMod = readRepo("apps/admin/src/features/workflow/mod.rs");
const adminWorkflowModel = readRepo(
  "apps/admin/src/features/workflow/model.rs",
);
const adminWorkflowTransport = readRepo(
  "apps/admin/src/features/workflow/transport/mod.rs",
);
const adminWorkflowGraphqlAdapter = readRepo(
  "apps/admin/src/features/workflow/transport/graphql_adapter.rs",
);
const adminWorkflowNativeAdapter = readRepo(
  "apps/admin/src/features/workflow/transport/native_server_adapter.rs",
);
const adminOauthAppsMod = readRepo("apps/admin/src/features/oauth_apps/mod.rs");
const adminOauthAppsModel = readRepo(
  "apps/admin/src/features/oauth_apps/model.rs",
);
const adminOauthAppsTransport = readRepo(
  "apps/admin/src/features/oauth_apps/transport/mod.rs",
);
const adminOauthAppsGraphqlAdapter = readRepo(
  "apps/admin/src/features/oauth_apps/transport/graphql_adapter.rs",
);
const adminOauthAppsNativeAdapter = readRepo(
  "apps/admin/src/features/oauth_apps/transport/native_server_adapter.rs",
);
const adminInstallerMod = readRepo("apps/admin/src/features/installer/mod.rs");
const adminInstallerModel = readRepo(
  "apps/admin/src/features/installer/model.rs",
);
const adminInstallerTransport = readRepo(
  "apps/admin/src/features/installer/transport/mod.rs",
);
...
const adminCacheTransport = readRepo(
  "apps/admin/src/features/cache/transport/mod.rs",
);
const adminCacheNativeAdapter = readRepo(
  "apps/admin/src/features/cache/transport/native_server_adapter.rs",
);
...
const adminDashboardModel = readRepo(
  "apps/admin/src/features/dashboard/model.rs",
);
const adminDashboardTransport = readRepo(
  "apps/admin/src/features/dashboard/transport/mod.rs",
);
const adminDashboardNativeAdapter = readRepo(
  "apps/admin/src/features/dashboard/transport/native_server_adapter.rs",
);
...
const adminEmailTransport = readRepo(
  "apps/admin/src/features/email/transport/mod.rs",
);
const adminEmailNativeAdapter = readRepo(
  "apps/admin/src/features/email/transport/native_server_adapter.rs",
);
...
const adminModulesTransport = readRepo(
  "apps/admin/src/features/modules/transport/mod.rs",
);
const adminModulesTransportClient = readRepo(
  "apps/admin/src/features/modules/transport/client.rs",
);
const adminModulesNativeAdapter = readRepo(
  "apps/admin/src/features/modules/transport/native_server_adapter.rs",
);
...
const nextAdminSharedApiIndex = readRepo(
  "apps/next-admin/src/shared/api/index.ts",
);
...
const nextFrontendPlan = readRepo(
  "apps/next-frontend/docs/implementation-plan.md",
);
const nextFrontendBlogEntrypoint = readRepo(
  "apps/next-frontend/packages/rustok-blog/src/index.tsx",
);
const nextFrontendBlogPosts = readRepo(
  "apps/next-frontend/packages/rustok-blog/src/api/posts.ts",
);
...
const nextFrontendModulesIndex = readRepo(
  "apps/next-frontend/src/modules/index.ts",
);
const nextFrontendRegistry = readRepo(
  "apps/next-frontend/src/modules/registry.ts",
);
...
for (const marker of [
  ...
  if (adminShellCore.includes(marker)) {
    fail(
      `apps/admin/src/widgets/app_shell/core.rs: host core must stay Leptos-free (${marker})`,
    );
  }
  if (storefrontHeaderCore.includes(marker)) {
    fail(
      `apps/storefront/src/widgets/header/core.rs: host core must stay Leptos-free (${marker})`,
    );
  }
}
...
assertContains(
  ...
  "apps/admin/src/widgets/app_shell/header.rs: global search must select the native transport path",
);
assertContains(
  adminHeader,
  "UiTransportPath::Graphql",
  "apps/admin/src/widgets/app_shell/header.rs: global search must keep the GraphQL transport path",
);
assertNotContains(
  adminHeader,
  "Err(_) => request",
  "apps/admin/src/widgets/app_shell/header.rs: global search must not fall back from native to GraphQL",
);
assertContains(
  storefrontHeader,
  "build_header_links",
...
for (const [contextPath, contextSource, adapterPath, adapterSource] of [
  ...
  assertContains(
    contextSource,
    "UiTransportPath",
    `${contextPath}: context must select its transport path`,
  );
  assertNotContains(
    contextSource,
    "#[server",
    `${contextPath}: server functions belong in its native adapter`,
  );
  assertContains(
    adapterSource,
    "#[server",
    `${adapterPath}: native adapter must own its server function`,
  );
}
...
assertContains(
  storefrontEnabledModulesAdapter,
  "storefront/list-enabled-modules",
  "storefront enabled-modules adapter must own its server endpoint",
);
assertContains(
  storefrontCanonicalRouteAdapter,
  "storefront/resolve-canonical-route",
  "storefront canonical-route adapter must own its server endpoint",
);
assertContains(
  storefrontSeoPageContextAdapter,
  "storefront/seo-page-context",
  "storefront SEO adapter must own its server endpoint",
);
...
assertContains(
  adminWorkflowMod,
  "pub mod model;",
  "apps/admin/src/features/workflow/mod.rs: workflow host feature must wire model",
);
assertContains(
  adminWorkflowMod,
  "pub mod transport;",
  "apps/admin/src/features/workflow/mod.rs: workflow host feature must wire transport facade",
);
assertNotContains(
  adminWorkflowMod,
  "pub mod api;",
  "apps/admin/src/features/workflow/mod.rs: workflow host feature must not wire api facade",
);
...
for (const marker of ["leptos::", "leptos_", "#[component]", "#[server]"]) {
  assertNotContains(
    adminWorkflowModel,
    marker,
    `apps/admin/src/features/workflow/model.rs: workflow model must stay framework/server-function free (${marker})`,
  );
}
...
for (const [relativePath, source] of workflowHostCallers) {
  assertContains(
    source,
    /workflow::(?:transport|\{[\s\S]*transport)/,
    `${relativePath}: workflow host caller must use the transport facade`,
  );
  assertNotContains(
    source,
    "workflow::api",
    `${relativePath}: workflow host caller must not use the removed api facade`,
  );
  assertNotContains(
    source,
    "native_server_adapter::",
    `${relativePath}: workflow host caller must not call native adapter directly`,
  );
  assertNotContains(
    source,
    "graphql_adapter::",
    `${relativePath}: workflow host caller must not call GraphQL adapter directly`,
  );
}
...
assertContains(
  adminOauthAppsMod,
  "pub mod model;",
  "apps/admin/src/features/oauth_apps/mod.rs: OAuth apps host feature must wire model",
);
assertContains(
  adminOauthAppsMod,
  "pub mod transport;",
  "apps/admin/src/features/oauth_apps/mod.rs: OAuth apps host feature must wire transport facade",
);
assertNotContains(
  adminOauthAppsMod,
  "pub mod api;",
  "apps/admin/src/features/oauth_apps/mod.rs: OAuth apps host feature must not wire api facade",
);
...
for (const marker of ["leptos::", "leptos_", "#[component]", "#[server]"]) {
  assertNotContains(
    adminOauthAppsModel,
    marker,
    `apps/admin/src/features/oauth_apps/model.rs: OAuth apps model must stay framework/server-function free (${marker})`,
  );
}
...
for (const [relativePath, source] of oauthAppsHostCallers) {
  assertContains(
    source,
    /oauth_apps::(?:transport|\{[\s\S]*transport)/,
    `${relativePath}: OAuth apps host caller must use the transport facade`,
  );
  assertNotContains(
    source,
    "oauth_apps::api",
    `${relativePath}: OAuth apps host caller must not use the removed api facade`,
  );
  assertNotContains(
    source,
    "native_server_adapter::",
    `${relativePath}: OAuth apps host caller must not call native adapter directly`,
  );
  assertNotContains(
    source,
    "graphql_adapter::",
    `${relativePath}: OAuth apps host caller must not call GraphQL adapter directly`,
  );
  assertNotContains(
    source,
    "crate::shared::api::request",
    `${relativePath}: OAuth apps host caller must not execute raw GraphQL requests directly`,
  );
}
...
assertContains(
  adminInstallerMod,
  "pub mod model;",
  "apps/admin/src/features/installer/mod.rs: installer host feature must wire model",
);
assertContains(
  adminInstallerMod,
  "pub mod transport;",
  "apps/admin/src/features/installer/mod.rs: installer host feature must wire transport facade",
);
assertNotContains(
  adminInstallerMod,
  "pub mod api;",
  "apps/admin/src/features/installer/mod.rs: installer host feature must not wire api facade",
);
...
for (const marker of ["leptos::", "leptos_", "#[component]", "#[server]"]) {
  assertNotContains(
    adminInstallerModel,
    marker,
    `apps/admin/src/features/installer/model.rs: installer model must stay framework/server-function free (${marker})`,
  );
  assertNotContains(
    adminInstallerTransport,
    marker,
    `apps/admin/src/features/installer/transport/mod.rs: installer transport must stay framework/server-function free (${marker})`,
  );
}
...
assertContains(
  adminCacheMod,
  "pub mod model;",
  "apps/admin/src/features/cache/mod.rs: cache host feature must wire a model",
);
assertContains(
  adminCacheMod,
  "pub mod transport;",
  "apps/admin/src/features/cache/mod.rs: cache host feature must wire a transport facade",
);
for (const marker of ["leptos::", "leptos_", "#[component]", "#[server]"]) {
  assertNotContains(
    adminCacheModel,
    marker,
    `apps/admin/src/features/cache/model.rs: cache model must stay framework/server-function free (${marker})`,
  );
}
assertContains(
  adminCacheTransport,
  "UiTransportPath::NativeServer",
  "apps/admin/src/features/cache/transport/mod.rs: cache transport must select the native path",
);
assertContains(
  adminCacheTransport,
  "UiTransportPath::Graphql",
  "apps/admin/src/features/cache/transport/mod.rs: cache transport must keep the GraphQL path",
);
assertNotContains(
  adminCacheTransport,
  "#[server",
  "apps/admin/src/features/cache/transport/mod.rs: server functions belong in native_server_adapter.rs",
);
assertContains(
  adminCacheNativeAdapter,
  "#[server",
  "apps/admin/src/features/cache/transport/native_server_adapter.rs: native adapter must own server-function endpoints",
);
assertContains(
  adminCachePage,
  "transport::fetch_cache_health",
  "apps/admin/src/pages/cache.rs: cache page must use the transport facade",
);
for (const marker of [
  "CACHE_HEALTH_QUERY",
  "crate::shared::api::{request",
  "native_server_adapter::cache_health_native",
]) {
  assertNotContains(
    adminCachePage,
    marker,
    `apps/admin/src/pages/cache.rs: cache page must not own raw transport (${marker})`,
  );
}
...
assertContains(
  adminDashboardMod,
  "pub mod model;",
  "apps/admin/src/features/dashboard/mod.rs: dashboard host feature must wire a model",
);
assertContains(
  adminDashboardMod,
  "pub mod transport;",
  "apps/admin/src/features/dashboard/mod.rs: dashboard host feature must wire a transport facade",
);
for (const marker of ["leptos::", "leptos_", "#[component]", "#[server]"]) {
  assertNotContains(
    adminDashboardModel,
    marker,
    `apps/admin/src/features/dashboard/model.rs: dashboard model must stay framework/server-function free (${marker})`,
  );
}
assertContains(
  adminDashboardTransport,
  "UiTransportPath::NativeServer",
  "apps/admin/src/features/dashboard/transport/mod.rs: dashboard transport must select the native path",
);
assertContains(
  adminDashboardTransport,
  "UiTransportPath::Graphql",
  "apps/admin/src/features/dashboard/transport/mod.rs: dashboard transport must keep the GraphQL path",
);
assertContains(
  adminDashboardNativeAdapter,
  "#[server",
  "apps/admin/src/features/dashboard/transport/native_server_adapter.rs: native adapter must own server-function endpoints",
);
for (const marker of [
  "transport::fetch_dashboard_stats",
  "transport::fetch_recent_activity",
]) {
  assertContains(
    adminDashboardPage,
    marker,
    `apps/admin/src/pages/dashboard.rs: dashboard page must use ${marker}`,
  );
}
for (const marker of [
  "DASHBOARD_STATS_QUERY",
  "RECENT_ACTIVITY_QUERY",
  "load_period_count_snapshot",
  ...
  assertNotContains(
    adminDashboardPage,
    marker,
    `apps/admin/src/pages/dashboard.rs: dashboard page must not own raw transport/runtime helpers (${marker})`,
  );
}
...
assertContains(
  adminEmailMod,
  "pub mod model;",
  "apps/admin/src/features/email/mod.rs: email host feature must wire a model",
);
assertContains(
  adminEmailMod,
  "pub mod transport;",
  "apps/admin/src/features/email/mod.rs: email host feature must wire a transport facade",
);
for (const marker of ["leptos::", "leptos_", "#[component]", "#[server]"]) {
  assertNotContains(
    adminEmailModel,
    marker,
    `apps/admin/src/features/email/model.rs: email model must stay framework/server-function free (${marker})`,
  );
}
assertContains(
  adminEmailTransport,
  "UiTransportPath::NativeServer",
  "apps/admin/src/features/email/transport/mod.rs: email transport must select the native read path",
);
assertContains(
  adminEmailTransport,
  "UiTransportPath::Graphql",
  "apps/admin/src/features/email/transport/mod.rs: email transport must keep the GraphQL read path",
);
assertNotContains(
  adminEmailTransport,
  "#[server",
  "apps/admin/src/features/email/transport/mod.rs: server functions belong in native_server_adapter.rs",
);
assertContains(
  adminEmailNativeAdapter,
  "#[server",
  "apps/admin/src/features/email/transport/native_server_adapter.rs: native adapter must own server-function endpoints",
);
for (const marker of [
  "transport::fetch_email_settings",
  "transport::update_email_settings",
]) {
  assertContains(
    adminEmailPage,
    marker,
    `apps/admin/src/pages/email_settings.rs: email page must use ${marker}`,
  );
}
for (const marker of [
  "PLATFORM_SETTINGS_QUERY",
  "UPDATE_PLATFORM_SETTINGS_MUTATION",
  "native_server_adapter::email_settings_native",
]) {
  assertNotContains(
    adminEmailPage,
    marker,
    `apps/admin/src/pages/email_settings.rs: email page must not own raw transport (${marker})`,
  );
}
...
assertContains(
  adminModulesMod,
  "pub mod transport;",
  "apps/admin/src/features/modules/mod.rs: modules host feature must wire transport boundary",
);
assertNotContains(
  adminModulesMod,
  "pub mod api;",
  "apps/admin/src/features/modules/mod.rs: modules host feature must not wire api boundary",
);
assertContains(
  adminModulesTransport,
  "pub mod client;",
  "apps/admin/src/features/modules/transport/mod.rs: modules transport must expose client helpers",
);
assertContains(
  adminModulesTransport,
...
for (const [relativePath, source] of modulesHostCallers) {
  assertContains(
    source,
    /modules::(?:transport|\{[\s\S]*transport)/,
    `${relativePath}: modules host caller must use the transport boundary`,
  );
  assertNotContains(
    source,
    "modules::api",
    `${relativePath}: modules host caller must not use the removed api boundary`,
  );
}
