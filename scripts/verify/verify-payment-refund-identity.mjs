...
requireMarker(rest, 'headers.get("idempotency-key")', `${restPath}: REST refund must require Idempotency-Key`);
requireMarker(rest, ".create_refund_idempotent(", `${restPath}: REST refund must use explicit idempotent API`);
requireMarker(graphql, "idempotency_key: String", `${graphqlPath}: GraphQL refund must require idempotencyKey`);
requireMarker(graphql, ".create_refund_idempotent(", `${graphqlPath}: GraphQL refund must use explicit idempotent API`);
requireMarker(graphqlReturn, 'format!("order_return:{return_id}:refund")', `${graphqlReturnPath}: GraphQL return refund identity drift`);
requireMarker(adminReturn, 'format!("order_return:{id}:refund")', `${adminReturnPath}: admin return refund identity drift`);
forbidMarker(rest, ".create_refund(tenant.id, id, input)", `${restPath}: REST must not call legacy refund creation`);
forbidMarker(graphql, ".create_refund(\n", `${graphqlPath}: GraphQL must not call legacy refund creation`);
requireMarker(plan, "refund `creation_key`", `${planPath}: main plan must track refund creation identity`);
requireMarker(plan, "verify-payment-refund-identity.mjs", `${planPath}: main plan must list refund identity verifier`);
