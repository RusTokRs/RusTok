...
const findScale = (comparison, scale) => {
  const result = comparison.scales.find((item) => item.scale === scale);
  if (!result) fail(`comparison is missing ${scale} evidence`);
  return result;
};
...
const validateComparison = (comparison) => {
  ...
  if (!Array.isArray(comparison.scales)) fail('comparison.scales must be an array');
  ...
  return { lower, upper, commit: lowerCommit };
};
...
const validateDecision = (decision, comparisonCommit) => {
  ...
  if (JSON.stringify(keys) !== JSON.stringify([...rejected].sort())) {
...
const storageRows = (lower, upper, comparison) => prototypes.map((prototype) => {
  ...
  const ratio = comparison.cross_scale_ratios?.prototypes?.find((item) => item.prototype === prototype);
  return {
    prototype,
    schema100k: read100k.schema_bytes,
    schema1m: read1m.schema_bytes,
    ...
    churn100k: maintenance100k.churn_growth_percent,
    churn1m: maintenance1m.churn_growth_percent,
    vacuum100k: maintenance100k.vacuum_duration_ms,
    vacuum1m: maintenance1m.vacuum_duration_ms,
  };
});

const readRows = (lower, upper, comparison) => {
  const rows = [];
  for (const prototype of prototypes) {
    const read100k = findPrototype(lower, 'read', prototype);
    const read1m = findPrototype(upper, 'read', prototype);
    const ratioPrototype = comparison.cross_scale_ratios?.prototypes?.find((item) => item.prototype === prototype);
    for (const workload100k of read100k.workloads) {
      ...
      rows.push({
        ...
        warm100k: workload100k.warm_median_execution_ms,
        warm1m: workload1m.warm_median_execution_ms,
        ratio: ratio?.warm_execution_ratio_1m_to_100k,
        plans100k: workload100k.plan_shape_variants,
        plans1m: workload1m.plan_shape_variants,
      });
    }
  }
  return rows;
...
const mutationRows = (lower, upper, comparison) => {
  const rows = [];
  for (const prototype of prototypes) {
    ...
    for (const workload100k of mutation100k.workloads) {
      ...
      rows.push({
        prototype,
        workload: workload100k.name,
        execution100k: workload100k.median_execution_ms,
        execution1m: workload1m.median_execution_ms,
        executionRatio: ratio?.execution_ratio_1m_to_100k,
        wal100k: workload100k.median_maximum_node_wal_bytes,
        wal1m: workload1m.median_maximum_node_wal_bytes,
        walRatio: ratio?.wal_bytes_ratio_1m_to_100k,
      });
    }
  }
  return rows;
...
const render = (comparison, decision, comparisonPath) => {
  const { lower, upper, commit } = validateComparison(comparison);
  validateDecision(decision, commit);
  const storage = storageRows(lower, upper, comparison);
  const reads = readRows(lower, upper, comparison);
  const mutations = mutationRows(lower, upper, comparison);
  const rejected = prototypes.filter((prototype) => prototype !== decision.selected_prototype);
  const lines = [
...
const markdown = render(comparison, decision, args.comparison);
