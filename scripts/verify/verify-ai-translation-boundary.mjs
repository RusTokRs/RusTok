...
const files = {
  ...
  aiNativeAdmin: 'crates/rustok-ai/admin/src/transport/native_server_adapter.rs',
  ...
  translationMachineEntity: 'crates/rustok-translation/src/entities/machine_operation.rs',
  ...
  translationMachineMigration: 'crates/rustok-translation/src/migrations/m20260729_000009_create_translation_machine_operations.rs',
  translationGraphqlMutation: 'crates/rustok-translation/src/graphql/mutation.rs',
  translationNativeAdmin: 'crates/rustok-translation/admin/src/transport/native_server_adapter.rs',
...
const translationMemoryBindingEntity = read(files.translationMemoryBindingEntity);
...
]) requireText(aiStructuredResult, marker, 'AI encrypted structured-result handoff');
for (const marker of [
...
]) requireText(aiGraphqlMutation, marker, 'AI structured-accounting GraphQL writes');
for (const marker of [
...
]) requireText(translationMachineEntity, marker, 'Translation machine operation entity');
for (const marker of [
  ...
  'unit_ordinal',
  'score_basis_points',
]) requireText(translationMemoryBindingEntity, marker, 'Translation machine memory binding entity');
for (const marker of [
  ...
  'idempotency_key',
  'request_hash',
]) requireText(translationCancellationEntity, marker, 'Translation machine cancellation entity');
for (const marker of [
...
]) requireText(translationMachineMigration, marker, 'Translation machine operation migration');
requireText(
...
for (const marker of ['rustok_ai', 'AiStructuredTaskPort', 'AiManagementService']) {
...
]) requireText(distributionSource, marker, 'AI Translation distribution composition');
requireText(
  serverComposition,
