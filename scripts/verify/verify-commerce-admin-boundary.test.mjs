...
function fixture(options = {}) {
  ...
  put(root, "crates/modules/rustok-commerce/admin/src/lib.rs", `${options.legacyModApi ? "mod api;\n" : ""}mod core;\nmod transport;\nmod ui;\npub use ui::CommerceAdmin;\n`);
  put(root, "crates/modules/rustok-commerce/admin/src/core/mod.rs", `${options.leptosCore ? "use leptos::prelude::*;" : ""}\npub fn build_shipping_profile_form_snapshot() {}\n`);
  put(root, "crates/modules/rustok-commerce/admin/src/ui/leptos.rs", `use crate::transport;\npub fn render() { let _ = transport::fetch_bootstrap; ${options.rawUi ? "let _ = api::fetch_bootstrap;" : ""} }\n`);
  put(root, "crates/modules/rustok-commerce/admin/src/transport/mod.rs", "mod order_change;\nmod promotion;\nmod native_server_adapter;\nmod shipping_profile;\npub use shipping_profile::fetch_bootstrap;\n");
  put(root, "crates/modules/rustok-commerce/admin/src/transport/shipping_profile.rs", "use super::native_server_adapter::{self, ApiError};\npub async fn fetch_bootstrap() -> Result<(), ApiError> { native_server_adapter::fetch_bootstrap().await }\n");
  put(root, "crates/modules/rustok-commerce/admin/src/transport/promotion.rs", "use super::native_server_adapter::{self, ApiError};\npub async fn preview_cart_promotion() -> Result<(), ApiError> { native_server_adapter::fetch_bootstrap().await }\n");
  put(root, "crates/modules/rustok-commerce/admin/src/transport/order_change.rs", "use super::native_server_adapter::{self, ApiError};\npub async fn fetch_order_changes() -> Result<(), ApiError> { native_server_adapter::fetch_bootstrap().await }\n");
  put(root, "crates/modules/rustok-commerce/admin/src/transport/native_server_adapter.rs", "use \npub enum ApiError { ServerFn(String) }\n#[server]\npub async fn fetch_bootstrap() -> Result<(), ApiError> { Ok(()) }\n");
  put(root, "crates/modules/rustok-commerce/src/lib.rs", "pub mod graphql;\npub mod state_machine;\n");
  put(root, "crates/modules/rustok-commerce/src/graphql/mutations/provider_operations.rs", `
  ...
  `);
  put(root, "crates/modules/rustok-commerce/src/graphql/mutations/fulfillment.rs", `
  ...
  `);
  put(root, "crates/modules/rustok-commerce/src/graphql_runtime.rs", `
  ...
  `);
  put(root, "crates/modules/rustok-commerce/src/services/fulfillment_orchestration_facade.rs", `
  ...
  put(root, "crates/modules/rustok-commerce/src/controllers/admin/changes.rs", `
    OrderChangeOrchestrationService::from_order_ports();
    runtime.order_read_port();
    runtime.order_post_order_command_port();
    service.apply_order_change_with_owner_ports();
  `);
  put(root, "crates/modules/rustok-commerce/src/services/order_change_orchestration.rs", `
  ...
  put(root, "crates/modules/rustok-commerce/docs/implementation-plan.md", "verify-commerce-admin-boundary.mjs admin/src/transport/native_server_adapter.rs root GraphQL and state-machine aliases");
