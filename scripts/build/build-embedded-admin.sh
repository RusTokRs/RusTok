...
usage() {
  cat <<'USAGE'
Usage: build-embedded-admin.sh [--tool-root DIR] [--target-dir DIR] [--skip-tool-install]
USAGE
}
...
grep -Fq '/admin/' "$index" || {
  echo "embedded admin output does not contain the /admin/ public URL" >&2
  exit 1
}

echo "✔ built deterministic embedded admin assets with Trunk 0.21.14"
