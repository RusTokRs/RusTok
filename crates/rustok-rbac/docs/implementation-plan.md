...
- Fixed in this pass: `draft PR #2843 removes the public permission-resolver mutation surface, the obsolete RoleAssignmentStore owner contract and the server-owned direct-persistence adapter. Runtime permission resolution is now read-only, while the remaining low-level assignment helper is test-only and existing-user production mutations stay on transaction-owned or committed RBAC entry points. A fail-closed source guard prevents the removed composition from returning.`
- Remaining risks or blockers: `#2843 is source-ready but unexecuted. Draft #2847 remains unmerged and lacks its repository-generated digest. Draft #2863 contains the Auth admin effective-no-op correction but is not present in main or this branch. The #2849, #2853, #2856 and #2862 packets remain source-only. Same-SHA formatting, API/Events/telemetry/RBAC/Admin/server/CLI compilation, focused Rust/Node/module gates, live negative transports, runtime incident evidence and FFA/FBA management evidence remain absent. Issue #2740 still blocks the known Rust-host path before the server build.`
- Evidence: `source review confirms PermissionResolver and RuntimePermissionResolver now expose lookup only; RoleAssignmentStore and ServerRoleAssignmentStore are removed; no repository-wide caller was found for the deleted resolver mutation methods; the remaining RbacService assignment helper is cfg(test). The new architecture guard locks the read-only owner and runtime composition. No execution evidence is claimed.`
- Next action: `run exact-head formatting, owner/server compilation and the read-only resolver architecture guard for #2843; resolve every branch-local failure; then reconcile #2847 and #2863 additively, generate the #2847 event digest and execute #2862/#2856/#2853/#2849 on one reconciled revision before module promotion`
- Resume command: `cargo fmt --all -- --check && cargo check -p rustok-rbac --all-features && cargo check -p rustok-server --lib && cargo test -p rustok-rbac --all-features && cargo test -p rustok-server --test rbac_permission_resolver_read_only_guard && cargo test -p rustok-server --test rbac_mutation_api_architecture_guard && node scripts/verify/verify-rbac-explicit-principal-kind.mjs && node scripts/verify/verify-rbac-invalidation-observability.mjs && cargo xtask module validate rbac && cargo xtask module test rbac`
