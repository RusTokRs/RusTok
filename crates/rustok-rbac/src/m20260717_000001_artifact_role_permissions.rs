...
/// tenant-composite role/user parents. Database foreign keys provide concurrency-safe
/// parent protection; trigger-only existence checks are intentionally not used.
    ...
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let statements: &[&str] = match manager.get_database_backend() {
            DbBackend::Postgres => &[
                ...
                "CREATE TABLE rbac_artifact_role_permissions (id UUID PRIMARY KEY, tenant_id UUID NOT NULL, role_id UUID NOT NULL, artifact_permission_id UUID NOT NULL, granted_by_actor_id UUID NOT NULL, granted_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT fk_rbac_artifact_grant_role FOREIGN KEY (tenant_id, role_id) REFERENCES roles (tenant_id, id) ON UPDATE RESTRICT ON DELETE RESTRICT, CONSTRAINT fk_rbac_artifact_grant_actor FOREIGN KEY (tenant_id, granted_by_actor_id) REFERENCES users (tenant_id, id) ON UPDATE RESTRICT ON DELETE RESTRICT, CONSTRAINT fk_rbac_artifact_grant_permission FOREIGN KEY (artifact_permission_id) REFERENCES rbac_artifact_permission_definitions (id) ON UPDATE RESTRICT ON DELETE RESTRICT, UNIQUE (tenant_id, role_id, artifact_permission_id))",
                ...
                "CREATE TABLE rbac_artifact_role_permission_operations (id UUID PRIMARY KEY, tenant_id UUID NOT NULL, idempotency_key TEXT NOT NULL, role_id UUID NOT NULL, artifact_permission_id UUID NOT NULL, actor_id UUID NOT NULL, granted BOOLEAN NOT NULL, applied_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT fk_rbac_artifact_operation_role FOREIGN KEY (tenant_id, role_id) REFERENCES roles (tenant_id, id) ON UPDATE RESTRICT ON DELETE RESTRICT, CONSTRAINT fk_rbac_artifact_operation_actor FOREIGN KEY (tenant_id, actor_id) REFERENCES users (tenant_id, id) ON UPDATE RESTRICT ON DELETE RESTRICT, CONSTRAINT fk_rbac_artifact_operation_permission FOREIGN KEY (artifact_permission_id) REFERENCES rbac_artifact_permission_definitions (id) ON UPDATE RESTRICT ON DELETE RESTRICT, UNIQUE (tenant_id, idempotency_key))",
            ],
            DbBackend::Sqlite => &[
                "CREATE UNIQUE INDEX IF NOT EXISTS uq_rbac_roles_tenant_id_id ON roles (tenant_id, id)",
                "CREATE UNIQUE INDEX IF NOT EXISTS uq_rbac_users_tenant_id_id ON users (tenant_id, id)",
