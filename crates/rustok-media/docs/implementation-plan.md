...
- Last checkpoint: FFA admin slice расширил Leptos-free presentation/error policy: list-card view-model теперь формирует size/dimensions/storage display вне Leptos, а context-error prefix formatting вынесен в `media_admin_context_error` без изменения transport parity.
