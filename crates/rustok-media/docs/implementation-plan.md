...
- Last checkpoint: REST upload/list/get/delete/translation handlers now accept narrow `MediaHttpRuntime` with explicit DB/storage handles; current Loco `AppContext` is isolated in the route-state adapter until the full Axum cutover. GraphQL field `mediaUsage` and DTO `MediaUsageStats` moved from `apps/server::SystemQuery` to `rustok-media::graphql::MediaQuery`; server is left only as a schema composition point. Boundary is locked by `apps/server/tests/module_surface_boundary_guard.rs` without compilation.
...
- Evidence:
  ...
  - `admin/src/transport/` owns the current native-first + GraphQL fallback + REST upload transport facade without changing external GraphQL/REST contracts; facade split is locked through `graphql_adapter.rs`, `rest_adapter.rs` and `native_server_adapter.rs`;
