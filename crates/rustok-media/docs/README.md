...
- module-owned admin UI package `rustok-media-admin` with FFA split `core`/`transport`/`ui/leptos`;
...
- upload remains a REST-first path, GraphQL is preserved for read/mutation flows without multipart extension, and the Leptos admin adapter calls the transport facade instead of the raw API module; the transport facade inside the admin package splits native server functions, GraphQL fallback and REST upload adapters, while upload/detail presentation state remains in Leptos-free `admin/src/core.rs`;
