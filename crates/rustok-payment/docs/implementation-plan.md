...
- Last checkpoint: Provider SPI baseline added `src/providers.rs` with manual provider descriptor/capabilities plus authorize/capture/cancel/refund adapter contract, and the FBA registry now records and verifies `provider_spi` metadata/source markers without moving lifecycle persistence out of `PaymentService`.
