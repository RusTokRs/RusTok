...
- Last checkpoint: Provider SPI baseline added `src/providers.rs` with manual provider descriptor/capabilities plus authorize/capture/cancel/refund adapter contract, and the FBA registry now records and verifies `provider_spi` metadata/source markers, module-owned SPI source path and `PaymentService` lifecycle ownership without moving persistence into transport.
