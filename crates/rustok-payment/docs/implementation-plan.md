...
- Evidence:
  ...
  - provider SPI baseline теперь опубликован в `src/providers.rs`: manual descriptor/capabilities, transport-neutral operation request/result и adapter trait для `authorize/capture/cancel/refund`; persisted lifecycle transitions остаются в `PaymentService`, а registry metadata фиксирует `provider_spi.status = manual_baseline_locked`;
