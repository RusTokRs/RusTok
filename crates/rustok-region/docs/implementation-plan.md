...
- Evidence:
  ...
  - admin transport profile locked as native-only single-adapter state: `admin/src/transport/mod.rs` delegates to native server-function endpoints from `admin/src/transport/native_server_adapter.rs`; GraphQL/REST admin selected path not yet declared, and legacy `admin/src/api.rs` removed;
