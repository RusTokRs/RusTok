...
- transport endpoint shell и GraphQL surface пока публикуются через `rustok-commerce`, admin UI ownership уже вынесен в `rustok-fulfillment/admin`, а storefront presentation/selection DTO/core/selection-update materialization/transport-fallback ownership — в `rustok-fulfillment/storefront`;
