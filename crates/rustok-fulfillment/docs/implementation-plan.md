...
- Current phase: storefront selection materialization ownership and provider SPI live-adapter evidence
- Last checkpoint: Fulfillment storefront now owns shipping-selection update materialization (`build_shipping_selection_updates`) and typed validation error text; commerce raw native/GraphQL adapters only convert owner updates into cart/GraphQL DTOs and keep checkout orchestration/reprice ownership. Earlier checkpoint: FBA maintenance slice перевёл select-shipping-option write path на shared `PortCallPolicy::write()` и сохранил list path на `PortCallPolicy::read()`.
- Next step: Move the remaining select-shipping-option server-function endpoint shell from commerce compatibility into a fulfillment-owned SSR adapter when host composition can provide the cart-update/reprice callback without introducing a `rustok-fulfillment-storefront -> rustok-commerce` dependency, then move from evidence-only external carrier contract execution to production adapter wiring in host composition.
- Open blockers: None.
...
- Evidence:
  ...
  - storefront shipping-selection plan matches delivery groups only by `shipping_profile_slug + seller_id`; `seller_scope` is no longer accepted as a fallback target identity;
  ...
  - storefront handoff + shipping-selection slice lives in `storefront/src/model.rs`, `storefront/src/core/mod.rs`, `storefront/src/transport.rs` and `storefront/src/ui/leptos.rs` as fulfillment-owned seller-aware delivery-group presentation/normalization plus native-first/GraphQL fallback policy consumed by commerce checkout orchestration; compatibility fallback is now MissingServer-only while the temporary commerce adapter remains, and fast guardrails `scripts/verify/verify-fulfillment-storefront-boundary.mjs` plus `scripts/verify/verify-commerce-storefront-transport-handoff.mjs` validate the owner UI/core/transport split, narrowed fallback policy and aggregate package wiring while commerce temporarily retains the SSR endpoint shell.
  - `storefront/src/transport.rs` now owns shipping-selection update materialization via `build_shipping_selection_updates` plus typed validation error text; commerce raw native/GraphQL adapters consume those owner updates and only map them into cart/GraphQL DTOs before checkout reprice.
