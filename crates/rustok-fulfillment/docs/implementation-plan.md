...
- Last checkpoint: Fulfillment storefront selection transport and delivery-group model DTO no longer expose `seller_scope` in owner command/update/presentation seams or missing-group error context; canonical targeting is only `shipping_profile_slug + seller_id`. Fulfillment storefront still owns shipping-selection update materialization (`build_shipping_selection_updates`) and typed validation error text; commerce raw native/GraphQL adapters only convert owner updates into cart DTOs / GraphQL payloads without sending `sellerScope`, while keeping checkout orchestration/reprice ownership.
