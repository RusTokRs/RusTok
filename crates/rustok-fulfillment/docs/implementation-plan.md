...
- Evidence:
  ...
  - provider SPI baseline теперь опубликован в `src/providers.rs`: manual carrier descriptor/capabilities, transport-neutral rate quote + shipment operation request/result и adapter trait для `quote_rates/create_label/cancel`; persisted lifecycle transitions остаются в `FulfillmentService`, а registry metadata фиксирует `provider_spi.status = manual_baseline_locked`;
