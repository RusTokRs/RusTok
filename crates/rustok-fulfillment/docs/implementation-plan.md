...
- Last checkpoint: Fulfillment storefront owns both shipping-selection transports. `storefront/src/transport/native_server_adapter/server_functions.rs` exposes `fulfillment/select-shipping-option`, validates/materializes owner selection updates and calls `rustok_commerce::storefront_checkout_runtime` with `HostRuntimeContext` DB/event-bus handles, while `storefront/src/transport/graphql_adapter.rs` keeps the parallel public GraphQL mutation fallback. `storefront/src/transport.rs` exposes the shared `execute_selected_transport` `select_shipping_option` facade without a commerce callback, and commerce no longer contains fulfillment GraphQL or native owner-operation wrappers. The storefront package no longer depends on `loco-rs` or `rustok-outbox/loco-adapter`.
...
- Evidence:
  ...
  - storefront handoff + shipping-selection slice lives in `storefront/src/model.rs`, `storefront/src/core/mod.rs`, `storefront/src/transport.rs`, `storefront/src/transport/graphql_adapter.rs`, `storefront/src/transport/native_server_adapter/server_functions.rs` and `storefront/src/ui/leptos.rs`; fulfillment owns seller-aware presentation/normalization, update materialization, typed errors, GraphQL mutation payload/mapping, the `fulfillment/select-shipping-option` server-function shell and shared `execute_selected_transport` fallback over the explicit commerce checkout runtime API.
...
- [x] keep the public single-group response shortcut only as DTO compatibility for carts with one delivery group;
