...
- Last checkpoint: No-compile provider SPI runtime-smoke evidence now locks missing-provider, unsupported/unknown operation, degraded-provider and unavailable-provider runtime-mode cases plus registration failure cases; previous registration slice added fulfillment-owned external carrier registration DTOs, an in-memory `FulfillmentProviderRegistry` composition seam with descriptor-id/adapter-id validation, duplicate registration guards, health/degraded-mode guards and static verifier source locks, while keeping lifecycle persistence and tracking webhook replay ownership in `FulfillmentService`.
- Next step: Move the remaining select-shipping-option server-function endpoint/body from commerce compatibility into a fulfillment-owned SSR adapter, preserve GraphQL fallback parity, then replace static/no-compile provider SPI evidence with live runtime contract execution against concrete external adapters.
- Open blockers: None.
...
- Evidence:
  ...
  - provider SPI runtime-smoke evidence теперь закреплён в `crates/rustok-fulfillment/contracts/evidence/fulfillment-provider-spi-runtime-smoke.json`: no-compile packet фиксирует missing-provider lookup, unsupported/unknown operation rejection, degraded fallback propagation, unavailable-provider non-executable mode, registration failure cases и webhook replay guardrails; `scripts/verify/verify-ecommerce-provider-spi-evidence.mjs` проверяет этот packet вместе со static matrix, но статус остаётся ниже `boundary_ready` до live adapter execution;
