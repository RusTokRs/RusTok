...
- admin/operator surface уже использует typed lifecycle для shipping options; storefront handoff presentation, request normalization, selection-update materialization, GraphQL mutation execution, native server-function execution и transport fallback policy живут в `rustok-fulfillment/storefront`, а commerce exposes only the shared checkout runtime API for cart update/reprice orchestration.
