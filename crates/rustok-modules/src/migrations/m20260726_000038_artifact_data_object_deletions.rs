...
/// Persists the result of one revision-guarded logical object deletion.
/// Physical bytes are queued separately for retention-aware collection.
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let statements: &[&str] = match manager.get_database_backend() {
            DbBackend::Postgres => &[
                "CREATE TABLE module_artifact_data_object_delete_operations (\
                ...
                "ALTER TABLE module_artifact_data_object_delete_operations ENABLE ROW LEVEL SECURITY",
                "CREATE POLICY module_artifact_data_object_delete_operations_scope \
                 ON module_artifact_data_object_delete_operations \
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("DROP TABLE module_artifact_data_object_delete_operations")
            .await?;
        Ok(())
    }
}
