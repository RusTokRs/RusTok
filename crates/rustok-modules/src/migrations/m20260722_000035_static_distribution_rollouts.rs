        ...
        let statements: &[&str] = match manager.get_database_backend() {
            DbBackend::Postgres => &[
                ...
                "CREATE TABLE module_static_distribution_rollout_assignments (\
                    ...
                    active_claim_id UUID NULL, claimed_by_agent TEXT NULL CHECK (claimed_by_agent IS NULL OR length(trim(claimed_by_agent)) BETWEEN 1 AND 128), claim_expires_at TIMESTAMPTZ NULL, last_claimed_at TIMESTAMPTZ NULL,\
...
mod tests {
    ...
    async fn sqlite_schema_uses_complete_bundle_observation_identity() {
        ...
        assert!(assignment_columns.contains("active_claim_id"));
        assert!(assignment_columns.contains("claimed_by_agent"));
        assert!(assignment_columns.contains("claim_expires_at"));
