        ...
        let statements: &[&str] = match manager.get_database_backend() {
            DbBackend::Postgres => &[
                ...
                "CREATE TABLE module_static_distribution_rollout_operations (\
                    ...
                    CHECK ((completed_at IS NULL AND rollout_id IS NULL AND rollout_revision IS NULL AND rollout_state_revision IS NULL AND rollout_status IS NULL AND node_id IS NULL AND role IS NULL AND observation_revision IS NULL AND assignment_phase IS NULL) OR\
                           (completed_at IS NOT NULL AND operation_kind = 'request' AND rollout_id IS NOT NULL AND rollout_revision IS NOT NULL AND rollout_state_revision IS NOT NULL AND rollout_status = 'preparing' AND node_id IS NULL AND role IS NULL AND observation_revision IS NULL AND assignment_phase IS NULL) OR\
            ...
            DbBackend::Sqlite => &[
                ...
                "CREATE TABLE module_static_distribution_rollout_operations (\
                    ...
                    CHECK ((completed_at IS NULL AND rollout_id IS NULL AND rollout_revision IS NULL AND rollout_state_revision IS NULL AND rollout_status IS NULL AND node_id IS NULL AND role IS NULL AND observation_revision IS NULL AND assignment_phase IS NULL) OR (completed_at IS NOT NULL AND operation_kind = 'request' AND rollout_id IS NOT NULL AND rollout_revision IS NOT NULL AND rollout_state_revision IS NOT NULL AND rollout_status = 'preparing' AND node_id IS NULL AND role IS NULL AND observation_revision IS NULL AND assignment_phase IS NULL) OR (completed_at IS NOT NULL AND operation_kind = 'report' AND rollout_id IS NOT NULL AND rollout_revision IS NOT NULL AND rollout_state_revision IS NOT NULL AND rollout_status IS NOT NULL AND node_id IS NOT NULL AND role IS NOT NULL AND observation_revision IS NOT NULL AND assignment_phase IS NOT NULL))\
                )",
            ],
            backend => {
...
mod tests {
    ...
    async fn sqlite_schema_uses_complete_bundle_observation_identity() {
        ...
        assert!(assignment_columns.contains("artifact_digest"));
