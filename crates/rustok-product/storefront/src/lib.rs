...
use crate::i18n::t;
use crate::model::{ProductDetail, ProductListItem, StorefrontProductsData};
...
pub fn ProductView() -> impl IntoView {
    ...
    let resource = Resource::new_blocking(
        ...
        move |(handle, locale)| async move { api::fetch_storefront_products(handle, locale).await },
    );
    ...
    view! {
        <article class="rounded-3xl border border-border bg-background p-8">
            ...
            <div class="mt-6 grid gap-3 md:grid-cols-2">
                <MetricCard title=t(locale.as_deref(), "product.selected.price", "Price") value=price />
