...
- Last checkpoint: Admin customer core теперь владеет shell/list/detail header view-models, refresh/open action-state policy, field placeholder DTOs, detail section/profile-empty copy и page/editor state helpers; Leptos adapter переводит host-provided locale в label DTOs, рендерит core-owned copy/action state и вызывает transport facade. Native Leptos server functions остаются в `admin/src/transport/native_server_adapter.rs`.
- Next step: Завершить оставшиеся мелкие render-only срезы в `admin/src/ui/leptos.rs` (metadata timestamp labels, detail profile row labels и возможные form control layout markers), затем собрать focused evidence для перевода customer admin к следующему FFA gate; при появлении второго transport path добавить explicit adapter рядом с `native_server_adapter.rs`.
- Open blockers: None.
...
- Last updated at (UTC): 2026-06-12T19:55:05Z
...
- Evidence:
  ...
  - admin FFA slice добавил framework-agnostic `admin/src/core.rs` list request policy, submit-command validation/preparation, form snapshot mapping, shell/list/detail header view-models, field placeholder DTOs, detail section/profile-empty copy, list/detail row view-model policy, active row CSS policy, page-level list/detail empty/error/loading states, refresh/open action-state policy and editor action-state policy; `admin/src/transport/mod.rs` remains the module-owned facade over native-only `admin/src/transport/native_server_adapter.rs` `#[server]` endpoints; explicit Leptos render adapter `admin/src/ui/leptos.rs` consumes core view-models/snapshots/states and no longer owns covered shell/list/detail header copy, list/detail fallback strings, form placeholders, detail section/profile-empty copy, refresh/open disabled policy, active-row class decisions or editor mode/disabled policy; legacy `admin/src/api.rs` удалён, `admin/src/lib.rs` только wires modules и re-export `CustomerAdmin`.
- Last verified at (UTC): 2026-06-12T19:55:05Z
