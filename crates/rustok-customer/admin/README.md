...
- Keeps framework-agnostic customer list defaults, submit-command validation/preparation, shell/list/detail header view models, field placeholder DTOs, detail section/profile-empty copy, list/detail view models, page-state view models, refresh/open action-state policy, editor action-state policy, active-row CSS policy, and form snapshots in `admin/src/core.rs` so render adapters do not own pagination, display fallback, header copy, form placeholders, detail section copy, action disabled state, editor mode/disabled, or form mapping policy.
