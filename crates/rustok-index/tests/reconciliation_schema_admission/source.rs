...
pub struct SchemaAdmissionSource {
    scans: Arc<AtomicUsize>,
}

impl SchemaAdmissionSource {
...
#[async_trait]
impl IndexSource for SchemaAdmissionSource {
    async fn scan(
        &self,
        ...
        match offset {
            0 => {
                ...
                IndexSourcePage::new(
                    &request,
                    vec![mutation(request.tenant_id(), 701, 14_701)],
                    Some(cursor),
                )
                .map_err(|_| fixture_failure())
            }
            1 => IndexSourcePage::new(
                &request,
                vec![mutation(request.tenant_id(), 702, 14_702)],
                None,
            )
            .map_err(|_| fixture_failure()),
...
fn fixture_failure() -> IndexSourceFailure {
    IndexSourceFailure::permanent("schema_admission_fixture_invalid")
