...
use super::connection::{DATABASE_ENV, TestResult, connect, database_url, scoped_connection};
...
impl TestDatabase {
    pub async fn setup() -> TestResult<Option<Self>> {
        let Some(database_url) = database_url() else {
            eprintln!(
                "{DATABASE_ENV} is not set to a PostgreSQL URL; skipping reconciliation schema admission harness"
            );
            return Ok(None);
        };
        let control = connect(&database_url).await?;
        let schema_name = format!("rustok_idx_rec_schema_{}", Uuid::new_v4().simple());
        control
