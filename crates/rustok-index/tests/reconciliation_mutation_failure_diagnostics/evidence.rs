...
pub async fn read_running(
    db: &DatabaseConnection,
    tenant_id: Uuid,
) -> TestResult<ActiveEvidence> {
    let row = db
        .query_one(Statement::from_sql_and_values(
            DbBackend::Postgres,
            "SELECT job_id, state, attempt_count::bigint AS attempt_count_value, (cursor->>'completed_passes')::bigint AS completed_passes, (cursor->>'pages_processed')::bigint AS pages_processed, lease_owner, COALESCE(lease_expires_at > CURRENT_TIMESTAMP, FALSE) AS lease_active FROM index_jobs WHERE tenant_id = $1 AND kind = 'reconcile' AND state = 'running'",
            vec![tenant_id.into()],
        ))
        .await?
        .ok_or_else(|| io::Error::other("running reconciliation mutation failure job is missing"))?;
    Ok(ActiveEvidence {
...
pub async fn read_failure(
    ...
    let row = db
        ...
        .await?
        .ok_or_else(|| io::Error::other("failed reconciliation mutation job is missing"))?;
    Ok(FailureEvidence {
        job_id: row.try_get("", "job_id")?,
        state: row.try_get("", "state")?,
        attempt_count: row.try_get("", "attempt_count_value")?,
