...
#[derive(Clone, Copy)]
pub enum SourceFailedProgressSource {
    FailSecondPage,
    RecoverSecondPage,
}

#[async_trait]
impl IndexSource for SourceFailedProgressSource {
    async fn scan(
        &self,
        ...
        match offset {
            0 => {
                ...
                IndexSourcePage::new(
                    &request,
                    vec![valid_mutation(request.tenant_id(), 801, 15_801)],
                    Some(cursor),
                )
                .map_err(|_| fixture_failure())
            }
            1 => match self {
                Self::FailSecondPage => Err(fixture_failure()),
                Self::RecoverSecondPage => IndexSourcePage::new(
                    &request,
                    vec![valid_mutation(request.tenant_id(), 802, 15_802)],
                    None,
                )
                .map_err(|_| fixture_failure()),
            },
...
fn fixture_failure() -> IndexSourceFailure {
    IndexSourceFailure::retryable(FAILURE_CODE).expect("fixture failure code must be valid")
}
