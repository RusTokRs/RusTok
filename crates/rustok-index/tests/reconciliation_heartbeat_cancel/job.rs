use std::io;

use sea_orm::{ConnectionTrait, DatabaseConnection, DbBackend, Statement};
use uuid::Uuid;

use super::connection::TestResult;

#[derive(Debug)]
pub struct DurableJob {
    pub job_id: Uuid,
    pub state: String,
    pub attempt_count: i64,
    pub completed_passes: i64,
    pub pages_processed: i64,
    pub lease_owner: Option<String>,
    pub lease_extended: bool,
    pub lease_released: bool,
    pub cancel_requested: bool,
}

pub async fn read_job(
    db: &DatabaseConnection,
    tenant_id: Uuid,
    state: &str,
) -> TestResult<DurableJob> {
    let row = db
        .query_one(Statement::from_sql_and_values(
            DbBackend::Postgres,
            "SELECT job_id, state, attempt_count::bigint AS attempt_count_value, (cursor->>'completed_passes')::bigint AS completed_passes, (cursor->>'pages_processed')::bigint AS pages_processed, lease_owner, COALESCE(lease_expires_at > CURRENT_TIMESTAMP + INTERVAL '50 minutes', FALSE) AS lease_extended, (lease_owner IS NULL AND lease_expires_at IS NULL) AS lease_released, cancel_requested FROM index_jobs WHERE tenant_id = $1 AND kind = 'reconcile' AND state = $2 ORDER BY created_at DESC LIMIT 1",
            vec![tenant_id.into(), state.to_owned().into()],
        ))
        .await?
        .ok_or_else(|| io::Error::other(format!("reconciliation {state} job row is missing")))?;
    Ok(DurableJob {
        job_id: row.try_get("", "job_id")?,
        state: row.try_get("", "state")?,
        attempt_count: row.try_get("", "attempt_count_value")?,
        completed_passes: row.try_get("", "completed_passes")?,
        pages_processed: row.try_get("", "pages_processed")?,
        lease_owner: row.try_get("", "lease_owner")?,
        lease_extended: row.try_get("", "lease_extended")?,
        lease_released: row.try_get("", "lease_released")?,
        cancel_requested: row.try_get("", "cancel_requested")?,
    })
}

pub async fn shorten_attempt_one(
    db: &DatabaseConnection,
    tenant_id: Uuid,
    job_id: Uuid,
) -> TestResult<()> {
    let result = db
        .execute(Statement::from_sql_and_values(
            DbBackend::Postgres,
            "UPDATE index_jobs SET lease_expires_at = CURRENT_TIMESTAMP + INTERVAL '30 minutes', updated_at = CURRENT_TIMESTAMP WHERE tenant_id = $1 AND job_id = $2 AND kind = 'reconcile' AND state = 'running' AND lease_owner = 'heartbeat-cancel-worker-a' AND attempt_count = 1",
            vec![tenant_id.into(), job_id.into()],
        ))
        .await?;
    assert_eq!(
        result.rows_affected(),
        1,
        "attempt one lease must shorten exactly once"
    );
    Ok(())
}

pub async fn count(db: &DatabaseConnection, table: &str) -> TestResult<i64> {
    let sql = match table {
        "index_entities" => "SELECT COUNT(*)::bigint AS value FROM index_entities",
        "index_inbox" => "SELECT COUNT(*)::bigint AS value FROM index_inbox",
        "index_jobs" => "SELECT COUNT(*)::bigint AS value FROM index_jobs WHERE kind = 'reconcile'",
        _ => panic!("unsupported fixture table"),
    };
    Ok(db
        .query_one(Statement::from_string(DbBackend::Postgres, sql.to_owned()))
        .await?
        .ok_or_else(|| io::Error::other("count query returned no row"))?
        .try_get("", "value")?)
}
