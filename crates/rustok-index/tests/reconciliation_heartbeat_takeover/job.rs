...
pub async fn read_job(db: &DatabaseConnection, tenant_id: Uuid) -> TestResult<DurableJob> {
    let row = db
        ...
        .await?
        .ok_or_else(|| io::Error::other("reconciliation heartbeat job row is missing"))?;
    Ok(DurableJob {
...
pub async fn shorten_attempt_one(
    ...
    let result = db
        .execute(Statement::from_sql_and_values(
            DbBackend::Postgres,
            "UPDATE index_jobs SET lease_expires_at = CURRENT_TIMESTAMP + INTERVAL '30 minutes', updated_at = CURRENT_TIMESTAMP WHERE tenant_id = $1 AND job_id = $2 AND kind = 'reconcile' AND state = 'running' AND lease_owner = 'heartbeat-worker-a' AND attempt_count = 1",
            vec![tenant_id.into(), job_id.into()],
        ))
        .await?;
    assert_eq!(result.rows_affected(), 1, "attempt one lease must shorten exactly once");
    Ok(())
}
...
pub async fn expire_attempt_one(
    ...
    let result = db
        .execute(Statement::from_sql_and_values(
            DbBackend::Postgres,
            "UPDATE index_jobs SET lease_expires_at = CURRENT_TIMESTAMP - INTERVAL '1 second', updated_at = CURRENT_TIMESTAMP WHERE tenant_id = $1 AND job_id = $2 AND kind = 'reconcile' AND state = 'running' AND lease_owner = 'heartbeat-worker-a' AND attempt_count = 1",
            vec![tenant_id.into(), job_id.into()],
        ))
        .await?;
