...
use crate::domain::{
    FieldPath, FilterExpr, IndexQuery, IndexValue, IndexValueType, LinkCardinality,
...
impl ExecutableQueryPlan {
    ...
    fn compile_postgres_with_cursor(
        ...
        let base = compile_base(self, &mut bindings);
        ...
        if let Some(filter) = &self.filter {
            predicates.push(compile_filter(self, filter, &mut bindings)?);
        }
        if let Some(cursor) = cursor {
            predicates.push(compile_keyset(self, cursor, &mut bindings)?);
        }

        let order = compile_order(self, &mut bindings);
        let pagination = compile_pagination(&self.pagination, &mut bindings)?;
        let sql = format!(
...
fn scalar_bind(
    ...
    match value {
        ...
        IndexValue::Decimal(value) => Ok(PostgresBindValue::Decimal(*value)),
        IndexValue::String(value) => Ok(PostgresBindValue::Text(value.clone())),
        IndexValue::Uuid(value) => Ok(PostgresBindValue::Uuid(*value)),
        IndexValue::Timestamp(value) => Ok(PostgresBindValue::Timestamp(*value)),
