...
const MAX_OWNER_NAME_BYTES: usize = 128;
...
impl IndexDriftRepairTarget {
    pub fn missing_entity(
        ...
        if key.tenant_id.is_nil() {
            return Err(IndexDriftRepairValidationError::NilTargetTenantId);
        }
    ...
    pub fn orphan_link(
        ...
        if source_key.tenant_id.is_nil() {
            return Err(IndexDriftRepairValidationError::NilTargetTenantId);
        }
        if indexed_source_version == 0 || target_absence_source_version == 0 {
            return Err(IndexDriftRepairValidationError::InvalidSourceVersion);
        }
        Ok(Self::OrphanLink {
            source_key,
    ...
    pub fn tenant_id(&self) -> Uuid {
        match self {
            Self::MissingEntity { key, .. } => key.tenant_id,
            Self::OrphanLink { source_key, .. } => source_key.tenant_id,
        }
    }
...
pub enum IndexDriftRepairValidationError {
    ...
    NilCommandId,
    #[error("Index drift repair target tenant id must not be nil")]
    NilTargetTenantId,
    ...
    InvalidDigest,
    #[error("Index drift repair owner name is invalid")]
    InvalidOwnerName,
    #[error("Index drift repair owner kind is registered more than once")]
    DuplicateOwnerKind,
    #[error("Index drift repair outcome code is invalid")]
    InvalidOutcomeCode,
}
...
impl IndexDriftRepairOwnerOutcome {
    ...
    pub fn not_applied(
        ...
        let code = code.into();
        if !valid_machine_name(&code, MAX_OUTCOME_CODE_BYTES) {
            return Err(IndexDriftRepairValidationError::InvalidOutcomeCode);
        }
...
impl IndexDriftRepairOwnerRegistry {
        ...
        I: IntoIterator<Item = Arc<dyn IndexDriftRepairOwner>>,
    {
        ...
        for owner in owners {
            if !valid_machine_name(owner.owner_name(), MAX_OWNER_NAME_BYTES) {
                return Err(IndexDriftRepairValidationError::InvalidOwnerName);
            }
...
impl IndexDriftRepairReceipt {
    pub(crate) fn new(
        ...
        if !valid_digest(&before_digest)
            || after_digest.as_deref().is_some_and(|value| !valid_digest(value))
        ...
        if let IndexDriftRepairReceiptOutcome::NotRepaired { code } = &outcome {
            if !valid_machine_name(code, MAX_OUTCOME_CODE_BYTES) {
                return Err(IndexDriftRepairValidationError::InvalidOutcomeCode);
            }
        }
...
impl IndexDriftRepairCompletion {
    pub(crate) fn new(
        ...
        if !valid_machine_name(&owner_name, MAX_OWNER_NAME_BYTES) {
            return Err(IndexDriftRepairValidationError::InvalidOwnerName);
        }
...
impl IndexDriftRepairService {
            ...
            IndexDriftRepairOwnerOutcome::Applied { receipt_digest } => {
                IndexDriftRepairCompletion::new(
                    ...
                    IndexDriftRepairReceiptOutcome::NotRepaired {
                        code: "after_not_converged".to_owned(),
                    },
                    before.digest().to_owned(),
                    Some(after.digest().to_owned()),
                    Some(receipt_digest),
    ...
    async fn complete_not_repaired(
        &self,
        ticket: &IndexDriftRepairTicket,
        owner_name: &str,
        before: &IndexDriftRepairEvidence,
        ...
        let completion = IndexDriftRepairCompletion::new(
            owner_name.to_owned(),
            IndexDriftRepairReceiptOutcome::NotRepaired {
                code: code.to_owned(),
            },
    ...
    async fn complete_ticket(
        ...
        let receipt = match self.store.complete(ticket, completion).await? {
            IndexDriftRepairStoreCompletionOutcome::Completed(receipt)
            | IndexDriftRepairStoreCompletionOutcome::AlreadyCompleted(receipt) => receipt,
        };
        Ok(match receipt.outcome() {
            IndexDriftRepairReceiptOutcome::Repaired => IndexDriftRepairOutcome::Repaired(receipt),
            IndexDriftRepairReceiptOutcome::NotRepaired { .. } => {
