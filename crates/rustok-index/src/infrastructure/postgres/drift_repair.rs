...
use crate::{
    ...
    IndexDriftRepairTicket, LinkName, LinkedEntityKey, LocaleKey, ModuleName, SchemaRef,
    SchemaVersion,
};
...
impl PostgresIndexDriftRepairStore {
    ...
    async fn reserve_in_transaction(
        ...
        if let Some(existing) = load_command(transaction, command.tenant_id(), command.command_id()).await? {
    ...
    async fn complete_in_transaction(
        ...
        lock_ticket_id(transaction, ticket).await?;
        ...
        let finding_open = exact_finding_is_open(transaction, existing.tenant_id, existing.finding_id).await?;
        ...
        let receipt = IndexDriftRepairReceipt::new(
            ticket.command_id(),
            ticket.finding_id(),
        ...
        .map_err(|_| permanent_failure(STORED_CONTRACT_INVALID))?;
        Ok(IndexDriftRepairStoreCompletionOutcome::Completed(receipt))
    }
}
...
async fn load_and_validate_open_finding(
    ...
    let details_contract = required_text(&row, "details_contract")?;
    if details_contract != FINDING_DETAILS_CONTRACT {
        return Err(permanent_failure(STORED_CONTRACT_INVALID));
    }
    ...
    let finding = IndexDriftRepairFinding::new(
        command.finding_id(),
        finding_key,
        expected_digest,
        actual_digest,
    ...
    .map_err(|_| permanent_failure(STORED_CONTRACT_INVALID))?;
    Ok(FindingLoadOutcome::Open(finding))
}
...
fn validate_target_commitment(
    ...
    if finding_key != derive_finding_key(target.tenant_id(), &expected_check, scope_key) {
        return Err(permanent_failure(STORED_CONTRACT_INVALID));
    }
...
async fn lock_command_id(
    ...
    let key = format!(
        "index-drift-repair-command\u{1f}{}\u{1f}{}",
        command.tenant_id(),
        command.command_id(),
    );
...
async fn lock_ticket_id(
    transaction: &DatabaseTransaction,
    ticket: &IndexDriftRepairTicket,
) -> Result<(), IndexDriftRepairFailure> {
    let key = format!(
        "index-drift-repair-command\u{1f}{}",
        ticket.command_id(),
    );
    advisory_lock(transaction, key).await
}
...
async fn load_command_by_ticket(
    ...
    query_command(
        transaction,
        "SELECT tenant_id, finding_id, payload_digest, target_kind, state, outcome, outcome_code, before_digest, after_digest, owner_receipt_digest FROM index_consistency_finding_repair_commands WHERE command_id = $1 AND finding_id = $2 AND payload_digest = $3 FOR UPDATE",
        vec![
            ticket.command_id().into(),
            ticket.finding_id().into(),
            ticket.reservation_digest().to_owned().into(),
...
fn decode_command(row: QueryResult) -> Result<StoredRepairCommand, IndexDriftRepairFailure> {
    ...
    Ok(StoredRepairCommand {
        ...
        target_kind: required_text(&row, "target_kind")?,
...
fn repair_ticket(
    ...
    IndexDriftRepairTicket::new(command.command_id(), command.finding_id(), payload_digest)
        .map_err(|_| permanent_failure(STORED_CONTRACT_INVALID))
}
...
fn missing_evidence_digest(target: &IndexDriftRepairTarget, state: &[u8]) -> String {
    ...
    hash_repair_target(&mut hasher, target);
    hex::encode(hasher.finalize())
}
