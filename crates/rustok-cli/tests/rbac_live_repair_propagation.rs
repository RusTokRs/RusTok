...
const CHILD_REDIS_URL_ENV: &str = "RUSTOK_RBAC_CLI_REPAIR_REDIS_URL";
...
async fn run_live_cli_repair_scenario() -> TestResult<()> {
    ...
    let scenario_result = async {
        ...
        wait_for_redis_subscribers(&redis_url, 2).await?;
...
async fn run_observer(
    ...
    let context = ServerRuntimeContext::new(db.clone(), settings_with_redis(redis_url));
    let cache = ensure_cache_service(&context);
    if !cache.redis_client_initialized() {
        return Err(test_error("observer did not initialize the Redis client"));
    }
    ...
    wait_for_applied_generation(0, Duration::from_secs(7)).await?;
