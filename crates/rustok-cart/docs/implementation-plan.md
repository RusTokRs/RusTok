...
- Last checkpoint: Cart storefront native server functions now consume `HostRuntimeContext` and a shared `TransactionalEventBus` host handle instead of Loco `AppContext` / `rustok_outbox::loco`, while preserving full shipping option summaries, storefront repricing in `cart/storefront-data`, and the GraphQL selected path.
...
- Last updated at (UTC): 2026-07-08T06:47:29Z
...
- Evidence:
  ...
  - cart storefront native server functions use `HostRuntimeContext::db_clone()` plus a shared `rustok_outbox::TransactionalEventBus` host handle for cart reads, repricing and safe line-item mutations, with no `loco-rs` / `rustok-outbox/loco-adapter` package dependency; `scripts/verify/verify-cart-storefront-boundary.mjs` now locks both the GraphQL selected path and the Loco-free native runtime boundary;
...
- Last verified at (UTC): 2026-07-08T06:47:29Z
