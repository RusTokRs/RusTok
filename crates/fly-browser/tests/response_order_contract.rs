...
fn public_bundle_orders_intent_responses_and_invalidates_on_stop() {
    ...
    assert!(
        FLY_BROWSER_ADAPTER_JS.contains("this.latestIntentRequestGeneration = requestGeneration")
    );
    assert!(
        FLY_BROWSER_ADAPTER_JS.contains("requestGeneration === this.latestIntentRequestGeneration")
    );
    ...
    assert!(
        FLY_BROWSER_ADAPTER_JS
            .contains("this.latestIntentRequestGeneration = ++this.intentRequestGeneration")
    );
}
