...
- Last checkpoint: Order storefront action UI now emits `CompleteCheckoutRequest` via the order-owned `storefront/src/transport.rs` facade, and the obsolete commerce-side checkout completion request builder was removed so the compatibility host only forwards the owner DTO.
