...
- Evidence:
  ...
  - `in_process_provider_impl` теперь закрепляет `OrderService` как owner implementation для `CheckoutCompletionPort`: write-path вызывает `require_write_semantics()?` перед owner `create_order`, read status вызывает `require_deadline_semantics()?` перед owner `get_order`, а cart-id result projection остаётся typed unavailable gap до появления storage projection; fast verifier проверяет эти semantics без полной компиляции;
