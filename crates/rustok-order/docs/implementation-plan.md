...
- Last checkpoint: `OrderStatsSnapshot` and `load_order_stats_snapshot` moved to `rustok-order`; `apps/server::RootQuery::dashboard_stats` only composes the owner helper behind feature `mod-order` and no longer contains SQL for `order.placed` events. The boundary is locked by `apps/server/tests/module_surface_boundary_guard.rs` without compilation.
