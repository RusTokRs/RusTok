...
- Last checkpoint: Order storefront action UI now emits `CompleteCheckoutRequest` with order-owned completion command metadata and create-fulfillment policy via the order-owned `storefront/src/transport.rs` facade, and the compatibility host forwards the owner DTO into native/GraphQL orchestration payload metadata instead of creating anonymous commerce-side command metadata.
...
- Evidence:
  ...
  - `in_process_provider_impl` теперь закрепляет `OrderService` как owner implementation для `CheckoutCompletionPort`: write-path вызывает `require_write_semantics()?` перед owner `create_order_with_channel`, подтверждает order lifecycle через `confirm_order` и reload-ит locale-aware snapshot при наличии locale context; read status вызывает `require_deadline_semantics()?` перед owner `get_order`, а cart-id result projection остаётся typed unavailable gap до появления storage projection; fast verifier проверяет эти semantics без полной компиляции;
...
- async transport adapters по-прежнему публикуются фасадом `rustok-commerce`, while complete-checkout command normalization is order-owned;
