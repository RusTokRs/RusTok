...
- REST endpoints в `apps/server`: `/api/v1/flex/schemas*` и `/api/v1/flex/schemas/{schema_id}/entries*` с теми же tenant-scoped RBAC gates; request/response DTO, command mapping и view mapping берутся из `flex::rest`
