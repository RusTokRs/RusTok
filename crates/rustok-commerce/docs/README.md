...
- Для shipping profiles metadata-backed baseline больше не является единственным source of truth: в `commerce` появился typed registry `shipping_profiles` + `ShippingProfileService`, а product/shipping option storage по-прежнему нормализуется в `shipping_profile.slug` и `shipping_profiles.allowed_slugs`.
