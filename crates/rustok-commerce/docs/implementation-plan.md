...
- Last checkpoint: Commerce storefront FFA cleanup removed aggregate payment-collection presentation from `storefront/src/ui/leptos`, so commerce only triggers cross-module checkout/payment orchestration while payment-owned UI remains outside the umbrella surface.
