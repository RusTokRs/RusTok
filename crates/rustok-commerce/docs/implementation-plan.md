...
- Current phase: admin FFA display/form/command hardening + FBA readiness hardening gate
- Last checkpoint: FFA-срез `rustok-commerce-admin` продолжил root/core separation: crate root только wires modules и re-export `ui::CommerceAdmin`; Leptos adapter готовит shipping-profile drafts/form state, cart-promotion/order-change commands, optional filters, badge classes, summary/display fallbacks, cart-adjustment view-models и resolution summary cards через framework-agnostic `admin/src/core.rs`; transport facade остаётся единственной точкой вызова raw `api::*` для covered admin flows.
...
- Evidence:
  ...
  - exchange/claim return-decision helper metadata теперь помечает создаваемые order changes `return_decision_action` и `return_decision_source`, а admin operator workspace показывает resolution summary cards из preview/metadata через framework-agnostic `admin/src/core.rs` helper без переноса domain rules в host или Leptos render adapter;
  - FFA admin form/command/view-model hardening: `admin/src/lib.rs` больше не содержит Leptos render/business code и только wires modules + re-export `CommerceAdmin`; shipping-profile form state/draft preparation, optional filter normalization, cart-promotion command preparation, order-change action draft preparation, lifecycle badge classes, summary/display fallback policy, cart-adjustment view-models и resolution summary mapping вынесены в `admin/src/core.rs`;
