...
- Last checkpoint: Commerce storefront checkout owner-fragment mapping moved out of the Leptos render adapter into Leptos-free `storefront/src/core/presentation.rs`: fulfillment delivery-group mapping, cart/payment/order labels and payment/order result DTO projection are now core-owned, while the UI consumes owner-module components only. Cart/commerce storefront GraphQL read queries no longer request legacy `sellerScope`; commerce GraphQL fallback selection payload no longer sends `sellerScope`; REST/GraphQL storefront shipping-selection inputs and GraphQL cart line-item/delivery-group outputs no longer expose `seller_scope`; fulfillment owner selection transport DTOs and commerce/fulfillment storefront delivery-group model DTOs no longer expose `seller_scope`; guardrails lock the no-`sellerScope` read/fallback paths and no-`seller_scope` input/output/owner/model seams.
- Next step: Continue only with owner-module checkout handoff slices that remove real umbrella presentation/request leakage, or finish moving payment/order/fulfillment async native/GraphQL adapters fully behind owner storefront packages; keep Next commerce pages behind the shared module guard.
- Open blockers: None.
...
- Last updated at (UTC): 2026-06-24T15:37:00Z
...
- Evidence:
  ...
  - REST/GraphQL cart line-item resolution и quantity update также передают variant/channel/quantity в `check_variant_availability_for_public_channel`; generic `InventoryReservationPort::check_availability` не используется для public-channel availability, что закреплено `verify-inventory-admin-boundary.mjs`;
  - storefront REST/GraphQL region lists вызывают `RegionReadPort::list_regions_for_tenant`; umbrella re-exports `rustok_commerce::RegionService` и `rustok_commerce::services::region` удалены, поэтому owner service больше не маскируется commerce facade-ом;
  - umbrella re-export `rustok_commerce::{services::inventory, InventoryService}` удалён; server migration smoke и commerce integration callers импортируют owner service только из `rustok-inventory`;
  - umbrella re-exports `rustok_commerce::{services::customer, CustomerService}` удалены; commerce transports и integration callers импортируют owner service только из `rustok-customer`;
  ...
  - FFA admin/storefront transport/core split: aggregate checkout route теперь строит `FetchCommerceRequest`, `CartCommandRequest`, `SelectShippingOptionRequest` и commerce-owned context fallback view-model (`tenant/channel/resolution`) в Leptos-free `storefront/src/core/` submodules (`requests`, `presentation`); cart totals/line-items/adjustments stay in `rustok-cart`, payment details stay in `rustok-payment`, order totals stay in `rustok-order`, fulfillment/shipping option details stay in `rustok-fulfillment`, and cart handoff status rendering is owned by `rustok-cart-storefront`, payment-collection card/create action rendering plus create/reuse command metadata and typed fallback facade are owned by `rustok-payment-storefront`, fulfillment/shipping handoff and seller-aware selection rendering plus selection-update materialization and typed fallback facade are owned by `rustok-fulfillment-storefront`, checkout result/order status and complete-checkout action presentation plus completion command metadata and typed fallback facade are owned by `rustok-order-storefront`, and commerce consumes those owner-module components/requests while retaining checkout orchestration transport; checkout owner-fragment label/data mappers now live in Leptos-free `storefront/src/core/presentation.rs` instead of `storefront/src/ui/leptos/mod.rs`, storefront cart/checkout GraphQL read queries and commerce GraphQL fallback selection payload no longer carry legacy `sellerScope`, REST/GraphQL storefront shipping-selection inputs and GraphQL cart line-item/delivery-group outputs no longer expose `seller_scope`, and fulfillment owner selection transport/model DTOs no longer expose `seller_scope`; `storefront/src/transport/mod.rs` owns единственную native-first + GraphQL fallback policy, duplicate combined fallback helpers удалены из legacy `storefront/src/api.rs`, compatibility fallback is now MissingServer-only for owner handoff commands and locked by `scripts/verify/verify-commerce-storefront-transport-handoff.mjs`, legacy `storefront/src/api.rs` удалён, raw native/GraphQL operations временно живут в `storefront/src/transport/raw_adapter.rs` but shipping-selection planning/error text is delegated to `rustok-fulfillment-storefront`; legacy `admin/src/api.rs` тоже удалён, admin raw native/GraphQL operations временно живут в `admin/src/transport/raw_adapter.rs`, а `scripts/verify/verify-commerce-admin-boundary.mjs` запрещает возврат legacy admin API;
...
- Last verified at (UTC): 2026-06-23T00:00:00Z
...
| `BL-14` | seller-aware grouping уже есть, но marketplace identity boundary исторически опиралась на `seller_scope` вместо стабильного opaque key | закрепить `seller_id` как canonical multivendor boundary в product/cart/order/fulfillment contracts и вычистить runtime fallback на `seller_scope` |
...
- `cart_shipping_selections` теперь персистит seller-aware key по `(shipping_profile_slug, seller_id)` и не fallback'ится к `seller_scope`;
...
- cart grouping, checkout fulfillment metadata and manual fulfillment validation теперь опираются на `(shipping_profile_slug, seller_id)` без fallback на `seller_scope`;
...
- seller display label больше не персистится в ecommerce storage, а `seller_scope` больше не используется как runtime identity fallback в cart grouping/selection или manual fulfillment metadata.

Ближайшие шаги:

- подготовить seller-owned read model/resolver для display label по `seller_id` и effective locale;
- продолжить вычищать оставшиеся storage/API упоминания `seller_scope` отдельными атомарными owner-module slices без runtime fallback;
...
- [x] payment provider SPI static evidence, typed webhook adapter operation и webhook ingress/replay contract;
- [x] fulfillment provider SPI static evidence, typed carrier webhook adapter operation и carrier webhook replay contract;
