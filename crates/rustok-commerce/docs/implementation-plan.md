...
- Current phase: provider SPI live-adapter execution contract hardening and storefront owner transport handoff
- Last checkpoint: Storefront owner transport handoff moved the payment and order MissingServer-only native/GraphQL fallback policy into owner packages: `rustok-payment/storefront` now exposes `PaymentCollectionTransportError` + `create_payment_collection_with_fallback`, `rustok-order/storefront` exposes `CheckoutCompletionTransportError` + `complete_checkout_with_fallback`, and commerce only maps temporary adapter errors while `verify-commerce-storefront-transport-handoff.mjs` locks the no-broad-fallback boundary without compilation.
