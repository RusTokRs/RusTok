...
- Current phase: admin FFA command/view-model hardening + FBA readiness hardening gate
- Last checkpoint: FFA-срез `rustok-commerce-admin` продолжил root/core separation: crate root только wires modules и re-export `ui::CommerceAdmin`; Leptos adapter готовит cart-promotion/order-change commands, badge classes и resolution summary cards через framework-agnostic `admin/src/core.rs`; transport facade остаётся единственной точкой вызова raw `api::*` для covered admin flows.
...
- Evidence:
  ...
  - FFA admin command/view-model hardening: `admin/src/lib.rs` больше не содержит Leptos render/business code и только wires modules + re-export `CommerceAdmin`; cart-promotion command preparation, order-change action draft preparation, lifecycle badge classes и resolution summary mapping вынесены в `admin/src/core.rs`;
