...
- Evidence:
  ...
  - `CheckoutService` принимает runtime-composed `Arc<dyn InventoryReservationPort>` через единственный constructor; checkout availability validation вызывает `check_availability` с tenant/channel/locale/deadline context, прямой inventory helper из orchestration удалён, старая сигнатура constructor не сохранена;
  - REST/GraphQL cart line-item resolution и quantity update также принимают `InventoryReservationPort` и вызывают `check_availability` с transport-neutral context; прямых вызовов `check_variant_availability_for_public_channel` в `rustok-commerce/src` больше нет;
