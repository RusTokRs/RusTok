...
- Current phase: post-checkout provider SPI runtime wiring
- Last checkpoint: Post-checkout payment orchestration now routes admin REST/GraphQL payment cancel and refund creation through `PaymentOrchestrationService`, invoking payment-owner `execute_cancel` / `execute_refund` registry seams before `PaymentService` lifecycle persistence; checkout authorize/capture and fulfillment label creation remain behind the same owner registry pattern, and the no-compile provider SPI verifier source-locks both checkout and post-order invocation markers.
- Next step: Extend provider runtime wiring into fulfillment cancel/tracking-webhook and payment webhook replay orchestration, then finish moving payment/order/fulfillment async native/GraphQL adapters fully behind owner storefront packages; keep Next commerce pages behind the shared module guard.
- Open blockers: None.
