...
- Last checkpoint: Provider SPI runtime-mode smoke evidence now lives beside owner payment/fulfillment registries: per-operation capability checks, missing-provider errors, unsupported/unknown operations, registration failures and health/degraded-mode fallback mapping are locked before invoking external gateway/carrier adapters, while lifecycle persistence stays in owner services; the aggregate verifier still runs without a compile step.
- Next step: Move payment/order/fulfillment async native/GraphQL adapters behind owner storefront packages when host routing can depend on those adapters without circular orchestration, then turn provider SPI webhook/registration no-compile smoke contracts into live runtime contract execution; keep Next commerce pages behind the shared module guard.
- Open blockers: None.
