...
- Current phase: admin FFA root/core hardening + FBA readiness hardening gate
- Last checkpoint: FFA-срез `rustok-commerce-admin` добил root/core separation: crate root теперь только wires modules и re-export `ui::CommerceAdmin`, Leptos adapter снова показывает resolution summary cards через framework-agnostic `core::order_change_resolution_summary`, а transport facade остаётся единственной точкой вызова raw `api::*` для covered admin flows.
...
- Evidence:
  ...
  - FFA admin root hardening: `admin/src/lib.rs` больше не содержит Leptos render/business code и только wires modules + re-export `CommerceAdmin`;
