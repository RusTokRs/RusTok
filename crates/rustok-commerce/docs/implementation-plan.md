...
- Last checkpoint: Commerce storefront FFA cleanup removed aggregate payment/shipping presentation leaks from `storefront/src/ui/leptos` and the private transport facade, so commerce only triggers cross-module checkout/payment orchestration while payment/fulfillment-owned UI remains outside the umbrella surface.
- Next step: Create/expand payment-owned UI in `rustok-payment` before showing payment collection details, or harden parity/evidence for existing native + GraphQL checkout/post-order paths without long full-workspace compilation.
- Open blockers: None.
...
- Last updated at (UTC): 2026-06-13T18:05:00Z
...
- Evidence:
  ...
  - FFA storefront transport/core split: aggregate checkout route теперь строит `FetchCommerceRequest` and `CartCommandRequest` и commerce-owned context fallback view-model (`tenant/channel/resolution`) в Leptos-free `storefront/src/core/` submodules (`requests`, `presentation`); cart totals/line-items/adjustments stay in `rustok-cart`, payment details stay in `rustok-payment`, order totals stay in `rustok-order`, fulfillment/shipping option details stay in `rustok-fulfillment`, and commerce renders only handoff/orchestration context, no longer renders cart/payment/order/fulfillment detail/status cards, and no longer exposes the unused shipping-selection facade from the private UI transport; `storefront/src/transport/mod.rs` owns единственную native-first + GraphQL fallback policy, duplicate combined fallback helpers удалены из `storefront/src/api.rs`, а `native_server_adapter.rs` / `graphql_adapter.rs` являются единственными местами storefront UI package, где вызываются raw native/GraphQL `api::*` functions;
