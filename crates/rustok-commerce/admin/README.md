...
- Routes shipping-profile, promotion, and order-change operations through `admin/src/transport.rs`; default form policy and order-change resolution summary mapping live in `admin/src/core.rs`, so the Leptos surface does not call raw `api::*` operations or own parsing/policy helpers for covered flows.
