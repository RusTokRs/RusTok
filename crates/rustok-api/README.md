...
- Provide neutral port context/error primitives, policy helpers (`PortCallPolicy`) and typed error constructors for module-owned ports; `rustok-region` and migrated product, customer, media, workflow, RBAC, tax, fulfillment, payment, pricing, cart, inventory, comments, search, and order read ports use these shared primitives for FBA/read-oriented projection boundaries.
