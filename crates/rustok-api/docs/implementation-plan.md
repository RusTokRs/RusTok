...
- Last checkpoint: Migrated `rustok-email` and `rustok-outbox` write-oriented FBA owner ports from package-local `PortContext`/`PortError` shims to shared `rustok_api::PortContext`/`PortError`; delivery/relay control now enforce `PortCallPolicy::write()` while module-owned request/projection DTOs stay local.
- Next step: Add targeted compile/tests for the migrated email/outbox write ports when compilations are allowed and continue auditing any newly discovered capability-local port shims.
- Open blockers: None.
...
- [~] документировать изменения host/API contracts одновременно с изменением runtime surface; (updated for index read/rebuild plus email/outbox write-policy consumer migration)
- [~] удерживать local docs и `README.md` синхронизированными; (updated for index read/rebuild plus email/outbox write-policy consumer migration)
