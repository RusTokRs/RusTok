...
- Current phase: shared read/write port policy consumer migration
- Last checkpoint: Continued shared policy consumer migration without compilation: `rustok-outbox` now imports shared port primitives through `rustok_api`, and customer/media read ports rely on `PortCallPolicy::read()` as the single shared read-policy gate instead of layering ad-hoc deadline checks above it.
- Next step: Add targeted compile/tests for the migrated email/outbox/customer/media policy consumers when compilations are allowed and continue auditing direct `require_*_semantics` use that can move to `PortCallPolicy`.
- Open blockers: None.
...
- [~] документировать изменения host/API contracts одновременно с изменением runtime surface; (updated for outbox rustok-api import plus customer/media read-policy cleanup)
- [~] удерживать local docs и `README.md` синхронизированными; (updated for outbox rustok-api import plus customer/media read-policy cleanup)
