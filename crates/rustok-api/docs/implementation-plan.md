...
- Last checkpoint: Extended `PortCallPolicy::read()` enforcement from the initial `rustok-region` slice to product, customer, media, workflow, RBAC, tax, fulfillment, payment, pricing, cart, inventory, comments, search, order checkout status/result reads, payment collection status reads, and fulfillment shipping-option list reads that already consumed shared `PortContext`/`PortError`.
