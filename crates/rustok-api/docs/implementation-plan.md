...
- Current phase: neutral port baseline
- Last checkpoint: Added baseline neutral port primitives (`PortContext`, `PortActor`, `PortError`, `PortErrorKind`) for transport-agnostic module ports.
...
- `PortContext`/`PortError` задают минимальный shared baseline для transport-agnostic ports до появления transport adapters;
...
- [~] продолжать выносить действительно shared transport/UI/port helpers из host/module-specific layers; (started: neutral port context/error primitives)
