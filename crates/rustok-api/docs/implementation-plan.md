...
- Last checkpoint: Migrated `rustok-index` FBA read/rebuild owner ports from package-local port context/error structs to shared `rustok_api::PortContext`/`PortError`; index reads now use `PortCallPolicy::read()` and rebuild orchestration uses `PortCallPolicy::write()` while index-owned request/projection types stay local.
- Next step: Continue auditing the remaining capability-local write-only shims (`rustok-email`, `rustok-outbox`) for whether they should stay local or adopt shared write policy, then add targeted tests when compilations are allowed.
- Open blockers: None.
...
- [~] документировать изменения host/API contracts одновременно с изменением runtime surface; (updated for index read/rebuild port policy consumer migration)
- [~] удерживать local docs и `README.md` синхронизированными; (updated for index read/rebuild port policy consumer migration)
