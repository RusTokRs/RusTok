...
- Last checkpoint: Extended `PortCallPolicy::read()` enforcement from the initial `rustok-region` slice to product, customer, media, workflow, RBAC, tax, fulfillment, payment, pricing, cart, inventory, and comments module-owned read ports that already consumed shared `PortContext`/`PortError`.
- Next step: Continue migrating the remaining module-owned read ports from ad-hoc `require_deadline_semantics()` checks to `PortCallPolicy::read()` where the operation is truly read-only, then add targeted tests when compilations are allowed.
- Open blockers: None.
- Hand-off notes for next agent: После каждого инкремента обновлять этот блок.
- Last updated at (UTC): 2026-06-20T13:36:46Z
...
- `PortContext`/`PortError` задают shared baseline для transport-agnostic ports, а `PortCallPolicy` фиксирует reusable read/write/event-replay/best-effort enforcement без module-specific logic; `rustok-region`, product, customer, media, workflow, RBAC, tax, fulfillment, payment, pricing, cart, inventory и comments read ports уже потребляют `PortCallPolicy::read()` для FBA/read-oriented projections;
...
- [~] продолжать выносить действительно shared transport/UI/port helpers из host/module-specific layers; (continued: neutral port context/error primitives, port call policies, typed error constructors and multi-module read-port consumer migration)
...
- [~] документировать изменения host/API contracts одновременно с изменением runtime surface; (updated for multi-module read-port policy consumer migration)
- [~] удерживать local docs и `README.md` синхронизированными; (updated for multi-module read-port policy consumer migration)
