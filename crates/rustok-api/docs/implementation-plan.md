...
- Last checkpoint: Continued shared policy consumer migration without compilation: remaining direct module-port write checks in inventory, comments, fulfillment, order, payment and page-builder now use `PortCallPolicy::write()`, leaving direct `require_*_semantics` calls only inside the core primitive implementation/tests.
- Next step: Add targeted compile/tests for the migrated email/outbox/customer/media/inventory/comments/fulfillment/order/payment/page-builder policy consumers when compilations are allowed and continue auditing new module ports for direct `require_*_semantics` regressions.
- Open blockers: None.
