...
impl DirectTaskHandler for ProductAttributesHandler {
    fn task_slug(&self) -> &'static str { "product_attributes" }

    async fn execute(&self, app_ctx: &AppContext, operator: &AiOperatorContext, request: DirectExecutionRequest) -> AiResult<DirectExecutionResult> {
        ...
        let trace = ToolTrace { tool_name: "direct.commerce.product_attributes".to_string(), input_payload: request.task_input_json.clone(), output_payload: Some(operation_payload.clone()), status: "completed".to_string(), duration_ms: started.elapsed().as_millis() as i64, sensitive: false, error_message: None, created_at: Utc::now() };
        let explanation = explain_result(&request.provider, &request.provider_config, request.system_prompt.as_deref(), request.resolved_locale.as_str(), input.assistant_prompt.as_deref(), &summary, &operation_payload, request.stream_emitter.clone()).await;
        Ok(DirectExecutionResult { execution_target: DirectExecutionTarget::Commerce, appended_messages: vec![explanation], traces: vec![trace], metadata: json!({"direct_task": request.task_slug,"requested_locale": request.requested_locale,"resolved_locale": request.resolved_locale,"product_id": input.product_id,"suggested_attributes": operation_payload,}), })
    }
}
