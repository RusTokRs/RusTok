...
| Agent principals and owner workflows | `in_progress` | Generic principal/catalog/workflow contracts, Alloy-owned descriptors and stage bindings, persisted principal/model-assignment/workflow schemas, GraphQL catalog/read/create paths, descriptor-capability enforcement, atomic stage claims, lease recovery, dependency promotion, canonical task-run stage execution, and approval-to-stage completion are implemented. Update/delete transport, scheduler hosting, and admin forms remain. |
