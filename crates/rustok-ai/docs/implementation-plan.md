...
| Agent principals and owner workflows | `in_progress` | Generic principal/catalog/workflow contracts, Alloy-owned code-agent descriptors, persisted principal/model-assignment/workflow schemas, GraphQL catalog/read/create paths, descriptor-capability enforcement, atomic stage claims, lease recovery, and dependency promotion are implemented. Update/delete transport, approval resolution for workflow stages, actual stage execution, and admin forms remain. |
