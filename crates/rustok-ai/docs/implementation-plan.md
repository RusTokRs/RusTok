...
| Agent principals and owner workflows | `in_progress` | Generic principal/catalog/workflow contracts, Alloy- and product-owned descriptors and stage bindings, persisted principal/model-assignment/workflow schemas, GraphQL catalog/read/create/update paths, native/GraphQL bootstrap catalog parity, descriptor-capability enforcement, atomic stage claims, lease recovery, dependency promotion, canonical task-run stage execution, and approval-to-stage completion are implemented. A typed internal validator registry binds every owner descriptor to exactly one owner-level stage input validator before selecting the canonical task, so no owner-specific execution path or string dispatch exists in the scheduler. Successful stages promote their persisted dependents, a single service derives the durable workflow aggregate status from stage state, and claim rejects stages whose parent workflow is terminal. Lease recovery rederives that aggregate, while promotion and every post-run stage write use `running + unexpired lease_token` or `pending` compare-and-set guards, so an expired worker cannot overwrite a later attempt. Successful claims record `started_at`; an expired lease is rejected even before the recovery loop requeues it. Owner-declared admission gates have a separate atomic approve/reject transition and audit metadata; an approval validates that the parent is still active, then rederives the aggregate status; a rejected gate fails its workflow, while model-tool approvals remain in their own lifecycle. The catalog rejects dependency cycles and cross-owner stage agents before any workflow can persist. Workflow admission requires an exact one-to-one binding set for its owner-declared stages; it cannot ignore unknown or incomplete bindings. A validated model assignment authorizes its own provider/model/mode selection through a private execution-authority type; transport callers still require `AI_ROUTER_OVERRIDE` for a manual selection. Agent create/update inputs no longer accept raw roles or permissions: they persist only descriptor-required permissions until the platform-owned tenant RBAC catalog can assign catalogued roles. Provider-profile GraphQL, native, and Leptos inputs likewise no longer accept raw restricted-role slugs; the persisted field is read-only migration state until that same catalog can supply a typed selection. Admission persists a server-owned initiator-permission/locale snapshot, and both initial execution and tool-approval continuation reconstruct a descriptor-constrained context from it; scheduler-worker permissions, approving-operator permissions, and caller metadata never reach owner direct handlers or MCP tools. A terminal parent workflow blocks both stage execution and tool-approval claim before any external effect. Agent-stage finalization is also compare-and-set by `run_id + waiting_approval`; cancellation immediately derives a distinct `cancelled` stage and workflow status rather than falsely recording a failure. Workflow admission and the executor both revalidate the current profile capabilities against the owner descriptor, rejecting stale assignments before persistence and before execution. The module-owned Leptos UI renders a read-only owner catalog with operations, capabilities, required permissions, workflow stages, configured-principal count, and each principal's persisted profile/model/mode assignment. The same assignment catalog is present in native and GraphQL bootstrap responses. Principals and assignments use audit-preserving deactivation rather than deletion. Scheduler hosting remains; agent-role forms are blocked on a platform-owned public tenant RBAC role/permission catalog. |
...
| Agent workflow platform contracts | `blocked_platform` | Proposed ADR `2026-07-13-agent-workflow-platform-contracts.md` requires a generic `TenantRbacCatalog` and `ModuleWorkScheduler`; no raw-role form or AI-specific host worker is permitted. |
...
context. The factory is the only remaining `AiHostRuntime::new` call; turning
