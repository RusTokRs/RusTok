...
| Agent principals and owner workflows | `in_progress` | Generic principal/catalog/workflow contracts, Alloy-owned descriptors and stage bindings, persisted principal/model-assignment/workflow schemas, GraphQL catalog/read/create/update paths, descriptor-capability enforcement, atomic stage claims, lease recovery, dependency promotion, canonical task-run stage execution, and approval-to-stage completion are implemented. Principals and assignments use audit-preserving deactivation rather than deletion. Scheduler hosting and admin forms remain. |
