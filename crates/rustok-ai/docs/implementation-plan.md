...
|---|---|---|---|
| R0. Static contract repair | `implemented_pending_tests` | Provider token fields are normalized into `ProviderUsage`; both direct and agent streaming paths use one provider-event normalizer before hub publication; the native DTO owns its conversion from the canonical event. `ProviderIntegration` now owns compiler-exhaustive factory capability binding, public GraphQL/native/admin catalogs iterate compiled integrations only, and `AiProviderTargetCatalog` validates compiled availability, settings schema, and server-owned egress when it loads. `fastembed` stays in the pinned snapshot but cannot be advertised or selected without its feature. | Target-load rejection, compiled-catalog filtering, and exact descriptor/factory parity tests are present. Execute the focused Rust tests in R4 after the no-long-compilation restriction is lifted. |
...
| R4. Final verification and evidence | `in_progress` | Observed fast evidence: changed-file `rustfmt --check`, `git diff --check`, `verify:ai:admin-boundary`, `verify:ai:rig-cutover`, `verify:ai:fba-baseline`, `verify:orchestrator:fba-runtime-order`, `verify:ai:domain-verticals`, and `verify:i18n:ui` pass. Rust gates passed before the registry-integrity repair: `rustok-secrets` 7/7, `rustok-ai` server 59/59 plus 1 ignored deployment-live probe, server + GraphQL 60/60 plus 1 ignored probe, and `rustok-ai-admin` SSR 17/17. Next Admin typecheck and lint pass. The aggregate FFA verifier is no longer blocked: every governed local plan declares its structural shape. `rustok-ai` is registered as `runtime = "extension"`, so `cargo xtask module validate ai` validates its package/transport/UI contract without making it tenant-toggled, emitting it into `ModuleRegistry`, or changing `apps/server`. The generic runtime-contribution implementation itself remains platform-owned. A workspace check remains. | All required gates pass, or each external failure is recorded with owner and reproducible evidence. |
