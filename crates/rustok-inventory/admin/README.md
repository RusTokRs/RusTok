...
- Keeps the existing commerce GraphQL access isolated inside the transitional `CommerceGraphqlInventoryReadAdapter` as a compatibility fallback until dedicated inventory write transport is available.
...
- Reads inventory product, variant, stock-health, and localized-copy fields through the inventory-owned facade; native server functions are the primary path, while the underlying commerce GraphQL adapter is transitional and must stay private to the package transport boundary.
...
Remove `CommerceGraphqlInventoryReadAdapter` after inventory has dedicated write transport and native read parity for:
