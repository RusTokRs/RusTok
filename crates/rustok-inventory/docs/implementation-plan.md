...
- Evidence:
  - in-process реализация `InventoryReservationPort for InventoryService` добавлена в `src/ports.rs`: availability read path вызывает owner `check_variant_availability`, reservation/release write paths требуют `PortContext::require_write_semantics` и мапят `CommerceError` в `PortError`;
