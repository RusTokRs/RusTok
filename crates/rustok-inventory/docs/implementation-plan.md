...
- Last verified at (UTC): 2026-06-07T06:45:00Z
...
- read transport теперь native-only через dedicated server functions; первые set-quantity/adjust-quantity/reserve-quantity/release-reservation write endpoints и availability-check facade вынесены в inventory-owned native facade, оставшийся non-admin mutation parity всё ещё дособирается из umbrella `rustok-commerce`;
...
- [x] вынести dedicated inventory admin read/write transport из umbrella `rustok-commerce` (admin read path native-only; первый admin write split: native set-quantity/adjust-quantity/reserve-quantity/release-reservation endpoints plus check-availability);
