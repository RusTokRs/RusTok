...
impl FlyUiStateMachine {
                ...
                let insertion = {
                    ...
                    if let Some(index) = index {
                        let candidate = drag.candidates.get(index).ok_or_else(|| {
                            UiError::IllegalDrop("drop candidate index is out of bounds".to_string())
                        })?;
                        if !candidate.legal {
    ...
    fn mark_dirty(&mut self) {
        ...
        self.state.dirty.command_sequence =
            self.state.dirty.command_sequence.saturating_add(1);
        self.state.dirty.save_failed = false;
    }
}
