...
async fn setup() -> (DatabaseConnection, Uuid, Uuid, Uuid) {
    ...
    create_current_menu_schema(&db).await;
...
async fn create_current_menu_schema(db: &DatabaseConnection) {
    for statement in [
        r#"
        CREATE TABLE menus (
            id TEXT PRIMARY KEY NOT NULL,
