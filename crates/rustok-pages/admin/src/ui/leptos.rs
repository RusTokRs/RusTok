...
pub fn PagesAdmin() -> impl IntoView {
    ...
    let pages_resource = local_resource(
        ...
        move |(token_value, tenant_value, _)| async move {
            transport::fetch_pages(token_value, tenant_value).await
        },
    );
    ...
    let edit_page = Callback::new(move |page_id: String| {
        ...
        spawn_local(async move {
            match transport::fetch_page(token_value, tenant_value, page_id.clone()).await {
                Ok(Some(page)) => {
    ...
    let submit_page = move |ev: SubmitEvent| {
        ...
        spawn_local(async move {
            let result = match editing_page {
                Some(page_id) => {
                    transport::update_page(token_value, tenant_value, page_id, draft).await
                }
                None => transport::create_page(token_value, tenant_value, draft).await,
            };
    ...
    let publish_page = Callback::new(move |(page_id, publish): (String, bool)| {
        ...
        spawn_local(async move {
            let result = if publish {
                transport::publish_page(token_value, tenant_value, page_id.clone()).await
            } else {
                transport::unpublish_page(token_value, tenant_value, page_id.clone()).await
            };
    ...
    let delete_page = Callback::new(move |page_id: String| {
        ...
        spawn_local(async move {
            match transport::delete_page(token_value, tenant_value, page_id.clone()).await {
                Ok(true) => {
