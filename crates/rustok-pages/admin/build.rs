...
fn main() {
    ...
    if landing.surface != "admin" || metadata.surface != "admin" {
        panic!(
            "Pages landing_blocks and metadata contribution roles must remain on the admin surface"
        );
    }
    ...
    let fly_builtin_version = normalized
        ...
        .unwrap_or_else(|| {
            panic!(
                "landing_blocks provider '{}' is not version-pinned",
                landing.provider
            )
        });
    let metadata_property_editor_id = metadata.property_editor_id.as_deref().unwrap_or_else(|| {
        panic!("metadata contribution must declare exactly one property editor")
    });
