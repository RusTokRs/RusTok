...
if (evidence.format !== "pages_explicit_artifact_binding_replacement_source_v4") failures.push("evidence format drifted");
if (evidence.status !== "pages_explicit_artifact_binding_replacement_rollback_anchor_recovery_source_unvalidated") failures.push("evidence status drifted");
if (!Array.isArray(evidence.execution) || evidence.execution.length !== 0) failures.push("evidence execution must remain empty");
...
for (const key of [
  ...
  if (evidence.source_contract?.[key] !== true) failures.push(`evidence source_contract.${key} must be true`);
}
for (const key of ["tests_run", "static_verifier_run", "cargo_run", "formatting_run", "migration_run", "database_scenario_run", "binding_replacement_run", "workflows_or_ci_run"]) {
  if (evidence.source_contract?.[key] !== false) failures.push(`evidence source_contract.${key} must remain false`);
}
...
for (const marker of ["sanitize_static_landing_project", "compile_materialized_static_landing", "append_rebuilt_in_tx", "page_body::Column::Content", "body.content", "delete_by_id(rebuild.source_artifact_id)", "PagesCacheInvalidationRuntime"]) {
...
requireOrdered(sources.service, [
  ...
  "ensure_sequential_missing_binding_recovery_version_chain_in_tx",
], "publish-or-rollback version admission");

for (const marker of ["pub(crate) async fn bind_existing_body_in_tx", "match page_published_landing_artifact::Entity::find_by_id(body.id)", "None =>", "page_published_landing_artifact::ActiveModel"]) {
...
for (const marker of ["Missing-binding activation after physical source-artifact loss", "Source-ready in this overlay", "Dedicated PostgreSQL execution pending"]) need(sources.actualization, marker, "historical parity actualization");
for (const marker of ["Rollback-Activated Artifact-Loss Recovery Actualization", "Exact rollback activation anchor", "Unexplained post-rollback version drift remains rejected"]) need(sources.rollbackActualization, marker, "current parity actualization");
