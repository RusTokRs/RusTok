...
- Latest maintenance update: admin UI получил явный FFA split `core` + `transport` + `ui/leptos`; GraphQL operations теперь живут в `admin/src/transport/graphql_adapter.rs`, legacy `admin/src/api.rs` удалён, а render/effect код вызывает только facade из `admin/src/transport/`.
- Latest FFA update: storefront UI получил matching split `core` + `transport` + `ui/leptos`; crate root re-export-ит `PagesView`, Leptos adapter вызывает только `storefront/src/transport/`, native server function живёт в `storefront/src/transport/native_server_adapter.rs`, GraphQL fallback живёт в `storefront/src/transport/graphql_adapter.rs`, legacy `storefront/src/api.rs` удалён. Быстрый guardrail `scripts/verify/verify-pages-ui-boundary.mjs` закрепляет admin/storefront boundary без full-workspace compile.
...
- Latest FFA maintenance update: admin capability-card presentation helpers (`publish_state_view`, `channel_count_label`, `legacy_block_snapshot_label`) и storefront list/empty-state helpers (`page_link_href`, `page_status_label`, `selected_page_empty_state`) вынесены в `core`, а `verify-pages-ui-boundary.mjs` теперь фиксирует эти no-compile boundary markers.
...
- Latest contract surface maintenance update: no-compile guardrail `verify-page-builder-pages-contract-surface.mjs` и no-compile fallback gates теперь фиксируют наличие публичных contract tests для CRUD/sanitize, builder round-trip, legacy blocks bridge, degraded fallback profiles, menu lifecycle, locale fallback, RBAC/channel visibility и manifest/provider drift; aggregate FBA baseline запускает этот guardrail без Cargo-компиляции.
...
- [x] контрактные тесты покрывают все публичные use-case для уже поставленных pages runtime surfaces (`verify-page-builder-pages-contract-surface.mjs` фиксирует coverage map без компиляции).
...
- targeted tests для CRUD, body sanitize, legacy blocks bridge, menus, locale fallback, builder round-trip, degraded fallback profiles, RBAC и channel visibility; no-compile coverage guardrail: `npm run verify:page-builder:pages:contract-surface`
...
### Тесты (Tests)
- [x] Добавить интеграционные тесты для degraded-режимов (publish_off, preview_off, builder_off) на уровне сервис-границ rustok-pages (`crates/rustok-pages/tests/page_service_kind_guard.rs` - `pages_builder_fallback_*` tests).
- [x] Написать автотесты для проверки корректности отображения ошибок маппинга FeatureDisabled в Leptos и Next.js UI-компонентах (`verify-page-builder-leptos-admin-parity.mjs` / `verify-page-builder-next-admin-parity.mjs`).
- [x] Реализовать тесты на обход visibility-ограничений (channel visibility, page channel visibility) для ролей с административными правами (`crates/rustok-pages/tests/rbac.rs`).
- [x] Зафиксировать карту публичных contract tests no-compile guardrail-ом `verify-page-builder-pages-contract-surface.mjs` и включить её в aggregate FBA baseline.

### Документация (Documentation)
- [x] Провести ревизию README.md и docs/README.md на соответствие реальной схеме БД (таблицы `pages`, `page_translations`, `page_bodies`, `page_blocks`, `page_channel_visibility`, `menus`, `menu_translations`, `menu_items`, `menu_item_translations`).
- [x] Документировать поведение degraded-режимов и toggle policy для tenant-флагов без необходимости передеплоя (`docs/README.md` + rollout policy section).
- [x] Зафиксировать в README архитектурное разделение ответственности между runtime-контуром pages и внешним провайдером page-builder.

### Инструменты и гейты верификации (Verification Gates)
- [x] Интегрировать скрипт verify-page-builder-pages-legacy-bridge.mjs в хуки pre-commit.
- [x] Внедрить запуск verify-pages-ui-boundary.mjs в pre-push пайплайн.
- [x] Реализовать автоматическую сверку схемы ошибок в CI между backend-слоем и UI-компонентами для предотвращения drift-а типов ошибок (`npm run verify:page-builder:error-catalog`, также входит в FBA baseline).
