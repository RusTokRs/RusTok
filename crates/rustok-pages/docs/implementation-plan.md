...
- Dependency evidence: storefront no-feature/hydrate profiles contain neither `rustok-core` nor backend `rustok-pages`; runtime dependencies are optional and enabled only by `ssr`. Pages reads its tenant-scoped builder settings through the public `rustok-tenant::TenantService::find_tenant_module` contract and never imports tenant entities.
