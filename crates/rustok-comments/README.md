...
- Own comment position allocation and thread counters below transport/service callers. Comment inserts lock the tenant thread row before deriving the next position; thread saves lock the same row and count active owner rows instead of trusting a stale caller value.
...
`comment::ActiveModelBehavior` is the position owner. It ignores a supplied
position on insert, serializes on the tenant thread row, and allocates the next
checked position. `comment_thread::ActiveModelBehavior` is the counter owner. It
recomputes `comment_count` from comments with `deleted_at IS NULL` while holding
the same owner lock.
...
`contracts/evidence/comments-thread-write-invariants.json`; the executable test
and source verifier are written but are not claimed as executed.
