...
pub(crate) async fn lock_active_topic_tag_write_in_tx(
    txn: &DatabaseTransaction,
    tenant_id: Uuid,
    topic_id: Uuid,
) -> ForumResult<forum_topic::Model> {
    let mut topics = lock_topic_rows_for_tags_in_tx(txn, tenant_id, &[topic_id]).await?;
    let topic = topics.pop().ok_or(ForumError::TopicNotFound(topic_id))?;
    if topic.status == TopicStatus::Archived {
        return Err(ForumError::Validation(
            "Forum topic tags cannot be changed after topic archival".to_string(),
        ));
    }
    Ok(topic)
}

pub(crate) async fn lock_topic_rows_for_tags_in_tx(
    txn: &DatabaseTransaction,
    tenant_id: Uuid,
    topic_ids: &[Uuid],
        ...
        let statement = match txn.get_database_backend() {
            ...
            backend => {
                return Err(ForumError::Validation(format!(
                    "Forum topic tag locking does not support database backend {backend:?}"
                )));
            }
        };
    ...
    if topics.len() != ids.len() {
        return Err(ForumError::Validation(
            "Forum topic tag lock lost a topic row".to_string(),
        ));
    }
...
pub(crate) async fn lock_topic_tag_scopes_in_tx(
    txn: &DatabaseTransaction,
    tenant_id: Uuid,
    topic_ids: &[Uuid],
) -> ForumResult<()> {
        ...
        DatabaseBackend::Postgres => {
            for topic_id in ids {
                txn.execute(Statement::from_sql_and_values(
                    ...
                    vec![format!("forum-topic-tag:{tenant_id}:{topic_id}").into()],
                ))
                .await?;
            }
        }
        DatabaseBackend::Sqlite => {
            for topic_id in ids {
                txn.execute(Statement::from_sql_and_values(
                    DatabaseBackend::Sqlite,
                    r#"
                    INSERT INTO forum_topic_tag_locks (
