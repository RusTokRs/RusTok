...
- Presents category, topic, reply-preview, localized route rename, merge, selected-reply split, reply-branch fork and reply-range move workflows as module-owned Forum pages.
...
- `admin/src/transport.rs` is the only UI-facing transport facade, selects exactly one merge transport per compile profile, and exposes slug rename, split, fork and reply-range GraphQL adapters without fallback.
