...
use crate::topic_split_model::{
    ForumTopicSplitIdentity, ForumTopicSplitReceipt, ForumTopicSplitReplyPage,
    build_forum_topic_split_command, forum_topic_split_candidate_label,
...
pub fn ForumTopicSplitAdmin() -> impl IntoView {
    ...
    let (selected_reply_ids, set_selected_reply_ids) = signal(Vec::<String>::new());
    let (target_locale, set_target_locale) = signal(requested_locale.clone());
    ...
    let (identity, set_identity) = signal(new_forum_topic_split_identity(""));
    ...
    let subtitle = t(
        ...
        "Create a new topic from one parent-closed set of replies through the idempotent Forum owner.",
    );
    ...
    let candidates = LocalResource::new(move || {
        ...
        async move { transport::fetch_topic_split_candidates(token, tenant, locale).await }
    });
    ...
    let submit = move |event: SubmitEvent| {
        ...
        let command = match build_forum_topic_split_command(
            &current_identity,
            source_topic_id.get_untracked().as_str(),
            &reply_page,
            selected_reply_ids.get_untracked().as_slice(),
    ...
    view! {
        <section class="space-y-6">
            <header class="rounded-[2rem] border border-border bg-gradient-to-br from-card via-card to-muted/40 px-6 py-7 shadow-sm lg:px-8">
                <p class="text-xs font-semibold uppercase tracking-[0.24em] text-muted-foreground">"FORUM-21V"</p>
            ...
            {move || receipt.get().map(|receipt| view! {
                <article class="rounded-[1.75rem] border border-emerald-500/30 bg-emerald-500/10 p-5 text-sm">
                    ...
                    <dl class="mt-3 grid gap-2 text-muted-foreground sm:grid-cols-2">
                        ...
                        <div><dt class="font-medium text-foreground">"Target published replies"</dt><dd>{receipt.target_resulting_published_reply_count}</dd></div>
                    </dl>
                </article>
            })}
            ...
            <form class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]" on:submit=submit>
                <section class="space-y-6 rounded-[1.75rem] border border-border bg-card p-6 shadow-sm">
                            ...
                            Ok(items) => view! {
                                <label class="block space-y-2 text-sm font-medium text-foreground">
                                    ...
                                    >
                                        ...
                                        {items.into_iter().filter(|item| item.reply_count >= 2).map(|item| {
                                            let label = forum_topic_split_candidate_label(&item);
                    ...
                    <section class="space-y-3">
                        <h2 class="text-sm font-semibold text-foreground">{replies_label}</h2>
                                ...
                                Ok(page) if !page.items.is_empty() => view! {
                                    <div class="max-h-96 space-y-2 overflow-y-auto rounded-2xl border border-border p-3">
                                        {page.items.into_iter().map(|reply| {
                                            ...
                                            let label = forum_topic_split_reply_label(&reply);
                                            view! {
                                                <label class="flex items-start gap-3 rounded-xl px-3 py-2 text-sm hover:bg-muted/50">
                                                    <input
                                                        class="mt-1"
                                                        type="checkbox"
                                                        prop:checked=move || selected_reply_ids.get().iter().any(|value| value == &checked_id)
                                                        on:change=move |event| {
                                                            let checked = event_target_checked(&event);
                                                            set_selected_reply_ids.update(|values| {
                                                                values.retain(|value| value != &change_id);
                                                                if checked {
                                                                    values.push(change_id.clone());
                                                                }
                                                            });
                                                            rotate_command_identity(
                                                                set_identity,
...
fn rotate_command_identity(
    set_identity: WriteSignal<ForumTopicSplitIdentity>,
    set_receipt: WriteSignal<Option<ForumTopicSplitReceipt>>,
