...
- держать REST handlers на узком `ForumHttpRuntime` с явными DB/event bus handles; текущий Loco `AppContext` остаётся только в route-state adapter до полного Axum cutover;
