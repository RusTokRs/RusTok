...
### Pre-audit terminal rows

Application rows that were already `applied`, `rejected` or `operator_review` before this lifecycle source became active are a distinct upgrade boundary. They are terminal/non-due, so they cannot pass through the new finalizer simply to create case/audit history. Their Moderation case may still reflect the earlier pre-audit state and truthful start/terminal audit facts may not exist.

This slice does not change those terminal operation truths, does not call Forum again and does not invent historical timestamps/events. The next bounded Moderation operator-recovery slice must include tenant/decision-scoped legacy-terminal reconciliation. For an already-applied operation it must preserve applied truth while reconciling the owner case explicitly; rejected/operator-review remain fail-closed and operator-visible. Forum owns none of this recovery state.

`moderation_events` remains an internal owner audit ledger. This slice does not freeze a typed `rustok-events` Moderation application event family. Bounded operator retry/requeue/re-review commands and legacy-terminal reconciliation remain the next owner code milestone.
