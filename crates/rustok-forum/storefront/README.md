...
The package keeps runtime-independent storefront policy in `src/core.rs`: route hrefs, rich-content summaries, count/slug labels, category/topic card view-models, accent fallback, and stable status badge class mapping. `src/transport.rs` remains the build-profile-selected native/GraphQL selected-path facade, while `src/ui/leptos.rs` is the explicit Leptos adapter. The fast non-compiling guardrail is `npm run verify:forum:storefront-boundary`.
