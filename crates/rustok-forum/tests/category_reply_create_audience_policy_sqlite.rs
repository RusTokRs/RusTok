...
use rustok_forum::{
    ...
    ForumCategoryReplyCreateAudiencePolicyService, ForumCategoryTopicCreateAudiencePolicyService,
    ForumError, ForumModule, SetForumCategoryReplyCreateAudiencePolicyInput,
};
...
async fn setup() -> DatabaseConnection {
    let db_url = format!(
        "sqlite:file:forum_category_reply_create_audience_{}?mode=memory&cache=shared",
        Uuid::new_v4()
    );
    ...
    let db = Database::connect(options)
        .await
        .expect("forum category reply-create audience sqlite database should connect");
...
#[tokio::test]
async fn category_reply_create_audience_is_separate_inherited_and_database_bounded() {
    let db = setup().await;
    ...
    let root_policy = policies
        ...
        .await
        .expect("root reply-create audience layer should persist");
    ...
    let topic_create_policy = ForumCategoryTopicCreateAudiencePolicyService::new(db.clone())
        .get(tenant_id, root, admin.clone())
        .await
        .expect("topic-create audience policy should remain readable");
    assert!(topic_create_policy.configured_constraints.is_none());
    assert!(topic_create_policy.effective_layers.is_empty());
    ...
    policies
        ...
        .await
        .expect("child reply-create audience layer should persist");

    let inherited = policies
        .get(tenant_id, leaf, admin.clone())
        .await
        .expect("leaf effective reply-create audience should resolve");
    ...
    let cleared = policies
        ...
        .await
        .expect("empty constraints should clear the local reply-create layer");
