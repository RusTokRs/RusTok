...
async fn profile_resolution_is_tenant_scoped_and_fail_closed() {
    ...
    let resolved = resolve_forum_mentions(
        &reader,
        tenant_id,
        ForumMentionCandidates::new(["alice".to_string()], []).expect("candidates"),
        Some("en"),
        Some("en"),
    )
    .await
    ...
    let error = resolve_forum_mentions(
        &reader,
        tenant_id,
        ForumMentionCandidates::new(["private-user".to_string()], []).expect("candidates"),
        Some("en"),
        Some("en"),
    )
    .await
...
#[test]
fn revision_diff_emits_only_new_targets_and_replay_is_immutable() {
    let tenant_id = Uuid::new_v4();
