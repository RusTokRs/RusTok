...
- `ConsumerPoisonIdentity` requires a non-nil deterministic delivery UUID, bounded consumer group/stream/topic, positive partition, representable offset, and non-empty exact payload.
...
- Poison receipt source coordinates are unique. Reuse with another deterministic delivery UUID, payload, error classification, or attempt count fails closed as an identity conflict.
