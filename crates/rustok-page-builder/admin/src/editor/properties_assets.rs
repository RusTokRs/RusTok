...
pub fn PropertiesAssetsPanel(runtime: AdminEditorRuntime) -> impl IntoView {
    ...
    let diagnostics_label = t(locale.as_deref(), "page_builder.panel.diagnostics", "Diagnostics");
    ...
    let attribute_name_label = t(locale.as_deref(), "page_builder.field.attributeName", "Attribute name");
    let attribute_value_label = t(locale.as_deref(), "page_builder.field.attributeValue", "Attribute value or JSON");
    let style_property_label = t(locale.as_deref(), "page_builder.field.styleProperty", "Style property");
    let style_value_label = t(locale.as_deref(), "page_builder.field.styleValue", "Style value");
    ...
    let selected_label = t(locale.as_deref(), "page_builder.field.selectedComponent", "Selected component");
    let none_label = t(locale.as_deref(), "page_builder.field.none", "None");
    let project_hash_label = t(locale.as_deref(), "page_builder.field.projectHash", "Project hash");
            ...
            if let Some(selected) = selected {
                ...
                style_value.set(
                    selected
                        ...
                        .and_then(|style| style.get(&style_property.get_untracked()))
                ...
                style_value.set(String::new());
            }
        }
    });

    let style_descriptors = builtin_style_properties();
        ...
        <aside class="space-y-4 overflow-auto rounded-xl border border-border bg-card p-3">
            <section class="space-y-3">
                ...
                <dl class="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1 text-sm">
                    ...
                    <dd class="break-all">{move || runtime.controller.with(|controller| {
                        controller.ui().state.selection.component_id.clone().unwrap_or_else(|| none_label.clone())
                    })}</dd>
                    <dt class="text-muted-foreground">"Type"</dt>
                    <dd>{move || runtime.controller.with(|controller| {
                    ...
                    <button
                        ...
                        on:click={
                            ...
                            move |_| {
                                ...
                                runtime.dispatch_result(selected_patch(&runtime, patch));
                                runtime.announce(apply_label.clone());
                            }
                        }
                ...
                <div class="flex gap-2">
                    <button
                        ...
                        on:click={
                            let runtime = runtime.clone();
                            move |_| runtime.dispatch_result(selected_patch(&runtime, ComponentPatch {
                                fields: Map::from_iter([("content".to_string(), Value::String(content_value.get_untracked()))]),
                ...
                <button
                    ...
                    on:click={
                        ...
                        move |_| {
                            ...
                            if name.is_empty() {
                                runtime.fail("attribute name must not be empty");
                                return;
                            }
                            runtime.dispatch_result(selected_patch(&runtime, ComponentPatch {
                                attributes: Map::from_iter([(name, parse_scalar(&attribute_value.get_untracked()))]),
                                ..ComponentPatch::default()
                            }));
                        }
                    }
                ...
                <div class="space-y-1">
                    {move || runtime.controller.with(|controller| controller.selected_component_view()).map(|selected| {
                            ...
                            view! {
                                <div class="flex items-start gap-2 rounded bg-muted/50 px-2 py-1 text-xs">
                                    ...
                                    <button
                                        ...
                                        on:click=move |_| runtime.dispatch_result(selected_patch(&runtime, ComponentPatch {
            ...
            <section class="space-y-2 border-t border-border pt-3">
                ...
                <label class="block text-sm font-medium">{style_property_label}</label>
                <select
                    class="w-full rounded border border-input bg-background px-2 py-1 text-sm"
                    ...
                    on:change={
                        ...
                        move |event| {
                            ...
                            style_value.set(runtime.controller.with(|controller| {
                                controller
                ...
                >
                    {style_descriptors.into_iter().map(|descriptor| view! {
                        <option value=descriptor.property>{format!("{:?} · {}", descriptor.group, descriptor.label)}</option>
                    }).collect_view()}
                </select>
                <label class="block text-sm font-medium">{style_value_label}</label>
                <input
                    class="w-full rounded border border-input bg-background px-2 py-1 text-sm"
                ...
                <div class="flex gap-2">
                    <button
                        ...
                        on:click={
                            let runtime = runtime.clone();
                            move |_| match style_patch([StyleEntry {
                                property: style_property.get_untracked(),
                                value: style_value.get_untracked(),
                            }]) {
                                Ok(patch) => runtime.dispatch_result(selected_patch(&runtime, patch)),
                                Err(errors) => runtime.fail(errors.into_iter().map(|error| format!("{}: {}", error.property, error.message)).collect::<Vec<_>>().join("; ")),
                            }
                        }
                    >{apply_label.clone()}</button>
                    <button
            ...
            <section class="space-y-2 border-t border-border pt-3">
                ...
                <button
                    ...
                    on:click={
                        ...
                        move |_| {
                            ...
                            if source.is_empty() {
                                runtime.fail("asset URL must not be empty");
                                return;
                            }
                            let id = asset_id.get_untracked().trim().to_string();
                            let asset = if id.is_empty() { json!({ "src": source }) } else { json!({ "id": id, "src": source }) };
                ...
                <div class="space-y-1">
                    {move || {
                        let catalog = runtime.controller.with(|controller| AssetCatalog::from_document(controller.editor().document()));
                                ...
                                <div class="rounded border border-border p-2 text-xs">
                                    ...
                                    <div class="mt-2 flex gap-2">
                                        ...
                                        <button
                                            ...
                                            on:click=move |_| remove_runtime.dispatch(UiIntent::Execute(EditorCommand::Asset {
                                                command: AssetCommand::Remove { asset_id: remove_id.clone() },
                                            }))
                                        >{remove_label}</button>
                                    </div>
                                </div>
            ...
            <section class="space-y-1 border-t border-border pt-3 text-sm">
                ...
                <p class="break-all">{move || runtime.controller.with(|controller| {
                    format!("{project_hash_label}: {}", controller.editor().revision().project_hash.hex())
                })}</p>
                {move || runtime.controller.with(|controller| controller.ui().state.diagnostics.clone()).into_iter().map(|diagnostic| view! {
