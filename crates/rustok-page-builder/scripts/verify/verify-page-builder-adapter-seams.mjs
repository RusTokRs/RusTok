...
for (const entrypoint of contract.canonical_entrypoints ?? []) {
  const source = entrypoint.includes("handle_page_builder_") ? readme : service + readme;
...
for (const seam of [contract.seams?.persistence?.trait, contract.seams?.rendering?.trait, contract.seams?.adapter_backed_service?.type]) {
  ...
  if (!readme.includes(seam)) fail(`adapter seam '${seam}' is not documented in local docs`);
}
...
for (const status of contract.seams?.adapter_backed_service?.operation_statuses ?? []) {
  ...
  if (!readme.includes(status)) fail(`adapter operation status '${status}' is not documented in local docs`);
}

for (const blocked of contract.blocked_patterns ?? []) {
  if (!centralPlan.includes(blocked) && !readme.includes(blocked)) {
