...
Server-side capability handlers enforce стабильную page permission map перед делегированием в provider service. `pages:manage` остаётся effective override для всех builder capabilities. `PageBuilderCapabilityPortPolicies` держит owner-side source-lock для registry/manifest policy names: `preview`, `tree` и `properties` требуют `PortCallPolicy::read()` deadline semantics, а `publish` требует `PortCallPolicy::write()` с deadline и idempotency key.
