...
The Social Graph command port also owns versioned durable receipts. Tenant-scoped normalized idempotency keys are bound to the complete relation command identity, exact replay returns the original committed response snapshot without rewinding newer live state, and mismatched key reuse fails with a typed conflict. Receipt reservation, relation mutation, response persistence, and completion share one transaction; raw keys and payloads remain owner-private and absent from telemetry.
...
2. **Finish followers-only and downstream presentation policy.**
   ...
   **Social Graph status:** completed for the required owner capability, public GraphQL transport, source-level command telemetry, and durable command receipts. `SocialRelationKind::Follow`, PostgreSQL/SQLite relation and receipt migrations, directional single reads, bounded/deduplicated batch reads, revision-bearing state reads, actor binding, tenant integrity, and targeted source tests are present. Profiles calls `SocialGraphPrivacyReadPort` in chunks of at most 100 and propagates owner failures instead of converting them into allow. `isFollowing`, `followState`, `followUser`, and `unfollowUser` expose tenant-gated human-user transport without relation storage leakage. Exact command replay is owner-transactional and mismatched key reuse is rejected without relation mutation.
   ...
   **Remaining:** configure production mutual TLS/client identity and reachable Media HTTP ingress, then collect compiled/runtime evidence for Blog, Forum, Customer Admin, storefront audience states, embedded/remote selection, public-image route/cache/degradation behavior, Local/S3 delivery, Social Graph receipt replay/conflict, and command telemetry outcomes.
...
5. **Add audit and operational capabilities from defined owner contracts.**
   **Status:** in progress. Stable GraphQL self-service, CLI backfill, and Social Graph relation-command operation names, success/failure outcomes, duration, stable non-value-bearing error codes, retryability, aggregate backfill counters/stages, `profile.updated` publication outcomes, and versioned durable Social Graph command receipts are source-complete and locked by source verifiers. `PresentationUnavailable` is handled by both GraphQL query and mutation error mappers.
   **Remaining:** add relation outbox events/reconciliation, define receipt retention/cleanup and rollout/rollback guidance, reconcile the central FFA/FBA readiness board, prove PostgreSQL receipt concurrency, and collect retained runtime evidence.
...
- Added versioned durable Social Graph command receipts with tenant/key uniqueness, exact response replay, mismatched-payload conflict, one-transaction relation completion, and fail-closed corrupt-record handling.
...
- `docs/modules/registry.md` still contains the pre-UI Profiles FFA/FBA row and remains the final central readiness reconciliation item.
...
- storefront route, module-package, en/ru i18n, Media provider/delivery/degradation, receipt replay/conflict, Profiles/Social Graph operation telemetry, and accessibility verification
...
10. Storefront follow controls must bind authenticated tenant/user through owner ports, suppress self-follow, use unique idempotency keys and optimistic revision semantics, require owner receipts to bind each key to one exact command identity, recover stale state through read-only refresh, never retry writes automatically, and avoid exposing internal identifiers or transport details.
11. Operational telemetry may record stable operation/error identifiers, tenant/user correlation for Profiles writes, tenant/source/target UUIDs for Social Graph commands, tenant scope and aggregate counters for backfill, mode flags, stages, outcomes, retryability, and duration only; it must not record source email, generated handle, display copy, bio, locale values, Media identifiers/URLs, object keys, provider endpoints, idempotency keys, expected revisions, request correlation, channel, claims, roles, or receipt payloads.
