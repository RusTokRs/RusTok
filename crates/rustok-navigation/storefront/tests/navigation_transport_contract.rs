...
fn navigation_owns_active_menu_transport_and_slot_views() {
    for marker in ["StorefrontMenuLocation", "fetch_active_menu", "navigation/active-menu", "MenuBindingService::new",
        "NavigationHeaderMenu", "NavigationView"] {
        assert!(MODEL.contains(marker) || TRANSPORT.contains(marker) || NATIVE.contains(marker) || UI.contains(marker), "missing `{marker}`");
    }
