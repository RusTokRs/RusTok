...
#[derive(Debug, Serialize)]
struct PoisonAmbiguityEvidenceArtifact {
    contract: &'static str,
    task: &'static str,
...
struct PostgresIggyPoisonEvidence {
...
impl PostgresIggyPoisonEvidence {
    async fn setup(
        database_url: &str,
        config: IggyConfig,
        scope: &str,
...
async fn raw_poison_publish_mark_ambiguity_obeys_configured_dedup_modes() -> TestResult<()> {
    ...
    let Some(dedup_enabled) = external_iggy_config(DEDUP_ENABLED_ADDRESS_ENV, "dedup-enabled")?
        ...
        eprintln!(
            "{DEDUP_ENABLED_ADDRESS_ENV} is not set; skipping Forum Search poison ambiguity proof"
        );
        return Ok(());
    };
    let Some(dedup_disabled) =
        ...
        eprintln!(
            "{DEDUP_DISABLED_ADDRESS_ENV} is not set; skipping Forum Search poison ambiguity proof"
        );
        return Ok(());
    };

    let enabled = exercise_publish_mark_ambiguity(
        &database_url,
        dedup_enabled,
        "dedup_enabled",
        1,
    ...
    .await?;
    let disabled = exercise_publish_mark_ambiguity(
        &database_url,
        dedup_disabled,
        "dedup_disabled",
        2,
    ...
    write_evidence(PoisonAmbiguityEvidenceArtifact {
...
async fn exercise_publish_mark_ambiguity(
    ...
    let evidence = PostgresIggyPoisonEvidence::setup(database_url, config, scope).await?;
    ...
    let first_payload = match expected_physical_dlq_messages {
        1 => vec![0xff, 0x00, 0x61, 0x01],
        2 => vec![0xff, 0x00, 0x62, 0x01],
        other => {
            return Err(invalid_data(format!(
                "unsupported expected DLQ message count {other}"
            ))
            .into());
        }
    };
    let second_payload = match expected_physical_dlq_messages {
        1 => vec![0xff, 0x00, 0x61, 0x02],
        2 => vec![0xff, 0x00, 0x62, 0x02],
        _ => unreachable!("validated expected count"),
    };
    ...
    if first_failure.raw_payload() != first_payload.as_slice() {
        return Err(invalid_data("first raw poison payload changed before receipt reservation").into());
    }
    ...
    {
        return Err(invalid_data("first poison publisher did not own the durable claim").into());
    }
    ...
    if deterministic_message_id != first_delivery_id {
        return Err(invalid_data("raw poison DLQ message ID differs from deterministic delivery ID").into());
    }
    ...
    {
        return Err(invalid_data("unexpired poison publisher claim was not reported busy").into());
    }
    ...
    if redelivered.offset() != first_offset
        ...
        || redelivered.raw_payload() != first_payload.as_slice()
    {
        return Err(invalid_data(
            "restarted consumer did not receive the exact unacknowledged raw poison delivery",
        )
        .into());
    }
    ...
    if store
        ...
        != ConsumerPoisonPublishClaim::Claimed
    {
        return Err(invalid_data("restarted publisher did not reclaim the expired receipt").into());
    }
    if !matches!(
        store.mark_published(&identity, first_publisher).await,
        Err(ConsumerPoisonReceiptError::ClaimLost)
    ) {
        return Err(invalid_data("stale poison publisher retained authority after lease takeover").into());
    }
    ...
    if next_failure.raw_payload() != second_payload.as_slice() || next_failure.offset() <= first_offset
    {
        return Err(invalid_data(
            "successful poison terminalization did not advance to the next source delivery",
        )
        .into());
    }
...
async fn receive_decode_failure(
    ...
    match delivery {
        ...
        PersistentContractDelivery::Event(_) => Err(invalid_data(
            "malformed Forum Search ambiguity fixture unexpectedly decoded as a contract event",
        )
        .into()),
    }
}
...
async fn assert_receipt_state(
    ...
    let receipt = store
        .find(identity)
        .await?
        .ok_or_else(|| invalid_data("expected Forum Search poison receipt was not found"))?;
...
async fn message_count(client: &IggyClient, stream: &str) -> TestResult<u64> {
    ...
    let topic = client
        ...
        .await?
        .ok_or_else(|| invalid_data("Forum Search poison ambiguity DLQ topic is missing"))?;
    let partition = topic
        .partitions
        .iter()
        .find(|partition| partition.id == 1)
        .ok_or_else(|| invalid_data("Forum Search poison ambiguity DLQ partition 1 is missing"))?;
...
async fn assert_message_count(client: &IggyClient, stream: &str, expected: u64) -> TestResult<()> {
    ...
    if observed != expected {
        return Err(invalid_data(format!(
            "expected {expected} physical Forum Search DLQ messages but observed {observed}"
        ))
        .into());
    }
    Ok(())
...
fn external_iggy_config(address_env: &'static str, scope: &str) -> TestResult<Option<IggyConfig>> {
    ...
    if username.is_empty() != password.is_empty() {
        return Err(invalid_data(
            "Forum Search ambiguity Iggy username and password must both be set or both be empty",
        )
        .into());
    }
...
fn ensure_distinct_mode_addresses() -> TestResult<()> {
    ...
    if let (Some(enabled), Some(disabled)) = (enabled, disabled) {
        ...
        if enabled == disabled {
            return Err(invalid_data(
                "dedup-enabled and dedup-disabled Forum Search evidence require distinct Iggy addresses",
            )
            .into());
        }
    }
...
fn connection_string(config: &ExternalConfig) -> Result<String, IoError> {
    let address = config
        .addresses
        .first()
        .ok_or_else(|| invalid_data("Forum Search ambiguity Iggy address is missing"))?;
    if config.protocol != "tcp" {
        return Err(invalid_data("Forum Search poison ambiguity evidence requires TCP"));
    }
    if config.tls_enabled || config.tls_domain.is_some() || config.tls_ca_file.is_some() {
        return Err(invalid_data(
            "Forum Search poison ambiguity source harness does not claim TLS coverage",
        ));
    }
...
fn unique_name(scope: &str) -> String {
    format!(
        "rustok-forum-search-poison-{scope}-{}",
        Uuid::new_v4().simple()
    )
}

fn write_evidence(artifact: PoisonAmbiguityEvidenceArtifact) -> TestResult<()> {
    ...
    fs::write(&path, bytes)?;
    eprintln!(
        "wrote Forum Search poison ambiguity evidence to {}",
        path.display()
    );
    Ok(())
}
