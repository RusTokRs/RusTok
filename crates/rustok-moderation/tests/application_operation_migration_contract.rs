...
impl PostgresHarness {
    async fn setup() -> TestResult<Option<Self>> {
        ...
        let control = connect_postgres(&database_url, "moderation_migration_contract_control").await?;
        Ok(Some(Self {
            control,
            database_url,
        }))
    ...
    async fn create_schema(&self, label: &str) -> TestResult<(String, DatabaseConnection)> {
        ...
        let db = connect_postgres(
            &self.database_url,
            &format!("moderation_migration_{label}"),
        )
        .await?;
    ...
    async fn drop_schema(&self, schema_name: &str) -> TestResult<()> {
        self.control
...
async fn postgres_clean_and_upgrade_application_operation_migration_contract() -> TestResult<()> {
    ...
    let upgrade_result = async {
        ...
        assert_eq!(migration_count(&upgrade_db).await?, 3);
...
"#,
    ...
    assert_eq!(typed.try_get::<String>("", "status")?, "pending");
    assert_eq!(typed.try_get::<i32>("", "attempt_count")?, 0);
    for flag in [
        "tenant_matches",
        "case_matches",
        "hash_matches",
        ...
        assert_flag(&typed, flag)?;
    }

    let typed_identity = db
        .query_one(Statement::from_string(
            db.get_database_backend(),
            format!(
    ...
    assert_eq!(typed_identity.try_get::<Uuid>("", "tenant_id")?, fixture.tenant_id);
    assert_eq!(typed_identity.try_get::<Uuid>("", "case_id")?, fixture.typed_case_id);
    assert_eq!(typed_identity.try_get::<Uuid>("", "subject_id")?, fixture.typed_subject_id);
    assert_eq!(
        typed_identity.try_get::<i64>("", "subject_revision")?,
...
SELECT
    ...
    created_at,
    updated_at
FROM moderation_application_operations
WHERE 1 = 0
"#
        .to_string(),
    ))
    ...
    let index_names = application_index_names(db).await?;
    assert!(index_names.contains("idx_moderation_application_operations_due"));
    assert!(index_names.contains("idx_moderation_application_operations_case"));
    Ok(())
}
...
async fn seed_legacy_decisions(db: &DatabaseConnection) -> TestResult<UpgradeFixture> {
    ...
    let untyped_subject_id = Uuid::new_v4();
...
"#,
    ...
    assert_eq!(
        scalar_i64(
            db,
            "SELECT COUNT(*) AS value FROM moderation_decisions".to_string(),
        )
        .await?,
        2
    );
    assert_eq!(
        scalar_i64(
            db,
            "SELECT COUNT(*) AS value FROM moderation_decision_effects".to_string(),
        )
        .await?,
        1
    );
...
async fn insert_legacy_decision(
    db: &DatabaseConnection,
    tenant_id: Uuid,
    decision_id: Uuid,
    case_id: Uuid,
...
async fn application_operation_count(db: &DatabaseConnection) -> TestResult<i64> {
    scalar_i64(
        db,
        "SELECT COUNT(*) AS value FROM moderation_application_operations".to_string(),
    )
    .await
}
...
async fn migration_count(db: &DatabaseConnection) -> TestResult<i64> {
    scalar_i64(
        db,
        "SELECT COUNT(*) AS value FROM seaql_migrations".to_string(),
    )
    .await
}
