...
- Keeps Leptos render/bind code in `admin/src/ui/leptos.rs`; `admin/src/lib.rs` only wires modules and re-exports `BlogAdmin`, while transport-specific GraphQL code stays under `admin/src/transport/`.
...
- Uses the `rustok-blog` GraphQL contract via the package transport facade (`transport/mod.rs` -> `transport/graphql_adapter.rs`), plus shared Leptos host libraries.
