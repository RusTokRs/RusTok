...
- Current phase: ffa_admin_presentation_classes
- Last checkpoint: FFA slice #102 moved admin editor field CSS class selection into the Leptos-free `BlogPostAdminEditorFieldClassesViewModel` / `blog_post_admin_editor_field_classes_view`, so text inputs, textareas, checkbox label and submit button classes are prepared by core while the adapter keeps only markup and event binding.
