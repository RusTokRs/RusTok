...
- Last checkpoint: FFA slice #95 moved admin status-badge presentation into `BlogPostStatusBadgeViewModel`, restored the raw-body warning memo as an explicit core-owned render payload, and memoized the form view-model so the Leptos adapter no longer rebuilds form labels separately for title/button fields.
- Current phase: ffa_admin_status_badge_form_view_boundary
- Next step: Continue with small admin render/command fragments that reduce real coupling without changing the dual-path contract; avoid shell-copy over-extraction and keep verifier fixture coverage synchronized with any new helper markers.
- Open blockers: None.
...
- [x] Slice 95: admin status-badge presentation moved into `BlogPostStatusBadgeViewModel`, raw-body warning rendering now uses an explicit memoized core payload, and form title/submit state now reuses one memoized `BlogPostAdminFormViewModel`; guardrail markers and fixture tests were updated for the new helper boundary. Evidence: `node scripts/verify/verify-blog-admin-boundary.mjs`; `node --test scripts/verify/verify-blog-admin-boundary.test.mjs`; no Cargo compilation was run by request.
