...
- Current phase: ffa_admin_route_query_intent_core_boundary
- Last checkpoint: FFA slice #87 moved admin selected-post route/query push/replace/clear intent selection into `BlogPostAdminRouteQueryIntent`, so the Leptos adapter only applies prepared writes through the host query writer.
