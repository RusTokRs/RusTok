...
- Current phase: ffa_admin_body_format_change
- Last checkpoint: FFA slice #100 moved admin body-format change normalization into Leptos-free `BlogPostAdminBodyFormatChangeViewModel` / `blog_post_admin_body_format_change_view`, so unsupported select events fall back to the canonical `markdown` format in core before the Leptos adapter mutates signal state.
