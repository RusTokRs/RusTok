...
- Current phase: ffa_admin_shell_header_core_boundary
- Last checkpoint: FFA slice #91 moved admin shell header badge/title/subtitle payload into `BlogPostAdminShellViewModel`, so the Leptos adapter renders prepared shell copy instead of owning header presentation literals.
...
- [x] Slice 91: admin shell header copy moved into `BlogPostAdminShellViewModel`; Leptos now asks core for badge/title/subtitle payload and renders markup only. Evidence: `node scripts/verify/verify-blog-admin-boundary.mjs`; no Cargo compilation was run by request.
