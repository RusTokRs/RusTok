...
- Current phase: ffa_admin_graphql_adapter_transport_boundary
- Last checkpoint: FFA slice #88 moved the admin GraphQL adapter from legacy root `admin/src/api.rs` into `admin/src/transport/graphql_adapter.rs`; the crate root now wires only `core`, `transport` and `ui`, and `transport/mod.rs` owns the GraphQL adapter boundary.
- Next step: Continue adapter-level parity evidence around transport failure classification, or take only slices that move real transport/state/policy ownership; do not add copy-only helpers.
- Open blockers: None.
...
- Last updated at (UTC): 2026-06-14T19:05:00Z
