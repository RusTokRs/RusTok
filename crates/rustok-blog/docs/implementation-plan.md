...
- Last checkpoint: FFA slice #72 moved storefront shell copy and route/query state into `storefront/src/core.rs` via `BlogStorefrontShellViewModel`, `BlogStorefrontRouteState`, and the stable `SELECTED_POST_QUERY_KEY` contract.
- Next step: Выбрать следующий один storefront/admin use-case for core extraction or split the existing storefront `transport.rs` facade into explicit native/GraphQL adapter modules without changing the dual-path contract.
- Open blockers: None.
...
- [x] Pass #1 (code/docs consistency): storefront helper extraction and `slug` route/query key contract reflected in tracker and local docs.
