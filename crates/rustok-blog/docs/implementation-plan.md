...
- Current phase: ffa_admin_warning_and_posts_load_core_boundary
- Last checkpoint: FFA slices #89-#90 moved admin raw-body warning visibility/message and posts-list load-result/contract-unavailable/error presentation into Leptos-free core view-models, so the Leptos adapter only renders prepared payloads and keeps transport classification at the facade edge.
- Next step: Continue with small admin render/command fragments that reduce real coupling without changing the dual-path contract, or add adapter-level parity evidence around remaining transport failure classification.
- Open blockers: None.
