...
- Current phase: fba_comments_consumer_static_metadata
- Last checkpoint: FBA comments consumer slice added `crates/rustok-blog/contracts/blog-fba-registry.json`, `crates/rustok-blog/contracts/evidence/blog-comments-consumer-static-matrix.json`, manifest consumer metadata and `npm run verify:blog:fba` to lock blog -> comments `CommentsThreadPort` dependency without raising status above `in_progress` before runtime fallback smoke.
- Previous checkpoint: FFA slice #96 moved admin posts-table header/empty-state/row normalization into Leptos-free `BlogPostAdminPostsTableViewModel` / `blog_post_admin_posts_table_view_from_items`, so the Leptos table adapter no longer rebuilds table and row view-models inside the row render loop; fast guardrail markers and fixtures were extended for the posts-table envelope.
- Next step: Add runtime contract execution/fallback smoke for blog comments degraded modes, or continue small admin render fragments without changing the dual-path contract.
- Open blockers: None.
...
- Evidence:
  ...
  - FBA consumer metadata now locks the blog -> comments boundary: `crates/rustok-blog/contracts/blog-fba-registry.json` declares the `blog_post_comments` consumer profile for `CommentsThreadPort` / `comments.thread.v1`, `crates/rustok-blog/contracts/evidence/blog-comments-consumer-static-matrix.json` mirrors planned consumer contract cases/fallback degraded modes, and `scripts/verify/verify-blog-fba.mjs` checks drift against `crates/rustok-comments/contracts/comments-fba-registry.json` without long compilation;
  - backend boundary пока работает в in-process модели; remote extraction readiness ведётся как эволюционный трек без смены ownership/contract, поэтому FBA status остаётся `in_progress` до runtime contract execution/fallback smoke;
