...
- Current phase: ffa_admin_table_classes
- Last checkpoint: FFA slice #103 moved admin posts-table CSS class selection into the Leptos-free `BlogPostAdminTableClassesViewModel` / `blog_post_admin_table_classes_view`, so table containers, headers, rows, cells and action button classes are prepared by core while the adapter keeps markup and event binding only.
- Previous checkpoint: FFA slice #102 moved admin editor field CSS class selection into the Leptos-free `BlogPostAdminEditorFieldClassesViewModel` / `blog_post_admin_editor_field_classes_view`, so text inputs, textareas, checkbox label and submit button classes are prepared by core while the adapter keeps only markup and event binding.
