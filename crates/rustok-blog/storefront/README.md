...
- Uses native `#[server] -> HostRuntimeContext -> PostService -> DB/event bus` on the SSR/hydrate path through the native adapter and uses the `rustok-blog` GraphQL adapter on the headless/CSR path.
