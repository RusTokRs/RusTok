        ...
        match manager.get_database_backend() {
            ...
            backend => {
                return Err(DbErr::Custom(format!(
                    "Blog Category Taxonomy backfill does not support {backend:?}",
                )));
            }
        }

        let txn = manager.get_connection().begin().await?;
        backfill_blog_categories(&txn).await?;
        txn.commit().await?;
        Ok(())
    }

    async fn down(&self, _manager: &SchemaManager) -> Result<(), DbErr> {
        // This is a monotonic copy. Blog-owned category rows and localized copy
        // remain authoritative until later CAT-6 read/write cutover slices, so
        // rollback does not delete truthful Taxonomy data or bindings.
        Ok(())
    }
}

async fn backfill_blog_categories(txn: &sea_orm::DatabaseTransaction) -> Result<(), DbErr> {
    let categories = blog_category::Entity::find()
        .order_by_asc(blog_category::Column::TenantId)
    ...
    // Every identity must exist before parent references are copied.
    for category in &categories {
        ensure_category_hierarchy(txn, category).await?;
    }

    for category in &categories {
        ensure_blog_binding(txn, category).await?;
    }
...
async fn ensure_taxonomy_term(
    ...
    let canonical_key = canonical_key_for_blog_category(category.id);
        ...
        .await?
    {
        if existing.tenant_id != category.tenant_id
            ...
            || existing.scope_value != BLOG_SCOPE_VALUE
            || existing.canonical_key != canonical_key
        {
            return Err(DbErr::Migration(format!(
                "Blog Category Taxonomy backfill blocked: UUID {} is already owned by an incompatible Taxonomy term",
                category.id,
            )));
        }
        return Ok(());
    ...
    if let Some(existing) = taxonomy_term::Entity::find()
        ...
        .filter(taxonomy_term::Column::ScopeValue.eq(BLOG_SCOPE_VALUE))
        .filter(taxonomy_term::Column::CanonicalKey.eq(&canonical_key))
        .one(txn)
        .await?
    {
        return Err(DbErr::Migration(format!(
            "Blog Category Taxonomy backfill blocked: canonical key {canonical_key:?} is already owned by Taxonomy Category {}",
            existing.id,
        )));
    }
    ...
    taxonomy_term::ActiveModel {
        ...
        scope_value: Set(BLOG_SCOPE_VALUE.to_owned()),
...
async fn ensure_category_translations_and_routes(
    ...
    let translations = blog_category_translation::Entity::find()
        .filter(blog_category_translation::Column::TenantId.eq(category.tenant_id))
        .filter(blog_category_translation::Column::CategoryId.eq(category.id))
        .order_by_asc(blog_category_translation::Column::Locale)
        .order_by_asc(blog_category_translation::Column::Id)
        .all(txn)
        .await?;
    if translations.is_empty() {
        return Err(DbErr::Migration(format!(
            "Blog Category Taxonomy backfill blocked: category {} has no localized copy",
            category.id,
        )));
    }
    ...
    for translation in translations {
        ...
        match existing {
            ...
            Some(_) => {
                return Err(DbErr::Migration(format!(
                    "Blog Category Taxonomy backfill blocked: Taxonomy localized copy already differs for category {} locale {locale}",
                    category.id,
                )));
            }
            None => {
...
async fn ensure_route_owner(
    ...
    let existing = taxonomy_term_route_key::Entity::find()
        ...
        .filter(taxonomy_term_route_key::Column::ScopeValue.eq(BLOG_SCOPE_VALUE))
    ...
    if let Some(existing) = existing {
        ...
        return Err(DbErr::Migration(format!(
            "Blog Category Taxonomy backfill blocked: route {locale}/{route_key} is already owned by Taxonomy Category {}",
            existing.term_id,
        )));
    }
...
async fn ensure_category_hierarchy(
        ...
        .await?
    {
        ...
        Some(_) => Err(DbErr::Migration(format!(
            "Blog Category Taxonomy backfill blocked: hierarchy already differs for category {}",
            category.id,
        ))),
        None => {
...
async fn ensure_blog_binding(
    txn: &sea_orm::DatabaseTransaction,
    category: &blog_category::Model,
) -> Result<(), DbErr> {
    if let Some(existing) =
        blog_category_taxonomy_binding::Entity::find_by_id((category.tenant_id, category.id))
            .one(txn)
            .await?
    {
        ...
        return Err(DbErr::Migration(format!(
            "Blog Category Taxonomy backfill blocked: Blog category {} is already bound to Taxonomy Category {}",
            category.id, existing.taxonomy_category_id,
        )));
    }

    if let Some(existing) = blog_category_taxonomy_binding::Entity::find()
        .filter(blog_category_taxonomy_binding::Column::TenantId.eq(category.tenant_id))
        .filter(blog_category_taxonomy_binding::Column::TaxonomyCategoryId.eq(category.id))
        .one(txn)
        .await?
    {
        return Err(DbErr::Migration(format!(
            "Blog Category Taxonomy backfill blocked: Taxonomy Category {} is already bound to Blog category {}",
            category.id, existing.blog_category_id,
        )));
    }

    blog_category_taxonomy_binding::ActiveModel {
        tenant_id: Set(category.tenant_id),
        blog_category_id: Set(category.id),
...
fn exact_taxonomy_locale(value: &str, category_id: Uuid) -> Result<String, DbErr> {
    let normalized = normalize_term_locale(value).ok_or_else(|| {
        DbErr::Migration(format!(
            "Blog Category Taxonomy backfill blocked: category {category_id} has invalid locale {value:?}",
        ))
    })?;
    if normalized != value {
        return Err(DbErr::Migration(format!(
            "Blog Category Taxonomy backfill blocked: category {category_id} locale {value:?} is not canonically normalized as {normalized:?}",
        )));
    }
    Ok(normalized)
}
...
fn exact_taxonomy_route_key(value: &str, category_id: Uuid, locale: &str) -> Result<String, DbErr> {
    let normalized = normalize_term_route_key(value).ok_or_else(|| {
        DbErr::Migration(format!(
            "Blog Category Taxonomy backfill blocked: category {category_id} locale {locale} has an empty route key",
        ))
    })?;
    if normalized != value {
        return Err(DbErr::Migration(format!(
            "Blog Category Taxonomy backfill blocked: category {category_id} locale {locale} route {value:?} normalizes to {normalized:?}",
        )));
    }
    if normalized.len() > TAXONOMY_ROUTE_KEY_MAX_BYTES {
        return Err(DbErr::Migration(format!(
            "Blog Category Taxonomy backfill blocked: category {category_id} locale {locale} route exceeds {TAXONOMY_ROUTE_KEY_MAX_BYTES} bytes",
        )));
    }
    Ok(normalized)
}
