...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        let rows = db
            .query_all(Statement::from_string(
                backend,
                "SELECT id, body FROM blog_post_translations".to_string(),
            ))
            .await?;
        ...
        for row in rows {
            ...
            if let Err(error) = parse_json(&body, RichTextProfile::Article) {
                return Err(DbErr::Migration(format!(
                    "Blog article translation {id} is not canonical article richtext ({:?}); convert stored Blog article bodies before retrying the migration",
                    error.code()
                )));
            }
        }
        ...
        manager
            .alter_table(
                Table::alter()
                    ...
                    .drop_column(BlogPostTranslations::BodyFormat)
                    .to_owned(),
            )
            .await
    }
    ...
    async fn down(&self, _manager: &SchemaManager) -> Result<(), DbErr> {
        Err(DbErr::Migration(
            "the Blog article richtext cutover is irreversible because the removed format selector cannot be reconstructed"
                .to_string(),
        ))
    }
}
...
mod tests {
    ...
    async fn legacy_database(body: &str) -> sea_orm::DatabaseConnection {
        let database = Database::connect(format!(
            "sqlite:file:blog_article_richtext_cutover_{}?mode=memory&cache=shared",
            Uuid::new_v4()
        ))
        .await
        ...
        manager
            .create_table(
                Table::create()
                    .table(BlogPostTranslations::Table)
                    .col(
                        ColumnDef::new(BlogPostTranslationFixture::Id)
                            .uuid()
                            .not_null()
                            .primary_key(),
                    )
                    .col(
                        ColumnDef::new(BlogPostTranslationFixture::Body)
                            .text()
                            .not_null(),
                    )
                    .col(
        ...
        database
            .execute(Statement::from_sql_and_values(
                DbBackend::Sqlite,
                "INSERT INTO blog_post_translations (id, body, body_format) VALUES (?, ?, ?)",
    ...
    #[derive(DeriveIden)]
    enum BlogPostTranslationFixture {
        Id,
        Body,
    }
}
