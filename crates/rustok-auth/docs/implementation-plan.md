...
- Last checkpoint: Production runtime extensions register one `ServerAuthAdminMutationProvider` behind both `OAuthAdminMutationRuntime` and `UserAdminMutationRuntime`. OAuth and user create/update/delete GraphQL resolvers plus native `#[server]` adapters consume the same typed ports; the module-owned facade is native-first and keeps GraphQL in parallel. User custom-field validation, tenant scoping, RBAC, role replacement, atomic create metadata/localized-value lifecycle, host-resolved locale propagation and case-insensitive role/status normalization now execute inside the shared provider instead of transport resolvers. Profile preference defaults are derived through core-owned host-locale policy instead of package-local UI locale constants, and the auth transport facade no longer reads the host admin locale storage key directly.
