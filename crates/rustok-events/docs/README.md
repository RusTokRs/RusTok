...
- a manually dispatched, read-only admission workflow runs the canonical Rust
  generator and archives the committed/generated artifacts plus an exact review
  patch without committing or pushing repository changes;
...
Digest values must be generated by the canonical Rust example. The manual
**Event contract digest admission** workflow offers `generate_patch` and
`verify` modes. It uses the pinned Rust toolchain, read-only repository
permissions, disabled checkout credentials, and archives an exact patch for
maintainer review. It never commits or pushes generated values.
