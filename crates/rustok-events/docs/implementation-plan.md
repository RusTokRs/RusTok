...
Draft PR #2847 adds sealed
`rbac.artifact_role_permission.assignment_changed` v1. Tenant and actor remain
envelope metadata; the bounded payload contains only operation, role,
installation, permission key, and grant state. RBAC owns state-change detection and
publishes through a host-neutral port using the same live owner transaction. The
server adapter delegates to the canonical transactional Outbox and is not a second
publisher owner. The contract digest is intentionally absent until generated by the
repository-owned digest command on the reconciled exact head.
...
1. **Keep reviewed event-contract digests synchronized.** Regenerate only through the
   deterministic example when a reviewed contract shape changes. Draft #2847 must not
   leave draft until its RBAC artifact-permission contract digest is generated and
   reviewed on the exact reconciled head.
...
## RBAC artifact-permission transactional event correction (2026-08-01)

- Status: `draft_pr_source_ready_unvalidated`.
- Draft PR: #2847.
- Severity: `P1` because a committed artifact role-permission mutation lacked an
  owner event in the canonical transactional Outbox.
- [x] Add sealed `RbacArtifactPermissionEvent::AssignmentChanged` v1 and bounded
  schema metadata.
- [x] Keep tenant and actor in envelope metadata and validate the bounded payload.
- [x] Add contract tests for registry membership, semantic validation, wire shape,
  and tenant/actor envelope metadata.
- [x] Keep publication timing in the RBAC owner transaction and transport execution in
  the server Outbox adapter.
- [ ] Generate and review `contracts/event-contract-digests.json` using the
  repository-owned command on the exact reconciled head.
- [ ] Execute contract, owner transaction, server adapter, source verifier, and module
  gates before treating the family as delivered source evidence.
