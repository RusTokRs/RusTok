...
- Execute the wired channel SQLite reader, no-Redis two-runtime and PostgreSQL jobs on one revision.
- Add channel resolved-value assertions for publication loss, local lag, Redis reconnect,
  database outage/recovery, generation regression and periodic reconciliation across replicas.
