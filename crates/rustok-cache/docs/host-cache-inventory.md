...
- Execute the wired channel SQLite reader, no-Redis two-runtime, missed-publication, local-lag,
  PostgreSQL and live Redis jobs on one revision.
- Add channel resolved-value assertions for local lag, Redis disconnect/reconnect, database
  outage/recovery and generation regression across replicas.
