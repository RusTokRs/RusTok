...
Для `offer` helper действует минимальная нормализация: `price` пишется только для finite значений, `priceCurrency` — только для валидного трёхбуквенного alphabetic кода (кроме `XXX`), `availability` — только для `http(s)://schema.org/...`.
