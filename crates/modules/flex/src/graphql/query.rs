...
use super::{
    ...
    FlexSchemaObject, StandaloneFieldPolicyObject, bad_user_input, map_attached_field_policy_error,
...
impl FlexQuery {
    ...
    async fn standalone_field_policies(
        ...
        let runtime = runtime(ctx)?;
        let schema =
            crate::find_schema(runtime.standalone_service().as_ref(), tenant.id, schema_id)
                .await
                .map_err(map_flex_error)?
                .ok_or_else(|| {
                    bad_user_input(format!(
                        "standalone Flex schema `{schema_id}` was not found"
                    ))
                })?;
