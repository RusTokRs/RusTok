...
- Uses native Leptos server functions backed by `AdminInventoryReadService` as the only inventory admin read transport, with DB access provided through `HostRuntimeContext` instead of package-local context.
