...
use crate::application_core::{
    prepare_group_application_policy_locale_catalog_query, prepare_group_application_policy_query,
...
pub fn GroupsPolicyEditorAdmin() -> impl IntoView {
    ...
    let on_load = Callback::new(move |_: ()| {
        ...
        spawn_local(async move {
            match load_group_admin_application_policy_locale_catalog(catalog_context, catalog_query)
                .await
            {
            ...
            match load_group_admin_application_policy_for_management(policy_context, policy_query)
                .await
            {
                Ok(policy) => {
    ...
    view! {
        <section class="groups-admin-policy-editor rounded-3xl border border-border bg-card p-6 shadow-sm">
            ...
            <form class="mt-6 space-y-6" on:submit=on_save>
                ...
                <Show when=move || !locale_options.get().is_empty()>
                    <p class="text-xs text-muted-foreground">{
                        let available_locales = available_locales.clone();
                        move || format!("{}: {}", available_locales, locale_options.get().join(", "))
                    }</p>
                </Show>
                <Show when=move || translation_exists.get().is_some()>
                    <p class="text-xs text-muted-foreground">{
                        let existing_translation = existing_translation.clone();
                        let new_translation = new_translation.clone();
                        move || if translation_exists.get().unwrap_or(false) { existing_translation.clone() } else { new_translation.clone() }
                    }</p>
                </Show>
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <button class="rounded-xl border border-border px-3 py-2 text-sm" type="button" on:click=add_question>{add_question_label.clone()}</button>
                    </div>
                    {move || {
                        let question_key = question_key.clone();
                        let question_prompt = question_prompt.clone();
                        let question_help = question_help.clone();
                        let question_required = question_required.clone();
                        let answer_limit = answer_limit.clone();
                        let question_move_up = question_move_up.clone();
                        let question_move_down = question_move_down.clone();
                        let question_remove = question_remove.clone();
                        questions.get().into_iter().enumerate().map(|(index, question)| {
                            ...
                            view! {
                                <div class="space-y-3 rounded-2xl border border-border p-4">
                                    <div class="grid gap-3 md:grid-cols-2">
                                        <input class="rounded-xl border border-border bg-background px-3 py-2 text-sm" placeholder=question_key prop:value=key_value on:input=move |event| questions.update(|items| if let Some(item) = items.get_mut(index) { item.key = event_target_value(&event); }) />
                                        <input class="rounded-xl border border-border bg-background px-3 py-2 text-sm" placeholder=question_prompt prop:value=prompt_value on:input=move |event| questions.update(|items| if let Some(item) = items.get_mut(index) { item.prompt = event_target_value(&event); }) />
                                    </div>
                                    <textarea class="min-h-20 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm" placeholder=question_help prop:value=help_value on:input=move |event| questions.update(|items| if let Some(item) = items.get_mut(index) { let value = event_target_value(&event); item.help_text = (!value.trim().is_empty()).then_some(value); })></textarea>
                                    <div class="flex flex-wrap items-center gap-3">
                                        <label class="flex items-center gap-2 text-sm"><input type="checkbox" prop:checked=question.required on:change=move |event| questions.update(|items| if let Some(item) = items.get_mut(index) { item.required = event_target_checked(&event); }) /><span>{question_required}</span></label>
                                        <input
                                            class="w-40 rounded-xl border border-border bg-background px-3 py-2 text-sm"
                                            aria-label=answer_limit
                                        ...
                                        />
                                        <button type="button" class="rounded-lg border border-border px-2 py-1 text-xs" on:click=move |_| move_item(questions, index, -1)>{question_move_up}</button>
                                        <button type="button" class="rounded-lg border border-border px-2 py-1 text-xs" on:click=move |_| move_item(questions, index, 1)>{question_move_down}</button>
                                        <button type="button" class="rounded-lg border border-destructive px-2 py-1 text-xs text-destructive" on:click=move |_| questions.update(|items| { if index < items.len() { items.remove(index); } })>{question_remove}</button>
                                    </div>
                                </div>
                            }
                        }).collect_view()
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <button class="rounded-xl border border-border px-3 py-2 text-sm" type="button" on:click=add_rule>{add_rule_label.clone()}</button>
                    </div>
                    {move || {
                        let rule_key = rule_key.clone();
                        let rule_title = rule_title.clone();
                        let rule_body = rule_body.clone();
                        let rule_required = rule_required.clone();
                        let rule_move_up = rule_move_up.clone();
                        let rule_move_down = rule_move_down.clone();
                        let rule_remove = rule_remove.clone();
                        rules.get().into_iter().enumerate().map(|(index, rule)| {
                            ...
                            view! {
                                <div class="space-y-3 rounded-2xl border border-border p-4">
                                    <div class="grid gap-3 md:grid-cols-2">
                                        <input class="rounded-xl border border-border bg-background px-3 py-2 text-sm" placeholder=rule_key prop:value=key_value on:input=move |event| rules.update(|items| if let Some(item) = items.get_mut(index) { item.key = event_target_value(&event); }) />
                                        <input class="rounded-xl border border-border bg-background px-3 py-2 text-sm" placeholder=rule_title prop:value=title_value on:input=move |event| rules.update(|items| if let Some(item) = items.get_mut(index) { item.title = event_target_value(&event); }) />
                                    </div>
                                    <textarea class="min-h-28 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm" placeholder=rule_body prop:value=body_value on:input=move |event| rules.update(|items| if let Some(item) = items.get_mut(index) { item.body = event_target_value(&event); })></textarea>
                                    <div class="flex flex-wrap items-center gap-3">
                                        <label class="flex items-center gap-2 text-sm"><input type="checkbox" prop:checked=rule.required on:change=move |event| rules.update(|items| if let Some(item) = items.get_mut(index) { item.required = event_target_checked(&event); }) /><span>{rule_required}</span></label>
                                        <button type="button" class="rounded-lg border border-border px-2 py-1 text-xs" on:click=move |_| move_item(rules, index, -1)>{rule_move_up}</button>
                                        <button type="button" class="rounded-lg border border-border px-2 py-1 text-xs" on:click=move |_| move_item(rules, index, 1)>{rule_move_down}</button>
                                        <button type="button" class="rounded-lg border border-destructive px-2 py-1 text-xs text-destructive" on:click=move |_| rules.update(|items| { if index < items.len() { items.remove(index); } })>{rule_remove}</button>
                                    </div>
                                </div>
                            }
                        }).collect_view()
                ...
                <button class="rounded-xl bg-primary px-4 py-2 text-sm font-medium text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50" type="submit" disabled=move || !management_loaded.get()>{save_label.clone()}</button>
            </form>
            ...
            <div class="mt-8 space-y-3">
                <h3 class="font-semibold text-card-foreground">{history_label.clone()}</h3>
                {
                    let empty_history = empty_history.clone();
                    let revision_label = revision_label.clone();
                    ...
                    move || {
                        ...
                        let changed_by = changed_by.clone();
                        if items.is_empty() {
                            view! { <p class="text-sm text-muted-foreground">{empty_history}</p> }.into_any()
                        } else {
                            view! {
...
fn policy_editor_copy(locale: Option<&str>) -> PolicyEditorCopy {
    PolicyEditorCopy {
        ...
        body: t(
            ...
            "Select an exact policy locale, edit its questions and rules, and save through owner CAS without changing the host UI locale.",
        ),
        ...
        existing_translation: t(
            ...
            "Editing an existing exact-locale translation.",
        ),
        new_translation: t(
            locale,
            "groups.admin.policyEditor.newTranslation",
            "This locale has no translation yet. Saving creates it with the current policy CAS precondition.",
        ),
        enabled: t(
            locale,
        ...
        loaded: t(
            locale,
            "groups.admin.policyEditor.loaded",
            "Policy locale loaded",
        ),
        saved: t(locale, "groups.admin.policyEditor.saved", "Policy saved"),
        stale: t(
            locale,
            "groups.admin.policyEditor.stale",
            "The owner service rejected this stale policy atomically. Reload the selected locale before saving again.",
        ),
        error: t(
            locale,
        ...
        invalid: t(
            ...
            "Load a valid group and exact locale before saving, then check the expected revision, question keys, text limits, and rule fields.",
        ),
    }
}
