...
- `MediaAssetReadPort::get_asset_reference_admission` publishes a bounded cross-module lifecycle fact: only `active + ready` assets are reference-admissible; delete-pending, deleted, failed, and not-ready states fail closed without exposing storage or delivery details.
- `MediaAssetWritePort::retain_asset_reference` / `release_asset_reference` publish durable Media-owned reference holds. Database lifecycle guards make retained references deletion-safe; shared write idempotency protects retries.
