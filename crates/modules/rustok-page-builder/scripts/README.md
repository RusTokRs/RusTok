...
- `verify/verify-page-builder-preview-runtime-contract.mjs` source-locks the contract 1.1 preview runtime DTO and contextual rendering port, DTO-owned context/scenario limits, Pages stale-response guards, the deterministic static runtime materialization envelope, SHA-256 evidence, Fly snapshot parity, the post-materialization public-resource security check, and Pages persistence/migration/storefront integrity rules including backward-compatible all-`NULL` legacy evidence and fail-closed partial evidence.
