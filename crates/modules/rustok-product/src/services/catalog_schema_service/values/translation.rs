...
const TRANSLATION_RESOURCE_KIND: &str = "attribute_value";
const VALUE_FIELD: &str = "value";
const OPERATION_APPLY_PATCH: &str = "translation_target_apply_attribute_value_patch";
const CHANGE_CURSOR_VERSION: &str = "v1";
const MAX_CHANGE_PAGE: u16 = 200;
const PROGRESS_STABILITY_ATTEMPTS: usize = 3;
const LOCALE_REVISION_NAMESPACE: &str = "rustok-product/attribute-value-translation-locale/v1";
const DELETED_REVISION_NAMESPACE: &str = "rustok-product/attribute-value-translation-deleted/v1";
const RESOURCE_REVISION_PREFIX: &str = "product-attribute-value-resource-v1:";

#[derive(Debug, Error)]
enum ProductAttributeValueTranslationError {
    #[error(transparent)]
    ...
    #[error("Product localized attribute value owner invariant failed: {0}")]
    OwnerInvariant(String),
}

impl From<sea_orm::DbErr> for ProductAttributeValueTranslationError {
    fn from(error: sea_orm::DbErr) -> Self {
        Self::Commerce(CommerceError::Database(error))
    }
}

type OwnerResult<T> = Result<T, ProductAttributeValueTranslationError>;
...
impl ChangeLifecycle {
    ...
    fn parse(value: &str) -> OwnerResult<Self> {
        match value {
            ...
            _ => Err(ProductAttributeValueTranslationError::OwnerInvariant(
...
#[derive(Clone)]
struct ProductAttributeValueTranslationTargetProvider {
    service: Arc<ProductCatalogSchemaService>,
}

impl ProductCatalogSchemaService {
    /// Builds the neutral Translation adapter for Product-owned localized EAV values.
    ///
    /// One canonical `product_attribute_values` row is one Translation resource. Dynamic
    /// category/form membership is context only and never changes the Translation identity.
    pub fn attribute_value_translation_target_provider(
        service: Arc<Self>,
    ) -> impl TranslationTargetProvider {
        ProductAttributeValueTranslationTargetProvider { service }
    }

    async fn list_attribute_value_translation_exact_resources(
        &self,
        tenant_id: Uuid,
        source_locale: &str,
        target_locale: &str,
        ...
        if limit == 0 || limit > 200 {
            return Err(CommerceError::Validation(
                "Product attribute-value translation resource page size must be between 1 and 200"
                    .to_string(),
            )
            .into());
        }
    ...
    async fn read_attribute_value_translation_exact_locale(
        &self,
        tenant_id: Uuid,
        value_id: Uuid,
        source_locale: &str,
    ...
    async fn apply_attribute_value_translation_exact_locale(
        &self,
        tenant_id: Uuid,
...
"#,
        ...
        if request.expected_target_revision != before.target_revision {
            return Err(ProductAttributeValueTranslationError::RevisionConflict {
                revision: "target",
            });
        }
...
"#,
        ...
        let target_revision = after.target_revision.clone().ok_or_else(|| {
            ProductAttributeValueTranslationError::OwnerInvariant(
                "exact target revision is absent after Product attribute-value translation apply"
                    .to_string(),
            )
        })?;
        let target_value = after.target_value.clone().ok_or_else(|| {
            ProductAttributeValueTranslationError::OwnerInvariant(
                "exact target value is absent after Product attribute-value translation apply"
                    .to_string(),
            )
        })?;
    ...
    async fn read_attribute_value_translation_progress(
        &self,
        tenant_id: Uuid,
        source_locale: &str,
        target_locale: &str,
...
INNER JOIN product_attributes pa
    ON pa.id = pav.attribute_id
   AND pa.tenant_id = pav.tenant_id
INNER JOIN product_attribute_value_translations source_translation
    ON source_translation.value_id = pav.id
   AND source_translation.locale = $2
LEFT JOIN product_attribute_value_translations target_translation
...
"#,
    ...
    async fn attribute_value_translation_change_highwater(
        &self,
        tenant_id: Uuid,
        ...
        let row = self
            ...
            .query_one_raw(Statement::from_sql_and_values(
                DatabaseBackend::Postgres,
                "SELECT MAX(change_seq) AS highwater FROM product_attribute_value_translation_change_journal WHERE tenant_id = $1",
                vec![tenant_id.into()],
            ))
            .await?
            .ok_or_else(|| {
    ...
    async fn read_attribute_value_translation_changes(
        &self,
        tenant_id: Uuid,
        after_seq: u64,
        through_seq: u64,
        ...
        if through_seq == 0 || after_seq > through_seq {
            return Err(CommerceError::Validation(
                "Product attribute-value translation change cursor bounds are invalid".to_string(),
            )
            .into());
        }
        if limit == 0 || limit > MAX_CHANGE_PAGE {
            return Err(CommerceError::Validation(format!(
                "Product attribute-value translation change page size must be between 1 and {MAX_CHANGE_PAGE}"
            ))
            .into());
        }
        ensure_postgres(self.db.get_database_backend())?;
...
FROM product_attribute_value_translation_change_journal
WHERE tenant_id = $1
  AND change_seq > $2
  AND change_seq <= $3
...
"#,
    ...
    pub(crate) async fn record_attribute_value_translation_changes_in_tx(
        ...
        if tenant_id.is_nil() || product_id.is_nil() || root_event_id.is_nil() {
            return Err(CommerceError::Validation(
                "Product attribute-value translation change identity must not be nil".to_string(),
            ));
        }
        ...
        let live_rows = LiveValueRow::find_by_statement(Statement::from_sql_and_values(
            DatabaseBackend::Postgres,
            r#"
SELECT pav.id AS value_id, pav.translation_revision
FROM product_attribute_values pav
INNER JOIN product_attributes pa
    ON pa.id = pav.attribute_id
   AND pa.tenant_id = pav.tenant_id
...
#[async_trait]
impl TranslationTargetProvider for ProductAttributeValueTranslationTargetProvider {
    ...
    async fn list_resources(
        ...
        let (resources, next_after) = self
            .service
            .list_attribute_value_translation_exact_resources(
                tenant_id,
    ...
    async fn read_progress(
        ...
        for _ in 0..PROGRESS_STABILITY_ATTEMPTS {
            let before = self
                .service
                .attribute_value_translation_change_highwater(tenant_id)
                .await
                .map_err(owner_error_to_port_error)?;
            let owner = self
                .service
                .read_attribute_value_translation_progress(
                    tenant_id,
                    request.source_locale.as_str(),
                    request.target_locale.as_str(),
                )
            ...
            facts.validate().map_err(|error| {
                PortError::invariant_violation(
                    "product.attribute_value_translation_progress_invalid",
                    error.to_string(),
                )
            })?;
            return Ok(facts);
        ...
        Err(PortError::unavailable(
            "product.attribute_value_translation_progress_unstable",
            "Product attribute-value translation progress changed while it was being aggregated",
        ))
    }
    ...
    async fn read_changes(
        ...
        let owner_changes = self
            .service
            .read_attribute_value_translation_changes(tenant_id, after, through, request.limit)
            .await
            .map_err(owner_error_to_port_error)?;
    ...
    async fn apply_patch(
        ...
        let result = async {
            ...
            let applied = with_product_operation_receipt(
                lease,
                self.service.apply_attribute_value_translation_exact_locale(
                    tenant_id,
                    security.user_id,
                    value_id,
                    target,
            ...
            if applied.operation_id != Some(lease.operation_id) {
                return Err(PortError::invariant_violation(
                    "product.attribute_value_translation_receipt_identity_invalid",
                    "Product attribute-value owner receipt is not bound to the active Translation operation",
                ));
            }
...
async fn load_owner_snapshot(
    ...
    let value_id = parse_identity(&request.identity)?;
    service
        .read_attribute_value_translation_exact_locale(
            tenant_id,
            value_id,
            request.source_locale.as_str(),
            request.target_locale.as_str(),
...
async fn load_attribute_value_aggregate<C>(
    db: &C,
    tenant_id: Uuid,
    value_id: Uuid,
...
SELECT
    ...
    pav.translation_revision,
    COALESCE((
        SELECT jsonb_agg(
            jsonb_build_object(
                'locale', pavt.locale,
                'value_text', pavt.value_text
            ) ORDER BY pavt.locale
        )
        FROM product_attribute_value_translations pavt
        WHERE pavt.value_id = pav.id
...
fn source_eligible_sql(with_after: bool) -> String {
    let after = if with_after { "AND pav.id > $3" } else { "" };
...
{}
INNER JOIN candidate ON candidate.id = pav.id
ORDER BY pav.id ASC
"#,
...
fn decode_aggregate(row: AggregateRow) -> OwnerResult<Aggregate> {
    ...
    {
        return Err(ProductAttributeValueTranslationError::OwnerInvariant(
            "persisted Product attribute-value identity is invalid".to_string(),
        ));
    }
...
fn locale_revision(
    ...
    format!(
        "product-attribute-value-locale-v1:{}",
...
fn deleted_revision(root_event_id: Uuid, value_id: Uuid) -> String {
    ...
    hash_uuid(&mut hasher, value_id);
    format!(
        "product-attribute-value-deleted-v1:{}",
        hex::encode(hasher.finalize())
    )
}
        ...
        exact_locales: owner
            ...
            .map(|locale| {
                TenantLocale::new(locale.clone()).map_err(|error| {
                    PortError::invariant_violation(
                        "product.attribute_value_translation_locale_invalid",
                        error.to_string(),
                    )
                })
            })
...
fn snapshot_from_owner(
    ...
    {
        return Err(PortError::invariant_violation(
            "product.attribute_value_translation_locale_identity_invalid",
            "Product attribute-value owner returned different exact locales",
        ));
    }
    ...
    snapshot.validate().map_err(|error| {
        PortError::invariant_violation(
            "product.attribute_value_translation_snapshot_invalid",
            error.to_string(),
        )
    })?;
    Ok(snapshot)
...
fn merged_target(
    ...
    if !values.is_empty() {
        return Err(PortError::validation(
            "product.attribute_value_translation_patch_field_unknown",
            "Product attribute-value translation patch contains an unknown field",
        ));
    }
    Ok(ExactLocaleApply {
...
fn application_receipt(
    ...
    let operation_id = owner_receipt.operation_id.ok_or_else(|| {
        PortError::invariant_violation(
            "product.attribute_value_translation_receipt_identity_missing",
            "Product attribute-value owner receipt is missing operation identity",
        )
    })?;
    Ok(TranslationApplicationReceipt {
        provider_receipt_id: format!("product-attribute-value:{operation_id}"),
...
async fn insert_change(
    ...
    txn.execute_raw(Statement::from_sql_and_values(
        DatabaseBackend::Postgres,
        r#"
INSERT INTO product_attribute_value_translation_change_journal (
    root_event_id, tenant_id, product_id, value_id, resource_revision, lifecycle, created_at
...
fn parse_tenant_id(context: &PortContext) -> Result<Uuid, PortError> {
    Uuid::parse_str(&context.tenant_id).map_err(|_| {
        PortError::validation(
            ...
            "Product attribute-value translation target context must carry a UUID tenant_id",
        )
    })
}
...
fn authorize(context: &PortContext, action: Action) -> Result<SecurityContext, PortError> {
    ...
    if security.get_scope(Resource::Products, action) == PermissionScope::None {
        return Err(PortError::forbidden(
            "product.attribute_value_translation_permission_denied",
...
fn parse_identity(identity: &TranslationResourceIdentity) -> Result<Uuid, PortError> {
    ...
    {
        return Err(PortError::validation(
            "product.attribute_value_translation_identity_invalid",
            "Product attribute-value identity must address product/attribute_value without a subresource",
        ));
    }
    Uuid::parse_str(identity.resource_id.as_str()).map_err(|_| {
        PortError::validation(
            "product.attribute_value_translation_resource_id_invalid",
            "Product attribute-value resource id must be a UUID",
        )
    })
}

fn attribute_value_identity(value_id: Uuid) -> TranslationResourceIdentity {
...
fn resource_cursor(value_id: Uuid) -> Result<OpaqueCursor, PortError> {
    OpaqueCursor::new(value_id.to_string()).map_err(|error| {
        PortError::invariant_violation(
            "product.attribute_value_translation_resource_cursor_invalid",
            error.to_string(),
        )
    })
}
...
fn parse_resource_cursor(value: &str) -> Result<Uuid, PortError> {
    Uuid::parse_str(value).map_err(|_| {
        PortError::validation(
            "product.attribute_value_translation_resource_cursor_invalid",
            "Product attribute-value resource cursor must be a UUID",
        )
    })
}
...
fn change_cursor(through: u64, after: u64) -> Result<OpaqueCursor, PortError> {
    OpaqueCursor::new(format!("{CHANGE_CURSOR_VERSION}:{through}:{after}")).map_err(|error| {
        PortError::invariant_violation(
            "product.attribute_value_translation_change_cursor_invalid",
            error.to_string(),
        )
    })
}
...
fn parse_change_cursor(cursor: &OpaqueCursor) -> Result<(u64, u64), PortError> {
        ...
        || after.is_none()
    {
        return Err(PortError::validation(
            "product.attribute_value_translation_change_cursor_invalid",
            "Product attribute-value translation change cursor is invalid",
        ));
    }
    ...
    if through == 0 || after == 0 || after > through {
        return Err(PortError::validation(
            "product.attribute_value_translation_change_cursor_invalid",
            "Product attribute-value translation change cursor bounds are invalid",
        ));
    }
    Ok((through, after))
}
...
fn opaque_revision(value: String, field: &'static str) -> Result<OpaqueRevision, PortError> {
    OpaqueRevision::new(value).map_err(|error| {
        PortError::invariant_violation(
            "product.attribute_value_translation_revision_invalid",
            format!("Product attribute-value {field} is invalid: {error}"),
        )
    })
}

fn owner_error_to_port_error(error: ProductAttributeValueTranslationError) -> PortError {
    match error {
        ProductAttributeValueTranslationError::ValueNotFound(_) => PortError::not_found(
            "product.attribute_value_translation_resource_not_found",
            "Product attribute-value translation resource was not found",
        ),
        ProductAttributeValueTranslationError::SourceLocaleNotFound { .. } => PortError::not_found(
            "product.attribute_value_translation_source_not_found",
            "Exact source Product attribute-value locale was not found",
        ),
        ProductAttributeValueTranslationError::RevisionConflict { .. } => PortError::conflict(
            "product.attribute_value_translation_revision_conflict",
            "Product attribute-value translation state conflicts with the requested mutation",
        ),
        ProductAttributeValueTranslationError::OwnerInvariant(_) => PortError::invariant_violation(
...
fn product_error_to_port_error(error: CommerceError) -> PortError {
    match error {
        CommerceError::Database(_) => PortError::unavailable(
            "product.attribute_value_translation_owner_unavailable",
            "Product attribute-value translation storage is temporarily unavailable",
        ),
        ...
        | CommerceError::CannotDeletePublished => PortError::conflict(
            "product.attribute_value_translation_owner_conflict",
            "Product state conflicts with the requested attribute-value translation mutation",
        ),
        CommerceError::Validation(_) | CommerceError::NoVariants => PortError::validation(
            "product.attribute_value_translation_owner_validation",
            "Product rejected the attribute-value translation mutation",
        ),
        CommerceError::Core(_) => PortError::invariant_violation(
            "product.attribute_value_translation_owner_invariant",
            "Product attribute-value translation state is invalid",
        ),
    }
}
...
fn validate_locale_pair(source_locale: &str, target_locale: &str) -> OwnerResult<()> {
    if source_locale == target_locale {
        return Err(CommerceError::Validation(
            "Product attribute-value translation source and target locale must differ".to_string(),
        )
        .into());
    }
    Ok(())
...
fn validate_stored_translation(translation: &StoredTranslation) -> OwnerResult<()> {
    if canonical_locale(&translation.locale)? != translation.locale {
        return Err(ProductAttributeValueTranslationError::OwnerInvariant(
            "persisted Product attribute-value translation locale is not canonical".to_string(),
        ));
    }
    Ok(())
}
...
fn validate_target_value(value: &str) -> OwnerResult<()> {
    if value.trim().is_empty() {
        return Err(CommerceError::Validation(
            "Product attribute-value translation target must not be blank".to_string(),
        )
        .into());
    }
    Ok(())
...
fn validate_uuid(value: Uuid, field: &str) -> OwnerResult<()> {
    if value.is_nil() {
        return Err(CommerceError::Validation(format!(
            "Product attribute-value translation {field} must not be nil"
        ))
        .into());
    }
    Ok(())
...
fn ensure_revision(revision: &'static str, expected: &str, current: &str) -> OwnerResult<()> {
    if expected != current {
        return Err(ProductAttributeValueTranslationError::RevisionConflict { revision });
    }
    Ok(())
}
...
fn ensure_postgres(backend: DatabaseBackend) -> OwnerResult<()> {
    if backend != DatabaseBackend::Postgres {
        return Err(CommerceError::Validation(
            "Product attribute-value Translation target requires PostgreSQL".to_string(),
        )
        .into());
    }
    Ok(())
...
fn nonnegative_count(value: i64, field: &str) -> OwnerResult<u64> {
    u64::try_from(value).map_err(|_| {
        ProductAttributeValueTranslationError::OwnerInvariant(format!(
            "Product attribute-value translation {field} count must be nonnegative"
        ))
    })
}
...
fn invalid_sequence(field: &str) -> ProductAttributeValueTranslationError {
    ProductAttributeValueTranslationError::OwnerInvariant(format!(
        "Product attribute-value translation change {field} sequence must be positive"
    ))
}
...
fn change_record_from_row(row: sea_orm::QueryResult) -> OwnerResult<ChangeRecord> {
    ...
    if value_id.is_nil() || resource_revision.trim().is_empty() {
        return Err(ProductAttributeValueTranslationError::OwnerInvariant(
            "Product attribute-value translation change row is invalid".to_string(),
        ));
    }
    Ok(ChangeRecord {
        change_seq,
...
fn owner_error_to_commerce(error: ProductAttributeValueTranslationError) -> CommerceError {
    match error {
        ProductAttributeValueTranslationError::Commerce(error) => error,
