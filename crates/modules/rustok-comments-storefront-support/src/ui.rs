...
pub fn CommentComposer(
    ...
    let sign_in_required = StoredValue::new(copy.sign_in_required.clone());
    let form_copy = StoredValue::new(copy);
    ...
    view! {
        <section class="mt-6 rounded-2xl border border-border bg-card/50 p-5">
            ...
            >
                <AuthenticatedCommentComposer
                    content_locale=content_locale.get_value()
                    submit_action
                    copy=form_copy.get_value()
                />
            </Show>
        </section>
    }
...
#[component]
fn AuthenticatedCommentComposer(
    content_locale: String,
    submit_action: Action<RichTextDocument, Result<(), String>>,
    copy: CommentComposerCopy,
) -> AnyView {
    ...
    view! {
        <form class="mt-4 space-y-3" on:submit=on_submit>
            <RichTextEditorFrame
                ...
                content_locale=Signal::derive(move || content_locale.get_value())
