...
pub(crate) async fn lock_active_topic_subscription_write_in_tx(
    txn: &DatabaseTransaction,
    tenant_id: Uuid,
    topic_id: Uuid,
) -> ForumResult<forum_topic::Model> {
    let mut topics = lock_topic_rows_for_subscription_in_tx(txn, tenant_id, &[topic_id]).await?;
    let topic = topics.pop().ok_or(ForumError::TopicNotFound(topic_id))?;
    if topic.status == TopicStatus::Archived {
        return Err(ForumError::Validation(
            "Forum topic subscriptions cannot be changed after topic archival".to_string(),
        ));
    }
    Ok(topic)
}

pub(crate) async fn lock_topic_rows_for_subscription_in_tx(
    txn: &DatabaseTransaction,
    tenant_id: Uuid,
    topic_ids: &[Uuid],
        ...
        let statement = match txn.get_database_backend() {
            ...
            backend => {
                return Err(ForumError::Validation(format!(
                    "Forum topic subscription locking does not support database backend {backend:?}"
                )));
            }
        };
    ...
    if topics.len() != ids.len() {
        return Err(ForumError::Validation(
            "Forum topic subscription lock lost a topic row".to_string(),
        ));
    }
...
pub(crate) async fn lock_topic_subscription_scopes_in_tx(
    txn: &DatabaseTransaction,
    tenant_id: Uuid,
    topic_ids: &[Uuid],
) -> ForumResult<()> {
        ...
        DatabaseBackend::Postgres => {
            for topic_id in ids {
                txn.execute_raw(Statement::from_sql_and_values(
                    DatabaseBackend::Postgres,
                    "SELECT pg_advisory_xact_lock(hashtextextended($1, 22))",
                    vec![format!("forum-topic-subscription:{tenant_id}:{topic_id}").into()],
                ))
                .await?;
            }
        }
        DatabaseBackend::Sqlite => {
            for topic_id in ids {
                txn.execute_raw(Statement::from_sql_and_values(
                    DatabaseBackend::Sqlite,
                    r#"
                    INSERT INTO forum_topic_subscription_locks (
