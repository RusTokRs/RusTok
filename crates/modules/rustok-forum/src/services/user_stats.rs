...
impl UserStatsService {
    ...
    async fn adjust_counts_in_tx(
        ...
        let statement = match txn.get_database_backend() {
            DatabaseBackend::Postgres => Statement::from_sql_and_values(
                DatabaseBackend::Postgres,
                "INSERT INTO forum_user_stats                  (tenant_id, user_id, topic_count, reply_count, solution_count, created_at, updated_at)                  VALUES ($1, $2, CASE WHEN $3 < 0 THEN 0 ELSE $3 END,                          CASE WHEN $4 < 0 THEN 0 ELSE $4 END,                          CASE WHEN $5 < 0 THEN 0 ELSE $5 END,                          CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)                  ON CONFLICT (tenant_id, user_id) DO UPDATE SET                     topic_count = CASE WHEN forum_user_stats.topic_count + $3 < 0                                        THEN 0 ELSE forum_user_stats.topic_count + $3 END,                     reply_count = CASE WHEN forum_user_stats.reply_count + $4 < 0                                        THEN 0 ELSE forum_user_stats.reply_count + $4 END,                     solution_count = CASE WHEN forum_user_stats.solution_count + $5 < 0                                           THEN 0 ELSE forum_user_stats.solution_count + $5 END,                     updated_at = CURRENT_TIMESTAMP",
                vec![
                    tenant_id.into(),
                    user_id.into(),
                    topic_delta.into(),
            ...
            DatabaseBackend::Sqlite => Statement::from_sql_and_values(
                DatabaseBackend::Sqlite,
                "INSERT INTO forum_user_stats                  (tenant_id, user_id, topic_count, reply_count, solution_count, created_at, updated_at)                  VALUES (?1, ?2, MAX(?3, 0), MAX(?4, 0), MAX(?5, 0),                          CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)                  ON CONFLICT (tenant_id, user_id) DO UPDATE SET                     topic_count = MAX(forum_user_stats.topic_count + ?3, 0),                     reply_count = MAX(forum_user_stats.reply_count + ?4, 0),                     solution_count = MAX(forum_user_stats.solution_count + ?5, 0),                     updated_at = CURRENT_TIMESTAMP",
                vec![
                    tenant_id.into(),
                    user_id.into(),
                    topic_delta.into(),
