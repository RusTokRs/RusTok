    ...
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let statements: &[&str] = match manager.get_database_backend() {
            DbBackend::Postgres => &[
                ...
                "CREATE INDEX module_artifact_data_snapshots_scope_idx ON module_artifact_data_snapshots (tenant_id, data_owner_id, namespace_instance_id, created_at, snapshot_id)",
                ...
                "CREATE TABLE module_artifact_data_restore_operations (\
                    ...
                    reason TEXT NOT NULL CHECK (length(trim(reason)) BETWEEN 1 AND 2000),\
                    completed_at TIMESTAMPTZ NOT NULL,\
                    PRIMARY KEY (tenant_id, data_owner_id, namespace_instance_id, idempotency_key)\
                )",
            ...
            DbBackend::Sqlite => &[
                "CREATE TABLE module_artifact_data_snapshots (snapshot_id TEXT PRIMARY KEY, tenant_id TEXT NOT NULL, data_owner_id TEXT NOT NULL, namespace_instance_id TEXT NOT NULL, policy_revision INTEGER NOT NULL CHECK (policy_revision > 0), source_namespace_revision INTEGER NOT NULL CHECK (source_namespace_revision > 0), status TEXT NOT NULL CHECK (status IN ('staging', 'ready', 'collecting')), retention_revision INTEGER NOT NULL CHECK (retention_revision > 0), request_digest TEXT NOT NULL CHECK (length(request_digest) = 71), manifest_digest TEXT NULL CHECK (manifest_digest IS NULL OR length(manifest_digest) = 71), actor_id TEXT NOT NULL, trace_id TEXT NOT NULL CHECK (length(trim(trace_id)) > 0 AND length(trace_id) <= 512), correlation_id TEXT NOT NULL, reason TEXT NOT NULL CHECK (length(trim(reason)) BETWEEN 1 AND 2000), idempotency_key TEXT NOT NULL, structured_record_count INTEGER NOT NULL CHECK (structured_record_count >= 0), object_count INTEGER NOT NULL CHECK (object_count >= 0), total_object_bytes INTEGER NOT NULL CHECK (total_object_bytes >= 0), retain_until TEXT NOT NULL, legal_hold INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL, ready_at TEXT NULL, UNIQUE (tenant_id, data_owner_id, namespace_instance_id, idempotency_key))",
                "CREATE INDEX module_artifact_data_snapshots_scope_idx ON module_artifact_data_snapshots (tenant_id, data_owner_id, namespace_instance_id, created_at, snapshot_id)",
                ...
                "CREATE TABLE module_artifact_data_snapshot_retention_operations (tenant_id TEXT NOT NULL, snapshot_id TEXT NOT NULL, idempotency_key TEXT NOT NULL, request_digest TEXT NOT NULL CHECK (length(request_digest) = 71), expected_retention_revision INTEGER NOT NULL CHECK (expected_retention_revision > 0), retention_revision INTEGER NOT NULL CHECK (retention_revision > 0), retain_until TEXT NOT NULL, legal_hold INTEGER NOT NULL, actor_id TEXT NOT NULL, trace_id TEXT NOT NULL CHECK (length(trim(trace_id)) > 0 AND length(trace_id) <= 512), correlation_id TEXT NOT NULL, reason TEXT NOT NULL CHECK (length(trim(reason)) BETWEEN 1 AND 2000), completed_at TEXT NOT NULL, PRIMARY KEY (tenant_id, snapshot_id, idempotency_key))",
                "CREATE TABLE module_artifact_data_snapshot_collections (collection_id TEXT PRIMARY KEY, tenant_id TEXT NOT NULL, snapshot_id TEXT NOT NULL UNIQUE, data_owner_id TEXT NOT NULL, namespace_instance_id TEXT NOT NULL, policy_snapshot_id TEXT NOT NULL CHECK (length(policy_snapshot_id) BETWEEN 1 AND 128), actor_id TEXT NOT NULL, trace_id TEXT NOT NULL CHECK (length(trim(trace_id)) > 0 AND length(trace_id) <= 512), correlation_id TEXT NOT NULL, idempotency_key TEXT NOT NULL, reason TEXT NOT NULL CHECK (length(trim(reason)) BETWEEN 1 AND 2000), object_count INTEGER NOT NULL CHECK (object_count >= 0), collecting_at TEXT NOT NULL, completed_at TEXT NULL)",
                "CREATE INDEX module_artifact_data_snapshot_collections_tenant_idx ON module_artifact_data_snapshot_collections (tenant_id, collecting_at, collection_id)",
                "CREATE TABLE module_artifact_data_restore_operations (tenant_id TEXT NOT NULL, data_owner_id TEXT NOT NULL, namespace_instance_id TEXT NOT NULL, idempotency_key TEXT NOT NULL, request_digest TEXT NOT NULL CHECK (length(request_digest) = 71), snapshot_id TEXT NOT NULL, expected_namespace_revision INTEGER NOT NULL CHECK (expected_namespace_revision > 0), namespace_revision INTEGER NOT NULL CHECK (namespace_revision > 0), restored_records INTEGER NOT NULL CHECK (restored_records >= 0), restored_objects INTEGER NOT NULL CHECK (restored_objects >= 0), actor_id TEXT NOT NULL, trace_id TEXT NOT NULL CHECK (length(trim(trace_id)) > 0 AND length(trace_id) <= 512), correlation_id TEXT NOT NULL, reason TEXT NOT NULL CHECK (length(trim(reason)) BETWEEN 1 AND 2000), completed_at TEXT NOT NULL, PRIMARY KEY (tenant_id, data_owner_id, namespace_instance_id, idempotency_key))",
            ],
            backend => {
