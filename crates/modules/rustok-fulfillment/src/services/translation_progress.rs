...
pub(crate) struct ShippingOptionTranslationExactProgressFacts {
    pub resources: u64,
...
struct ShippingOptionTranslationExactProgressRow {
    resources: i64,
...
impl ShippingOptionTranslationService {
    pub(crate) async fn read_exact_progress(
        ...
        if source_locale == target_locale {
            return Err(ShippingOptionTranslationExactLocaleError::Validation(
                "Fulfillment translation progress source and target locale must differ".to_string(),
            ));
        }

        query_shipping_option_translation_exact_progress(
            self.database(),
            tenant_id,
            &source_locale,
            &target_locale,
...
async fn query_shipping_option_translation_exact_progress<C>(
    db: &C,
    tenant_id: Uuid,
    source_locale: &str,
    target_locale: &str,
    ...
    C: ConnectionTrait,
{
    ...
    let (sql, values) = match backend {
        DatabaseBackend::Postgres => (
            SHIPPING_OPTION_TRANSLATION_PROGRESS_POSTGRES_SQL,
            vec![
                tenant_id.into(),
        ...
        _ => (
            SHIPPING_OPTION_TRANSLATION_PROGRESS_QUESTION_MARK_SQL,
            vec![
                target_locale.to_string().into(),
                tenant_id.into(),
                source_locale.to_string().into(),
    ...
    let row = ShippingOptionTranslationExactProgressRow::find_by_statement(
    ...
    .ok_or_else(|| {
        ShippingOptionTranslationExactLocaleError::Validation(
            "Fulfillment translation progress aggregate returned no row".to_string(),
        )
    })?;

    let facts = ShippingOptionTranslationExactProgressFacts {
    ...
    if facts.exact_required_units > facts.resources || facts.complete_resources > facts.resources {
        return Err(ShippingOptionTranslationExactLocaleError::Validation(
            "Fulfillment translation progress aggregate is internally inconsistent".to_string(),
        ));
    }
    Ok(facts)
}
...
fn progress_count(
    ...
    u64::try_from(value).map_err(|_| {
        ShippingOptionTranslationExactLocaleError::Validation(format!(
            "Fulfillment translation progress {field} must not be negative"
        ))
    })
}

const SHIPPING_OPTION_TRANSLATION_PROGRESS_POSTGRES_SQL: &str = r#"
SELECT
    COUNT(*) AS resources,
...
FROM shipping_option_translations AS source_translation
INNER JOIN shipping_options AS shipping_option
    ON shipping_option.id = source_translation.shipping_option_id
LEFT JOIN shipping_option_translations AS target_translation
    ON target_translation.shipping_option_id = source_translation.shipping_option_id
   AND target_translation.locale = $3
WHERE shipping_option.tenant_id = $1
  AND source_translation.locale = $2
"#;

const SHIPPING_OPTION_TRANSLATION_PROGRESS_QUESTION_MARK_SQL: &str = r#"
SELECT
    COUNT(*) AS resources,
    COUNT(CASE WHEN TRIM(COALESCE(target_translation.name, '')) <> '' THEN 1 END)
        AS exact_required_units,
...
WHERE shipping_option.tenant_id = ?
