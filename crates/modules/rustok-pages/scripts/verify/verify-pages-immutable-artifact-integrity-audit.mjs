...
for (const key of [
  ...
  "id_scan_projection_only",
  "loads_full_payloads_one_record_at_a_time",
  "streaming_audit_identity_hash",
...
const idQuery = sliceBetween(
  auditMethod,
  "let artifact_id_query = ||",
  "let mut artifact_ids = match txn.get_database_backend()",
  "artifact id projection",
);
for (const marker of [
  ".select_only()",
  ".column(page_static_landing_artifact::Column::Id)",
  ".limit(fetch_limit)",
]) need(idQuery, marker, "artifact id projection");
for (const marker of [
  "DocumentHtml",
  "BodyHtml",
  "Css",
...
]) forbid(idQuery, marker, "artifact id projection");
for (const marker of [
  "ActiveModel",
  ".insert(",
  ".update(",
