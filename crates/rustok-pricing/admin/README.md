...
- Keeps pricing visibility and price-health UX inside the pricing-owned package.
- Participates in manifest-driven admin composition through `rustok-module.toml`.
- Uses the existing `rustok-commerce` GraphQL product contract for current read-side price visibility while dedicated pricing write transport is still being split out.
...
- Reads price-related product and variant fields from the current `rustok-commerce` GraphQL contract in parallel with the ongoing ecommerce UI split.
