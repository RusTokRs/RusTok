...
| Split `ProductCatalogSchemaService` into attributes, schemas, categories, values, and virtual categories | partial | Category creation, groups, bindings, schema modes, and listing are isolated in `src/services/catalog_schema_service/categories.rs`; schema creation/listing/groups/bindings are isolated in `src/services/catalog_schema_service/schemas.rs`; attribute reads/writes are isolated in `src/services/catalog_schema_service/attributes.rs`; virtual-category rule reference validation is isolated in `src/services/catalog_schema_service/virtual_categories.rs`. Values and effective-form projection remain in the main service file. |
...
| Remove transitional columns from products, options, images, translations, and variants | partial | Obsolete variant inventory fields are removed. The obsolete cross-module `product_images.media_id -> media.id` foreign key is removed from the clean-install migration and existing PostgreSQL databases; the field now stores a Media-owned UUID validated through the public owner contract. Other legacy columns still need an audited consumer inventory. |
...
| Item | Status | Review result and evidence |
| --- | --- | --- |
| PostgreSQL `up/down/up` migration integration tests | resolved | `product_postgres_migrations_support_up_down_up` creates an isolated PostgreSQL database, installs the minimal tenant/taxonomy/Flex owner prerequisites, applies every Product migration, tears the Product schema down, and reapplies it. The 2026-07-25 local run passed. The full platform clean-install smoke separately stops later in an unrelated `ledger_reversals` foreign-key migration. |
...
| Tenant-isolation tests for product/catalog storage | partial | The GraphQL runtime suite rejects a substituted tenant on every current product read root before storage access. The PostgreSQL constraint fixture additionally proves cross-tenant tag rejection, but persistence coverage for categories, schemas, attributes, values, and translations is not yet complete. |
...
- `RUSTOK_MIGRATION_SMOKE_ADMIN_URL=<local-dev-admin-url> postgres_zero_migration_smoke product_postgres_migrations_support_up_down_up --ignored --exact` passed on 2026-07-25, proving the Product migration `up/down/up` lifecycle against PostgreSQL.
- `RUSTOK_MIGRATION_SMOKE_ADMIN_URL=<local-dev-admin-url> postgres_zero_migration_smoke product_postgres_constraints_reject_invalid_and_racing_writes --ignored --exact` passed on 2026-07-25, proving concurrent handle/SKU uniqueness plus cross-tenant tag, typed EAV, canonical primary-category, root-slug, category-cycle, and closure-drift rejection against PostgreSQL.
- `RUSTOK_MIGRATION_SMOKE_ADMIN_URL=<local-dev-admin-url> postgres_zero_migration_smoke product_tenant_integrity_migration_rejects_dirty_data_and_maps_inventory --ignored --exact` passed on 2026-07-25, proving dirty handle/SKU/root-slug preflight failures and the legacy inventory backfill/drop contract.
- `RUSTOK_MIGRATION_SMOKE_ADMIN_URL=<local-dev-admin-url> postgres_zero_migration_smoke product_catalog_read_port_executes_against_postgres --ignored --exact` passed on 2026-07-25, proving the live in-process Product read port against PostgreSQL.
...
2. Keep the PostgreSQL lifecycle and invariant fixtures in the release gate; lifecycle, race, tag, EAV, category-tree/closure, primary-category, root-slug, dirty-data preflight, and legacy inventory checks now pass.
