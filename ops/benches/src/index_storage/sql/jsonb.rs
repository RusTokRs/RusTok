...
pub fn mutation_workloads(context: &WorkloadContext) -> Vec<MutationWorkload> {
    ...
    vec![
        ...
        MutationWorkload {
            ...
            sql: format!(
                "WITH targets AS (SELECT product_id FROM idx_bench_source.product WHERE tenant_no = 1 AND locale = {locale} AND product_no <= {batch}), deleted_links AS (DELETE FROM idx_bench_jsonb.link AS link USING targets WHERE link.tenant_id = {tenant} AND link.source_module = 'product' AND link.source_entity = 'product' AND link.source_schema_version = 1 AND link.source_locale = {locale} AND link.source_entity_id = targets.product_id RETURNING 1), deleted_entities AS (DELETE FROM idx_bench_jsonb.entity AS entity USING targets WHERE entity.tenant_id = {tenant} AND entity.module_name = 'product' AND entity.entity_name = 'product' AND entity.schema_version = 1 AND entity.locale = {locale} AND entity.entity_id = targets.product_id RETURNING entity.entity_id) SELECT (SELECT count(*) FROM deleted_entities)::bigint AS affected_entities, (SELECT count(*) FROM deleted_links)::bigint AS affected_links, {deleted_links}::bigint AS expected_links"
            ),
            expected_affected_entities: i64::from(batch),
        },
    ]
