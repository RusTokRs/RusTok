...
SELECT tenant_id, 'variant', variant_id, locale, 'sku', 0, sku
FROM idx_bench_source.variant;

INSERT INTO idx_bench_eav.field_value (
    tenant_id, entity_name, entity_id, locale, field_name, ordinal, value_int
)
SELECT tenant_id, 'variant', variant_id, locale, 'price_minor', 0, price_minor
FROM idx_bench_source.variant;

INSERT INTO idx_bench_eav.field_value (
    tenant_id, entity_name, entity_id, locale, field_name, ordinal, value_uuid
)
SELECT tenant_id, 'variant', variant_id, locale, 'product_id', 0, product_id
FROM idx_bench_source.variant;

INSERT INTO idx_bench_eav.field_value (
    tenant_id, entity_name, entity_id, locale, field_name, ordinal, value_text
)
SELECT tenant_id, 'sales_channel', channel_id, '', 'code', 0, code
FROM idx_bench_source.channel
UNION ALL
SELECT tenant_id, 'sales_channel', channel_id, '', 'region_code', 0, region_code
FROM idx_bench_source.channel;
...
pub fn workloads(context: &WorkloadContext) -> Vec<Workload> {
    ...
    vec![
        Workload {
            ...
            sql: format!(
                "SELECT entity.entity_id AS entity_id FROM idx_bench_eav.entity AS entity JOIN idx_bench_eav.field_value AS status ON status.tenant_id = entity.tenant_id AND status.entity_name = entity.entity_name AND status.entity_id = entity.entity_id AND status.locale = entity.locale AND status.field_name = 'status' WHERE entity.tenant_id = {tenant} AND entity.entity_name = 'product' AND entity.locale = {locale} AND status.value_text = 'published' ORDER BY entity_id LIMIT 100"
            ),
        },
        Workload {
            name: "price_range_sort",
            sql: format!(
                "SELECT entity.entity_id AS entity_id, price.value_int AS price_minor FROM idx_bench_eav.entity AS entity JOIN idx_bench_eav.field_value AS price ON price.tenant_id = entity.tenant_id AND price.entity_name = entity.entity_name AND price.entity_id = entity.entity_id AND price.locale = entity.locale AND price.field_name = 'price_minor' WHERE entity.tenant_id = {tenant} AND entity.entity_name = 'product' AND entity.locale = {locale} AND price.value_int BETWEEN 20000 AND 80000 ORDER BY price_minor, entity_id LIMIT 100"
            ),
        },
        Workload {
            name: "multi_value_tag",
            sql: format!(
                "SELECT DISTINCT entity.entity_id AS entity_id FROM idx_bench_eav.entity AS entity JOIN idx_bench_eav.field_value AS tag ON tag.tenant_id = entity.tenant_id AND tag.entity_name = entity.entity_name AND tag.entity_id = entity.entity_id AND tag.locale = entity.locale AND tag.field_name = 'tags' WHERE entity.tenant_id = {tenant} AND entity.entity_name = 'product' AND entity.locale = {locale} AND tag.value_text = 'tag-3' ORDER BY entity_id LIMIT 100"
            ),
        },
        Workload {
            name: "two_hop_channel_filter",
            sql: format!(
                "SELECT DISTINCT product.entity_id AS entity_id FROM idx_bench_eav.entity AS product JOIN idx_bench_eav.link AS product_variant ON product_variant.tenant_id = product.tenant_id AND product_variant.source_entity = 'product' AND product_variant.source_entity_id = product.entity_id AND product_variant.source_locale = product.locale AND product_variant.link_name = 'variants' AND product_variant.target_entity = 'variant' JOIN idx_bench_eav.link AS variant_channel ON variant_channel.tenant_id = product_variant.tenant_id AND variant_channel.source_entity = 'variant' AND variant_channel.source_entity_id = product_variant.target_entity_id AND variant_channel.source_locale = product_variant.target_locale AND variant_channel.link_name = 'sales_channels' AND variant_channel.target_entity = 'sales_channel' JOIN idx_bench_eav.field_value AS channel_code ON channel_code.tenant_id = variant_channel.tenant_id AND channel_code.entity_name = 'sales_channel' AND channel_code.entity_id = variant_channel.target_entity_id AND channel_code.locale = variant_channel.target_locale AND channel_code.field_name = 'code' WHERE product.tenant_id = {tenant} AND product.entity_name = 'product' AND product.locale = {locale} AND channel_code.value_text = 'channel-1' ORDER BY entity_id LIMIT 100"
            ),
        },
        Workload {
            name: "keyset_page",
            sql: format!(
                "SELECT entity.entity_id AS entity_id, price.value_int AS price_minor FROM idx_bench_eav.entity AS entity JOIN idx_bench_eav.field_value AS price ON price.tenant_id = entity.tenant_id AND price.entity_name = entity.entity_name AND price.entity_id = entity.entity_id AND price.locale = entity.locale AND price.field_name = 'price_minor' WHERE entity.tenant_id = {tenant} AND entity.entity_name = 'product' AND entity.locale = {locale} AND (price.value_int, entity.entity_id) > ({anchor_price}, {anchor_id}) ORDER BY price_minor, entity_id LIMIT 100"
            ),
        },
        Workload {
            name: "exact_count",
