...
pub fn workloads(context: &WorkloadContext) -> Vec<Workload> {
    ...
    vec![
        Workload {
            ...
            sql: format!(
                "SELECT entity.entity_id AS entity_id FROM idx_bench_eav.entity AS entity JOIN idx_bench_eav.field_value AS status ON status.tenant_id = entity.tenant_id AND status.entity_name = entity.entity_name AND status.entity_id = entity.entity_id AND status.locale = entity.locale AND status.field_name = 'status' WHERE entity.tenant_id = {tenant} AND entity.module_name = 'product' AND entity.entity_name = 'product' AND entity.schema_version = 1 AND entity.locale = {locale} AND status.value_text = 'published' ORDER BY entity_id LIMIT 100"
            ),
        },
        Workload {
        ...
        Workload {
            ...
            sql: format!(
                "SELECT DISTINCT entity.entity_id AS entity_id FROM idx_bench_eav.entity AS entity JOIN idx_bench_eav.field_value AS tag ON tag.tenant_id = entity.tenant_id AND tag.entity_name = entity.entity_name AND tag.entity_id = entity.entity_id AND tag.locale = entity.locale AND tag.field_name = 'tags' WHERE entity.tenant_id = {tenant} AND entity.module_name = 'product' AND entity.entity_name = 'product' AND entity.schema_version = 1 AND entity.locale = {locale} AND tag.value_text = 'tag-3' ORDER BY entity_id LIMIT 100"
            ),
        },
        Workload {
        ...
        Workload {
            name: "exact_count",
            sql: format!(
                "SELECT count(*)::bigint AS result_count FROM idx_bench_eav.entity AS entity JOIN idx_bench_eav.field_value AS status ON status.tenant_id = entity.tenant_id AND status.entity_name = entity.entity_name AND status.entity_id = entity.entity_id AND status.locale = entity.locale AND status.field_name = 'status' WHERE entity.tenant_id = {tenant} AND entity.module_name = 'product' AND entity.entity_name = 'product' AND entity.schema_version = 1 AND entity.locale = {locale} AND status.value_text = 'published'"
            ),
        },
    ]
}
