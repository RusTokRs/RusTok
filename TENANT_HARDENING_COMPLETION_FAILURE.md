# Tenant hardening completion failure

```text
/home/runner/work/RusTok/RusTok/scripts/migrations/complete_tenant_isolation_coverage.py:201: SyntaxWarning: invalid escape sequence '\.'
  '''requireMatch(resolution, /resolve_explicit_slug/, "self-resolving transports must produce typed resolutions");
/home/runner/work/RusTok/RusTok/scripts/migrations/complete_tenant_isolation_coverage.py:210: SyntaxWarning: invalid escape sequence '\('
  '''requireMatch(runtime, /record_resolution_source\(resolution\.source\)/, "all tenant transports must emit telemetry from typed resolution results");
/home/runner/work/RusTok/RusTok/scripts/migrations/complete_tenant_isolation_coverage.py:213: SyntaxWarning: invalid escape sequence '\('
  '''requireMatch(runtime, /record_resolution_outcome/, "tenant resolution outcomes must use dedicated telemetry");
/home/runner/work/RusTok/RusTok/scripts/migrations/complete_tenant_isolation_coverage.py:227: SyntaxWarning: invalid escape sequence '\/'
  '''forbidMatch(graphql, /models::tenants::Entity::find_by_slug/, "GraphQL WebSocket must not query tenants directly");
✔ typed tenant resolution architecture verified
[1m[92m    Updating[0m crates.io index
[1m[92m Downloading[0m crates ...
[1m[92m  Downloaded[0m atomic-waker v1.1.2
[1m[92m  Downloaded[0m async-trait v0.1.89
[1m[92m  Downloaded[0m anyhow v1.0.103
[1m[92m  Downloaded[0m form_urlencoded v1.2.2
[1m[92m  Downloaded[0m futures-core v0.3.32
[1m[92m  Downloaded[0m cfg-if v1.0.4
[1m[92m  Downloaded[0m ipnet v2.12.0
[1m[92m  Downloaded[0m base64 v0.22.1
[1m[92m  Downloaded[0m icu_locale_core v2.2.0
[1m[92m  Downloaded[0m signal-hook-registry v1.4.8
[1m[92m  Downloaded[0m rand v0.9.5
[1m[92m  Downloaded[0m hyper-util v0.1.20
[1m[92m  Downloaded[0m aho-corasick v1.1.4
[1m[92m  Downloaded[0m futures-util v0.3.32
[1m[92m  Downloaded[0m idna v1.1.0
[1m[92m  Downloaded[0m protobuf v3.7.2
[1m[92m  Downloaded[0m procfs-core v0.17.0
[1m[92m  Downloaded[0m parking_lot_core v0.9.12
[1m[92m  Downloaded[0m procfs v0.17.0
[1m[92m  Downloaded[0m percent-encoding v2.3.2
[1m[92m  Downloaded[0m protobuf-support v3.7.2
[1m[92m  Downloaded[0m regex-syntax v0.8.11
[1m[92m  Downloaded[0m serde v1.0.228
[1m[92m  Downloaded[0m smallvec v1.15.2
[1m[92m  Downloaded[0m futures-macro v0.3.32
[1m[92m  Downloaded[0m hashbrown v0.17.1
[1m[92m  Downloaded[0m thiserror v1.0.69
[1m[92m  Downloaded[0m prost-derive v0.14.4
[1m[92m  Downloaded[0m stable_deref_trait v1.2.1
[1m[92m  Downloaded[0m bytes v1.12.1
[1m[92m  Downloaded[0m prost-types v0.14.4
[1m[92m  Downloaded[0m thiserror-impl v2.0.18
[1m[92m  Downloaded[0m futures-task v0.3.32
[1m[92m  Downloaded[0m rand_core v0.9.5
[1m[92m  Downloaded[0m url v2.5.8
[1m[92m  Downloaded[0m scopeguard v1.2.0
[1m[92m  Downloaded[0m tonic v0.14.6
[1m[92m  Downloaded[0m unicode-ident v1.0.24
[1m[92m  Downloaded[0m tokio-stream v0.1.18
[1m[92m  Downloaded[0m zerovec-derive v0.11.3
[1m[92m  Downloaded[0m tinystr v0.8.3
[1m[92m  Downloaded[0m tracing-log v0.2.0
[1m[92m  Downloaded[0m fnv v1.0.7
[1m[92m  Downloaded[0m itoa v1.0.18
[1m[92m  Downloaded[0m sync_wrapper v1.0.2
[1m[92m  Downloaded[0m yoke-derive v0.8.2
[1m[92m  Downloaded[0m thiserror-impl v1.0.69
[1m[92m  Downloaded[0m displaydoc v0.2.6
[1m[92m  Downloaded[0m rustix v0.38.44
[1m[92m  Downloaded[0m thread_local v1.1.10
[1m[92m  Downloaded[0m futures-io v0.3.32
[1m[92m  Downloaded[0m tower-layer v0.3.3
[1m[92m  Downloaded[0m tracing-subscriber v0.3.23
[1m[92m  Downloaded[0m zerotrie v0.2.4
[1m[92m  Downloaded[0m zmij v1.0.22
[1m[92m  Downloaded[0m equivalent v1.0.2
[1m[92m  Downloaded[0m tracing-attributes v0.1.31
[1m[92m  Downloaded[0m errno v0.3.14
[1m[92m  Downloaded[0m futures-executor v0.3.32
[1m[92m  Downloaded[0m nu-ansi-term v0.50.3
[1m[92m  Downloaded[0m pin-project-lite v0.2.17
[1m[92m  Downloaded[0m ppv-lite86 v0.2.21
[1m[92m  Downloaded[0m thiserror v2.0.18
[1m[92m  Downloaded[0m hex v0.4.3
[1m[92m  Downloaded[0m http-body v1.0.1
[1m[92m  Downloaded[0m idna_adapter v1.2.2
[1m[92m  Downloaded[0m futures-sink v0.3.32
[1m[92m  Downloaded[0m potential_utf v0.1.5
[1m[92m  Downloaded[0m socket2 v0.6.4
[1m[92m  Downloaded[0m rand_chacha v0.9.0
[1m[92m  Downloaded[0m slab v0.4.12
[1m[92m  Downloaded[0m http-body-util v0.1.3
[1m[92m  Downloaded[0m hyper-timeout v0.5.2
[1m[92m  Downloaded[0m opentelemetry-http v0.32.0
[1m[92m  Downloaded[0m sharded-slab v0.1.7
[1m[92m  Downloaded[0m tokio-macros v2.7.0
[1m[92m  Downloaded[0m bitflags v2.13.0
[1m[92m  Downloaded[0m either v1.16.0
[1m[92m  Downloaded[0m futures-channel v0.3.32
[1m[92m  Downloaded[0m litemap v0.8.2
[1m[92m  Downloaded[0m lock_api v0.4.14
[1m[92m  Downloaded[0m log v0.4.33
[1m[92m  Downloaded[0m once_cell v1.21.4
[1m[92m  Downloaded[0m parking_lot v0.12.5
[1m[92m  Downloaded[0m pin-project v1.1.13
[1m[92m  Downloaded[0m proc-macro2 v1.0.106
[1m[92m  Downloaded[0m quote v1.0.46
[1m[92m  Downloaded[0m reqwest v0.13.4
[1m[92m  Downloaded[0m serde_core v1.0.228
[1m[92m  Downloaded[0m synstructure v0.13.2
[1m[92m  Downloaded[0m tonic-types v0.14.6
[1m[92m  Downloaded[0m tracing-serde v0.2.0
[1m[92m  Downloaded[0m getrandom v0.3.4
[1m[92m  Downloaded[0m icu_normalizer v2.2.0
[1m[92m  Downloaded[0m icu_normalizer_data v2.2.0
[1m[92m  Downloaded[0m icu_properties v2.2.0
[1m[92m  Downloaded[0m icu_provider v2.2.0
[1m[92m  Downloaded[0m memchr v2.8.3
[1m[92m  Downloaded[0m opentelemetry-otlp v0.32.0
[1m[92m  Downloaded[0m tracing-core v0.1.36
[1m[92m  Downloaded[0m zerofrom v0.1.8
[1m[92m  Downloaded[0m icu_collections v2.2.0
[1m[92m  Downloaded[0m mio v1.2.1
[1m[92m  Downloaded[0m opentelemetry v0.32.0
[1m[92m  Downloaded[0m prometheus v0.14.0
[1m[92m  Downloaded[0m indexmap v2.14.0
[1m[92m  Downloaded[0m serde_json v1.0.150
[1m[92m  Downloaded[0m try-lock v0.2.5
[1m[92m  Downloaded[0m itertools v0.14.0
[1m[92m  Downloaded[0m tracing-opentelemetry v0.33.0
[1m[92m  Downloaded[0m icu_properties_data v2.2.0
[1m[92m  Downloaded[0m zerofrom-derive v0.1.7
[1m[92m  Downloaded[0m hyper v1.10.1
[1m[92m  Downloaded[0m opentelemetry-proto v0.32.0
[1m[92m  Downloaded[0m tokio-util v0.7.18
[1m[92m  Downloaded[0m writeable v0.6.3
[1m[92m  Downloaded[0m opentelemetry_sdk v0.32.1
[1m[92m  Downloaded[0m syn v2.0.118
[1m[92m  Downloaded[0m libc v0.2.186
[1m[92m  Downloaded[0m tower v0.5.3
[1m[92m  Downloaded[0m h2 v0.4.15
[1m[92m  Downloaded[0m utf8_iter v1.0.4
[1m[92m  Downloaded[0m linux-raw-sys v0.4.15
[1m[92m  Downloaded[0m regex-automata v0.4.15
[1m[92m  Downloaded[0m tower-service v0.3.3
[1m[92m  Downloaded[0m tonic-prost v0.14.6
[1m[92m  Downloaded[0m tower-http v0.6.11
[1m[92m  Downloaded[0m http v1.4.2
[1m[92m  Downloaded[0m zerovec v0.11.6
[1m[92m  Downloaded[0m want v0.3.1
[1m[92m  Downloaded[0m httparse v1.10.1
[1m[92m  Downloaded[0m yoke v0.8.3
[1m[92m  Downloaded[0m tracing v0.1.44
[1m[92m  Downloaded[0m zerocopy v0.8.54
[1m[92m  Downloaded[0m tokio v1.52.3
[1m[92m  Downloaded[0m pin-project-internal v1.1.13
[1m[92m  Downloaded[0m lazy_static v1.5.0
[1m[92m  Downloaded[0m prost v0.14.4
[1m[92m  Downloaded[0m matchers v0.2.0
[1m[92m   Compiling[0m proc-macro2 v1.0.106
[1m[92m   Compiling[0m unicode-ident v1.0.24
[1m[92m   Compiling[0m quote v1.0.46
[1m[92m   Compiling[0m libc v0.2.186
[1m[92m   Compiling[0m cfg-if v1.0.4
[1m[92m   Compiling[0m smallvec v1.15.2
[1m[92m   Compiling[0m pin-project-lite v0.2.17
[1m[92m   Compiling[0m once_cell v1.21.4
[1m[92m   Compiling[0m futures-core v0.3.32
[1m[92m   Compiling[0m bytes v1.12.1
[1m[92m   Compiling[0m parking_lot_core v0.9.12
[1m[92m   Compiling[0m tracing-core v0.1.36
[1m[92m   Compiling[0m syn v2.0.118
[1m[92m   Compiling[0m scopeguard v1.2.0
[1m[92m   Compiling[0m futures-sink v0.3.32
[1m[92m   Compiling[0m lock_api v0.4.14
[1m[92m   Compiling[0m errno v0.3.14
[1m[92m   Compiling[0m memchr v2.8.3
[1m[92m   Compiling[0m itoa v1.0.18
[1m[92m   Compiling[0m parking_lot v0.12.5
[1m[92m   Compiling[0m signal-hook-registry v1.4.8
[1m[92m   Compiling[0m mio v1.2.1
[1m[92m   Compiling[0m socket2 v0.6.4
[1m[92m   Compiling[0m stable_deref_trait v1.2.1
[1m[92m   Compiling[0m slab v0.4.12
[1m[92m   Compiling[0m http v1.4.2
[1m[92m   Compiling[0m equivalent v1.0.2
[1m[92m   Compiling[0m futures-io v0.3.32
[1m[92m   Compiling[0m hashbrown v0.17.1
[1m[92m   Compiling[0m futures-task v0.3.32
[1m[92m   Compiling[0m percent-encoding v2.3.2
[1m[92m   Compiling[0m fnv v1.0.7
[1m[92m   Compiling[0m httparse v1.10.1
[1m[92m   Compiling[0m http-body v1.0.1
[1m[92m   Compiling[0m indexmap v2.14.0
[1m[92m   Compiling[0m futures-channel v0.3.32
[1m[92m   Compiling[0m try-lock v0.2.5
[1m[92m   Compiling[0m atomic-waker v1.1.2
[1m[92m   Compiling[0m tower-service v0.3.3
[1m[92m   Compiling[0m writeable v0.6.3
[1m[92m   Compiling[0m litemap v0.8.2
[1m[92m   Compiling[0m want v0.3.1
[1m[92m   Compiling[0m icu_normalizer_data v2.2.0
[1m[92m   Compiling[0m utf8_iter v1.0.4
[1m[92m   Compiling[0m icu_properties_data v2.2.0
[1m[92m   Compiling[0m sync_wrapper v1.0.2
[1m[92m   Compiling[0m ipnet v2.12.0
[1m[92m   Compiling[0m thiserror v2.0.18
[1m[92m   Compiling[0m anyhow v1.0.103
[1m[92m   Compiling[0m base64 v0.22.1
[1m[92m   Compiling[0m tower-layer v0.3.3
[1m[92m   Compiling[0m bitflags v2.13.0
[1m[92m   Compiling[0m either v1.16.0
[1m[92m   Compiling[0m zerocopy v0.8.54
[1m[92m   Compiling[0m synstructure v0.13.2
[1m[92m   Compiling[0m getrandom v0.3.4
[1m[92m   Compiling[0m itertools v0.14.0
[1m[92m   Compiling[0m http-body-util v0.1.3
[1m[92m   Compiling[0m serde_core v1.0.228
[1m[92m   Compiling[0m log v0.4.33
[1m[92m   Compiling[0m rand_core v0.9.5
[1m[92m   Compiling[0m form_urlencoded v1.2.2
[1m[92m   Compiling[0m tracing-attributes v0.1.31
[1m[92m   Compiling[0m zerofrom-derive v0.1.7
[1m[92m   Compiling[0m tracing v0.1.44
[1m[92m   Compiling[0m zerofrom v0.1.8
[1m[92m   Compiling[0m yoke-derive v0.8.2
[1m[92m   Compiling[0m tokio-macros v2.7.0
[1m[92m   Compiling[0m zerovec-derive v0.11.3
[1m[92m   Compiling[0m tokio v1.52.3
[1m[92m   Compiling[0m yoke v0.8.3
[1m[92m   Compiling[0m displaydoc v0.2.6
[1m[92m   Compiling[0m futures-macro v0.3.32
[1m[92m   Compiling[0m zerovec v0.11.6
[1m[92m   Compiling[0m zerotrie v0.2.4
[1m[92m   Compiling[0m tinystr v0.8.3
[1m[92m   Compiling[0m potential_utf v0.1.5
[1m[92m   Compiling[0m icu_locale_core v2.2.0
[1m[92m   Compiling[0m icu_collections v2.2.0
[1m[92m   Compiling[0m futures-util v0.3.32
[1m[92m   Compiling[0m thiserror-impl v2.0.18
[1m[92m   Compiling[0m icu_provider v2.2.0
[1m[92m   Compiling[0m icu_normalizer v2.2.0
[1m[92m   Compiling[0m icu_properties v2.2.0
[1m[92m   Compiling[0m prost-derive v0.14.4
[1m[92m   Compiling[0m async-trait v0.1.89
[1m[92m   Compiling[0m tokio-util v0.7.18
[1m[92m   Compiling[0m idna_adapter v1.2.2
[1m[92m   Compiling[0m tokio-stream v0.1.18
[1m[92m   Compiling[0m h2 v0.4.15
[1m[92m   Compiling[0m tower v0.5.3
[1m[92m   Compiling[0m pin-project-internal v1.1.13
[1m[92m   Compiling[0m prost v0.14.4
[1m[92m   Compiling[0m pin-project v1.1.13
[1m[92m   Compiling[0m idna v1.1.0
[1m[92m   Compiling[0m opentelemetry v0.32.0
[1m[92m   Compiling[0m ppv-lite86 v0.2.21
[1m[92m   Compiling[0m thiserror v1.0.69
[1m[92m   Compiling[0m hyper v1.10.1
[1m[92m   Compiling[0m serde v1.0.228
[1m[92m   Compiling[0m zmij v1.0.22
[1m[92m   Compiling[0m rand_chacha v0.9.0
[1m[92m   Compiling[0m url v2.5.8
[1m[92m   Compiling[0m thiserror-impl v1.0.69
[1m[92m   Compiling[0m hyper-util v0.1.20
[1m[92m   Compiling[0m rustix v0.38.44
[1m[92m   Compiling[0m serde_json v1.0.150
[1m[92m   Compiling[0m regex-syntax v0.8.11
[1m[92m   Compiling[0m lazy_static v1.5.0
[1m[92m   Compiling[0m tower-http v0.6.11
[1m[92m   Compiling[0m rand v0.9.5
[1m[92m   Compiling[0m hyper-timeout v0.5.2
[1m[92m   Compiling[0m tonic v0.14.6
[1m[92m   Compiling[0m regex-automata v0.4.15
[1m[92m   Compiling[0m futures-executor v0.3.32
[1m[92m   Compiling[0m linux-raw-sys v0.4.15
[1m[92m   Compiling[0m hex v0.4.3
[1m[92m   Compiling[0m protobuf v3.7.2
[1m[92m   Compiling[0m procfs v0.17.0
[1m[92m   Compiling[0m procfs-core v0.17.0
[1m[92m   Compiling[0m matchers v0.2.0
[1m[92m   Compiling[0m tonic-prost v0.14.6
[1m[92m   Compiling[0m opentelemetry_sdk v0.32.1
[1m[92m   Compiling[0m tracing-serde v0.2.0
[1m[92m   Compiling[0m reqwest v0.13.4
[1m[92m   Compiling[0m protobuf-support v3.7.2
[1m[92m   Compiling[0m sharded-slab v0.1.7
[1m[92m   Compiling[0m prost-types v0.14.4
[1m[92m   Compiling[0m tracing-log v0.2.0
[1m[92m   Compiling[0m thread_local v1.1.10
[1m[92m   Compiling[0m prometheus v0.14.0
[1m[92m   Compiling[0m nu-ansi-term v0.50.3
[1m[92m   Compiling[0m tonic-types v0.14.6
[1m[92m   Compiling[0m tracing-subscriber v0.3.23
[1m[92m   Compiling[0m opentelemetry-http v0.32.0
[1m[92m   Compiling[0m opentelemetry-proto v0.32.0
[1m[92m   Compiling[0m tracing-opentelemetry v0.33.0
[1m[92m   Compiling[0m opentelemetry-otlp v0.32.0
[1m[92m   Compiling[0m rustok-telemetry v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-telemetry)
[1m[92m    Finished[0m `test` profile [unoptimized + debuginfo] target(s) in 34.65s
[1m[92m     Running[0m tests/metrics_test.rs (target/debug/deps/metrics_test-b180d16b10055285)

running 21 tests
test test_circuit_breaker_state_values ... ok
test test_cache_metrics ... ok
test test_circuit_breaker_metrics ... ok
test test_counter_increments ... ok
test test_database_metrics ... ok
test test_event_bus_metrics ... ok
test test_gauge_updates ... ok
test test_high_cardinality_labels ... ok
test test_histogram_buckets ... ok
test test_metric_labels ... ok
test test_large_values ... ok
test test_concurrent_metric_updates ... ok
test test_module_entrypoint_metrics ... ok
test test_module_error_metrics ... ok
test test_multiple_services ... ok
test test_metric_naming_conventions ... ok
test test_metric_help_text ... ok
test test_span_metrics ... ok
test test_tenant_resolution_metrics ... ok
test test_zero_values ... ok
test test_register_all_metrics ... ok

test result: ok. 21 passed; 0 failed; 0 ignored; 0 measured; 0 filtered out; finished in 0.00s

[1m[92m Downloading[0m crates ...
[1m[92m  Downloaded[0m as-slice v0.2.1
[1m[92m  Downloaded[0m arrayref v0.3.9
[1m[92m  Downloaded[0m asn1-rs-derive v0.6.0
[1m[92m  Downloaded[0m arcstr v1.2.0
[1m[92m  Downloaded[0m asynk-strim v0.1.5
[1m[92m  Downloaded[0m aws-smithy-observability v0.3.0
[1m[92m  Downloaded[0m aes v0.8.4
[1m[92m  Downloaded[0m anstream v1.0.0
[1m[92m  Downloaded[0m async-scoped v0.9.0
[1m[92m  Downloaded[0m base64 v0.21.7
[1m[92m  Downloaded[0m azure_identity v1.0.0
[1m[92m  Downloaded[0m aws-types v1.4.0
[1m[92m  Downloaded[0m async-dropper v0.3.1
[1m[92m  Downloaded[0m inherent v1.0.13
[1m[92m  Downloaded[0m async-task v4.7.1
[1m[92m  Downloaded[0m aws-sdk-ssooidc v1.105.0
[1m[92m  Downloaded[0m azure_core v1.1.0
[1m[92m  Downloaded[0m bigdecimal v0.4.10
[1m[92m  Downloaded[0m attribute-derive v0.10.5
[1m[92m  Downloaded[0m async-lock v3.4.2
[1m[92m  Downloaded[0m darling_macro v0.21.3
[1m[92m  Downloaded[0m darling_macro v0.23.0
[1m[92m  Downloaded[0m default-struct-builder v0.5.1
[1m[92m  Downloaded[0m aws-sdk-secretsmanager v1.109.0
[1m[92m  Downloaded[0m aws-smithy-async v1.3.0
[1m[92m  Downloaded[0m databake v0.2.1
[1m[92m  Downloaded[0m databake-derive v0.2.1
[1m[92m  Downloaded[0m derive_arbitrary v1.4.2
[1m[92m  Downloaded[0m dunce v1.0.5
[1m[92m  Downloaded[0m bit-set v0.8.0
[1m[92m  Downloaded[0m dirs v6.0.0
[1m[92m  Downloaded[0m embedded-io v0.6.1
[1m[92m  Downloaded[0m equator v0.4.2
[1m[92m  Downloaded[0m equator-macro v0.4.2
[1m[92m  Downloaded[0m err_trail v0.11.1
[1m[92m  Downloaded[0m err_trail_macros v0.11.1
[1m[92m  Downloaded[0m fast_chemail v0.9.6
[1m[92m  Downloaded[0m float-cmp v0.9.0
[1m[92m  Downloaded[0m Inflector v0.11.4
[1m[92m  Downloaded[0m darling_macro v0.20.11
[1m[92m  Downloaded[0m dirs-sys v0.5.0
[1m[92m  Downloaded[0m embedded-io v0.4.0
[1m[92m  Downloaded[0m erased v0.1.2
[1m[92m  Downloaded[0m eventsource-stream v0.2.3
[1m[92m  Downloaded[0m ext-trait-proc_macros v1.0.1
[1m[92m  Downloaded[0m dashmap v5.5.3
[1m[92m  Downloaded[0m dashmap v6.2.1
[1m[92m  Downloaded[0m data-url v0.3.2
[1m[92m  Downloaded[0m addr2line v0.26.1
[1m[92m  Downloaded[0m dyn-clone v1.0.20
[1m[92m  Downloaded[0m event-listener-strategy v0.5.4
[1m[92m  Downloaded[0m extension-traits v1.0.1
[1m[92m  Downloaded[0m aws-runtime v1.8.1
[1m[92m  Downloaded[0m aws-smithy-types v1.6.1
[1m[92m  Downloaded[0m crc-catalog v2.5.0
[1m[92m  Downloaded[0m crypto-common v0.1.7
[1m[92m  Downloaded[0m digest v0.10.7
[1m[92m  Downloaded[0m elsa v1.11.2
[1m[92m  Downloaded[0m email-encoding v0.4.1
[1m[92m  Downloaded[0m fastbloom v0.17.0
[1m[92m  Downloaded[0m ff v0.13.1
[1m[92m  Downloaded[0m find-msvc-tools v0.1.9
[1m[92m  Downloaded[0m deranged v0.5.8
[1m[92m  Downloaded[0m derive_builder_core v0.20.2
[1m[92m  Downloaded[0m dotenvy v0.15.7
[1m[92m  Downloaded[0m fixedbitset v0.5.7
[1m[92m  Downloaded[0m foldhash v0.2.0
[1m[92m  Downloaded[0m arg_enum_proc_macro v0.3.4
[1m[92m  Downloaded[0m crlify v1.0.5
[1m[92m  Downloaded[0m enumset v1.1.13
[1m[92m  Downloaded[0m crossbeam v0.8.4
[1m[92m  Downloaded[0m cron v0.17.0
[1m[92m  Downloaded[0m crossbeam-deque v0.8.7
[1m[92m  Downloaded[0m event-listener v5.4.1
[1m[92m  Downloaded[0m fdeflate v0.3.7
[1m[92m  Downloaded[0m crc32fast v1.5.0
[1m[92m  Downloaded[0m eyre v0.6.12
[1m[92m  Downloaded[0m aws-sdk-sso v1.103.0
[1m[92m  Downloaded[0m azure_security_keyvault_secrets v1.0.0
[1m[92m  Downloaded[0m foldhash v0.1.5
[1m[92m  Downloaded[0m crypto-common v0.2.2
[1m[92m  Downloaded[0m async-recursion v1.1.1
[1m[92m  Downloaded[0m anstyle-parse v1.0.0
[1m[92m  Downloaded[0m derive-where v1.6.1
[1m[92m  Downloaded[0m digest v0.11.3
[1m[92m  Downloaded[0m email_address v0.2.9
[1m[92m  Downloaded[0m fixed_decimal v0.7.2
[1m[92m  Downloaded[0m allocator-api2 v0.2.21
[1m[92m  Downloaded[0m crossbeam-queue v0.3.13
[1m[92m  Downloaded[0m darling v0.20.11
[1m[92m  Downloaded[0m darling_core v0.23.0
[1m[92m  Downloaded[0m drain_filter_polyfill v0.1.3
[1m[92m  Downloaded[0m erased-serde v0.4.10
[1m[92m  Downloaded[0m crossbeam-epoch v0.9.20
[1m[92m  Downloaded[0m csv-core v0.1.13
[1m[92m  Downloaded[0m darling_core v0.21.3
[1m[92m  Downloaded[0m der-parser v10.0.0
[1m[92m  Downloaded[0m aes-gcm v0.10.3
[1m[92m  Downloaded[0m av1-grain v0.2.5
[1m[92m  Downloaded[0m der v0.7.10
[1m[92m  Downloaded[0m elliptic-curve v0.13.8
[1m[92m  Downloaded[0m flate2 v1.1.9
[1m[92m  Downloaded[0m fluent-uri v0.4.1
[1m[92m  Downloaded[0m flume v0.12.0
[1m[92m  Downloaded[0m memmap2 v0.8.0
[1m[92m  Downloaded[0m arrayvec v0.7.8
[1m[92m  Downloaded[0m aws-sdk-sts v1.108.0
[1m[92m  Downloaded[0m crypto-bigint v0.5.5
[1m[92m  Downloaded[0m darling_core v0.20.11
[1m[92m  Downloaded[0m derive_more v2.1.1
[1m[92m  Downloaded[0m fancy-regex v0.18.0
[1m[92m  Downloaded[0m flume v0.11.1
[1m[92m  Downloaded[0m aws-config v1.9.0
[1m[92m  Downloaded[0m exr v1.74.2
[1m[92m  Downloaded[0m jobserver v0.1.35
[1m[92m  Downloaded[0m never-say-never v6.6.666
[1m[92m  Downloaded[0m derive_builder v0.20.2
[1m[92m  Downloaded[0m aws-smithy-xml v0.61.1
[1m[92m  Downloaded[0m paste v1.0.15
[1m[92m  Downloaded[0m arc-swap v1.9.2
[1m[92m  Downloaded[0m base16ct v0.2.0
[1m[92m  Downloaded[0m darling v0.23.0
[1m[92m  Downloaded[0m axum-core v0.5.6
[1m[92m  Downloaded[0m crossbeam-channel v0.5.16
[1m[92m  Downloaded[0m num v0.4.3
[1m[92m  Downloaded[0m password-hash v0.6.1
[1m[92m  Downloaded[0m ecdsa v0.16.9
[1m[92m  Downloaded[0m sqlx-core v0.8.6
[1m[92m  Downloaded[0m deunicode v1.6.2
[1m[92m  Downloaded[0m aligned-vec v0.6.4
[1m[92m  Downloaded[0m png v0.17.16
[1m[92m  Downloaded[0m async-graphql-derive v7.2.1
[1m[92m  Downloaded[0m manyhow-macros v0.11.4
[1m[92m  Downloaded[0m pest_derive v2.8.7
[1m[92m  Downloaded[0m rayon v1.12.0
[1m[92m  Downloaded[0m leptos_hot_reload v0.8.6
[1m[92m  Downloaded[0m tokio-native-tls v0.3.1
[1m[92m  Downloaded[0m jsonschema-regex v0.46.10
[1m[92m  Downloaded[0m av-scenechange v0.14.1
[1m[92m  Downloaded[0m rustc-demangle v0.1.28
[1m[92m  Downloaded[0m raw-cpuid v11.6.0
[1m[92m  Downloaded[0m aligned v0.4.3
[1m[92m  Downloaded[0m async-dropper-simple v0.2.6
[1m[92m  Downloaded[0m sha1_smol v1.0.1
[1m[92m  Downloaded[0m base64-simd v0.8.0
[1m[92m  Downloaded[0m ndarray v0.16.1
[1m[92m  Downloaded[0m crc-fast v1.10.0
[1m[92m  Downloaded[0m nougat-proc_macros v0.2.4
[1m[92m  Downloaded[0m zstd v0.13.3
[1m[92m  Downloaded[0m serde_qs v0.15.0
[1m[92m  Downloaded[0m aws-credential-types v1.3.0
[1m[92m  Downloaded[0m object v0.39.1
[1m[92m  Downloaded[0m zune-inflate v0.2.54
[1m[92m  Downloaded[0m loop9 v0.1.5
[1m[92m  Downloaded[0m async-dropper-derive v0.3.1
[1m[92m  Downloaded[0m tokio-rustls v0.26.4
[1m[92m  Downloaded[0m typespec_client_core v1.1.0
[1m[92m  Downloaded[0m utf8-width v0.1.9
[1m[92m  Downloaded[0m winnow v0.7.15
[1m[92m  Downloaded[0m aws-sdk-bedrockruntime v1.136.0
[1m[92m  Downloaded[0m ouroboros v0.18.5
[1m[92m  Downloaded[0m pest v2.8.7
[1m[92m  Downloaded[0m manyhow-macros v0.10.4
[1m[92m  Downloaded[0m adler2 v2.0.1
[1m[92m  Downloaded[0m ptr_meta_derive v0.1.4
[1m[92m  Downloaded[0m moxcms v0.8.1
[1m[92m  Downloaded[0m rustls-platform-verifier v0.7.0
[1m[92m  Downloaded[0m syn_derive v0.2.0
[1m[92m  Downloaded[0m urlencoding v2.1.3
[1m[92m  Downloaded[0m wasmtime-internal-jit-icache-coherence v46.0.1
[1m[92m  Downloaded[0m asn1-rs-impl v0.2.0
[1m[92m  Downloaded[0m proc-macro-error-attr2 v2.0.0
[1m[92m  Downloaded[0m spki v0.7.3
[1m[92m  Downloaded[0m rust-embed-utils v8.12.0
[1m[92m  Downloaded[0m subtle v2.6.1
[1m[92m  Downloaded[0m ascii_utils v0.9.3
[1m[92m  Downloaded[0m backon v1.6.0
[1m[92m  Downloaded[0m proc-macro-error v1.0.4
[1m[92m  Downloaded[0m sha1 v0.10.7
[1m[92m  Downloaded[0m ureq-proto v0.6.0
[1m[92m  Downloaded[0m aws-smithy-runtime-api-macros v1.1.0
[1m[92m  Downloaded[0m wyz v0.5.1
[1m[92m  Downloaded[0m server_fn_macro v0.8.10
[1m[92m  Downloaded[0m proc-macro-error-attr v1.0.4
[1m[92m  Downloaded[0m serde_path_to_error v0.1.20
[1m[92m  Downloaded[0m tinyvec_macros v0.1.1
[1m[92m  Downloaded[0m strum_macros v0.27.2
[1m[92m  Downloaded[0m aws-smithy-eventstream v0.61.1
[1m[92m  Downloaded[0m protoc-bin-vendored-linux-x86_64 v3.2.0
[1m[92m  Downloaded[0m serde_derive_internals v0.29.1
[1m[92m  Downloaded[0m async-stream-impl v0.3.6
[1m[92m  Downloaded[0m wasmtime-internal-core v46.0.1
[1m[92m  Downloaded[0m vcpkg v0.2.15
[1m[92m  Downloaded[0m num-traits v0.2.19
[1m[92m  Downloaded[0m sha1 v0.11.0
[1m[92m  Downloaded[0m combine v4.6.7
[1m[92m  Downloaded[0m pkcs8 v0.10.2
[1m[92m  Downloaded[0m y4m v0.8.0
[1m[92m  Downloaded[0m attribute-derive v0.9.2
[1m[92m  Downloaded[0m aws-smithy-http v0.64.0
[1m[92m  Downloaded[0m cipher v0.4.4
[1m[92m  Downloaded[0m blake3 v1.8.5
[1m[92m  Downloaded[0m attribute-derive-macro v0.9.2
[1m[92m  Downloaded[0m compression-core v0.4.32
[1m[92m  Downloaded[0m pest_meta v2.8.7
[1m[92m  Downloaded[0m anstyle-query v1.1.5
[1m[92m  Downloaded[0m aws-smithy-json v0.63.0
[1m[92m  Downloaded[0m rkyv_derive v0.7.46
[1m[92m  Downloaded[0m compio-macros v0.1.2
[1m[92m  Downloaded[0m as-any v0.3.2
[1m[92m  Downloaded[0m fs_extra v1.3.0
[1m[92m  Downloaded[0m core_maths v0.1.1
[1m[92m  Downloaded[0m colorchoice v1.0.5
[1m[92m  Downloaded[0m futures v0.3.32
[1m[92m  Downloaded[0m google-cloud-type v1.6.0
[1m[92m  Downloaded[0m aws-sigv4 v1.5.1
[1m[92m  Downloaded[0m aws-smithy-runtime v1.12.0
[1m[92m  Downloaded[0m http-auth v0.1.10
[1m[92m  Downloaded[0m icu_pattern v0.4.2
[1m[92m  Downloaded[0m leptos_query v0.5.3
[1m[92m  Downloaded[0m macro_rules_attribute v0.1.3
[1m[92m  Downloaded[0m syn_derive v0.1.8
[1m[92m  Downloaded[0m jwt v0.16.0
[1m[92m  Downloaded[0m inout v0.1.4
[1m[92m  Downloaded[0m argon2 v0.5.3
[1m[92m  Downloaded[0m aws-smithy-query v0.61.1
[1m[92m  Downloaded[0m async-graphql-parser v7.2.1
[1m[92m  Downloaded[0m macro_rules_attribute-proc_macro v0.1.3
[1m[92m  Downloaded[0m toml v1.1.2+spec-1.1.0
[1m[92m  Downloaded[0m iggy_binary_protocol v0.10.0
[1m[92m  Downloaded[0m typespec_macros v1.0.0
[1m[92m  Downloaded[0m async-once-cell v0.5.4
[1m[92m  Downloaded[0m anstyle v1.0.14
[1m[92m  Downloaded[0m async-graphql-value v7.2.1
[1m[92m  Downloaded[0m cranelift-codegen-shared v0.133.1
[1m[92m  Downloaded[0m json5 v1.3.1
[1m[92m  Downloaded[0m atoi v2.0.0
[1m[92m  Downloaded[0m avif-serialize v0.8.9
[1m[92m  Downloaded[0m miniz_oxide v0.8.9
[1m[92m  Downloaded[0m kstring v2.0.2
[1m[92m  Downloaded[0m num-iter v0.1.46
[1m[92m  Downloaded[0m base64ct v1.8.3
[1m[92m  Downloaded[0m sea-bae v0.2.1
[1m[92m  Downloaded[0m option-ext v0.2.0
[1m[92m  Downloaded[0m polonius-the-crab v0.2.1
[1m[92m  Downloaded[0m const-str v1.1.0
[1m[92m  Downloaded[0m nom v7.1.3
[1m[92m  Downloaded[0m cmake v0.1.58
[1m[92m  Downloaded[0m num-modular v0.6.4
[1m[92m  Downloaded[0m funty v2.0.0
[1m[92m  Downloaded[0m heapless v0.7.17
[1m[92m  Downloaded[0m leptos_config v0.8.10
[1m[92m  Downloaded[0m proc-macro-utils v0.10.0
[1m[92m  Downloaded[0m protoc-bin-vendored-linux-ppcle_64 v3.2.0
[1m[92m  Downloaded[0m rand_chacha v0.3.1
[1m[92m  Downloaded[0m iggy_common v0.10.0
[1m[92m  Downloaded[0m inventory v0.3.24
[1m[92m  Downloaded[0m leptos_reactive v0.6.15
[1m[92m  Downloaded[0m moka v0.12.15
[1m[92m  Downloaded[0m or_poisoned v0.1.0
[1m[92m  Downloaded[0m cfg_aliases v0.2.1
[1m[92m  Downloaded[0m lending-iterator v0.1.7
[1m[92m  Downloaded[0m leptos-struct-table v0.19.0
[1m[92m  Downloaded[0m typenum v1.20.1
[1m[92m  Downloaded[0m protoc-bin-vendored-macos-x86_64 v3.2.0
[1m[92m  Downloaded[0m typespec v1.1.0
[1m[92m  Downloaded[0m webpki-roots v0.26.11
[1m[92m  Downloaded[0m proc-macro-crate v3.5.0
[1m[92m  Downloaded[0m byte-unit v5.2.5
[1m[92m  Downloaded[0m leptos-struct-table-macro v0.15.0
[1m[92m  Downloaded[0m parse-zoneinfo v0.3.1
[1m[92m  Downloaded[0m xxhash-rust v0.8.16
[1m[92m  Downloaded[0m pulldown-cmark-to-cmark v22.0.0
[1m[92m  Downloaded[0m utf16_iter v1.0.5
[1m[92m  Downloaded[0m bytemuck v1.25.1
[1m[92m  Downloaded[0m serde-aux v4.7.0
[1m[92m  Downloaded[0m leptos_macro v0.6.15
[1m[92m  Downloaded[0m primeorder v0.13.6
[1m[92m  Downloaded[0m pkcs1 v0.7.5
[1m[92m  Downloaded[0m sha2 v0.10.9
[1m[92m  Downloaded[0m proc-macro-utils v0.8.0
[1m[92m  Downloaded[0m blake2 v0.10.6
[1m[92m  Downloaded[0m gloo-utils v0.2.0
[1m[92m  Downloaded[0m manyhow v0.10.4
[1m[92m  Downloaded[0m wast v253.0.0
[1m[92m  Downloaded[0m ulid v1.2.1
[1m[92m  Downloaded[0m sea-orm v1.1.20
[1m[92m  Downloaded[0m compio-tls v0.9.1
[1m[92m  Downloaded[0m cranelift-codegen-meta v0.133.1
[1m[92m  Downloaded[0m google-cloud-auth v1.14.0
[1m[92m  Downloaded[0m stringprep v0.1.5
[1m[92m  Downloaded[0m utf8-zero v0.8.1
[1m[92m  Downloaded[0m reactive_stores v0.4.3
[1m[92m  Downloaded[0m time-macros v0.2.31
[1m[92m  Downloaded[0m rstml v0.11.2
[1m[92m  Downloaded[0m tiny-skia v0.11.4
[1m[92m  Downloaded[0m tungstenite v0.29.0
[1m[92m  Downloaded[0m unsafe-libyaml-norway v0.2.15
[1m[92m  Downloaded[0m icu_list v2.2.0
[1m[92m  Downloaded[0m reactive_stores_macro v0.4.3
[1m[92m  Downloaded[0m num-integer v0.1.46
[1m[92m  Downloaded[0m headers-core v0.3.0
[1m[92m  Downloaded[0m strum v0.26.3
[1m[92m  Downloaded[0m http-body v0.4.6
[1m[92m  Downloaded[0m is_terminal_polyfill v1.70.2
[1m[92m  Downloaded[0m regex v1.13.0
[1m[92m  Downloaded[0m sec1 v0.7.3
[1m[92m  Downloaded[0m constant_time_eq v0.4.2
[1m[92m  Downloaded[0m iggy v0.10.0
[1m[92m  Downloaded[0m sea-orm-macros v1.1.20
[1m[92m  Downloaded[0m typed-builder v0.23.2
[1m[92m  Downloaded[0m icu_plurals v2.2.0
[1m[92m  Downloaded[0m wasm-bindgen v0.2.126
[1m[92m  Downloaded[0m zstd-safe v7.2.4
[1m[92m  Downloaded[0m lru v0.16.4
[1m[92m  Downloaded[0m hydration_context v0.3.1
[1m[92m  Downloaded[0m unsafe-libyaml v0.2.11
[1m[92m  Downloaded[0m fraction v0.15.4
[1m[92m  Downloaded[0m unicase v2.9.0
[1m[92m  Downloaded[0m wasmi_collections v1.1.0
[1m[92m  Downloaded[0m v_frame v0.3.9
[1m[92m  Downloaded[0m fontdb v0.15.0
[1m[92m  Downloaded[0m ordered-float v2.10.1
[1m[92m  Downloaded[0m weezl v0.1.12
[1m[92m  Downloaded[0m rustc-hash v2.1.3
[1m[92m  Downloaded[0m leptos-chartistry v0.2.3
[1m[92m  Downloaded[0m opentelemetry-semantic-conventions v0.32.1
[1m[92m  Downloaded[0m prettyplease v0.2.37
[1m[92m  Downloaded[0m rand v0.10.2
[1m[92m  Downloaded[0m unicode-bidi v0.3.18
[1m[92m  Downloaded[0m whoami v1.6.1
[1m[92m  Downloaded[0m aws-sdk-s3 v1.138.0
[1m[92m  Downloaded[0m google-cloud-secretmanager-v1 v1.11.0
[1m[92m  Downloaded[0m leptos-next-metadata v1.5.0
[1m[92m  Downloaded[0m regalloc2 v0.15.1
[1m[92m  Downloaded[0m icu_calendar v2.2.1
[1m[92m  Downloaded[0m zopfli v0.8.3
[1m[92m  Downloaded[0m send_wrapper v0.6.0
[1m[92m  Downloaded[0m toml_writer v1.1.1+spec-1.1.0
[1m[92m  Downloaded[0m cranelift-entity v0.133.1
[1m[92m  Downloaded[0m icu_time v2.2.0
[1m[92m  Downloaded[0m rustc_version v0.4.1
[1m[92m  Downloaded[0m sea-schema-derive v0.3.0
[1m[92m  Downloaded[0m rmcp v1.8.0
[1m[92m  Downloaded[0m bytes-utils v0.1.4
[1m[92m  Downloaded[0m secrecy v0.10.3
[1m[92m  Downloaded[0m tonic-build v0.14.6
[1m[92m  Downloaded[0m sha2 v0.11.0
[1m[92m  Downloaded[0m icu_casemap v2.2.0
[1m[92m  Downloaded[0m sqlx-macros-core v0.8.6
[1m[92m  Downloaded[0m foreign-types-shared v0.1.1
[1m[92m  Downloaded[0m shellexpand v3.1.2
[1m[92m  Downloaded[0m hkdf v0.12.4
[1m[92m  Downloaded[0m compio-buf v0.8.3
[1m[92m  Downloaded[0m google-cloud-rpc v1.6.0
[1m[92m  Downloaded[0m icu_datetime v2.2.0
[1m[92m  Downloaded[0m oid-registry v0.8.1
[1m[92m  Downloaded[0m hashbrown v0.16.1
[1m[92m  Downloaded[0m wasm-bindgen-futures v0.4.76
[1m[92m  Downloaded[0m glob v0.3.3
[1m[92m  Downloaded[0m ghash v0.5.1
[1m[92m  Downloaded[0m num-bigint v0.4.8
[1m[92m  Downloaded[0m winnow v1.0.3
[1m[92m  Downloaded[0m clap_derive v4.6.1
[1m[92m  Downloaded[0m compio-log v0.1.0
[1m[92m  Downloaded[0m chrono v0.4.45
[1m[92m  Downloaded[0m cmov v0.5.4
[1m[92m  Downloaded[0m encoding_rs v0.8.35
[1m[92m  Downloaded[0m leptos_dom v0.6.15
[1m[92m  Downloaded[0m next_tuple v0.1.0
[1m[92m  Downloaded[0m pest_generator v2.8.7
[1m[92m  Downloaded[0m termcolor v1.4.1
[1m[92m  Downloaded[0m wasmtime-internal-jit-debug v46.0.1
[1m[92m  Downloaded[0m unicode-bidi-mirroring v0.1.0
[1m[92m  Downloaded[0m leptos_macro v0.8.17
[1m[92m  Downloaded[0m libsqlite3-sys v0.30.1
[1m[92m  Downloaded[0m openssl-macros v0.1.1
[1m[92m  Downloaded[0m no-std-compat v0.4.1
[1m[92m  Downloaded[0m tiny-keccak v2.0.2
[1m[92m  Downloaded[0m validator_derive v0.20.1
[1m[92m  Downloaded[0m proc-macro-error2 v2.0.1
[1m[92m  Downloaded[0m hyper-rustls v0.27.9
[1m[92m  Downloaded[0m num_cpus v1.17.0
[1m[92m  Downloaded[0m borrow-or-share v0.2.4
[1m[92m  Downloaded[0m google-cloud-iam-v1 v1.11.0
[1m[92m  Downloaded[0m gloo-storage v0.4.0
[1m[92m  Downloaded[0m bitstream-io v4.10.0
[1m[92m  Downloaded[0m portable-atomic v1.13.1
[1m[92m  Downloaded[0m sea-query-binder v0.7.0
[1m[92m  Downloaded[0m const-random-macro v0.1.16
[1m[92m  Downloaded[0m quote-use-macros v0.8.4
[1m[92m  Downloaded[0m pathdiff v0.2.3
[1m[92m  Downloaded[0m usvg-tree v0.36.0
[1m[92m  Downloaded[0m compio-quic v0.7.2
[1m[92m  Downloaded[0m hash32 v0.2.1
[1m[92m  Downloaded[0m icu_casemap_data v2.2.0
[1m[92m  Downloaded[0m os_pipe v1.2.3
[1m[92m  Downloaded[0m cranelift-assembler-x64-meta v0.133.1
[1m[92m  Downloaded[0m io-uring v0.7.13
[1m[92m  Downloaded[0m md-5 v0.11.0
[1m[92m  Downloaded[0m siphasher v1.0.3
[1m[92m  Downloaded[0m httpdate v1.0.3
[1m[92m  Downloaded[0m liquid-derive v0.26.10
[1m[92m  Downloaded[0m phf_codegen v0.11.3
[1m[92m  Downloaded[0m rmcp v2.2.0
[1m[92m  Downloaded[0m ciborium-io v0.2.2
[1m[92m  Downloaded[0m ouroboros_macro v0.18.5
[1m[92m  Downloaded[0m wasm-streams v0.4.2
[1m[92m  Downloaded[0m reactive_graph v0.2.14
[1m[92m  Downloaded[0m rctree v0.5.0
[1m[92m  Downloaded[0m proc-macro-error3 v3.0.2
[1m[92m  Downloaded[0m heck v0.5.0
[1m[92m  Downloaded[0m serial_test v3.5.0
[1m[92m  Downloaded[0m lending-iterator-proc_macros v0.1.7
[1m[92m  Downloaded[0m ptr_meta v0.1.4
[1m[92m  Downloaded[0m redis v1.3.0
[1m[92m  Downloaded[0m iana-time-zone v0.1.65
[1m[92m  Downloaded[0m num-bigint-dig v0.8.6
[1m[92m  Downloaded[0m serde_qs v0.12.0
[1m[92m  Downloaded[0m rayon-core v1.13.0
[1m[92m  Downloaded[0m base16 v0.2.1
[1m[92m  Downloaded[0m pastey v0.2.3
[1m[92m  Downloaded[0m wasm-encoder v0.251.0
[1m[92m  Downloaded[0m simdutf8 v0.1.5
[1m[92m  Downloaded[0m serde-wasm-bindgen v0.6.5
[1m[92m  Downloaded[0m server_fn_macro_default v0.6.15
[1m[92m  Downloaded[0m icu v2.2.0
[1m[92m  Downloaded[0m md-5 v0.10.6
[1m[92m  Downloaded[0m quote-use v0.8.4
[1m[92m  Downloaded[0m rustversion v1.0.23
[1m[92m  Downloaded[0m tachys v0.2.18
[1m[92m  Downloaded[0m bs58 v0.5.1
[1m[92m  Downloaded[0m color_quant v1.1.0
[1m[92m  Downloaded[0m twox-hash v2.1.2
[1m[92m  Downloaded[0m png v0.18.1
[1m[92m  Downloaded[0m self_cell v1.2.2
[1m[92m  Downloaded[0m calendrical_calculations v0.2.4
[1m[92m  Downloaded[0m rustls-webpki v0.103.13
[1m[92m  Downloaded[0m block-buffer v0.10.4
[1m[92m  Downloaded[0m spin v0.10.0
[1m[92m  Downloaded[0m tokio-tungstenite v0.29.0
[1m[92m  Downloaded[0m typed-builder v0.18.2
[1m[92m  Downloaded[0m unicode-properties v0.1.4
[1m[92m  Downloaded[0m wasmtime-internal-cranelift v46.0.1
[1m[92m  Downloaded[0m ref-cast v1.0.25
[1m[92m  Downloaded[0m tinyvec v1.12.0
[1m[92m  Downloaded[0m unic-langid v0.9.6
[1m[92m  Downloaded[0m svgtypes v0.12.0
[1m[92m  Downloaded[0m wasm-bindgen-macro-support v0.2.126
[1m[92m  Downloaded[0m roxmltree v0.18.1
[1m[92m  Downloaded[0m v_escape-base v0.1.0
[1m[92m  Downloaded[0m const-oid v0.9.6
[1m[92m  Downloaded[0m sqlx-postgres v0.8.6
[1m[92m  Downloaded[0m google-cloud-api v1.6.0
[1m[92m  Downloaded[0m async-graphql v7.2.1
[1m[92m  Downloaded[0m gloo-timers v0.4.0
[1m[92m  Downloaded[0m tar v0.4.46
[1m[92m  Downloaded[0m ureq v3.3.0
[1m[92m  Downloaded[0m yansi v1.0.1
[1m[92m  Downloaded[0m bitvec v1.1.1
[1m[92m  Downloaded[0m thin-vec v0.2.18
[1m[92m  Downloaded[0m wasm-streams v0.5.0
[1m[92m  Downloaded[0m leptos v0.8.20
[1m[92m  Downloaded[0m aead v0.5.2
[1m[92m  Downloaded[0m concurrent-queue v2.5.0
[1m[92m  Downloaded[0m autocfg v1.5.1
[1m[92m  Downloaded[0m any_spawner v0.3.0
[1m[92m  Downloaded[0m icu_provider_registry v2.2.0
[1m[92m  Downloaded[0m manyhow v0.11.4
[1m[92m  Downloaded[0m ixdtf v0.6.5
[1m[92m  Downloaded[0m jsonwebtoken v10.4.0
[1m[92m  Downloaded[0m chacha20 v0.10.1
[1m[92m  Downloaded[0m cranelift-isle v0.133.1
[1m[92m  Downloaded[0m phf_macros v0.11.3
[1m[92m  Downloaded[0m nougat v0.2.4
[1m[92m  Downloaded[0m bytecheck_derive v0.6.12
[1m[92m  Downloaded[0m gloo-utils v0.3.0
[1m[92m  Downloaded[0m base64 v0.13.1
[1m[92m  Downloaded[0m wasm-encoder v0.253.0
[1m[92m  Downloaded[0m group v0.13.0
[1m[92m  Downloaded[0m icu_codepointtrie_builder v0.6.0
[1m[92m  Downloaded[0m pem-rfc7468 v0.7.0
[1m[92m  Downloaded[0m serde_urlencoded v0.7.1
[1m[92m  Downloaded[0m gloo-net v0.6.0
[1m[92m  Downloaded[0m spin v0.5.2
[1m[92m  Downloaded[0m lebe v0.5.3
[1m[92m  Downloaded[0m fastrand v2.4.1
[1m[92m  Downloaded[0m azure_core_macros v1.0.0
[1m[92m  Downloaded[0m core_affinity v0.8.3
[1m[92m  Downloaded[0m cpufeatures v0.2.17
[1m[92m  Downloaded[0m openssl-probe v0.2.1
[1m[92m  Downloaded[0m strsim v0.11.1
[1m[92m  Downloaded[0m borsh v1.7.0
[1m[92m  Downloaded[0m rustls-native-certs v0.8.4
[1m[92m  Downloaded[0m ahash v0.8.12
[1m[92m  Downloaded[0m aws-smithy-runtime-api v1.13.0
[1m[92m  Downloaded[0m icu_experimental v0.5.0
[1m[92m  Downloaded[0m rig-vertexai v0.40.0
[1m[92m  Downloaded[0m gloo-timers v0.3.0
[1m[92m  Downloaded[0m phf_shared v0.11.3
[1m[92m  Downloaded[0m aliasable v0.1.3
[1m[92m  Downloaded[0m schemars v1.2.1
[1m[92m  Downloaded[0m fontconfig-parser v0.5.8
[1m[92m  Downloaded[0m async-io v2.6.0
[1m[92m  Downloaded[0m async-stream v0.3.6
[1m[92m  Downloaded[0m leptos_router v0.8.14
[1m[92m  Downloaded[0m leptos_meta v0.8.6
[1m[92m  Downloaded[0m asn1-rs v0.7.2
[1m[92m  Downloaded[0m aws-smithy-checksums v0.65.0
[1m[92m  Downloaded[0m gimli v0.33.0
[1m[92m  Downloaded[0m interpolator v0.5.0
[1m[92m  Downloaded[0m liquid v0.26.11
[1m[92m  Downloaded[0m indoc v2.0.7
[1m[92m  Downloaded[0m protoc-bin-vendored v3.2.0
[1m[92m  Downloaded[0m seize v0.5.1
[1m[92m  Downloaded[0m filetime v0.2.29
[1m[92m  Downloaded[0m human-repr v1.1.0
[1m[92m  Downloaded[0m pulley-interpreter v46.0.1
[1m[92m  Downloaded[0m enumset_derive v0.15.0
[1m[92m  Downloaded[0m serde_spanned v0.6.9
[1m[92m  Downloaded[0m pad-adapter v0.1.1
[1m[92m  Downloaded[0m quick-error v2.0.1
[1m[92m  Downloaded[0m scoped-tls v1.0.1
[1m[92m  Downloaded[0m uuid-simd v0.8.0
[1m[92m  Downloaded[0m attribute-derive-macro v0.10.5
[1m[92m  Downloaded[0m aws-lc-rs v1.17.1
[1m[92m  Downloaded[0m memoffset v0.9.1
[1m[92m  Downloaded[0m nom v8.0.0
[1m[92m  Downloaded[0m password-hash v0.5.0
[1m[92m  Downloaded[0m qoi v0.4.1
[1m[92m  Downloaded[0m static_assertions_next v1.1.2
[1m[92m  Downloaded[0m chrono-tz-build v0.3.0
[1m[92m  Downloaded[0m leptos-use v0.18.3
[1m[92m  Downloaded[0m leptos-use v0.19.0
[1m[92m  Downloaded[0m leptos_server v0.6.15
[1m[92m  Downloaded[0m toml_write v0.1.2
[1m[92m  Downloaded[0m imgref v1.12.2
[1m[92m  Downloaded[0m simd-adler32 v0.3.9
[1m[92m  Downloaded[0m siphasher v0.3.11
[1m[92m  Downloaded[0m static_assertions v1.1.0
[1m[92m  Downloaded[0m tap v1.0.1
[1m[92m  Downloaded[0m ahash v0.7.8
[1m[92m  Downloaded[0m aws-smithy-http-client v1.2.0
[1m[92m  Downloaded[0m google-cloud-lro v1.8.0
[1m[92m  Downloaded[0m data-encoding v2.11.0
[1m[92m  Downloaded[0m imagesize v0.12.0
[1m[92m  Downloaded[0m compio-driver v0.11.4
[1m[92m  Downloaded[0m darling v0.21.3
[1m[92m  Downloaded[0m leptos_config v0.6.15
[1m[92m  Downloaded[0m unicode-ccc v0.1.2
[1m[92m  Downloaded[0m untrusted v0.7.1
[1m[92m  Downloaded[0m config v0.15.25
[1m[92m  Downloaded[0m pxfm v0.1.30
[1m[92m  Downloaded[0m rig-derive v0.40.0
[1m[92m  Downloaded[0m usvg-text-layout v0.36.0
[1m[92m  Downloaded[0m async-compression v0.4.42
[1m[92m  Downloaded[0m ordered-float v4.6.0
[1m[92m  Downloaded[0m unicode-segmentation v1.13.3
[1m[92m  Downloaded[0m derive_more-impl v2.1.1
[1m[92m  Downloaded[0m usvg-parser v0.36.0
[1m[92m  Downloaded[0m clap_lex v1.1.0
[1m[92m  Downloaded[0m strum_macros v0.28.0
[1m[92m  Downloaded[0m utf-8 v0.7.6
[1m[92m  Downloaded[0m wasmi_ir v1.1.0
[1m[92m  Downloaded[0m ciborium v0.2.2
[1m[92m  Downloaded[0m synchrony v0.1.8
[1m[92m  Downloaded[0m ttf-parser v0.20.0
[1m[92m  Downloaded[0m axum-extra v0.12.6
[1m[92m  Downloaded[0m leptos_i18n_build v0.6.2
[1m[92m  Downloaded[0m cranelift-bitset v0.133.1
[1m[92m  Downloaded[0m ctutils v0.4.2
[1m[92m  Downloaded[0m micromap v0.3.0
[1m[92m  Downloaded[0m fax v0.2.7
[1m[92m  Downloaded[0m async-broadcast v0.7.2
[1m[92m  Downloaded[0m profiling-procmacros v1.0.18
[1m[92m  Downloaded[0m config v0.14.1
[1m[92m  Downloaded[0m leptos_i18n v0.6.2
[1m[92m  Downloaded[0m rfc6979 v0.4.0
[1m[92m  Downloaded[0m google-cloud-gax-internal v0.7.15
[1m[92m  Downloaded[0m hmac v0.12.1
[1m[92m  Downloaded[0m liquid-core v0.26.11
[1m[92m  Downloaded[0m ordered-float v5.3.0
[1m[92m  Downloaded[0m tiny-skia-path v0.11.4
[1m[92m  Downloaded[0m tungstenite v0.28.0
[1m[92m  Downloaded[0m oco_ref v0.1.1
[1m[92m  Downloaded[0m bit-vec v0.8.0
[1m[92m  Downloaded[0m rusticata-macros v4.1.0
[1m[92m  Downloaded[0m unicode-normalization v0.1.25
[1m[92m  Downloaded[0m leb128fmt v0.1.0
[1m[92m  Downloaded[0m olpc-cjson v0.1.4
[1m[92m  Downloaded[0m reqwest v0.12.28
[1m[92m  Downloaded[0m new_debug_unreachable v1.0.6
[1m[92m  Downloaded[0m num-cmp v0.1.0
[1m[92m  Downloaded[0m openssl v0.10.81
[1m[92m  Downloaded[0m leptos_axum v0.8.10
[1m[92m  Downloaded[0m hashlink v0.10.0
[1m[92m  Downloaded[0m ctr v0.9.2
[1m[92m  Downloaded[0m fontdue v0.8.0
[1m[92m  Downloaded[0m reqwest-tracing v0.7.1
[1m[92m  Downloaded[0m smartstring v1.0.1
[1m[92m  Downloaded[0m ext-trait v1.0.1
[1m[92m  Downloaded[0m serde_spanned v1.1.1
[1m[92m  Downloaded[0m serde_with v3.21.0
[1m[92m  Downloaded[0m leptos_i18n_macro v0.6.2
[1m[92m  Downloaded[0m anymap2 v0.13.0
[1m[92m  Downloaded[0m arbitrary v1.4.2
[1m[92m  Downloaded[0m mime_guess v2.0.5
[1m[92m  Downloaded[0m tiff v0.11.3
[1m[92m  Downloaded[0m unicode-script v0.5.8
[1m[92m  Downloaded[0m aws-lc-sys v0.42.0
[1m[92m  Downloaded[0m linux-raw-sys v0.12.1
[1m[92m  Downloaded[0m rust-embed-impl v8.12.0
[1m[92m  Downloaded[0m oci-distribution v0.11.0
[1m[92m  Downloaded[0m resvg v0.36.0
[1m[92m  Downloaded[0m time-core v0.1.9
[1m[92m  Downloaded[0m byteorder-lite v0.1.0
[1m[92m  Downloaded[0m icu_calendar_data v2.2.0
[1m[92m  Downloaded[0m multimap v0.10.1
[1m[92m  Downloaded[0m aws-smithy-schema v0.2.0
[1m[92m  Downloaded[0m compio-net v0.11.1
[1m[92m  Downloaded[0m heck v0.4.1
[1m[92m  Downloaded[0m leptos_i18n_parser v0.6.2
[1m[92m  Downloaded[0m rhai v1.25.1
[1m[92m  Downloaded[0m cookie v0.18.1
[1m[92m  Downloaded[0m hmac v0.13.0
[1m[92m  Downloaded[0m utf8parse v0.2.2
[1m[92m  Downloaded[0m icu_time_data v2.2.0
[1m[92m  Downloaded[0m const_format v0.2.36
[1m[92m  Downloaded[0m cranelift-native v0.133.1
[1m[92m  Downloaded[0m google-cloud-aiplatform-v1 v1.13.0
[1m[92m  Downloaded[0m p256 v0.13.2
[1m[92m  Downloaded[0m icu_plurals_data v2.2.0
[1m[92m  Downloaded[0m pulldown-cmark v0.13.4
[1m[92m  Downloaded[0m rhai_codegen v3.2.0
[1m[92m  Downloaded[0m slug v0.1.6
[1m[92m  Downloaded[0m axum v0.8.9
[1m[92m  Downloaded[0m cc v1.2.67
[1m[92m  Downloaded[0m leptos_integration_utils v0.8.8
[1m[92m  Downloaded[0m quinn v0.11.11
[1m[92m  Downloaded[0m bon-macros v3.9.3
[1m[92m  Downloaded[0m sqlx-macros v0.8.6
[1m[92m  Downloaded[0m universal-hash v0.5.1
[1m[92m  Downloaded[0m kurbo v0.9.5
[1m[92m  Downloaded[0m leptos v0.6.15
[1m[92m  Downloaded[0m phf v0.11.3
[1m[92m  Downloaded[0m crc v3.4.0
[1m[92m  Downloaded[0m either_of v0.1.9
[1m[92m  Downloaded[0m image v0.25.10
[1m[92m  Downloaded[0m num-conv v0.2.2
[1m[92m  Downloaded[0m parking v2.2.1
[1m[92m  Downloaded[0m ident_case v1.0.1
[1m[92m  Downloaded[0m radium v0.7.0
[1m[92m  Downloaded[0m reqwest-retry v0.9.1
[1m[92m  Downloaded[0m wasmtime-internal-component-macro v46.0.1
[1m[92m  Downloaded[0m zune-jpeg v0.5.15
[1m[92m  Downloaded[0m chrono-tz v0.9.0
[1m[92m  Downloaded[0m cranelift-codegen v0.133.1
[1m[92m  Downloaded[0m cranelift-control v0.133.1
[1m[92m  Downloaded[0m lru-slab v0.1.2
[1m[92m  Downloaded[0m rcgen v0.14.8
[1m[92m  Downloaded[0m sea-schema v0.16.2
[1m[92m  Downloaded[0m derive_builder_macro v0.20.2
[1m[92m  Downloaded[0m powerfmt v0.2.0
[1m[92m  Downloaded[0m leptos-next-metadata-macros v1.5.0
[1m[92m  Downloaded[0m pgvector v0.4.2
[1m[92m  Downloaded[0m icu_decimal_data v2.2.0
[1m[92m  Downloaded[0m rand_pcg v0.10.2
[1m[92m  Downloaded[0m regex-lite v0.1.9
[1m[92m  Downloaded[0m handlebars v6.4.3
[1m[92m  Downloaded[0m tempfile v3.27.0
[1m[92m  Downloaded[0m typed-builder-macro v0.18.2
[1m[92m  Downloaded[0m ulid v2.0.1
[1m[92m  Downloaded[0m no_std_io2 v0.9.4
[1m[92m  Downloaded[0m toml v0.8.23
[1m[92m  Downloaded[0m rkyv v0.7.46
[1m[92m  Downloaded[0m server_fn_macro_default v0.8.5
[1m[92m  Downloaded[0m unicode-xid v0.2.6
[1m[92m  Downloaded[0m ciborium-ll v0.2.2
[1m[92m  Downloaded[0m crunchy v0.2.4
[1m[92m  Downloaded[0m simple_asn1 v0.6.4
[1m[92m  Downloaded[0m validator v0.20.0
[1m[92m  Downloaded[0m outref v0.5.2
[1m[92m  Downloaded[0m sea-query-derive v0.4.3
[1m[92m  Downloaded[0m compio v0.18.0
[1m[92m  Downloaded[0m const-random v0.1.18
[1m[92m  Downloaded[0m polling v3.11.0
[1m[92m  Downloaded[0m wasmprinter v0.251.0
[1m[92m  Downloaded[0m wasmtime-internal-wit-bindgen v46.0.1
[1m[92m  Downloaded[0m cranelift-bforest v0.133.1
[1m[92m  Downloaded[0m home v0.5.12
[1m[92m  Downloaded[0m lettre v0.11.22
[1m[92m  Downloaded[0m tonic-prost-build v0.14.6
[1m[92m  Downloaded[0m wat v1.253.0
[1m[92m  Downloaded[0m ryu v1.0.23
[1m[92m  Downloaded[0m serde-value v0.7.0
[1m[92m  Downloaded[0m icu_decimal v2.2.0
[1m[92m  Downloaded[0m rand_core v0.10.1
[1m[92m  Downloaded[0m ravif v0.13.0
[1m[92m  Downloaded[0m referencing v0.46.10
[1m[92m  Downloaded[0m untrusted v0.9.0
[1m[92m  Downloaded[0m wasm_split_helpers v0.2.3
[1m[92m  Downloaded[0m wasmtime-environ v46.0.1
[1m[92m  Downloaded[0m prost-build v0.14.4
[1m[92m  Downloaded[0m rmcp-macros v1.8.0
[1m[92m  Downloaded[0m unic-langid-impl v0.9.6
[1m[92m  Downloaded[0m wasmtime-internal-fiber v46.0.1
[1m[92m  Downloaded[0m rmcp-macros v2.2.0
[1m[92m  Downloaded[0m sqlx-mysql v0.8.6
[1m[92m  Downloaded[0m num-rational v0.4.2
[1m[92m  Downloaded[0m utoipa v5.5.0
[1m[92m  Downloaded[0m cpp_demangle v0.4.5
[1m[92m  Downloaded[0m pkg-config v0.3.33
[1m[92m  Downloaded[0m profiling v1.0.18
[1m[92m  Downloaded[0m tower-http v0.7.0
[1m[92m  Downloaded[0m unicode-width v0.2.2
[1m[92m  Downloaded[0m futures-rustls v0.26.0
[1m[92m  Downloaded[0m toml_parser v1.1.2+spec-1.1.0
[1m[92m  Downloaded[0m half v2.7.1
[1m[92m  Downloaded[0m hashbrown v0.12.3
[1m[92m  Downloaded[0m libm v0.2.16
[1m[92m  Downloaded[0m unicode-vo v0.1.0
[1m[92m  Downloaded[0m headers v0.4.1
[1m[92m  Downloaded[0m itertools v0.12.1
[1m[92m  Downloaded[0m nix v0.31.3
[1m[92m  Downloaded[0m rustix v1.1.4
[1m[92m  Downloaded[0m sea-orm-cli v1.1.20
[1m[92m  Downloaded[0m hyper-tls v0.6.0
[1m[92m  Downloaded[0m server_fn_macro v0.6.15
[1m[92m  Downloaded[0m pem v3.0.6
[1m[92m  Downloaded[0m tracing-futures v0.2.5
[1m[92m  Downloaded[0m wasmtime v46.0.1
[1m[92m  Downloaded[0m webpki-roots v1.0.8
[1m[92m  Downloaded[0m x509-parser v0.18.1
[1m[92m  Downloaded[0m block-buffer v0.12.1
[1m[92m  Downloaded[0m bon v3.9.3
[1m[92m  Downloaded[0m collection_literals v1.0.3
[1m[92m  Downloaded[0m crossbeam-utils v0.8.22
[1m[92m  Downloaded[0m gif v0.12.0
[1m[92m  Downloaded[0m google-cloud-longrunning v1.11.0
[1m[92m  Downloaded[0m google-cloud-wkt v1.6.0
[1m[92m  Downloaded[0m http-range-header v0.4.2
[1m[92m  Downloaded[0m icu_datetime_data v2.2.0
[1m[92m  Downloaded[0m liquid-lib v0.26.11
[1m[92m  Downloaded[0m simd_helpers v0.1.0
[1m[92m  Downloaded[0m wasm-bindgen-shared v0.2.126
[1m[92m  Downloaded[0m zune-core v0.5.1
[1m[92m  Downloaded[0m compio-runtime v0.11.0
[1m[92m  Downloaded[0m js-sys v0.3.103
[1m[92m  Downloaded[0m v_htmlescape v0.17.0
[1m[92m  Downloaded[0m cobs v0.3.0
[1m[92m  Downloaded[0m gif v0.14.2
[1m[92m  Downloaded[0m multer v3.1.0
[1m[92m  Downloaded[0m syn v1.0.109
[1m[92m  Downloaded[0m typed-builder-macro v0.23.2
[1m[92m  Downloaded[0m proc-macro-error-attr3 v3.0.2
[1m[92m  Downloaded[0m rav1e v0.8.1
[1m[92m  Downloaded[0m seahash v4.1.0
[1m[92m  Downloaded[0m wasmi v1.1.0
[1m[92m  Downloaded[0m compio-ws v0.3.1
[1m[92m  Downloaded[0m pin-utils v0.1.0
[1m[92m  Downloaded[0m toml_edit v0.22.27
[1m[92m  Downloaded[0m borsh-derive v1.7.0
[1m[92m  Downloaded[0m bumpalo v3.20.3
[1m[92m  Downloaded[0m const_format_proc_macros v0.2.34
[1m[92m  Downloaded[0m getrandom v0.4.3
[1m[92m  Downloaded[0m mac_address v1.1.8
[1m[92m  Downloaded[0m matchit v0.8.4
[1m[92m  Downloaded[0m matrixmultiply v0.3.10
[1m[92m  Downloaded[0m strict-num v0.1.1
[1m[92m  Downloaded[0m vsimd v0.8.0
[1m[92m  Downloaded[0m built v0.8.1
[1m[92m  Downloaded[0m postcard v1.1.3
[1m[92m  Downloaded[0m walkdir v2.5.0
[1m[92m  Downloaded[0m yasna v0.6.0
[1m[92m  Downloaded[0m reborrow v0.5.5
[1m[92m  Downloaded[0m rustybuzz v0.10.0
[1m[92m  Downloaded[0m bit-vec v0.9.1
[1m[92m  Downloaded[0m maybe-rayon v0.1.1
[1m[92m  Downloaded[0m openssl-sys v0.9.117
[1m[92m  Downloaded[0m getrandom v0.2.17
[1m[92m  Downloaded[0m rand_core v0.6.4
[1m[92m  Downloaded[0m sqlx-sqlite v0.8.6
[1m[92m  Downloaded[0m signature v2.2.0
[1m[92m  Downloaded[0m bytecheck v0.6.12
[1m[92m  Downloaded[0m convert_case v0.11.0
[1m[92m  Downloaded[0m icu_provider_export v2.2.0
[1m[92m  Downloaded[0m rsa v0.9.10
[1m[92m  Downloaded[0m rustls v0.23.41
[1m[92m  Downloaded[0m rustls-pki-types v1.15.0
[1m[92m  Downloaded[0m slotmap v1.1.1
[1m[92m  Downloaded[0m toml_edit v0.25.12+spec-1.1.0
[1m[92m  Downloaded[0m ucd-trie v0.1.7
[1m[92m  Downloaded[0m version-ranges v0.1.3
[1m[92m  Downloaded[0m wasm-bindgen-macro v0.2.126
[1m[92m  Downloaded[0m wasm_split_macros v0.2.2
[1m[92m  Downloaded[0m wasmparser v0.239.0
[1m[92m  Downloaded[0m zerocopy-derive v0.8.54
[1m[92m  Downloaded[0m compression-codecs v0.4.38
[1m[92m  Downloaded[0m convert_case v0.6.0
[1m[92m  Downloaded[0m convert_case_extras v0.2.0
[1m[92m  Downloaded[0m foreign-types v0.3.2
[1m[92m  Downloaded[0m generic-array v0.14.7
[1m[92m  Downloaded[0m icu_provider_baked v2.2.0
[1m[92m  Downloaded[0m indexmap v1.9.3
[1m[92m  Downloaded[0m konst v0.2.20
[1m[92m  Downloaded[0m quinn-proto v0.11.16
[1m[92m  Downloaded[0m tera v2.0.0
[1m[92m  Downloaded[0m serde_yaml v0.9.34+deprecated
[1m[92m  Downloaded[0m roxmltree v0.20.0
[1m[92m  Downloaded[0m rstml v0.12.1
[1m[92m  Downloaded[0m schemars_derive v1.2.1
[1m[92m  Downloaded[0m io_uring_buf_ring v0.2.3
[1m[92m  Downloaded[0m opaque-debug v0.3.1
[1m[92m  Downloaded[0m target-lexicon v0.13.5
[1m[92m  Downloaded[0m jsonschema v0.46.10
[1m[92m  Downloaded[0m cranelift-frontend v0.133.1
[1m[92m  Downloaded[0m hybrid-array v0.4.13
[1m[92m  Downloaded[0m noop_proc_macro v0.3.0
[1m[92m  Downloaded[0m memfd v0.6.5
[1m[92m  Downloaded[0m codee v0.3.5
[1m[92m  Downloaded[0m leptos_dom v0.8.8
[1m[92m  Downloaded[0m zeroize v1.9.0
[1m[92m  Downloaded[0m id-arena v2.3.0
[1m[92m  Downloaded[0m native-tls v0.2.18
[1m[92m  Downloaded[0m protoc-bin-vendored-macos-aarch_64 v3.2.0
[1m[92m  Downloaded[0m server_fn v0.6.15
[1m[92m  Downloaded[0m simplecss v0.2.2
[1m[92m  Downloaded[0m phf_generator v0.11.3
[1m[92m  Downloaded[0m strum v0.28.0
[1m[92m  Downloaded[0m thin-cell v0.1.2
[1m[92m  Downloaded[0m throw_error v0.3.1
[1m[92m  Downloaded[0m compio-fs v0.11.0
[1m[92m  Downloaded[0m console_error_panic_hook v0.1.7
[1m[92m  Downloaded[0m google-cloud-location v1.11.0
[1m[92m  Downloaded[0m icu_list_data v2.2.0
[1m[92m  Downloaded[0m leptos_router_macro v0.8.6
[1m[92m  Downloaded[0m oco_ref v0.2.1
[1m[92m  Downloaded[0m pico-args v0.5.0
[1m[92m  Downloaded[0m rand v0.8.7
[1m[92m  Downloaded[0m rust-embed v8.12.0
[1m[92m  Downloaded[0m icu_locale v2.2.0
[1m[92m  Downloaded[0m mime v0.3.17
[1m[92m  Downloaded[0m sqlx v0.8.6
[1m[92m  Downloaded[0m leptos_i18n_codegen v0.6.2
[1m[92m  Downloaded[0m pulley-macros v46.0.1
[1m[92m  Downloaded[0m time v0.3.53
[1m[92m  Downloaded[0m pastey v0.1.1
[1m[92m  Downloaded[0m strum v0.27.2
[1m[92m  Downloaded[0m zstd-sys v2.0.16+zstd.1.5.7
[1m[92m  Downloaded[0m rig-core v0.40.0
[1m[92m  Downloaded[0m icu_collator v2.2.1
[1m[92m  Downloaded[0m cpufeatures v0.3.0
[1m[92m  Downloaded[0m retry-policies v0.5.2
[1m[92m  Downloaded[0m rgb v0.8.53
[1m[92m  Downloaded[0m typeid v1.0.3
[1m[92m  Downloaded[0m leptos_server v0.8.7
[1m[92m  Downloaded[0m wasmi_core v1.1.0
[1m[92m  Downloaded[0m ring v0.17.14
[1m[92m  Downloaded[0m polyval v0.6.2
[1m[92m  Downloaded[0m serde_norway v0.9.42
[1m[92m  Downloaded[0m convert_case v0.10.0
[1m[92m  Downloaded[0m serial_test_derive v3.5.0
[1m[92m  Downloaded[0m bitflags v1.3.2
[1m[92m  Downloaded[0m rend v0.4.2
[1m[92m  Downloaded[0m cranelift-assembler-x64 v0.133.1
[1m[92m  Downloaded[0m serde_derive v1.0.228
[1m[92m  Downloaded[0m server_fn v0.8.13
[1m[92m  Downloaded[0m serde_with_macros v3.21.0
[1m[92m  Downloaded[0m unicode-general-category v1.1.0
[1m[92m  Downloaded[0m num-complex v0.4.6
[1m[92m  Downloaded[0m rawpointer v0.2.1
[1m[92m  Downloaded[0m leptos_hot_reload v0.6.15
[1m[92m  Downloaded[0m pubgrub v0.4.0
[1m[92m  Downloaded[0m rig-bedrock v0.40.0
[1m[92m  Downloaded[0m wit-parser v0.251.0
[1m[92m  Downloaded[0m xmlparser v0.13.6
[1m[92m  Downloaded[0m const_str_slice_concat v0.1.0
[1m[92m  Downloaded[0m num-order v1.2.0
[1m[92m  Downloaded[0m comfy-table v7.2.2
[1m[92m  Downloaded[0m num-derive v0.4.2
[1m[92m  Downloaded[0m google-cloud-gax v1.12.0
[1m[92m  Downloaded[0m spin v0.9.8
[1m[92m  Downloaded[0m rust_decimal v1.42.1
[1m[92m  Downloaded[0m jpeg-decoder v0.3.2
[1m[92m  Downloaded[0m protoc-bin-vendored-linux-s390_64 v3.2.0
[1m[92m  Downloaded[0m protoc-bin-vendored-win32 v3.2.0
[1m[92m  Downloaded[0m pulp v0.22.3
[1m[92m  Downloaded[0m wasmtime-internal-versioned-export-macros v46.0.1
[1m[92m  Downloaded[0m priority-queue v2.7.0
[1m[92m  Downloaded[0m sea-orm-migration v1.1.20
[1m[92m  Downloaded[0m utoipa-gen v5.5.0
[1m[92m  Downloaded[0m zip v2.4.2
[1m[92m  Downloaded[0m write16 v1.0.0
[1m[92m  Downloaded[0m cranelift-srcgen v0.133.1
[1m[92m  Downloaded[0m version_check v0.9.5
[1m[92m  Downloaded[0m futures-lite v2.6.1
[1m[92m  Downloaded[0m pulp-wasm-simd-flag v0.1.1
[1m[92m  Downloaded[0m hashbrown v0.14.5
[1m[92m  Downloaded[0m same-file v1.0.6
[1m[92m  Downloaded[0m petgraph v0.8.3
[1m[92m  Downloaded[0m quoted_printable v0.5.2
[1m[92m  Downloaded[0m icu_provider_source v2.2.0
[1m[92m  Downloaded[0m semver v1.0.28
[1m[92m  Downloaded[0m tokio-tungstenite v0.28.0
[1m[92m  Downloaded[0m toml_datetime v1.1.1+spec-1.1.0
[1m[92m  Downloaded[0m usvg v0.36.0
[1m[92m  Downloaded[0m zeroize_derive v1.5.0
[1m[92m  Downloaded[0m futures-timer v3.0.4
[1m[92m  Downloaded[0m trait-variant v0.1.2
[1m[92m  Downloaded[0m wasmparser v0.251.0
[1m[92m  Downloaded[0m ref-cast-impl v1.0.25
[1m[92m  Downloaded[0m wasmtime-internal-component-util v46.0.1
[1m[92m  Downloaded[0m icu_locale_data v2.2.0
[1m[92m  Downloaded[0m terminal_size v0.4.4
[1m[92m  Downloaded[0m web-sys v0.3.103
[1m[92m  Downloaded[0m http v0.2.12
[1m[92m  Downloaded[0m guardian v1.3.0
[1m[92m  Downloaded[0m shlex v2.0.1
[1m[92m  Downloaded[0m nix v0.29.0
[1m[92m  Downloaded[0m const-oid v0.10.2
[1m[92m  Downloaded[0m hashbrown v0.15.5
[1m[92m  Downloaded[0m indenter v0.3.4
[1m[92m  Downloaded[0m futures-intrusive v0.5.0
[1m[92m  Downloaded[0m tagptr v0.2.0
[1m[92m  Downloaded[0m clap v4.6.1
[1m[92m  Downloaded[0m schemars v0.9.0
[1m[92m  Downloaded[0m clap_builder v4.6.0
[1m[92m  Downloaded[0m konst_macro_rules v0.2.19
[1m[92m  Downloaded[0m byteorder v1.5.0
[1m[92m  Downloaded[0m wasmparser v0.253.0
[1m[92m  Downloaded[0m rustc-hash v1.1.0
[1m[92m  Downloaded[0m sea-query v0.32.7
[1m[92m  Downloaded[0m ttf-parser v0.19.2
[1m[92m  Downloaded[0m humantime v2.4.0
[1m[92m  Downloaded[0m compio-io v0.9.1
[1m[92m  Downloaded[0m xmlwriter v0.1.0
[1m[92m  Downloaded[0m reqwest-middleware v0.5.2
[1m[92m  Downloaded[0m papaya v0.2.4
[1m[92m  Downloaded[0m bytecount v0.6.9
[1m[92m  Downloaded[0m html-escape v0.2.14
[1m[92m  Downloaded[0m bytemuck_derive v1.11.0
[1m[92m  Downloaded[0m rig-gemini-grpc v0.40.0
[1m[92m  Downloaded[0m protoc-bin-vendored-linux-x86_32 v3.2.0
[1m[92m  Downloaded[0m string-interner v0.19.0
[1m[92m  Downloaded[0m parse-zoneinfo v0.5.0
[1m[92m  Downloaded[0m minimal-lexical v0.2.1
[1m[92m  Downloaded[0m uuid v1.23.5
[1m[92m  Downloaded[0m image-webp v0.2.4
[1m[92m  Downloaded[0m wasmtime-internal-unwinder v46.0.1
[1m[92m  Downloaded[0m camino v1.2.4
[1m[92m  Downloaded[0m quinn-udp v0.5.15
[1m[92m  Downloaded[0m proc-macro2-diagnostics v0.10.1
[1m[92m  Downloaded[0m icu_segmenter v2.2.0
[1m[92m  Downloaded[0m protoc-bin-vendored-linux-aarch_64 v3.2.0
[1m[92m  Downloaded[0m icu_experimental_data v0.5.0
[1m[92m  Downloaded[0m csv v1.4.0
[1m[92m  Downloaded[0m bit_field v0.10.3
[1m[92m  Downloaded[0m toml_datetime v0.6.11
[1m[92m   Compiling[0m proc-macro2 v1.0.106
[1m[92m   Compiling[0m unicode-ident v1.0.24
[1m[92m   Compiling[0m serde v1.0.228
[1m[92m   Compiling[0m quote v1.0.46
[1m[92m   Compiling[0m serde_core v1.0.228
[1m[92m   Compiling[0m libc v0.2.186
[1m[92m   Compiling[0m foldhash v0.2.0
[1m[92m   Compiling[0m portable-atomic v1.13.1
[1m[92m   Compiling[0m syn v2.0.118
[1m[92m   Compiling[0m version_check v0.9.5
[1m[92m   Compiling[0m futures-io v0.3.32
[1m[92m   Compiling[0m futures-task v0.3.32
[1m[92m   Compiling[0m jobserver v0.1.35
[1m[92m   Compiling[0m once_cell v1.21.4
[1m[92m   Compiling[0m hashbrown v0.17.1
[1m[92m   Compiling[0m shlex v2.0.1
[1m[92m   Compiling[0m find-msvc-tools v0.1.9
[1m[92m   Compiling[0m getrandom v0.2.17
[1m[92m   Compiling[0m cc v1.2.67
[1m[92m   Compiling[0m indexmap v2.14.0
[1m[92m   Compiling[0m socket2 v0.6.4
[1m[92m   Compiling[0m autocfg v1.5.1
[1m[92m   Compiling[0m typenum v1.20.1
[1m[92m   Compiling[0m subtle v2.6.1
[1m[92m   Compiling[0m errno v0.3.14
[1m[92m   Compiling[0m signal-hook-registry v1.4.8
[1m[92m   Compiling[0m synstructure v0.13.2
[1m[92m   Compiling[0m mio v1.2.1
[1m[92m   Compiling[0m tracing-core v0.1.36
[1m[92m   Compiling[0m pkg-config v0.3.33
[1m[92m   Compiling[0m serde_json v1.0.150
[1m[92m   Compiling[0m stable_deref_trait v1.2.1
[1m[92m   Compiling[0m ring v0.17.14
[1m[92m   Compiling[0m zerocopy v0.8.54
[1m[92m   Compiling[0m untrusted v0.9.0
[1m[92m   Compiling[0m rand_core v0.6.4
[1m[92m   Compiling[0m generic-array v0.14.7
[1m[92m   Compiling[0m cmake v0.1.58
[1m[92m   Compiling[0m serde_derive v1.0.228
[1m[92m   Compiling[0m zeroize_derive v1.5.0
[1m[92m   Compiling[0m futures-macro v0.3.32
[1m[92m   Compiling[0m zeroize v1.9.0
[1m[92m   Compiling[0m thiserror-impl v2.0.18
[1m[92m   Compiling[0m futures-util v0.3.32
[1m[92m   Compiling[0m thiserror v2.0.18
[1m[92m   Compiling[0m zerofrom-derive v0.1.7
[1m[92m   Compiling[0m zerofrom v0.1.8
[1m[92m   Compiling[0m yoke-derive v0.8.2
[1m[92m   Compiling[0m smallvec v1.15.2
[1m[92m   Compiling[0m bytes v1.12.1
[1m[92m   Compiling[0m parking_lot_core v0.9.12
[1m[92m   Compiling[0m parking_lot v0.12.5
[1m[92m   Compiling[0m displaydoc v0.2.6
[1m[92m   Compiling[0m zerovec-derive v0.11.3
[1m[92m   Compiling[0m tokio-macros v2.7.0
[1m[92m   Compiling[0m tracing-attributes v0.1.31
[1m[92m   Compiling[0m tokio v1.52.3
[1m[92m   Compiling[0m http v1.4.2
[1m[92m   Compiling[0m tracing v0.1.44
[1m[92m   Compiling[0m zerocopy-derive v0.8.54
[1m[92m   Compiling[0m dunce v1.0.5
[1m[92m   Compiling[0m fs_extra v1.3.0
[1m[92m   Compiling[0m aws-lc-sys v0.42.0
[1m[92m   Compiling[0m num-traits v0.2.19
[1m[92m   Compiling[0m aws-lc-rs v1.17.1
[1m[92m   Compiling[0m rustls-pki-types v1.15.0
[1m[92m   Compiling[0m crypto-common v0.1.7
[1m[92m   Compiling[0m form_urlencoded v1.2.2
[1m[92m   Compiling[0m untrusted v0.7.1
[1m[92m   Compiling[0m tokio-util v0.7.18
[1m[92m   Compiling[0m yoke v0.8.3
[1m[92m   Compiling[0m ryu v1.0.23
[1m[92m   Compiling[0m rustls v0.23.41
[1m[92m   Compiling[0m zerovec v0.11.6
[1m[92m   Compiling[0m const-oid v0.9.6
[1m[92m   Compiling[0m http-body v1.0.1
[1m[92m   Compiling[0m writeable v0.6.3
[1m[92m   Compiling[0m block-buffer v0.10.4
[1m[92m   Compiling[0m litemap v0.8.2
[1m[92m   Compiling[0m digest v0.10.7
[1m[92m   Compiling[0m zerotrie v0.2.4
[1m[92m   Compiling[0m http-body-util v0.1.3
[1m[92m   Compiling[0m either v1.16.0
[1m[92m   Compiling[0m cpufeatures v0.2.17
[1m[92m   Compiling[0m tinystr v0.8.3
[1m[92m   Compiling[0m potential_utf v0.1.5
[1m[92m   Compiling[0m icu_locale_core v2.2.0
[1m[92m   Compiling[0m icu_collections v2.2.0
[1m[92m   Compiling[0m webpki-roots v1.0.8
[1m[92m   Compiling[0m crossbeam-utils v0.8.22
[1m[92m   Compiling[0m crc32fast v1.5.0
[1m[92m   Compiling[0m httpdate v1.0.3
[1m[92m   Compiling[0m time-core v0.1.9
[1m[92m   Compiling[0m icu_provider v2.2.0
[1m[92m   Compiling[0m icu_properties v2.2.0
[1m[92m   Compiling[0m icu_normalizer v2.2.0
[1m[92m   Compiling[0m num-conv v0.2.2
[1m[92m   Compiling[0m h2 v0.4.15
[1m[92m   Compiling[0m time-macros v0.2.31
[1m[92m   Compiling[0m idna_adapter v1.2.2
[1m[92m   Compiling[0m idna v1.1.0
[1m[92m   Compiling[0m url v2.5.8
[1m[92m   Compiling[0m futures-executor v0.3.32
[1m[92m   Compiling[0m deranged v0.5.8
[1m[92m   Compiling[0m rand_core v0.10.1
[1m[92m   Compiling[0m powerfmt v0.2.0
[1m[92m   Compiling[0m getrandom v0.4.3
[1m[92m   Compiling[0m regex-syntax v0.8.11
[1m[92m   Compiling[0m time v0.3.53
[1m[92m   Compiling[0m hyper v1.10.1
[1m[92m   Compiling[0m aho-corasick v1.1.4
[1m[92m   Compiling[0m tower v0.5.3
[1m[92m   Compiling[0m hyper-util v0.1.20
[1m[92m   Compiling[0m getrandom v0.3.4
[1m[92m   Compiling[0m semver v1.0.28
[1m[92m   Compiling[0m mime v0.3.17
[1m[92m   Compiling[0m async-trait v0.1.89
[1m[92m   Compiling[0m serde_urlencoded v0.7.1
[1m[92m   Compiling[0m ident_case v1.0.1
[1m[92m   Compiling[0m rustc_version v0.4.1
[1m[92m   Compiling[0m cpufeatures v0.3.0
[1m[92m   Compiling[0m ppv-lite86 v0.2.21
[1m[92m   Compiling[0m spin v0.9.8
[1m[92m   Compiling[0m libm v0.2.16
[1m[92m   Compiling[0m strsim v0.11.1
[1m[92m   Compiling[0m anyhow v1.0.103
[1m[92m   Compiling[0m futures v0.3.32
[1m[92m   Compiling[0m encoding_rs v0.8.35
[1m[92m   Compiling[0m unicode-segmentation v1.13.3
[1m[92m   Compiling[0m adler2 v2.0.1
[1m[92m   Compiling[0m pin-project-internal v1.1.13
[1m[92m   Compiling[0m rand_core v0.9.5
[1m[92m   Compiling[0m rand_chacha v0.9.0
[1m[92m   Compiling[0m pin-project v1.1.13
[1m[92m   Compiling[0m chacha20 v0.10.1
[1m[92m   Compiling[0m num-integer v0.1.46
[1m[92m   Compiling[0m rand v0.10.2
[1m[92m   Compiling[0m rand v0.9.5
[1m[92m   Compiling[0m simd-adler32 v0.3.9
[1m[92m   Compiling[0m openssl-probe v0.2.1
[1m[92m   Compiling[0m winnow v1.0.3
[1m[92m   Compiling[0m iana-time-zone v0.1.65
[1m[92m   Compiling[0m miniz_oxide v0.8.9
[1m[92m   Compiling[0m chrono v0.4.45
[1m[92m   Compiling[0m regex-automata v0.4.15
[1m[92m   Compiling[0m rustls-native-certs v0.8.4
[1m[92m   Compiling[0m concurrent-queue v2.5.0
[1m[92m   Compiling[0m parking v2.2.1
[1m[92m   Compiling[0m flate2 v1.1.9
[1m[92m   Compiling[0m unicode-xid v0.2.6
[1m[92m   Compiling[0m sha1 v0.10.7
[1m[92m   Compiling[0m unicase v2.9.0
[1m[92m   Compiling[0m event-listener v5.4.1
[1m[92m   Compiling[0m webpki-roots v0.26.11
[1m[92m   Compiling[0m heck v0.5.0
[1m[92m   Compiling[0m arrayvec v0.7.8
[1m[92m   Compiling[0m allocator-api2 v0.2.21
[1m[92m   Compiling[0m data-encoding v2.11.0
[1m[92m   Compiling[0m itertools v0.14.0
[1m[92m   Compiling[0m proc-macro-error-attr2 v2.0.0
[1m[92m   Compiling[0m proc-macro-error2 v2.0.1
[1m[92m   Compiling[0m darling_core v0.23.0
[1m[92m   Compiling[0m thiserror-impl v1.0.69
[1m[92m   Compiling[0m thiserror v1.0.69
[1m[92m   Compiling[0m mime_guess v2.0.5
[1m[92m   Compiling[0m rustversion v1.0.23
[1m[92m   Compiling[0m hybrid-array v0.4.13
[1m[92m   Compiling[0m darling_macro v0.23.0
[1m[92m   Compiling[0m darling v0.23.0
[1m[92m   Compiling[0m yansi v1.0.1
[1m[92m   Compiling[0m block-buffer v0.12.1
[1m[92m   Compiling[0m crypto-common v0.2.2
[1m[92m   Compiling[0m multer v3.1.0
[1m[92m   Compiling[0m const-oid v0.10.2
[1m[92m   Compiling[0m matchit v0.8.4
[1m[92m   Compiling[0m uuid v1.23.5
[1m[92m   Compiling[0m proc-macro2-diagnostics v0.10.1
[1m[92m   Compiling[0m event-listener-strategy v0.5.4
[1m[92m   Compiling[0m sha2 v0.10.9
[1m[92m   Compiling[0m fastrand v2.4.1
[1m[92m   Compiling[0m compression-core v0.4.32
[1m[92m   Compiling[0m compression-codecs v0.4.38
[1m[92m   Compiling[0m regex v1.13.0
[1m[92m   Compiling[0m axum-core v0.5.6
[1m[92m   Compiling[0m serde_path_to_error v0.1.20
[1m[92m   Compiling[0m http-range-header v0.4.2
[1m[92m   Compiling[0m cmov v0.5.4
[1m[92m   Compiling[0m async-compression v0.4.42
[1m[92m   Compiling[0m ctutils v0.4.2
[1m[92m   Compiling[0m async-lock v3.4.2
[1m[92m   Compiling[0m tokio-stream v0.1.18
[1m[92m   Compiling[0m toml_writer v1.1.1+spec-1.1.0
[1m[92m   Compiling[0m digest v0.11.3
[1m[92m   Compiling[0m tower-http v0.6.11
[1m[92m   Compiling[0m toml_parser v1.1.2+spec-1.1.0
[1m[92m   Compiling[0m toml_datetime v1.1.1+spec-1.1.0
[1m[92m   Compiling[0m rustix v1.1.4
[1m[92m   Compiling[0m sha2 v0.11.0
[1m[92m   Compiling[0m hmac v0.12.1
[1m[92m   Compiling[0m linux-raw-sys v0.12.1
[1m[92m   Compiling[0m num-bigint v0.4.8
[1m[92m   Compiling[0m tinyvec_macros v0.1.1
[1m[92m   Compiling[0m tinyvec v1.12.0
[1m[92m   Compiling[0m ucd-trie v0.1.7
[1m[92m   Compiling[0m darling_core v0.20.11
[1m[92m   Compiling[0m vcpkg v0.2.15
[1m[92m   Compiling[0m crossbeam-queue v0.3.13
[1m[92m   Compiling[0m darling_macro v0.20.11
[1m[92m   Compiling[0m serde_spanned v1.1.1
[1m[92m   Compiling[0m darling v0.20.11
[1m[92m   Compiling[0m toml v1.1.2+spec-1.1.0
[1m[92m   Compiling[0m foldhash v0.1.5
[1m[92m   Compiling[0m byteorder v1.5.0
[1m[92m   Compiling[0m hashbrown v0.16.1
[1m[92m   Compiling[0m rust_decimal v1.42.1
[1m[92m   Compiling[0m toml_edit v0.25.12+spec-1.1.0
[1m[92m   Compiling[0m proc-macro-crate v3.5.0
[1m[92m   Compiling[0m unicode-properties v0.1.4
[1m[92m   Compiling[0m pest v2.8.7
[1m[92m   Compiling[0m bigdecimal v0.4.10
[1m[92m   Compiling[0m crossbeam-epoch v0.9.20
[1m[92m   Compiling[0m heck v0.4.1
[1m[92m   Compiling[0m static_assertions v1.1.0
[1m[92m   Compiling[0m unicode-bidi v0.3.18
[1m[92m   Compiling[0m opentelemetry v0.32.0
[1m[92m   Compiling[0m hashbrown v0.15.5
[1m[92m   Compiling[0m unicode-normalization v0.1.25
[1m[92m   Compiling[0m prost-derive v0.14.4
[1m[92m   Compiling[0m matchers v0.2.0
[1m[92m   Compiling[0m tracing-serde v0.2.0
[1m[92m   Compiling[0m tracing-log v0.2.0
[1m[92m   Compiling[0m crc-catalog v2.5.0
[1m[92m   Compiling[0m crc v3.4.0
[1m[92m   Compiling[0m tracing-subscriber v0.3.23
[1m[92m   Compiling[0m hashlink v0.10.0
[1m[92m   Compiling[0m pest_meta v2.8.7
[1m[92m   Compiling[0m libsqlite3-sys v0.30.1
[1m[92m   Compiling[0m futures-intrusive v0.5.0
[1m[92m   Compiling[0m pest_generator v2.8.7
[1m[92m   Compiling[0m opentelemetry_sdk v0.32.1
[1m[92m   Compiling[0m convert_case v0.10.0
[1m[92m   Compiling[0m rand_chacha v0.3.1
[1m[92m   Compiling[0m atoi v2.0.0
[1m[92m   Compiling[0m async-stream-impl v0.3.6
[1m[92m   Compiling[0m zstd-sys v2.0.16+zstd.1.5.7
[1m[92m   Compiling[0m bumpalo v3.20.3
[1m[92m   Compiling[0m dotenvy v0.15.7
[1m[92m   Compiling[0m async-stream v0.3.6
[1m[92m   Compiling[0m rand v0.8.7
[1m[92m   Compiling[0m derive_more-impl v2.1.1
[1m[92m   Compiling[0m pest_derive v2.8.7
[1m[92m   Compiling[0m stringprep v0.1.5
[1m[92m   Compiling[0m hkdf v0.12.4
[1m[92m   Compiling[0m flume v0.11.1
[1m[92m   Compiling[0m md-5 v0.10.6
[1m[92m   Compiling[0m home v0.5.12
[1m[92m   Compiling[0m whoami v1.6.1
[1m[92m   Compiling[0m tracing-opentelemetry v0.33.0
[1m[92m   Compiling[0m derive_more v2.1.1
[1m[92m   Compiling[0m sea-query-derive v0.4.3
[1m[92m   Compiling[0m ordered-float v4.6.0
[1m[92m   Compiling[0m inherent v1.0.13
[1m[92m   Compiling[0m zstd-safe v7.2.4
[1m[92m   Compiling[0m unsafe-libyaml v0.2.11
[1m[92m   Compiling[0m sea-query v0.32.7
[1m[92m   Compiling[0m derive_builder_core v0.20.2
[1m[92m   Compiling[0m derive_builder_macro v0.20.2
[1m[92m   Compiling[0m sea-bae v0.2.1
[1m[92m   Compiling[0m ouroboros_macro v0.18.5
[1m[92m   Compiling[0m lru v0.16.4
[1m[92m   Compiling[0m async-graphql-value v7.2.1
[1m[92m   Compiling[0m strum_macros v0.27.2
[1m[92m   Compiling[0m async-io v2.6.0
[1m[92m   Compiling[0m aliasable v0.1.3
[1m[92m   Compiling[0m utf8parse v0.2.2
[1m[92m   Compiling[0m num-modular v0.6.4
[1m[92m   Compiling[0m num-order v1.2.0
[1m[92m   Compiling[0m anstyle-parse v1.0.0
[1m[92m   Compiling[0m strum v0.27.2
[1m[92m   Compiling[0m ouroboros v0.18.5
[1m[92m   Compiling[0m async-graphql-parser v7.2.1
[1m[92m   Compiling[0m sea-orm-macros v1.1.20
[1m[92m   Compiling[0m zstd v0.13.3
[1m[92m   Compiling[0m derive_builder v0.20.2
[1m[92m   Compiling[0m prost v0.14.4
[1m[92m   Compiling[0m polling v3.11.0
[1m[92m   Compiling[0m hyper-timeout v0.5.2
[1m[92m   Compiling[0m futures-lite v2.6.1
[1m[92m   Compiling[0m ascii_utils v0.9.3
[1m[92m   Compiling[0m strum v0.26.3
[1m[92m   Compiling[0m Inflector v0.11.4
[1m[92m   Compiling[0m anstyle v1.0.14
[1m[92m   Compiling[0m is_terminal_polyfill v1.70.2
[1m[92m   Compiling[0m anstyle-query v1.1.5
[1m[92m   Compiling[0m colorchoice v1.0.5
[1m[92m   Compiling[0m async-graphql-derive v7.2.1
[1m[92m   Compiling[0m anstream v1.0.0
[1m[92m   Compiling[0m fast_chemail v0.9.6
[1m[92m   Compiling[0m handlebars v6.4.3
[1m[92m   Compiling[0m tempfile v3.27.0
[1m[92m   Compiling[0m terminal_size v0.4.4
[1m[92m   Compiling[0m asynk-strim v0.1.5
[1m[92m   Compiling[0m clap_lex v1.1.0
[1m[92m   Compiling[0m static_assertions_next v1.1.2
[1m[92m   Compiling[0m clap_builder v4.6.0
[1m[92m   Compiling[0m clap_derive v4.6.1
[1m[92m   Compiling[0m glob v0.3.3
[1m[92m   Compiling[0m async-graphql v7.2.1
[1m[92m   Compiling[0m clap v4.6.1
[1m[92m   Compiling[0m crossbeam-channel v0.5.16
[1m[92m   Compiling[0m paste v1.0.15
[1m[92m   Compiling[0m prost-types v0.14.4
[1m[92m   Compiling[0m protobuf-support v3.7.2
[1m[92m   Compiling[0m tagptr v0.2.0
[1m[92m   Compiling[0m wasm-bindgen-shared v0.2.126
[1m[92m   Compiling[0m moka v0.12.15
[1m[92m   Compiling[0m protobuf v3.7.2
[1m[92m   Compiling[0m sea-schema-derive v0.3.0
[1m[92m   Compiling[0m email_address v0.2.9
[1m[92m   Compiling[0m unsafe-libyaml-norway v0.2.15
[1m[92m   Compiling[0m serde_norway v0.9.42
[1m[92m   Compiling[0m prometheus v0.14.0
[1m[92m   Compiling[0m sea-orm-cli v1.1.20
[1m[92m   Compiling[0m rustls-webpki v0.103.13
[1m[92m   Compiling[0m utoipa-gen v5.5.0
[1m[92m   Compiling[0m ulid v2.0.1
[1m[92m   Compiling[0m v_escape-base v0.1.0
[1m[92m   Compiling[0m v_htmlescape v0.17.0
[1m[92m   Compiling[0m wasm-bindgen-macro-support v0.2.126
[1m[92m   Compiling[0m serde_yaml v0.9.34+deprecated
[1m[92m   Compiling[0m wasm-bindgen v0.2.126
[1m[92m   Compiling[0m rustc-hash v2.1.3
[1m[92m   Compiling[0m prettyplease v0.2.37
[1m[92m   Compiling[0m tokio-rustls v0.26.4
[1m[92m   Compiling[0m hyper-rustls v0.27.9
[1m[92m   Compiling[0m tungstenite v0.29.0
[1m[92m   Compiling[0m rustls-platform-verifier v0.7.0
[1m[92m   Compiling[0m reqwest v0.13.4
[1m[92m   Compiling[0m tokio-tungstenite v0.29.0
[1m[92m   Compiling[0m axum v0.8.9
[1m[92m   Compiling[0m sqlx-core v0.8.6
[1m[92m   Compiling[0m sqlx-postgres v0.8.6
[1m[92m   Compiling[0m sqlx-sqlite v0.8.6
[1m[92m   Compiling[0m tonic v0.14.6
[1m[92m   Compiling[0m sqlx v0.8.6
[1m[92m   Compiling[0m sea-query-binder v0.7.0
[1m[92m   Compiling[0m sea-orm v1.1.20
[1m[92m   Compiling[0m tonic-prost v0.14.6
[1m[92m   Compiling[0m opentelemetry-proto v0.32.0
[1m[92m   Compiling[0m tonic-types v0.14.6
[1m[92m   Compiling[0m opentelemetry-http v0.32.0
[1m[92m   Compiling[0m opentelemetry-otlp v0.32.0
[1m[92m   Compiling[0m sea-schema v0.16.2
[1m[92m   Compiling[0m utoipa v5.5.0
[1m[92m   Compiling[0m rustok-telemetry v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-telemetry)
[1m[92m   Compiling[0m wasm-bindgen-macro v0.2.126
[1m[92m   Compiling[0m rustok-api v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-api)
[1m[92m   Compiling[0m sea-orm-migration v1.1.20
[1m[92m   Compiling[0m rustok-events v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-events)
[1m[92m   Compiling[0m js-sys v0.3.103
[1m[92m   Compiling[0m convert_case v0.11.0
[1m[92m   Compiling[0m rustok-core v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-core)
[1m[92m   Compiling[0m slotmap v1.1.1
[1m[92m   Compiling[0m konst_macro_rules v0.2.19
[1m[92m   Compiling[0m konst v0.2.20
[1m[92m   Compiling[0m const_format_proc_macros v0.2.34
[1m[92m   Compiling[0m proc-macro-utils v0.10.0
[1m[92m   Compiling[0m const_format v0.2.36
[1m[92m   Compiling[0m xxhash-rust v0.8.16
[1m[92m   Compiling[0m same-file v1.0.6
[1m[92m   Compiling[0m walkdir v2.5.0
[1m[92m   Compiling[0m convert_case v0.6.0
[1m[92m   Compiling[0m derive-where v1.6.1
[1m[92m   Compiling[0m send_wrapper v0.6.0
[1m[92m   Compiling[0m quote-use-macros v0.8.4
[1m[92m   Compiling[0m or_poisoned v0.1.0
[1m[92m   Compiling[0m camino v1.2.4
[1m[92m   Compiling[0m quote-use v0.8.4
[1m[92m   Compiling[0m collection_literals v1.0.3
[1m[92m   Compiling[0m interpolator v0.5.0
[1m[92m   Compiling[0m html-escape v0.2.14
[1m[92m   Compiling[0m throw_error v0.3.1
[1m[92m   Compiling[0m vsimd v0.8.0
[1m[92m   Compiling[0m outref v0.5.2
[1m[92m   Compiling[0m wasm-bindgen-futures v0.4.76
[1m[92m   Compiling[0m web-sys v0.3.103
[1m[92m   Compiling[0m server_fn_macro v0.8.10
[1m[92m   Compiling[0m reactive_graph v0.2.14
[1m[92m   Compiling[0m drain_filter_polyfill v0.1.3
[1m[92m   Compiling[0m pathdiff v0.2.3
[1m[92m   Compiling[0m any_spawner v0.3.0
[1m[92m   Compiling[0m hydration_context v0.3.1
[1m[92m   Compiling[0m syn_derive v0.2.0
[1m[92m   Compiling[0m http v0.2.12
[1m[92m   Compiling[0m inventory v0.3.24
[1m[92m   Compiling[0m guardian v1.3.0
[1m[92m   Compiling[0m http-body v0.4.6
[1m[92m   Compiling[0m base64-simd v0.8.0
[1m[92m   Compiling[0m manyhow-macros v0.11.4
[1m[92m   Compiling[0m reactive_stores_macro v0.4.3
[1m[92m   Compiling[0m tachys v0.2.18
[1m[92m   Compiling[0m bytes-utils v0.1.4
[1m[92m   Compiling[0m pin-utils v0.1.0
[1m[92m   Compiling[0m aws-smithy-types v1.6.1
[1m[92m   Compiling[0m reactive_stores v0.4.3
[1m[92m   Compiling[0m manyhow v0.11.4
[1m[92m   Compiling[0m either_of v0.1.9
[1m[92m   Compiling[0m server_fn v0.8.13
[1m[92m   Compiling[0m oco_ref v0.2.1
[1m[92m   Compiling[0m const_str_slice_concat v0.1.0
[1m[92m   Compiling[0m base64ct v1.8.3
[1m[92m   Compiling[0m erased v0.1.2
[1m[92m   Compiling[0m next_tuple v0.1.0
[1m[92m   Compiling[0m attribute-derive-macro v0.10.5
[1m[92m   Compiling[0m server_fn_macro_default v0.8.5
[1m[92m   Compiling[0m rstml v0.12.1
[1m[92m   Compiling[0m leptos_macro v0.8.17
[1m[92m   Compiling[0m serde_qs v0.15.0
[1m[92m   Compiling[0m typed-builder-macro v0.23.2
[1m[92m   Compiling[0m const-str v1.1.0
[1m[92m   Compiling[0m base16 v0.2.1
[1m[92m   Compiling[0m typed-builder v0.23.2
[1m[92m   Compiling[0m wasm_split_macros v0.2.2
[1m[92m   Compiling[0m attribute-derive v0.10.5
[1m[92m   Compiling[0m leptos_hot_reload v0.8.6
[1m[92m   Compiling[0m config v0.15.25
[1m[92m   Compiling[0m convert_case_extras v0.2.0
[1m[92m   Compiling[0m leptos v0.8.20
[1m[92m   Compiling[0m aws-smithy-async v1.3.0
[1m[92m   Compiling[0m codee v0.3.5
[1m[92m   Compiling[0m aws-smithy-runtime-api-macros v1.1.0
[1m[92m   Compiling[0m async-once-cell v0.5.4
[1m[92m   Compiling[0m aws-smithy-runtime-api v1.13.0
[1m[92m   Compiling[0m wasm_split_helpers v0.2.3
[1m[92m   Compiling[0m leptos_config v0.8.10
[1m[92m   Compiling[0m rustok-outbox v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-outbox)
[1m[92m   Compiling[0m log v0.4.33
[1m[92m   Compiling[0m hmac v0.13.0
[1m[92m   Compiling[0m pem-rfc7468 v0.7.0
[1m[92m   Compiling[0m der v0.7.10
[1m[92m   Compiling[0m signature v2.2.0
[1m[92m   Compiling[0m ff v0.13.1
[1m[92m   Compiling[0m proc-macro-error-attr3 v3.0.2
[1m[92m   Compiling[0m base16ct v0.2.0
[1m[92m   Compiling[0m proc-macro-error3 v3.0.2
[1m[92m   Compiling[0m group v0.13.0
[1m[92m   Compiling[0m spki v0.7.3
[1m[92m   Compiling[0m gloo-utils v0.2.0
[1m[92m   Compiling[0m gloo-net v0.6.0
[1m[92m   Compiling[0m pkcs8 v0.10.2
[1m[92m   Compiling[0m sec1 v0.7.3
[1m[92m   Compiling[0m crypto-bigint v0.5.5
[1m[92m   Compiling[0m elliptic-curve v0.13.8
[1m[92m   Compiling[0m wasm-streams v0.5.0
[1m[92m   Compiling[0m validator_derive v0.20.1
[1m[92m   Compiling[0m aws-smithy-schema v0.2.0
[1m[92m   Compiling[0m aws-smithy-eventstream v0.61.1
[1m[92m   Compiling[0m aws-smithy-http v0.64.0
[1m[92m   Compiling[0m validator v0.20.0
[1m[92m   Compiling[0m rfc6979 v0.4.0
[1m[92m   Compiling[0m ecdsa v0.16.9
[1m[92m   Compiling[0m primeorder v0.13.6
[1m[92m   Compiling[0m aws-credential-types v1.3.0
[1m[92m   Compiling[0m aws-types v1.4.0
[1m[92m   Compiling[0m p256 v0.13.2
[1m[92m   Compiling[0m aws-smithy-http-client v1.2.0
[1m[92m   Compiling[0m aws-smithy-observability v0.3.0
[1m[92m   Compiling[0m arc-swap v1.9.2
[1m[92m   Compiling[0m xmlparser v0.13.6
[1m[92m   Compiling[0m aws-sigv4 v1.5.1
[1m[92m   Compiling[0m aws-smithy-runtime v1.12.0
[1m[92m   Compiling[0m aws-smithy-json v0.63.0
[1m[92m   Compiling[0m cobs v0.3.0
[1m[92m   Compiling[0m regex-lite v0.1.9
[1m[92m   Compiling[0m aws-runtime v1.8.1
[1m[92m   Compiling[0m leptos_router v0.8.14
[1m[92m   Compiling[0m leptos_router_macro v0.8.6
[1m[92m   Compiling[0m urlencoding v2.1.3
[1m[92m   Compiling[0m aws-smithy-xml v0.61.1
[1m[92m   Compiling[0m aws-smithy-query v0.61.1
[1m[92m   Compiling[0m aws-sdk-sts v1.108.0
[1m[92m   Compiling[0m aws-sdk-ssooidc v1.105.0
[1m[92m   Compiling[0m leptos_dom v0.8.8
[1m[92m   Compiling[0m leptos_server v0.8.7
[1m[92m   Compiling[0m aws-sdk-sso v1.103.0
[1m[92m   Compiling[0m leptos_meta v0.8.6
[1m[92m   Compiling[0m spin v0.10.0
[1m[92m   Compiling[0m crc-fast v1.10.0
[1m[92m   Compiling[0m aws-config v1.9.0
[1m[92m   Compiling[0m md-5 v0.11.0
[1m[92m   Compiling[0m sha1 v0.11.0
[1m[92m   Compiling[0m rustok-ui-core v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-ui-core)
[1m[92m   Compiling[0m deunicode v1.6.2
[1m[92m   Compiling[0m leptos_integration_utils v0.8.8
[1m[92m   Compiling[0m leptos_axum v0.8.10
[1m[92m   Compiling[0m aws-smithy-checksums v0.65.0
[1m[92m   Compiling[0m aws-sdk-s3 v1.138.0
[1m[92m   Compiling[0m slug v0.1.6
[1m[92m   Compiling[0m rustok-web v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-web)
[1m[92m   Compiling[0m rustok-ui-i18n v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-ui-i18n)
[1m[92m   Compiling[0m rustok-ui-i18n-leptos v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-ui-i18n-leptos)
[1m[92m   Compiling[0m rustok-content v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-content)
[1m[92m   Compiling[0m rustok-seo-targets v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-seo-targets)
[1m[92m   Compiling[0m rustok-graphql v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-graphql)
[1m[92m   Compiling[0m databake-derive v0.2.1
[1m[92m   Compiling[0m databake v0.2.1
[1m[92m   Compiling[0m rustok-ui-transport v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-ui-transport)
[1m[92m   Compiling[0m bytemuck_derive v1.11.0
[1m[92m   Compiling[0m bytemuck v1.25.1
[1m[92m   Compiling[0m rustok-commerce-foundation v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-commerce-foundation)
[1m[92m   Compiling[0m flex v0.1.0 (/home/runner/work/RusTok/RusTok/crates/flex)
[1m[92m   Compiling[0m rustok-taxonomy v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-taxonomy)
[1m[92m   Compiling[0m crunchy v0.2.4
[1m[92m   Compiling[0m tiny-keccak v2.0.2
[1m[92m   Compiling[0m siphasher v1.0.3
[1m[92m   Compiling[0m typeid v1.0.3
[1m[92m   Compiling[0m cookie v0.18.1
[1m[92m   Compiling[0m erased-serde v0.4.10
[1m[92m   Compiling[0m const-random-macro v0.1.16
[1m[92m   Compiling[0m serde_with_macros v3.21.0
[1m[92m   Compiling[0m unic-langid-impl v0.9.6
[1m[92m   Compiling[0m ahash v0.8.12
[1m[92m   Compiling[0m unic-langid v0.9.6
[1m[92m   Compiling[0m const-random v0.1.18
[1m[92m   Compiling[0m serde_with v3.21.0
[1m[92m   Compiling[0m postcard v1.1.3
[1m[92m   Compiling[0m default-struct-builder v0.5.1
[1m[92m   Compiling[0m rustok-inventory v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-inventory)
[1m[92m   Compiling[0m leptos-ui-routing v0.1.0 (/home/runner/work/RusTok/RusTok/crates/leptos-ui-routing)
[1m[92m   Compiling[0m leb128fmt v0.1.0
[1m[92m   Compiling[0m winnow v0.7.15
[1m[92m   Compiling[0m leptos-use v0.19.0
[1m[92m   Compiling[0m crossbeam-deque v0.8.7
[1m[92m   Compiling[0m gloo-timers v0.4.0
[1m[92m   Compiling[0m rustok-channel v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-channel)
[1m[92m   Compiling[0m cfg_aliases v0.2.1
[1m[92m   Compiling[0m wasmtime-internal-core v46.0.1
[1m[92m   Compiling[0m ref-cast v1.0.25
[1m[92m   Compiling[0m dyn-clone v1.0.20
[1m[92m   Compiling[0m google-cloud-wkt v1.6.0
[1m[92m   Compiling[0m rustok-profiles v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-profiles)
[1m[92m   Compiling[0m rustok-storage v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-storage)
[1m[92m   Compiling[0m rustok-media v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-media)
[1m[92m   Compiling[0m ref-cast-impl v1.0.25
[1m[92m   Compiling[0m cranelift-srcgen v0.133.1
[1m[92m   Compiling[0m cranelift-assembler-x64-meta v0.133.1
[1m[92m   Compiling[0m leptos-auth v0.1.0 (/home/runner/work/RusTok/RusTok/crates/leptos-auth)
[1m[92m   Compiling[0m rustok-product v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-product)
[1m[92m   Compiling[0m heapless v0.7.17
[1m[92m   Compiling[0m phf_shared v0.11.3
[1m[92m   Compiling[0m hash32 v0.2.1
[1m[92m   Compiling[0m pulley-macros v46.0.1
[1m[92m   Compiling[0m cranelift-bitset v0.133.1
[1m[92m   Compiling[0m google-cloud-rpc v1.6.0
[1m[92m   Compiling[0m google-cloud-auth v1.14.0
[1m[92m   Compiling[0m cranelift-isle v0.133.1
[1m[92m   Compiling[0m icu_locale_data v2.2.0
[1m[92m   Compiling[0m google-cloud-gax v1.12.0
[1m[92m   Compiling[0m phf_generator v0.11.3
[1m[92m   Compiling[0m pulley-interpreter v46.0.1
[1m[92m   Compiling[0m openssl-sys v0.9.117
[1m[92m   Compiling[0m num-rational v0.4.2
[1m[92m   Compiling[0m google-cloud-gax-internal v0.7.15
[1m[92m   Compiling[0m cranelift-codegen-shared v0.133.1
[1m[92m   Compiling[0m rayon-core v1.13.0
[1m[92m   Compiling[0m target-lexicon v0.13.5
[1m[92m   Compiling[0m cranelift-codegen-meta v0.133.1
[1m[92m   Compiling[0m cranelift-entity v0.133.1
[1m[92m   Compiling[0m cranelift-assembler-x64 v0.133.1
[1m[92m   Compiling[0m icu_plurals_data v2.2.0
[1m[92m   Compiling[0m opentelemetry-semantic-conventions v0.32.1
[1m[92m   Compiling[0m cranelift-bforest v0.133.1
[1m[92m   Compiling[0m cranelift-codegen v0.133.1
[1m[92m   Compiling[0m icu_locale v2.2.0
[1m[92m   Compiling[0m num-complex v0.4.6
[1m[92m   Compiling[0m gimli v0.33.0
[1m[92m   Compiling[0m arbitrary v1.4.2
[1m[92m   Compiling[0m regalloc2 v0.15.1
[1m[92m   Compiling[0m cranelift-control v0.133.1
[1m[92m   Compiling[0m fixed_decimal v0.7.2
[1m[92m   Compiling[0m rustok-pricing v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-pricing)
[1m[92m   Compiling[0m openssl v0.10.81
[1m[92m   Compiling[0m unicode-width v0.2.2
[1m[92m   Compiling[0m foreign-types-shared v0.1.1
[1m[92m   Compiling[0m object v0.39.1
[1m[92m   Compiling[0m foreign-types v0.3.2
[1m[92m   Compiling[0m wasmparser v0.251.0
[1m[92m   Compiling[0m icu_plurals v2.2.0
[1m[92m   Compiling[0m rustok-fulfillment v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-fulfillment)
[1m[92m   Compiling[0m rustok-tax v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-tax)
[1m[92m   Compiling[0m fly v0.1.0 (/home/runner/work/RusTok/RusTok/crates/fly)
[1m[33mwarning[0m[1m: unused import: `Map`[0m
 [1m[94m--> [0mcrates/fly/src/localized_route.rs:8:18
  [1m[94m|[0m
[1m[94m8[0m [1m[94m|[0m use serde_json::{Map, Value};
  [1m[94m|[0m                  [1m[33m^^^[0m
  [1m[94m|[0m
  [1m[94m= [0m[1mnote[0m: `#[warn(unused_imports)]` (part of `#[warn(unused)]`) on by default

[1m[92m   Compiling[0m openssl-macros v0.1.1
[1m[92m   Compiling[0m serde_derive_internals v0.29.1
[1m[92m   Compiling[0m native-tls v0.2.18
[1m[92m   Compiling[0m wasmtime-internal-component-util v46.0.1
[1m[92m   Compiling[0m termcolor v1.4.1
[1m[92m   Compiling[0m wasmprinter v0.251.0
[1m[92m   Compiling[0m schemars_derive v1.2.1
[1m[92m   Compiling[0m rustok-cart v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-cart)
[1m[33mwarning[0m: `fly` (lib) generated 1 warning (run `cargo fix --lib -p fly` to apply 1 suggestion)
[1m[92m   Compiling[0m phf_macros v0.11.3
[1m[92m   Compiling[0m rustok-customer v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-customer)
[1m[92m   Compiling[0m google-cloud-type v1.6.0
[1m[92m   Compiling[0m wasm-encoder v0.251.0
[1m[92m   Compiling[0m rustok-region v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-region)
[1m[92m   Compiling[0m typespec v1.1.0
[1m[92m   Compiling[0m azure_core v1.1.0
[1m[92m   Compiling[0m typespec_macros v1.0.0
[1m[92m   Compiling[0m secrecy v0.10.3
[1m[92m   Compiling[0m equator-macro v0.4.2
[1m[92m   Compiling[0m wasmtime-internal-versioned-export-macros v46.0.1
[1m[92m   Compiling[0m wasmtime-internal-unwinder v46.0.1
[1m[92m   Compiling[0m id-arena v2.3.0
[1m[92m   Compiling[0m equator v0.4.2
[1m[92m   Compiling[0m typespec_client_core v1.1.0
[1m[92m   Compiling[0m wit-parser v0.251.0
[1m[92m   Compiling[0m phf v0.11.3
[1m[92m   Compiling[0m wasmtime-environ v46.0.1
[1m[92m   Compiling[0m google-cloud-iam-v1 v1.11.0
[1m[92m   Compiling[0m schemars v1.2.1
[1m[92m   Compiling[0m google-cloud-location v1.11.0
[1m[92m   Compiling[0m phf_codegen v0.11.3
[1m[92m   Compiling[0m azure_core_macros v1.0.0
[1m[92m   Compiling[0m hashbrown v0.14.5
[1m[92m   Compiling[0m rustok-comments v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-comments)
[1m[92m   Compiling[0m rustok-order v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-order)
[1m[92m   Compiling[0m rustok-tenant v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-tenant)
[1m[92m   Compiling[0m parse-zoneinfo v0.3.1
[1m[92m   Compiling[0m compio-buf v0.8.3
[1m[92m   Compiling[0m smartstring v1.0.1
[1m[92m   Compiling[0m io-uring v0.7.13
[1m[92m   Compiling[0m wasmtime-internal-cranelift v46.0.1
[1m[92m   Compiling[0m wasmtime-internal-component-macro v46.0.1
[1m[92m   Compiling[0m minimal-lexical v0.2.1
[1m[92m   Compiling[0m arrayref v0.3.9
[1m[92m   Compiling[0m nom v7.1.3
[1m[92m   Compiling[0m chrono-tz-build v0.3.0
[1m[92m   Compiling[0m tokio-native-tls v0.3.1
[1m[92m   Compiling[0m wasmtime-internal-wit-bindgen v46.0.1
[1m[92m   Compiling[0m aligned-vec v0.6.4
[1m[92m   Compiling[0m wasmtime v46.0.1
[1m[92m   Compiling[0m cranelift-native v0.133.1
[1m[92m   Compiling[0m cranelift-frontend v0.133.1
[1m[92m   Compiling[0m rustok-page-builder v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-page-builder)
[1m[92m   Compiling[0m rustok-payment v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-payment)
[1m[92m   Compiling[0m num-iter v0.1.46
[1m[92m   Compiling[0m core_maths v0.1.1
[1m[92m   Compiling[0m half v2.7.1
[1m[92m   Compiling[0m synchrony v0.1.8
[1m[92m   Compiling[0m unicode-general-category v1.1.0
[1m[92m   Compiling[0m borrow-or-share v0.2.4
[1m[92m   Compiling[0m syn v1.0.109
[1m[92m   Compiling[0m bit-vec v0.8.0
[1m[92m   Compiling[0m bit-set v0.8.0
[1m[92m   Compiling[0m fluent-uri v0.4.1
[1m[92m   Compiling[0m calendrical_calculations v0.2.4
[1m[92m   Compiling[0m num v0.4.3
[1m[92m   Compiling[0m hyper-tls v0.6.0
[1m[92m   Compiling[0m chrono-tz v0.9.0
[1m[92m   Compiling[0m rustok-blog v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-blog)
[1m[92m   Compiling[0m cron v0.17.0
[1m[92m   Compiling[0m wasmtime-internal-jit-icache-coherence v46.0.1
[1m[92m   Compiling[0m compio-driver v0.11.4
[1m[92m   Compiling[0m wasm-encoder v0.253.0
[1m[33mwarning[0m[1m: method `reserved_refund_amount_in_tx` is never used[0m
   [1m[94m--> [0mcrates/rustok-payment/src/services/payment.rs:690:14
    [1m[94m|[0m
[1m[94m 45[0m [1m[94m|[0m impl PaymentService {
    [1m[94m|[0m [1m[94m-------------------[0m [1m[94mmethod in this implementation[0m
[1m[94m...[0m
[1m[94m690[0m [1m[94m|[0m     async fn reserved_refund_amount_in_tx<C>(
    [1m[94m|[0m              [1m[33m^^^^^^^^^^^^^^^^^^^^^^^^^^^^[0m
    [1m[94m|[0m
    [1m[94m= [0m[1mnote[0m: `#[warn(dead_code)]` (part of `#[warn(unused)]`) on by default

[1m[92m   Compiling[0m icu_pattern v0.4.2
[1m[92m   Compiling[0m memfd v0.6.5
[1m[92m   Compiling[0m thin-vec v0.2.18
[1m[92m   Compiling[0m rhai_codegen v3.2.0
[1m[92m   Compiling[0m pem v3.0.6
[1m[92m   Compiling[0m proc-macro-error-attr v1.0.4
[1m[92m   Compiling[0m httparse v1.10.1
[1m[92m   Compiling[0m base64 v0.13.1
[1m[92m   Compiling[0m utf16_iter v1.0.5
[1m[92m   Compiling[0m ixdtf v0.6.5
[1m[92m   Compiling[0m rustok-commerce v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-commerce)
[1m[92m   Compiling[0m wasmparser v0.239.0
[1m[92m   Compiling[0m micromap v0.3.0
[1m[92m   Compiling[0m write16 v1.0.0
[1m[92m   Compiling[0m referencing v0.46.10
[1m[33mwarning[0m: `rustok-payment` (lib) generated 1 warning
[1m[92m   Compiling[0m icu_calendar v2.2.1
[1m[92m   Compiling[0m jwt v0.16.0
[1m[92m   Compiling[0m rhai v1.25.1
[1m[92m   Compiling[0m icu_decimal v2.2.0
[1m[92m   Compiling[0m wast v253.0.0
[1m[92m   Compiling[0m io_uring_buf_ring v0.2.3
[1m[92m   Compiling[0m reqwest v0.12.28
[1m[92m   Compiling[0m fraction v0.15.4
[1m[92m   Compiling[0m fancy-regex v0.18.0
[1m[33mwarning[0m[1m: function `resolve_return_refund_collection_id` is never used[0m
   [1m[94m--> [0mcrates/rustok-commerce/src/controllers/admin/mod.rs:512:21
    [1m[94m|[0m
[1m[94m512[0m [1m[94m|[0m pub(crate) async fn resolve_return_refund_collection_id(
    [1m[94m|[0m                     [1m[33m^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^[0m
    [1m[94m|[0m
    [1m[94m= [0m[1mnote[0m: `#[warn(dead_code)]` (part of `#[warn(unused)]`) on by default

[1m[33mwarning[0m[1m: function `build_exchange_resolution_return_completion` is never used[0m
   [1m[94m--> [0mcrates/rustok-commerce/src/graphql/mutations/helpers.rs:215:21
    [1m[94m|[0m
[1m[94m215[0m [1m[94m|[0m pub(crate) async fn build_exchange_resolution_return_completion(
    [1m[94m|[0m                     [1m[33m^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^[0m

[1m[33mwarning[0m[1m: function `build_claim_resolution_return_completion` is never used[0m
   [1m[94m--> [0mcrates/rustok-commerce/src/graphql/mutations/helpers.rs:268:21
    [1m[94m|[0m
[1m[94m268[0m [1m[94m|[0m pub(crate) async fn build_claim_resolution_return_completion(
    [1m[94m|[0m                     [1m[33m^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^[0m

[1m[33mwarning[0m[1m: function `attach_return_order_change_context_gql` is never used[0m
   [1m[94m--> [0mcrates/rustok-commerce/src/graphql/mutations/helpers.rs:319:15
    [1m[94m|[0m
[1m[94m319[0m [1m[94m|[0m pub(crate) fn attach_return_order_change_context_gql(
    [1m[94m|[0m               [1m[33m^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^[0m

[1m[33mwarning[0m[1m: variant `TenantId` is never constructed[0m
   [1m[94m--> [0mcrates/rustok-commerce/src/migrations/m20260716_000006_create_return_completion_commands.rs:293:5
    [1m[94m|[0m
[1m[94m290[0m [1m[94m|[0m enum OrderReturns {
    [1m[94m|[0m      [1m[94m------------[0m [1m[94mvariant in this enum[0m
[1m[94m...[0m
[1m[94m293[0m [1m[94m|[0m     TenantId,
    [1m[94m|[0m     [1m[33m^^^^^^^^[0m

[1m[92m   Compiling[0m azure_security_keyvault_secrets v1.0.0
[1m[92m   Compiling[0m azure_identity v1.0.0
[1m[92m   Compiling[0m google-cloud-secretmanager-v1 v1.11.0
[1m[92m   Compiling[0m aws-sdk-secretsmanager v1.109.0
[1m[92m   Compiling[0m uuid-simd v0.8.0
[1m[92m   Compiling[0m olpc-cjson v0.1.4
[1m[92m   Compiling[0m json5 v1.3.1
[1m[92m   Compiling[0m flume v0.12.0
[1m[92m   Compiling[0m wasmi_core v1.1.0
[1m[92m   Compiling[0m jsonschema-regex v0.46.10
[1m[92m   Compiling[0m compio-log v0.1.0
[1m[92m   Compiling[0m version-ranges v0.1.3
[1m[92m   Compiling[0m matrixmultiply v0.3.10
[1m[92m   Compiling[0m priority-queue v2.7.0
[1m[92m   Compiling[0m proc-macro-error v1.0.4
[1m[92m   Compiling[0m num_cpus v1.17.0
[1m[92m   Compiling[0m http-auth v0.1.10
[1m[92m   Compiling[0m bytecount v0.6.9
[1m[92m   Compiling[0m pulldown-cmark v0.13.4
[1m[92m   Compiling[0m num-cmp v0.1.0
[1m[92m   Compiling[0m pastey v0.2.3
[1m[92m   Compiling[0m thin-cell v0.1.2
[1m[92m   Compiling[0m icu_provider_registry v2.2.0
[1m[92m   Compiling[0m jsonschema v0.46.10
[1m[92m   Compiling[0m rustok-secrets v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-secrets)
[1m[92m   Compiling[0m oci-distribution v0.11.0
[1m[92m   Compiling[0m core_affinity v0.8.3
[1m[92m   Compiling[0m pubgrub v0.4.0
[1m[92m   Compiling[0m wasmi_ir v1.1.0
[1m[92m   Compiling[0m leptos_i18n_parser v0.6.2
[1m[92m   Compiling[0m rustok-sandbox v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-sandbox)
[1m[92m   Compiling[0m wat v1.253.0
[1m[92m   Compiling[0m icu_time v2.2.0
[1m[92m   Compiling[0m compio-io v0.9.1
[1m[92m   Compiling[0m icu_casemap v2.2.0
[1m[92m   Compiling[0m ordered-float v2.10.1
[1m[92m   Compiling[0m rustok-forum v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-forum)
[1m[92m   Compiling[0m icu_list v2.2.0
[1m[92m   Compiling[0m rustok-runtime v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-runtime)
[1m[92m   Compiling[0m proc-macro-utils v0.8.0
[1m[92m   Compiling[0m serde_spanned v0.6.9
[1m[92m   Compiling[0m toml_datetime v0.6.11
[1m[92m   Compiling[0m float-cmp v0.9.0
[1m[92m   Compiling[0m rawpointer v0.2.1
[1m[92m   Compiling[0m zip v2.4.2
[1m[92m   Compiling[0m wasmi_collections v1.1.0
[1m[92m   Compiling[0m scoped-tls v1.0.1
[1m[92m   Compiling[0m async-task v4.7.1
[1m[92m   Compiling[0m wasmi v1.1.0
[1m[92m   Compiling[0m compio-runtime v0.11.0
[1m[92m   Compiling[0m strict-num v0.1.1
[1m[92m   Compiling[0m toml_edit v0.22.27
[1m[92m   Compiling[0m ureq-proto v0.6.0
[1m[92m   Compiling[0m manyhow-macros v0.10.4
[1m[92m   Compiling[0m rustok-modules v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-modules)
[1m[92m   Compiling[0m icu_experimental v0.5.0
[1m[33mwarning[0m[1m: unused import: `ModuleArtifactDescriptor`[0m
  [1m[94m--> [0mcrates/rustok-modules/src/data.rs:25:5
   [1m[94m|[0m
[1m[94m25[0m [1m[94m|[0m     ModuleArtifactDescriptor, ModuleInstallationScope, ModuleRuntimeBinding,
   [1m[94m|[0m     [1m[33m^^^^^^^^^^^^^^^^^^^^^^^^[0m
   [1m[94m|[0m
   [1m[94m= [0m[1mnote[0m: `#[warn(unused_imports)]` (part of `#[warn(unused)]`) on by default

[1m[33mwarning[0m: `rustok-commerce` (lib) generated 5 warnings
[1m[92m   Compiling[0m serde-value v0.7.0
[1m[92m   Compiling[0m icu_datetime v2.2.0
[1m[92m   Compiling[0m leptos_i18n_codegen v0.6.2
[1m[92m   Compiling[0m icu_collator v2.2.1
[1m[92m   Compiling[0m zopfli v0.8.3
[1m[33mwarning[0m[1m: unused import: `TryGetable`[0m
 [1m[94m--> [0mcrates/rustok-modules/src/binding_idempotency.rs:5:82
  [1m[94m|[0m
[1m[94m5[0m [1m[94m|[0m     ConnectionTrait, DatabaseConnection, DbBackend, Statement, TransactionTrait, TryGetable,
  [1m[94m|[0m                                                                                  [1m[33m^^^^^^^^^^[0m

[1m[33mwarning[0m[1m: unused import: `TryGetable`[0m
  [1m[94m--> [0mcrates/rustok-modules/src/schedule_materializer.rs:15:5
   [1m[94m|[0m
[1m[94m15[0m [1m[94m|[0m     TryGetable, Value as SqlValue,
   [1m[94m|[0m     [1m[33m^^^^^^^^^^[0m

[1m[33mwarning[0m[1m: unused import: `TryGetable`[0m
  [1m[94m--> [0mcrates/rustok-modules/src/event_delivery.rs:12:5
   [1m[94m|[0m
[1m[94m12[0m [1m[94m|[0m     TryGetable, Value as SqlValue,
   [1m[94m|[0m     [1m[33m^^^^^^^^^^[0m

[1m[33mwarning[0m[1m: unused import: `TryGetable`[0m
  [1m[94m--> [0mcrates/rustok-modules/src/schedule_delivery.rs:13:5
   [1m[94m|[0m
[1m[94m13[0m [1m[94m|[0m     TryGetable, Value as SqlValue,
   [1m[94m|[0m     [1m[33m^^^^^^^^^^[0m

[1m[92m   Compiling[0m rustok-pages v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-pages)
[1m[92m   Compiling[0m icu_segmenter v2.2.0
[1m[33mwarning[0m[1m: unused variable: `release`[0m
   [1m[94m--> [0mcrates/rustok-modules/src/dispatcher.rs:214:48
    [1m[94m|[0m
[1m[94m214[0m [1m[94m|[0m         let ModuleDefinitionSource::Artifact { release } = &definition.source else {
    [1m[94m|[0m                                                [1m[33m^^^^^^^[0m [1m[33mhelp: try ignoring the field: `release: _`[0m
    [1m[94m|[0m
    [1m[94m= [0m[1mnote[0m: `#[warn(unused_variables)]` (part of `#[warn(unused)]`) on by default

[1m[92m   Compiling[0m server_fn_macro v0.6.15
[1m[92m   Compiling[0m darling_core v0.21.3
[1m[92m   Compiling[0m filetime v0.2.29
[1m[92m   Compiling[0m nom v8.0.0
[1m[33mwarning[0m[1m: unused variable: `config_size`[0m
   [1m[94m--> [0mcrates/rustok-modules/src/oci.rs:632:17
    [1m[94m|[0m
[1m[94m632[0m [1m[94m|[0m             let config_size = config.len() as u64;
    [1m[94m|[0m                 [1m[33m^^^^^^^^^^^[0m [1m[33mhelp: if this is intentional, prefix it with an underscore: `_config_size`[0m

[1m[92m   Compiling[0m csv-core v0.1.13
[1m[92m   Compiling[0m utf8-zero v0.8.1
[1m[92m   Compiling[0m ciborium-io v0.2.2
[1m[92m   Compiling[0m crlify v1.0.5
[1m[92m   Compiling[0m rmcp v1.8.0
[1m[92m   Compiling[0m fixedbitset v0.5.7
[1m[92m   Compiling[0m petgraph v0.8.3
[1m[92m   Compiling[0m icu_provider_baked v2.2.0
[1m[92m   Compiling[0m ciborium-ll v0.2.2
[1m[92m   Compiling[0m ureq v3.3.0
[1m[92m   Compiling[0m csv v1.4.0
[1m[92m   Compiling[0m tar v0.4.46
[1m[92m   Compiling[0m darling_macro v0.21.3
[1m[92m   Compiling[0m icu v2.2.0
[1m[92m   Compiling[0m ndarray v0.16.1
[1m[92m   Compiling[0m alloy v0.1.0 (/home/runner/work/RusTok/RusTok/crates/alloy)
[1m[33mwarning[0m[1m: unused imports: `RhaiBindingInput` and `RhaiBindingOutput`[0m
 [1m[94m--> [0mcrates/alloy/src/bridge/http.rs:6:60
  [1m[94m|[0m
[1m[94m6[0m [1m[94m|[0m     CapabilityCall, CapabilityCallContext, CapabilityName, RhaiBindingInput, RhaiBindingOutput,
  [1m[94m|[0m                                                            [1m[33m^^^^^^^^^^^^^^^^[0m  [1m[33m^^^^^^^^^^^^^^^^^[0m
  [1m[94m|[0m
  [1m[94m= [0m[1mnote[0m: `#[warn(unused_imports)]` (part of `#[warn(unused)]`) on by default

[1m[33mwarning[0m[1m: unused import: `crate::artifact::RHAI_MODULE_ABI`[0m
  [1m[94m--> [0mcrates/alloy/src/bridge/http.rs:11:5
   [1m[94m|[0m
[1m[94m11[0m [1m[94m|[0m use crate::artifact::RHAI_MODULE_ABI;
   [1m[94m|[0m     [1m[33m^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^[0m

[1m[92m   Compiling[0m pulldown-cmark-to-cmark v22.0.0
[1m[92m   Compiling[0m rayon v1.12.0
[1m[92m   Compiling[0m syn_derive v0.1.8
[1m[92m   Compiling[0m serde-aux v4.7.0
[1m[92m   Compiling[0m manyhow v0.10.4
[1m[92m   Compiling[0m toml v0.8.23
[1m[92m   Compiling[0m tiny-skia-path v0.11.4
[1m[92m   Compiling[0m icu_codepointtrie_builder v0.6.0
[1m[92m   Compiling[0m fastbloom v0.17.0
[1m[92m   Compiling[0m elsa v1.11.2
[1m[92m   Compiling[0m rmcp-macros v1.8.0
[1m[92m   Compiling[0m kurbo v0.9.5
[1m[92m   Compiling[0m rand_pcg v0.10.2
[1m[92m   Compiling[0m inout v0.1.4
[1m[92m   Compiling[0m toml_write v0.1.2
[1m[92m   Compiling[0m multimap v0.10.1
[1m[92m   Compiling[0m lru-slab v0.1.2
[1m[92m   Compiling[0m indoc v2.0.7
[1m[92m   Compiling[0m twox-hash v2.1.2
[1m[92m   Compiling[0m bitflags v1.3.2
[1m[92m   Compiling[0m parse-zoneinfo v0.5.0
[1m[92m   Compiling[0m siphasher v0.3.11
[1m[92m   Compiling[0m svgtypes v0.12.0
[1m[92m   Compiling[0m icu_provider_source v2.2.0
[1m[92m   Compiling[0m rig-derive v0.40.0
[1m[33mwarning[0m: `rustok-modules` (lib) generated 7 warnings (run `cargo fix --lib -p rustok-modules` to apply 3 suggestions)
[1m[92m   Compiling[0m quinn-proto v0.11.16
[1m[92m   Compiling[0m prost-build v0.14.4
[1m[92m   Compiling[0m cipher v0.4.4
[1m[33mwarning[0m: `alloy` (lib) generated 2 warnings (run `cargo fix --lib -p alloy` to apply 2 suggestions)
[1m[92m   Compiling[0m icu_provider_export v2.2.0
[1m[92m   Compiling[0m attribute-derive-macro v0.9.2
[1m[92m   Compiling[0m rstml v0.11.2
[1m[92m   Compiling[0m darling v0.21.3
[1m[92m   Compiling[0m rustok-seo v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-seo)
[1m[92m   Compiling[0m ciborium v0.2.2
[1m[92m   Compiling[0m server_fn_macro_default v0.6.15
[1m[92m   Compiling[0m compio-net v0.11.1
[1m[92m   Compiling[0m ext-trait-proc_macros v1.0.1
[1m[92m   Compiling[0m v_frame v0.3.9
[1m[92m   Compiling[0m eventsource-stream v0.2.3
[1m[92m   Compiling[0m dashmap v5.5.3
[1m[92m   Compiling[0m compio-quic v0.7.2
[1m[92m   Compiling[0m compio-fs v0.11.0
[1m[92m   Compiling[0m wasm-streams v0.4.2
[1m[92m   Compiling[0m rustok-search v0.1.0 (/home/runner/work/RusTok/RusTok/crates/rustok-search)
[1m[92m   Compiling[0m password-hash v0.5.0
[1m[92m   Compiling[0m serde-wasm-bindgen v0.6.5
[1m[92m   Compiling[0m tonic-build v0.14.6
[1m[92m   Compiling[0m futures-rustls v0.26.0
[1m[92m   Compiling[0m simple_asn1 v0.6.4
[1m[92m   Compiling[0m serde_qs v0.12.0
[1m[91merror[E0382][0m[1m: use of moved value: `service`[0m
   [1m[94m--> [0mcrates/rustok-seo/src/services/services_base.rs:147:24
    [1m[94m|[0m
[1m[94m143[0m [1m[94m|[0m         let service = Self::new(db, event_bus, registry);
    [1m[94m|[0m             [1m[94m-------[0m [1m[94mmove occurs because `service` has type `SeoService`, which does not implement the `Copy` trait[0m
[1m[94m...[0m
[1m[94m146[0m [1m[94m|[0m             .map(|provider| service.with_media_asset_read_port(provider.port()))
    [1m[94m|[0m                  [1m[94m----------[0m [1m[94m-------[0m [1m[94mvariable moved due to use in closure[0m
    [1m[94m|[0m                  [1m[94m|[0m
    [1m[94m|[0m                  [1m[94mvalue moved into closure here[0m
[1m[94m147[0m [1m[94m|[0m             .unwrap_or(service))
    [1m[94m|[0m                        [1m[91m^^^^^^^[0m [1m[91mvalue used here after move[0m
    [1m[94m|[0m
[1m[96mhelp[0m: consider cloning the value if the performance cost is acceptable
    [1m[94m|[0m
[1m[94m146[0m [1m[94m| [0m            .map(|provider| service[92m.clone()[0m.with_media_asset_read_port(provider.port()))
    [1m[94m|[0m                                    [92m++++++++[0m

[1mFor more information about this error, try `rustc --explain E0382`.[0m
[1m[91merror[0m: could not compile `rustok-seo` (lib) due to 1 previous error
[1m[33mwarning[0m: build failed, waiting for other jobs to finish...
```
