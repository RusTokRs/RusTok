---
id: doc://docs/operations/script-script-command-makefile-target-dev-start.md
kind: operations_documentation
language: en
source_language: en
entities:
  - script-command://Makefile#target:dev-start
last_verified_snapshot: snap_jsonl_00000021
status: verified
---

# Script Command `make dev-start`

## Purpose

Document when this operational item is used, who owns it, and what successful execution or operation means.

## Contract

- Canonical entity: `script-command://Makefile#target:dev-start`
- Entity kind: `script_command`
- Source: `Makefile`:42

## Procedure

1. Review the source definition and confirm prerequisites.
2. Execute or operate this item according to the project runbook.
3. Record outcomes, rollback notes, and follow-up actions.

## Evidence

- `Makefile:42`

## Notes

Generated from diagnostic `diag_script_7d34734c3d4ddfe1`. Review this page before relying on it as operational documentation.
