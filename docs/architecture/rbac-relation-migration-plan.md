...
- [~] **Фаза 5 — Dual-read и cutover (частично):**
  ...
  - Добавлен gate helper `scripts/rbac_cutover_gate.sh` для финальной проверки обязательных артефактов (staging pre/dry-run/apply/rollback/post-rollback + cutover baseline + auth release gate report) и fail-fast в случае `gate_status != pass`, ненулевых `mismatch_delta/shadow_compare_failures_delta` или ненулевых post-rollback инвариантов (`users_without_roles_total`, `orphan_user_roles_total`, `orphan_role_permissions_total`); helper покрыт тестами (`scripts/tests/rbac_cutover_gate_test.sh` + smoke `scripts/test_rbac_cutover_gate.sh`).
