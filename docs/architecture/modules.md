...
- Module migration ordering фиксируется descriptor contract: module crates экспортируют `migration_dependencies()` рядом с `migrations()` и возвращают его через `MigrationSource::migration_dependencies()`, server migrator агрегирует descriptors через module contract для всех module crates, чьи migrations включены в server migrator, делает topological sort, сохраняет deterministic lexical tie-breaker для независимых migrations и падает на missing dependency/cycle.
