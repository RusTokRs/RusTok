...
Tenant enable/disable must go through `ModuleLifecycleService::toggle_module()` with an authenticated actor UUID, non-nil idempotency UUID, and reviewed static lifecycle revision: the operation journal
is written before changing tenant state, including an explicit no-op intent, compat `on_enable`/`on_disable` hooks are executed as pre-hooks, and the successful state
change and operation transition to `committed` are fixed in one commit. Static settings and recovery share the same tenant/module lifecycle aggregate, so their revision cannot race enablement. An exact idempotency retry returns that original operation. GraphQL and Leptos SSR/admin surfaces
do not own lifecycle taxonomy or journal metadata: the server GraphQL mapper publishes canonical error codes
(`BAD_USER_INPUT`, `IDEMPOTENCY_CONFLICT`, `MODULE_HOOK_FAILED`, `INTERNAL_ERROR`) and recovery/journal fields, while Leptos SSR/admin
...
- Tenant module lifecycle has owner-owned toggle, settings, and recovery entrypoints with authenticated actor, idempotency, and expected-revision identities. They share `module_static_tenant_lifecycle`, whose inherited/default revision is zero and whose execution claim fails concurrent commands closed. Direct model-level writes and admin-side SQL/bypass are not contract surfaces.
- Lifecycle journal uses statuses `validated/running/committed/failed`; it binds a static toggle correlation to its idempotency UUID, commits an explicit no-op receipt, and post-hook failure does not roll back committed tenant state but creates a failed operation with recovery metadata (`status`, `issue`, `retryable`, `recommended_action`, `correlation_id`, `requested_by`, `error_message`).
- Recovery is performed only through canonical GraphQL/service surface: `moduleOperationRecoveryPlan`, `failedModuleOperationRecoveryPlans`, `retryFailedModuleOperationPostHook`, `compensateFailedModuleOperation`. The authenticated transport derives tenant and actor UUIDs and supplies a non-nil idempotency UUID plus expected aggregate revision to the typed owner recovery command; it never provides actor display text. Compensation is allowed only for `post_hook_failed` operations when the current effective state still matches the committed requested state.
- GraphQL mapper owns error taxonomy (`BAD_USER_INPUT`, `IDEMPOTENCY_CONFLICT`, `REVISION_CONFLICT`, `MODULE_HOOK_FAILED`, `INTERNAL_ERROR`); Leptos SSR/admin layers must pass through and must not remap taxonomy/journal/recovery fields. `updateModuleSettings` and reviewed `compareAndSwapModuleSettings` use the same revision/receipt contract and return the owner-issued next revision.
