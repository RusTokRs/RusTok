...
- storefront shipping discovery, cart context patch и checkout также используют shipping profile compatibility: product/shipping option storage по-прежнему нормализуется в metadata-backed `shipping_profile.slug` и `shipping_profiles.allowed_slugs`, но source of truth теперь вынесен в typed registry `shipping_profiles`.
