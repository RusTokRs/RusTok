...
- `npm run verify:i18n:ui` and `npm run verify:storefront:routes` are part of the live contract; docs and code must not describe i18n parity or locale-aware generic routing as informal/manual conventions.
...
- Remaining gaps are now limited to outbound email/template flows beyond the built-in auth mailers and to future locale expansion work; the package-owned UI migration wave is closed, including the `ai admin` surface.
- The canonical residual scope after closing the native i18n/UI migration wave now lives in `docs/concepts/platform-remediation-execution-plan.md`; agents should start from that execution plan instead of reopening package-owned UI migration as pending work.
