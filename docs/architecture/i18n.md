...
- Password reset outbound mail now also consumes the effective request locale: GraphQL uses the request `Locale`, REST uses `RequestContext.locale`, and both `loco` and `smtp` providers render the same localized built-in templates.
...
- Remaining gaps are now limited to other outbound email/template flows and to future locale expansion work; the package-owned UI migration wave is closed, including the `ai admin` surface.
