...
- ✅ **Performance** - Base rows stay compact while localized reads remain indexable
...
- **Regional**: BCP 47 format supported (e.g., `en-US`, `pt-BR`, `zh-Hant`, `es-419`)
- **Storage**: `VARCHAR(32)` for canonical locale columns in live platform-controlled tables
- **Validation**: normalized BCP-47-style tags; RusToK runtime accepts language, region,
  script, and numeric region subtags used by current modules
...
### 1.1 Canonical Storage Shape

For multilingual business data, the canonical target shape is:

- base entity row with only language-agnostic fields;
- `*_translations` rows for localized short text;
- `*_bodies` rows for localized heavy content when the content payload is large or format-aware;
- tenant locale policy stored separately from entity content.

This is the normative storage direction for new work. Legacy mixed patterns can exist during migration, but they are not the target architecture.
...
- `tenant.default_locale` and `tenant_locales` are policy-layer records, not storage-shape switches
- Fallback chain: `requested → default → 'en'`
- Services handle locale resolution, not database
- Allows dynamic locale switching

### 4. Runtime Locale Negotiation Contract

Current server-side negotiation order for HTTP requests:

1. `locale` query parameter
2. `x-medusa-locale` header
3. locale cookie (currently `rustok-admin-locale` for Next Admin)
4. `Accept-Language` header
5. `tenant.default_locale`
6. final fallback: `en`

### 4.1 Normative Guardrails

The following rules are normative for all hosts, module-owned UI packages, and agents working in this repository:

- `RequestContext.locale` is the canonical server/runtime source of truth for HTTP locale selection.
- Leptos hosts must propagate the effective UI locale through `UiRouteContext.locale`; module-owned Leptos packages must consume that value instead of inventing package-local locale selection.
- Next hosts and package-owned Next surfaces must consume the host/runtime locale provider and must not introduce an alternative query/header/cookie fallback chain.
- `[provides.*_ui.i18n]` declares where package-owned translation bundles live; it does **not** define a separate locale negotiation contract.
- Package-owned bundles may extend visible copy, but they must not override the platform fallback order, `Content-Language`, or host-level locale persistence rules.
- `npm run verify:i18n:ui`, `npm run verify:i18n:contract`, and `npm run verify:storefront:routes` are part of the live contract; docs and code must not describe i18n parity or locale-aware generic routing as informal/manual conventions.

Notes:

- `apps/server` now applies the same fallback semantics to HTTP read-endpoints that previously hardcoded `"en"`.
- HTTP list/read endpoints now treat omitted `locale` as `RequestContext.locale`, so the effective chain remains `request -> tenant default -> platform fallback`.
- Domain read/list services now reuse the same chain when a caller supplies `fallback_locale`, instead of immediately defaulting to `"en"` when `locale` is absent.
- GraphQL queries in `apps/server` now reuse the same request locale from `RequestContext` when the locale argument/filter is omitted.
- Canonical request-level locale resolution now lives in `apps/server/src/common/context.rs`; `middleware/locale.rs` remains a compatibility layer for handlers that only need `Extension<Locale>`.
- Shared locale normalization, validation, tag matching, and candidate building for the safe migration scope now live in `rustok-core::locale`; `rustok-api`, `apps/server` manifest i18n wiring, `rustok-content`, `rustok-profiles`, and `rustok-ai` reuse that helper instead of maintaining divergent package-local implementations.
- Foundation locale storage now follows the same contract: `tenants.default_locale` and `tenant_locales.{locale,fallback_locale}` use a platform-safe width for the live BCP47-like locale format instead of a short `xx-YY`-only assumption.
- Detail API responses for blog/forum content now expose `requested_locale` separately from `effective_locale`; legacy `locale` fields remain as the requested locale for backward compatibility.
- `pages` detail responses now follow the same contract on locale-aware read paths (`get_by_slug`, GraphQL `page`/`page_by_slug`): selected translation/body are resolved explicitly and the response exposes `requested_locale`, `effective_locale`, and `available_locales`.
- Generic GraphQL content detail (`content.node`) now also accepts optional `locale` and exposes the selected `translation`/`body` alongside raw `translations`/`bodies`, keeping the low-level payload while making fallback behavior explicit.
- Locale tags are normalized to a BCP47-like form (`ru`, `en`, `ru-RU`, `pt-BR`) and validated before entering request context.
- `RequestContext.locale` represents the effective locale selected by this chain.
- `rustok-ai` reuses the same chain when session/task-job locale is omitted: admin/operator
  surfaces pass `locale` as an optional override, so empty locale falls back to the effective
  request locale first, then tenant default locale, then platform fallback.
- Next Admin, Next Frontend, and the Leptos admin host now align on the same platform fallback locale (`en`); package-owned Next features must read locale from the host/runtime provider instead of introducing feature-level defaults such as hardcoded `'en'`.
- Storefront host routing now supports locale-prefixed routes (`/{locale}` and `/{locale}/modules/{route_segment}`) with backward-compatible fallback to the legacy `?lang=` query parameter.
- Built-in auth outbound mail now also consumes the effective request locale: GraphQL password reset uses the request `Locale`, REST password reset and email verification use `RequestContext.locale`, and both `loco` and `smtp` providers render the same localized built-in auth templates.
- Module-owned UI packages can now opt into a manifest-level translation bundle contract via `[provides.*_ui.i18n]`, which records `default_locale`, `supported_locales`, and explicit `locales/*.json` / `messages/*.json` paths for package-owned bundles.
- Leptos admin host now also propagates the selected UI locale into `UiRouteContext.locale`, so module-owned admin crates read the same effective locale instead of inventing package-local language selection logic.
- The first module-owned Leptos admin packages (`workflow`, `rbac`, `tenant`, `index`, `outbox`, `pages`, `comments`, `channel`, `forum`, `search`, `commerce`) now ship `admin/locales/en.json` + `admin/locales/ru.json` bundles and resolve visible chrome strings from those package-owned bundles.
- The capability-owned Leptos admin package `rustok-ai-admin` now follows the same native i18n runtime contract through package-owned `admin/locales/*.json` bundles and `UiRouteContext.locale`, even though it is not declared through `rustok-module.toml`.
- `flex` is part of the same multilingual cleanup: standalone schema copy now belongs in parallel localized records instead of inline base-row fields, while attached-mode field-definition locale maps and localized entry-value storage remain follow-up migration work rather than approved end-state patterns.
- Module-owned storefront packages `blog`, `pages`, `commerce`, `forum`, and `search` now also ship `storefront/locales/en.json` + `storefront/locales/ru.json` bundles and resolve their visible chrome and empty-state copy from package-owned bundles instead of hardcoded English literals.
- Module-owned Leptos storefront packages now preserve the effective locale in their internal generic page links through `UiRouteContext::module_route_base()`, so locale-prefixed host routes remain canonical while legacy `/modules/{route_segment}` links stay backward-compatible.
- The route-side i18n contract for module-owned storefront packages is now machine-checked through `npm run verify:storefront:routes`, complementing bundle parity checks from `npm run verify:i18n:ui`.
- Remaining gaps are now limited to outbound email/template flows beyond the built-in auth mailers, future locale expansion work, and the deferred ecommerce locale alignment that remains intentionally out of scope while ecommerce modules are under active development.
- The residual scope after closing the native i18n/UI migration wave is now documented directly in the live docs: this i18n contract and `apps/server/docs/loco-core-integration-plan.md`; agents should update those documents instead of reopening package-owned UI migration as pending work.
...
### 4.2 HTTP locale runtime contract (April 2026)

Current request-level locale order is:

1. `locale` query parameter
2. `x-medusa-locale` header
3. locale cookie (`rustok-admin-locale`)
4. `Accept-Language` with q-values
5. `tenant.default_locale`
6. fallback `en`

Additional runtime rules:

- `RequestContext` is the canonical HTTP locale source of truth.
- `apps/server/src/middleware/locale.rs` is a transport adapter that enriches extensions and emits `Content-Language`.
- `tenant_locales` may constrain the final effective locale by filtering disabled locales and applying tenant default/fallback choices.
