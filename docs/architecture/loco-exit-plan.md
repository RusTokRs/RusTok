...
- Last checkpoint: introduced `rustok_api::HostRuntimeContext`, `apps/server` passes it into Leptos `#[server]` functions; `rustok-index-admin` and `rustok-outbox-admin` no longer import `loco_rs::app::AppContext`; `rustok-content-orchestration` no longer imports Loco and receives DB/event bus/GraphQL data handles explicitly; Alloy runtime core is built from explicit `DatabaseConnection`, does not import Loco, registers through `ServerRuntimeContext`, Alloy GraphQL reads `SharedAlloyRuntime` from schema data, and Alloy HTTP handlers use `AlloyHttpRuntime`; `rustok-ai` no longer depends on Loco and uses its own `AiHostRuntime` for GraphQL/service/direct/MCP execution; `rustok-commerce` storefront checkout orchestration uses `StorefrontCheckoutRuntime` with explicit DB/event bus handles, and shared/admin product, storefront product/catalog/order/cart/checkout, admin order/change/return, admin fulfillment, admin shipping and admin payment HTTP handlers use `CommerceHttpRuntime`; remaining commerce transport adapters are still Loco-boundary until next slices; `rustok-blog` HTTP post/comment handlers use a narrow `BlogHttpRuntime`, `rustok-pages` HTTP handlers use `PagesHttpRuntime`, `rustok-forum` REST handlers use `ForumHttpRuntime`, `rustok-media` HTTP handlers use `MediaHttpRuntime`, `rustok-workflow` HTTP/webhook handlers use `WorkflowHttpRuntime`, and `rustok-seo` HTTP handlers use `SeoHttpRuntime`; Loco `AppContext` in these modules remains only in controller state adapters until full Axum route cutover; `scripts/verify/verify-loco-inventory.mjs` classifies remaining Loco entrypoints; ADR `2026-07-02-axum-runtime-and-ops-cli-boundary` accepted; `SettingsService`, event bus, event transport, module dispatcher/runtime extensions, email, app runtime helpers, rate-limit bootstrap, GraphQL schema service, worker lifecycle orchestration, auth lifecycle providers, build/release/runtime guardrail services migrated to `Serve... (line truncated to 2000 chars)
...
| Leptos server-function context | Migration to `rustok_api::HostRuntimeContext` started; `index/outbox/tenant/region/comments` admin already migrated | `rustok-api` + server context provider |
...
Current classified inventory baseline after the `rustok-comments-admin` native transport cutover:
...
| `module_ui_adapter` | 183 | Largest remaining non-core surface: module-owned Leptos/native adapters still reading `AppContext`. |
...
| `dependency_manifest` / `lockfile` | 104 | Cleanup after code paths stop requiring `loco-rs` and `loco-adapter`. |
| `verification_guard` / `docs` / `scaffold_template` | 222 | Guardrails, historical docs and generated templates to update/archive last. |
...
Expected execution size: roughly 25-40 focused cutover slices if each slice migrates one coherent runtime or package boundary and updates guardrails/docs in the same change.
...
- [x] Migrate first module UI adapters (`rustok-index-admin`, `rustok-outbox-admin`, `rustok-tenant-admin`, `rustok-region-admin`, `rustok-comments-admin`) from Loco context.
...
- [x] Narrow `rustok-blog` HTTP controller state: post/comment handlers accept `BlogHttpRuntime` and build services from explicit DB/event bus handles; current Loco `AppContext` remains only in `FromRef` state adapter until the general Axum route cutover.
- [x] Narrow `rustok-pages` HTTP controller state: page/block handlers accept `PagesHttpRuntime` and do not use `rustok_outbox::loco`; current Loco `AppContext` remains only in `FromRef` state adapter until the general Axum route cutover.
- [x] Narrow `rustok-forum` REST controller state: category/topic/reply/user/widget handlers accept `ForumHttpRuntime`, topic/reply services receive event bus without `rustok_outbox::loco`, and current Loco `AppContext` remains only in `FromRef` state adapter until the general Axum route cutover.
- [x] Narrow `rustok-media` HTTP controller state: upload/list/get/delete/translation handlers accept `MediaHttpRuntime` with explicit DB/storage handles; current Loco `AppContext` remains only in `FromRef` state adapter until the general Axum route cutover.
- [x] Narrow `rustok-workflow` HTTP controller state: workflow/step/execution/webhook handlers accept `WorkflowHttpRuntime` with explicit DB handle; current Loco `AppContext` remains only in `FromRef` state adapter until the general Axum route cutover.
- [x] Narrow `rustok-seo` HTTP controller state: REST handlers accept `SeoHttpRuntime` with explicit DB/event bus/runtime extensions handles and do not use `rustok_outbox::loco`; current Loco `AppContext` remains only in `FromRef` state adapter until the general Axum route cutover.
...
- [ ] Replace Loco initializers with ordered bootstrap phases.
