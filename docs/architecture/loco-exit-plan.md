...
| Leptos server-function context | Migration to `rustok_api::HostRuntimeContext` started; `index/outbox/tenant/region/comments/workflow` admin already migrated | `rustok-api` + server context provider |
...
Current classified inventory baseline after the `rustok-workflow-admin` native transport cutover:
...
| `module_ui_adapter` | 181 | Largest remaining non-core surface: module-owned Leptos/native adapters still reading `AppContext`. |
...
| `dependency_manifest` / `lockfile` | 101 | Cleanup after code paths stop requiring `loco-rs` and `loco-adapter`. |
| `verification_guard` / `docs` / `scaffold_template` | 229 | Guardrails, historical docs and generated templates to update/archive last. |
