...
- [~] Migrate seed profiles to `rustok-cli seed`: canonical profile parsing and seed sequencing belong to `rustok-installer`; effective module-set calculation now belongs to `rustok-modules`, while the server remains a manifest/override loader; tenant provisioning uses the owner-owned `TenantService::ensure_tenant` API, identity provisioning uses `AuthUserBootstrapDbWriter`, and RBAC role assignment uses `RbacRoleAssignmentDbWriter` with host cache invalidation. Module lifecycle persistence, dependency checks, hooks and operation journaling remain the final owner-specific extraction before the typed CLI provider can replace the remaining Loco seed hook.
