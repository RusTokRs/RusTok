...
| Digital/physical fulfillment requirement | Product publishes a typed requirement; Cart/Order snapshot it; Commerce skips shipping/Fulfillment for digital lines and limits Fulfillment to physical plans | Product owns product kind; Cart/Order own immutable line snapshots; Fulfillment owns shipping execution | Implemented through Product/Cart/Order typed snapshots and checkout plan validation; remaining work is owner-port parity outside the typed checkout path |
