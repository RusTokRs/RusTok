...
Этот документ — каноническая входная точка для разработчика и AI-агента, который
пишет новый модуль или расширяет существующий. Он не дублирует архитектурные
документы построчно, а фиксирует практический contract: что считать модулем,
как не сломать platform boundaries и в какие документы смотреть дальше.

Если нужен один короткий ответ на вопрос «с чего начать», то он такой:

1. Определите ownership и runtime-роль модуля.
2. Соберите backend по platform contract.
3. Только после этого добавляйте UI-поверхности как module-owned package.
4. Обновите local docs модуля и central docs.
...
Support/crate/capability слой может жить рядом с модулем, но это не делает его
tenant-toggled модулем автоматически. Это особенно важно для `rustok-core`,
`rustok-api`, `rustok-storage`, `rustok-mcp`, `rustok-ai`, `flex` и похожих слоёв.
...
- `rustok-module.toml` с корректными `module.slug`, `module.version`,
  `module.description`, `module.ui_classification`;
...
- transport ownership идёт через `apps/server`, но business/domain contract
  остаётся у модуля;
- Leptos `#[server]` — default internal data layer для Leptos surfaces, но
  GraphQL остаётся параллельно;
- REST нужен для явного HTTP contract: integrations, webhooks, ops, module-owned
  routes;
...
### 4. Не выносите platform правила в строки и ad-hoc JSON
...
- держать canonical read contract в произвольном `details` JSON, если уже есть
  typed schema;
- смешивать public contract и internal audit storage.

Если нужен actor/principal/read-model, делайте typed contract, а не строковые
эвристики.
...
- Next.js surfaces — через соответствующие host packages, но ownership UI
  contract всё равно остаётся у модуля;
...
- нельзя заменять уже существующий GraphQL только потому, что появился
  `#[server]` path.
...
Сам факт существования `admin/` или `storefront/` каталога ещё не означает, что
surface интегрирован корректно. Канонический source of truth здесь — manifest.
...
4. UI package docs и host docs обновлены, если поменялся contract
...
- хранить локализуемый business text прямо в base rows, если модуль уже идёт по
...
Если агенту или разработчику нужно быстро принять решение, используйте такой
порядок:
...
Этот checklist нужен для любого нового модуля, крупного module refactor или
изменения module contract. Его можно прогонять перед PR или во время review.
...
4. Обновлены central docs в `docs/`, если поменялся platform/module/API/i18n/storage contract.
5. `docs/index.md` обновлён, если появился новый canonical doc или изменился входной путь.
6. Документация описывает фактическое состояние кода, а не план «на потом».

## Связанные документы

- [Обзор модульной платформы](./overview.md)
- [Архитектура модулей](../architecture/modules.md)
- [Контракт `modules.toml` и `rustok-module.toml`](./manifest.md)
- [Схема данных платформы](../architecture/database.md)
