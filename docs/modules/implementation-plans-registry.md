...
| `rustok-inventory` | `rustok-inventory` | `crates/rustok-inventory/docs/implementation-plan.md` | `in_progress` | `55%` | `agent` | `2026-06-06T00:00:00Z` | `Wave 5 inventory admin boundary: backend AdminInventoryReadService/DTO + native read server functions live; set/adjust quantity write endpoints use inventory-owned native facade with typed InventoryQuantityWriteResult plus backend inStock derivation/wire-shape evidence; transitional commerce GraphQL adapter is package-private read-only fallback with removal criteria and boundary coverage` | Перевести следующий remaining inventory write mutation из umbrella `rustok-commerce` на inventory-owned native/API facade и добавить targeted mutation semantics test | Первый CI migration-smoke прогон ещё нужно наблюдать; remaining inventory mutation parity не завершён | `cargo test -p rustok-inventory --lib`; `cargo test -p rustok-inventory-admin --test boundary` |
