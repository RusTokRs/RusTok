...
| `ai` | [plan](../../crates/rustok-ai/docs/implementation-plan.md) | `in_progress` | Complete Rig protocol/live evidence and approval/stream recovery tests; typed dispatch, provider-target selection, cancellation sequencing, and stream usage/tool-call mapping are implemented. Wait for the platform-owned generic host contribution before claiming the AI boundary complete. |
