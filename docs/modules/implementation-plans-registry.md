...
| `rustok-ai` | `rustok-ai` | `crates/rustok-ai/docs/implementation-plan.md` | `in_progress` | `82%` | `agent` | `2026-06-20T00:00:00Z` | `AI ownership evidence hardening: media runtime fallback smoke and alloy policy registry added while runtime composition stays in rustok-ai` | Continue executable targeted tests/evidence when compilation is allowed | compile/test evidence отложен по явному ограничению итерации: без компиляций | `npm run verify:ai-media:fba`; `npm run verify:ai-alloy:policy`; `cargo test -p rustok-ai --lib` |
| `rustok-ai-content` | `rustok-ai-content` | `crates/rustok-ai-content/docs/implementation-plan.md` | `in_progress` | `58%` | `agent` | `2026-06-21T00:00:00Z` | `content support crate now owns content moderation/blog draft policy matrix and rustok-ai consumes moderation sensitive-tool defaults for approval routing` | Расширить blog contract тестами и добавить targeted verification при разрешённых компиляциях | compile/test evidence отложен по явному ограничению итерации: без компиляций | `cargo test -p rustok-ai-content --lib`; `cargo test -p rustok-ai --lib` |
