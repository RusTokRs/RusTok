...
| `ai` | [plan](../../crates/rustok-ai/docs/implementation-plan.md) | `in_progress` | Run the deferred R4 Rust/frontend/module verification when compilation is allowed. R0-R3 implementation is complete; the generic host contribution remains a separate platform-owned blocker. |
...
| `installer` | [plan](../../crates/rustok-installer/docs/implementation-plan.md) | `in_progress` | Versioned topology, shared apply sequencing, and CLI provider are implemented; next is typed distributed deployment hand-off and per-role receipts. |
