...
| `cache` | [plan](../../crates/rustok-cache/docs/implementation-plan.md) | `in_progress` | Execute compiled, PostgreSQL, full local CAS and isolated Redis latency/restart jobs on one reconciled `main` revision; fix failures and record the verified revision. |
