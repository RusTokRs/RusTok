...
| `cache` | [plan](../../crates/rustok-cache/docs/implementation-plan.md) | `in_progress` | Run the permanent compiled cache gate on a reconciled `main`, then record isolated Redis 7 live evidence. |
...
| `channel` | [plan](../../crates/rustok-channel/docs/implementation-plan.md) | `in_progress` | Execute the permanent compiled gate and multi-replica durable-generation recovery scenarios, then collect full ChannelReadPort and middleware runtime evidence. |
