...
| `ai` | [plan](../../crates/rustok-ai/docs/implementation-plan.md) | `in_progress` | Close Rig registry-integrity checks (factory binding, compiled catalog, and deployment-target load validation), then run the workspace gate. The FFA structural contract and extension-scoped manifest validation are available; the generic host contribution remains separately platform-owned. |
