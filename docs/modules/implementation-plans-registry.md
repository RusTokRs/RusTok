...
| `cache` | [plan](../../crates/rustok-cache/docs/implementation-plan.md) | `in_progress` | Run the compiled, PostgreSQL channel-generation, and isolated Redis jobs on one reconciled `main` revision; fix failures and record the verified revision. |
...
| `channel` | [plan](../../crates/rustok-channel/docs/implementation-plan.md) | `in_progress` | Execute the wired generation, missed-publication, lag, PostgreSQL and Redis resolved-value evidence, then add disconnect/reconnect and regression fault injection. |
