...
| `next_plan_id` | `rustok-iggy-connector` | ID записи, которую должен взять следующий агент |
| `last_rotation_at` | `2026-06-20T00:00:00Z` | Когда указатель был сдвинут последний раз |
...
| `flex` | `flex` | `crates/flex/docs/implementation-plan.md` | `in_progress` | `84%` | `agent` | `2026-06-15T00:00:00Z` | `no-compile инкремент: standalone guardrails tightened for JSON object payloads, normalized status, max 50 fields per schema and tenant-scoped localized upsert lookup` | При разрешённых компиляциях прогнать `cargo test -p flex --lib`, затем `cargo test -p rustok-server --lib` + flex-targeted integration и зафиксировать evidence | compile/test evidence отложен по явному ограничению итерации: без компиляций | `cargo test -p flex --lib` |
...
| `rustok-iggy-connector` | `rustok-iggy-connector` | `crates/rustok-iggy-connector/docs/implementation-plan.md` | `in_progress` | `20%` | `agent` | `2026-06-20T00:00:00Z` | `no-compile: canonical simulated ack token helper shared by remote/embedded subscriber metadata; source assertions cover token format` | Связать metadata с реальным SDK subscriber path; заменить no-compile evidence targeted tests при разрешении компиляций | `compile/test evidence отложен по явному ограничению итерации: без компиляций` | `cargo test -p rustok-iggy-connector --lib` |
