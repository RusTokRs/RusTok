...
| `index` | [plan](../../crates/rustok-index/docs/implementation-plan.md) | `in_progress` | Add schema-application leases and secondary-index lifecycle, then execute PostgreSQL Testcontainers, concurrency, rollback, stale/redelivery, and tenant/locale evidence for the atomic mutation adapter. |
...
| `profiles` | [plan](../../crates/rustok-profiles/docs/implementation-plan.md) | `in_progress` | Collect compiled/runtime evidence for storefront transports, privacy, Media, Profiles/CLI telemetry, Social Graph follow command outcomes, and backfill failure stages; then define durable command receipts and relation reconciliation. |
