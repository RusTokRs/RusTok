...
| `ai` | [plan](../../crates/rustok-ai/docs/implementation-plan.md) | `in_progress` | Rig and host-contribution evidence pass. Agent principal/workflow persistence, owner catalog validation, atomic workflow-stage admission approvals, and native/GraphQL assignment-catalog parity are active, with a module-owned read-only Leptos catalog. Neither agents nor provider profiles accept raw role slugs; legacy provider role policy is fail-closed and AI no longer reads RBAC tables. The generic tenant RBAC catalog is now published by the platform; AI role-selection and scheduler hosting await the generic scheduler contract, with no host-specific AI coupling. |
