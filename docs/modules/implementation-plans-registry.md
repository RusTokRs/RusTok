...
| `rustok-mcp` | `rustok-mcp` | `crates/rustok-mcp/docs/implementation-plan.md` | `in_progress` | `93%` | `agent` | `2026-06-30T20:20:00Z` | Alloy scaffold apply в `apps/server` усилен транзакционным claim `staged -> applying`, `apply_started` audit в той же транзакции, success audit при `applied` и failure audit с возвратом в `staged` при ошибке workspace write; Next и Leptos FFA owner UI остаются параллельными owner surfaces | Добавить browser parity smoke для Next и Leptos management workflows | - | `npm run verify:mcp:admin-boundary`; `cargo test -p rustok-mcp-admin --lib`; `cargo check -p rustok-mcp-admin --features ssr`; `cargo check -p rustok-server --offline`; `cargo test -p rustok-mcp --lib --offline`; `npm --prefix apps/next-admin run typecheck`; `npm --prefix apps/next-admin run format:check` |
