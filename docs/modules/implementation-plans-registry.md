...
| `rustok-commerce` | `rustok-commerce` | `crates/rustok-commerce/docs/implementation-plan.md` | `in_progress` | `45%` | `agent` | `2026-05-28T00:00:00Z` | `Phase 10.1 returns foundation продвинут: admin REST/GraphQL parity для order returns show/list/create/complete/cancel поверх order-owned service lifecycle` | Закрыть item-level return lines и связку returns -> refunds/exchanges без host-owned logic | default server OpenAPI test блокируется существующими compile errors вне commerce; targeted order lifecycle + commerce check проходят | `cargo check -p rustok-commerce` |
...
| `rustok-seo` | `rustok-seo` | `crates/rustok-seo/docs/implementation-plan.md` | `in_progress` | `88%` | `agent` | `2026-05-30T00:00:00Z` | `начат D2: добавлены typed terminal bulk events, scope-sensitive idempotency keys и seo_event_deliveries migration/entity skeleton` | Завершить D2 transactional delivery tracking: писать seo_event_deliveries в publish path, связать с outbox envelope id и добавить duplicate-emission integration evidence | локальная verification обновляется в текущем PR; для D2/D3 нужен cross-owner sign-off `seo + outbox + index` | `cargo check -p rustok-seo --tests --config profile.dev.debug=0` |
...
| `rustok-seo-admin-support` | `rustok-seo-admin-support` | `crates/rustok-seo-admin-support/docs/implementation-plan.md` | `in_progress` | `55%` | `agent` | `2026-05-28T23:58:00Z` | `план синхронизирован с SEO Phase D: зафиксированы D6 observability/remediation widgets, transport fallback parity и D8 verification backlog` | Закрыть D6.1: reusable event delivery status cards + diagnostics remediation hints для owner-module panels | verification blocked в VM (`cargo` missing); REST parity helpers зависят от `rustok-seo` Batch D4 | `cargo check -p rustok-seo-admin-support --tests --config profile.dev.debug=0` |
