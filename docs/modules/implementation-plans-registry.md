...
| `cli-registry` | [plan](../../crates/rustok-cli-registry/docs/implementation-plan.md) | `in_progress` | Add the next approved owner-local provider and collect runtime evidence for `media reconcile`. |
| `comments` | [plan](../../crates/rustok-comments/docs/implementation-plan.md) | `in_progress` | Retain the completed canonical richtext owner/Blog-consumer cutover, add offline conversion evidence for pre-existing Markdown rows, and close runtime contract/fallback plus embedded/native compatibility evidence; keep the native-only admin exception host-neutral. |
...
| `sandbox` | [plan](../../crates/rustok-sandbox/docs/implementation-plan.md) | `in_progress` | Route Alloy execution through the implemented Rhai/Wasmtime runtime, then add cancellation, admission, durable audit and capability hardening. |
...
| `translation` | [plan](../../crates/rustok-translation/docs/implementation-plan.md) | `in_progress` | Exact provider coverage and opaque-cursor freshness are implemented with Media first. Complete required-locale policy and QA, then publish native/GraphQL progress and workflow parity before adding AI. |
