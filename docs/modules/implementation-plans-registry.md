...
| `rustok-seo` | `rustok-seo` | `crates/rustok-seo/docs/implementation-plan.md` | `in_progress` | `83%` | `agent` | `2026-05-24T20:45:00Z` | `в C1 начата runtime-декомпозиция sitemaps: submission-aggregation и submission-adapters вынесены в отдельные дочерние модули без изменения public SeoSitemapStatusRecord` | Закрыть третий дочерний модуль `index-generation`, затем зафиксировать provider seam как отдельный runtime object и прогнать `cargo test -p rustok-seo --lib sitemaps` | `provider-specific adapters блокируются отсутствием tenant-secret/rotation contract` | `cargo test -p rustok-seo --lib sitemaps` |
