...
| `rustok-pages` | `rustok-pages` | `crates/rustok-pages/docs/implementation-plan.md` | `in_progress` | `72%` | `agent` | `2026-05-25T00:00:00Z` | `PB-FBA-1A закрыт: machine-readable builder registry + anti-drift gate фиксируют provider/consumer versions, consumer_min_version и fallback profiles; scope остаётся PB-FBA-1B..1D` | Закрыть host-level PB-FBA-1B evidence (`admin list/read`, `storefront read`, typed errors parity); service-level fallback gate по `all_on/publish_off/preview_off/builder_off` уже покрыт | Wave 1 readiness остаётся `hold`, если есть waiver по anti-drift/fallback или нет полного evidence packet | `node crates/rustok-page-builder/scripts/verify/verify-page-builder-fba-baseline.mjs` |
