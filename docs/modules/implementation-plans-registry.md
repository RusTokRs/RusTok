...
| `last_rotation_at` | `2026-06-17T00:00:00Z` | Когда указатель был сдвинут последний раз |
...
| `rustok-commerce` | `rustok-commerce` | `crates/rustok-commerce/docs/implementation-plan.md` | `in_progress` | `53%` | `agent` | `2026-06-14T12:45:00Z` | `Next Admin commerce route group now has a shared module guard for shipping profiles, cart promotions, return decisions, and order changes, keeping host pages thin auth/options adapters while respecting enabled-module state.` | Продолжить сжатие оставшихся commerce compatibility transport paths toward owner payment/order async transports once those module-owned server-function/GraphQL adapters are available | compile/test evidence отложен по явному ограничению итерации: без компиляций; выполнены static source guardrails for Next commerce route guard | `node scripts/verify/verify-inventory-admin-boundary.mjs`; `./scripts/verify/verify-all.sh inventory-admin-boundary`; `cargo check -p rustok-commerce`; targeted storefront/cart/checkout inventory availability integration tests после добавления coverage |
