...
| `rustok-seo` | `rustok-seo` | `crates/rustok-seo/docs/implementation-plan.md` | `in_progress` | `80%` | `agent` | `2026-05-21T12:05:47Z` | `выполнен plan-sync: подтверждены typed schema/bulk/diagnostics и sitemap submission foundation; сформированы итерации C1/C2/C3 с конкретными verification шагами` | Стартовать Iteration C1: adapter seam для external sitemap submissions + regression tests | `для полноценных search-provider adapters нужен tenant-secret contract` | `cargo test -p rustok-seo --lib` |
