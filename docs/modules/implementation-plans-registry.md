...
| `last_rotation_at` | `2026-06-15T16:10:00Z` | Когда указатель был сдвинут последний раз |
...
| `rustok-forum` | `rustok-forum` | `crates/rustok-forum/docs/implementation-plan.md` | `in_progress` | `90%` | `agent` | `2026-06-15T16:10:00Z` | `FW-2 fallback hardening завершён: подготовлен и добавлен структурированный runtime-smoke шаблон для профилей all_on, publish_off, preview_off, builder_off в fw2-fallback-static-matrix.json; тесты верификации проходят успешно без компиляции.` | `FW-3: Подготовить пилотный пакет (Pilot readiness) для 1–2 low-traffic tenants (metadata/fallback/observability/rollback) и observability correlation после закрытия P5.` | `pilot delivery по FW-2..FW-4 остаётся blocked до central P5 Wave 1 Go/No-Go + cross-runtime parity evidence + owner sign-off.` | `npm run verify:page-builder:consumer:forum`; `cargo test -p rustok-forum --lib` |
