...
| `rustok-api` | `rustok-api` | `crates/rustok-api/docs/implementation-plan.md` | `in_progress` | `35%` | `agent` | `2026-06-20T13:36:46Z` | `shared PortContext/PortError baseline with PortCallPolicy::read() migrated across region plus product/customer/media/workflow/RBAC/tax/fulfillment/payment/pricing/cart/inventory/comments read ports` | Довести оставшиеся read ports до shared policy и добавить targeted verification при разрешённых компиляциях | compile/test evidence отложен по явному ограничению итерации: без компиляций | `cargo test -p rustok-api --lib`; targeted consumer crate tests |
