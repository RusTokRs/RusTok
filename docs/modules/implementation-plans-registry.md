...
| `ai-content` | [plan](../../crates/rustok-ai-content/docs/implementation-plan.md) | `in_progress` | Composed moderation and Blog-draft owner-service evidence pass; render the owned admin package with native/GraphQL parity. |
...
| `ai-product` | [plan](../../crates/rustok-ai-product/docs/implementation-plan.md) | `boundary_ready` | Product-copy direct persistence preserves non-target locales through the product owner; attributes are review-only without a product write. Execute catalog-read, workflow, and host transport evidence next. |
...
| `cache` | [plan](../../crates/rustok-cache/docs/implementation-plan.md) | `in_progress` | Run the permanent compiled cache gate on a reconciled `main` after the unrelated inventory baseline blocker is fixed, then record isolated Redis 7 live evidence and remove temporary diagnostics. |
