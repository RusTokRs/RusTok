...
| `ai` | [plan](../../crates/rustok-ai/docs/implementation-plan.md) | `in_progress` | Finish typed Rig factory dispatch and durable approval/stream evidence; provider-target selection and cancellation sequencing are implemented. Wait for the platform-owned generic host contribution before claiming the AI boundary complete. |
...
| `ai-product` | [plan](../../crates/rustok-ai-product/docs/implementation-plan.md) | `boundary_ready` | Execute catalog-read evidence, then prove localized generated-write safety and host transport parity. |
...
| `installer` | [plan](../../crates/rustok-installer/docs/implementation-plan.md) | `in_progress` | Add the versioned monolith/distributed topology descriptor, then move apply sequencing behind installer ports and register the CLI provider. |
...
| `web` | [plan](../../crates/rustok-web/docs/implementation-plan.md) | `not_started` | Migrate repeated Loco controller helpers through the shared Axum boundary. |
