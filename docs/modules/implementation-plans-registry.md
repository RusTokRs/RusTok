...
| `rustok-seo` | `rustok-seo` | `crates/rustok-seo/docs/implementation-plan.md` | `in_progress` | `81%` | `agent` | `2026-05-24T17:58:10Z` | `подготовлен execution checkpoint для C1: зафиксирован runtime-only scope, acceptance criteria для endpoint aggregation и regression matrix без изменения public SeoSitemapStatusRecord` | Реализовать `submit_sitemap_index` + default HTTP adapter wiring и закрыть `cargo test -p rustok-seo --lib sitemaps` по C1 matrix | `provider-specific adapters блокируются отсутствием tenant-secret/rotation contract` | `cargo test -p rustok-seo --lib sitemaps` |
