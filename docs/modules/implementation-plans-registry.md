...
| `events` | [plan](../../crates/rustok-events/docs/implementation-plan.md) | `in_progress` | Prove real-Iggy cursor/position/receipt recovery, exercise broker-success/receipt-mark ambiguity, PostgreSQL concurrency and bounded replay repair, then define undecodable poison handling. |
...
| `iggy` | [plan](../../crates/rustok-iggy/docs/implementation-plan.md) | `in_progress` | Prove real SDK consume/commit, every-partition position observation, exact-byte DLQ receipt recovery and publish-confirmation behavior; then execute broker-backed replay resilience. |
...
| `profiles` | [plan](../../crates/rustok-profiles/docs/implementation-plan.md) | `in_progress` | Prove replay repair, PostgreSQL receipt/concurrency, real-Iggy consumer/lag/receipt recovery, broker confirmation behavior, and retained storefront/privacy/Media/CLI evidence. |
