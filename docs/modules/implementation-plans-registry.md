...
| `index` | [plan](../../crates/rustok-index/docs/implementation-plan.md) | `in_progress` | Implement the atomic JSONB entity/link mutation adapter over the new seven-table M3 schema, then add schema leases, secondary-index lifecycle and PostgreSQL failure/concurrency evidence. |
