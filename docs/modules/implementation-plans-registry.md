...
| `ai` | [plan](../../crates/rustok-ai/docs/implementation-plan.md) | `in_progress` | Execute the new Rig registry-integrity tests, then run the workspace gate. The FFA structural contract and extension-scoped manifest validation are available; the generic host contribution remains separately platform-owned. |
