...
| `cache` | [plan](../../crates/rustok-cache/docs/implementation-plan.md) | `in_progress` | Run the compiled, PostgreSQL channel-generation, listener-lag/value and isolated Redis jobs on one reconciled `main` revision; fix failures and record the verified revision. |
...
| `channel` | [plan](../../crates/rustok-channel/docs/implementation-plan.md) | `in_progress` | Execute the complete durable-generation, lag/value, PostgreSQL and Redis restart evidence, then add bounded Redis latency and circuit-breaker recovery coverage. |
