...
| `rustok-commerce` | `rustok-commerce` | `crates/rustok-commerce/docs/implementation-plan.md` | `in_progress` | `46%` | `agent` | `2026-05-30T09:22:58Z` | `Phase 10.4 claims scope зафиксирован: return decision tree поддерживает order-change-backed claim рядом с return_only/refund/exchange через PostOrderOrchestrationService` | Расширить post-order operator UX и связать exchange/claim return decisions с order-change apply/cancel lifecycle без host-owned logic | default server OpenAPI test блокируется существующими compile errors вне commerce; package-wide commerce test сейчас также блокируется существующими graphql_runtime_parity_test compile errors; targeted bridge/checkout tests и commerce check проходят | `cargo check -p rustok-commerce`; `cargo test -p rustok-commerce --test order_returns_bridge_test`; `cargo test -p rustok-commerce --test checkout_service_test complete_checkout_rejects_reentry_for_checking_out_cart_without_artifacts -- --exact` |
...
| `rustok-seo` | `rustok-seo` | `crates/rustok-seo/docs/implementation-plan.md` | `in_progress` | `92%` | `agent` | `2026-05-30T12:00:00Z` | `закрыт D2: publish path пишет seo_event_deliveries, связывает delivery с outbox envelope id и блокирует duplicate emission; продвинут D4: REST diagnostics/sitemaps/bulk jobs + GraphQL seoSitemapJobs/seoSitemapJob` | Закрыть D4 error-envelope unification и перейти к D3 SEO->index consumer seam (tenant/kind reindex + retry/DLQ policy) | для D3 нужен cross-owner sign-off `seo + outbox + index`; D4 error envelope требует единый contract decision между REST и GraphQL owners | `cargo check -p rustok-seo --tests --config profile.dev.debug=0` |
