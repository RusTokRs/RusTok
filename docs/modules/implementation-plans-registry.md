...
| `rustok-mcp` | `rustok-mcp` | `crates/rustok-mcp/docs/implementation-plan.md` | `in_progress` | `95%` | `agent` | `2026-07-01T00:00:00Z` | Leptos native stage/apply mutations делегируют каноническому `McpManagementService` через `McpManagementMutationPort`; `apps/admin` монтирует owner component на `/mcp` во всех CSR/hydrate/SSR профилях, а boundary gate запрещает scaffold writes внутри UI adapter и transport logic внутри host | Добавить authenticated browser parity smoke для Next `/dashboard/mcp` и Leptos `/mcp` | - | `npm run verify:mcp:admin-boundary`; `cargo check -p rustok-admin --offline`; `cargo check -p rustok-admin --no-default-features --features ssr --offline`; `cargo check -p rustok-mcp-admin --features hydrate --target wasm32-unknown-unknown --offline`; `cargo test -p rustok-mcp-admin --lib --offline`; `cargo check -p rustok-server --offline`; `cargo test -p rustok-mcp --lib --offline`; `npm --prefix apps/next-admin run typecheck` |
