...
| `rustok-runtime` | `rustok-runtime` | `crates/rustok-runtime/docs/implementation-plan.md` | `in_progress` | `5%` | `agent` | `2026-07-10T00:00:00Z` | `foundation scaffold created for typed host runtime helpers while rustok-api is kept contract-focused` | Define host-neutral CLI runtime composition for database, settings and selected module handles before moving a DB/storage-backed Loco task | `-` | `cargo test -p rustok-runtime --lib` |
...
| `rustok-cli-core` | `rustok-cli-core` | `crates/rustok-cli-core/docs/implementation-plan.md` | `in_progress` | `20%` | `agent` | `2026-07-10T00:00:00Z` | `asynchronous provider execution supports database/storage-backed module adapters without blocking the runner` | Connect the first module-local provider after host-neutral CLI runtime composition exists | `-` | `cargo test -p rustok-cli-core --lib` |
...
| `rustok-cli` | `rustok-cli` | `crates/rustok-cli/docs/implementation-plan.md` | `in_progress` | `25%` | `agent` | `2026-07-10T00:00:00Z` | `async runner awaits selected providers while preserving list, JSON inventory, namespace filtering, duplicate rejection and normalized command args` | Construct a host-neutral CLI runtime, then connect the first real module-local provider | `-` | `cargo test -p rustok-cli --quiet`; `cargo run -p rustok-cli --quiet -- core version` |
