...
| `rustok-comments` | `rustok-comments` | `crates/rustok-comments/docs/implementation-plan.md` | `in_progress` | `25%` | `agent` | `2026-06-07T00:00:00Z` | `Comments admin получил FFA split: core status parsing, native-only transport facade and explicit ui/leptos render adapter` | Расширить admin core view-model helpers и спланировать GraphQL/headless fallback parity adapter для выхода из temporary single-adapter state | GraphQL fallback для comments admin пока не реализован; FBA остается not_started до boundary evidence | `cargo check -p rustok-comments-admin --config profile.dev.debug=0`; `cargo test -p rustok-comments-admin --lib --config profile.dev.debug=0` |
| `rustok-commerce` | `rustok-commerce` | `crates/rustok-commerce/docs/implementation-plan.md` | `in_progress` | `48%` | `agent` | `2026-06-07T08:10:00Z` | `Inventory availability compatibility tail moved into commerce roadmap: storefront/checkout callers consume rustok-inventory public-channel availability/projection facades instead of direct loaders/backorder branching; post-order operator work remains active` | Добавить channel-aware inventory visibility integration coverage через storefront catalog/cart/checkout path, продолжая post-order operator parity work без host-owned logic | default server OpenAPI test блокируется существующими compile errors вне commerce; fast inventory boundary verifier and selected commerce/inventory source guardrails pass | `node scripts/verify/verify-inventory-admin-boundary.mjs`; `./scripts/verify/verify-all.sh inventory-admin-boundary`; `cargo check -p rustok-commerce`; targeted storefront/cart/checkout inventory availability integration tests после добавления coverage |
