...
| `rustok-mcp` | `rustok-mcp` | `crates/rustok-mcp/docs/implementation-plan.md` | `in_progress` | `78%` | `agent` | `2026-06-30T00:00:00Z` | Restored executable `rustok-mcp` evidence against `rmcp 2.0.0`: `cargo check -p rustok-mcp` and `cargo test -p rustok-mcp --lib` passed after `ContentBlock::text` response update | Add admin/UI surface for MCP access management and Alloy scaffold draft review while preserving server-owned remote MCP bootstrap and persisted draft-store boundary | `-` | `cargo check -p rustok-mcp`; `cargo test -p rustok-mcp --lib` |
