...
| `rustok-ai` | `rustok-ai` | `crates/rustok-ai/docs/implementation-plan.md` | `in_progress` | `80%` | `agent` | `2026-06-19T06:15:00Z` | `AI ownership evidence hardening: media/alloy plans added, order payload validation tightened, central plan/index synced` | Continue parity/evidence hardening without changing runtime composition ownership | compile/test evidence отложен по явному ограничению итерации: без компиляций | `cargo test -p rustok-ai --lib` |
...
| `rustok-ai-media` | `rustok-ai-media` | `crates/rustok-ai-media/docs/implementation-plan.md` | `in_progress` | `45%` | `agent` | `2026-06-19T06:15:00Z` | `media support crate owns image_asset descriptors, handler adapter API, and image-size normalization validation consumed by rustok-ai` | Расширить media-owned generated artifact contract and targeted verification | compile/test evidence отложен по явному ограничению итерации: без компиляций | `cargo test -p rustok-ai-media --lib` |
| `rustok-ai-alloy` | `rustok-ai-alloy` | `crates/rustok-ai-alloy/docs/implementation-plan.md` | `in_progress` | `45%` | `agent` | `2026-06-19T06:15:00Z` | `alloy support crate owns alloy_code descriptors, handler adapter API, and runtime payload JSON validation consumed by rustok-ai` | Расширить Alloy-owned execution policy metadata and targeted verification | compile/test evidence отложен по явному ограничению итерации: без компиляций | `cargo test -p rustok-ai-alloy --lib` |
