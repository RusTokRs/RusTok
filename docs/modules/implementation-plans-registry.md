...
| `rustok-forum` | `rustok-forum` | `crates/rustok-forum/docs/implementation-plan.md` | `done` | `100%` | `agent` | `2026-06-22T00:00:00Z` | `FW-11 refresh section shape hardening: focused freshness и aggregate consumer gates теперь требуют непустую форму обязательных refresh sections, fixture suite покрывает empty metrics, root package.json снова валиден для npm no-compile gates.` | `Steady-state: refresh Wave evidence before next_due_at and keep consumer/freshness fixture no-compile gates green.` | `None` | `npm run verify:page-builder:consumer:forum`; `npm run verify:forum:wave-evidence-freshness`; `npm run test:verify:forum:wave-evidence-freshness`; `cargo test -p rustok-forum --lib` |
