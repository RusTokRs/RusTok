...
| `rustok-pages` | `rustok-pages` | `crates/rustok-pages/docs/implementation-plan.md` | `in_progress` | `80%` | `agent` | `2026-06-14T12:00:00Z` | `устранён drift в central registry по legacy-bridge гейту, добавлены детальные пункты для Quality Backlog в локальном плане, обновлён статус релиза.` | `Провести реальный PB-FBA-1C control-plane dry-run и заменить синтетический packet; начать выполнение задач из детального Quality Backlog.` | Wave 1 readiness остаётся `hold`, если есть waiver по anti-drift/fallback или нет полного фактического evidence packet | `node crates/rustok-page-builder/scripts/verify/verify-page-builder-fba-baseline.mjs` |
