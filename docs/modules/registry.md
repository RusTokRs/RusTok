...
| `region` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-region/docs/implementation-plan.md` (slice #20: admin/storefront crate roots wire module layers and re-export explicit `ui/leptos.rs` render adapters; admin core now owns list item view-models, selected-row CSS policy, detail meta formatting, editor form-state snapshots/defaults and detail policy-section rows/fallbacks; storefront keeps native/GraphQL transport split) |
