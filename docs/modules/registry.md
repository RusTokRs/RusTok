...
| `inventory` | admin | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-inventory/docs/implementation-plan.md` (Wave 5: backend `AdminInventoryReadService`/DTO + primary native server-function read path + первые native write endpoints set/adjust/reserve quantity + native check-availability validation endpoint + UI targeted set-quantity/+/-1 adjustment/reserve/check-availability controls + admin core/read/write facade + dedicated `native.rs` server-function transport + package-private native-unavailable transitional commerce GraphQL read fallback + explicit `ui/leptos.rs` adapter, unit/compatibility tests на inventory read model/normalized variables/facade request builders/error mapping + boundary tests на GraphQL isolation/separate native read + read-only transitional adapter/removal criteria + native-only write facade/UI markers + reservation/availability typed result evidence + reservation-aware admin read availability from inventory levels) |
