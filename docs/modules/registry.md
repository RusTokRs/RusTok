...
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (slice #82 moved admin table-level empty/count/header state and editor title/submit-button state into Leptos-free `BlogPostAdminTableViewModel` / `BlogPostAdminFormViewModel`; slice #81 moved write-path issue banner presentation into core; slice #79/#80 moved row display/action/archive/delete policy into core; slice #76 moved save-command preparation into core; fast boundary guardrail `scripts/verify/verify-blog-admin-boundary.mjs`; explicit admin/storefront `ui/leptos.rs` adapters and native/GraphQL contract remain unchanged) |
...
| `fulfillment` | `rustok-fulfillment` | — | Shipping options, fulfillments, fulfillment-owned shipping-option admin UI и storefront shipping handoff presentation |
