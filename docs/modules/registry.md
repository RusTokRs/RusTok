...
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (latest slices moved admin save-result post-apply/refresh/query-replace policy, selected-post route/query push/replace/clear intent selection and edit-banner visibility/copy/action-label presentation into Leptos-free `BlogPostSaveResultViewModel`, `BlogPostAdminRouteQueryIntent` and `BlogPostAdminEditBannerViewModel` in `admin/src/core.rs`; slice #77 fast boundary guardrail `scripts/verify/verify-blog-admin-boundary.mjs` now checks route/query intent and edit-banner ownership; explicit admin/storefront `ui/leptos.rs` adapters and native/GraphQL contract remain unchanged) |
