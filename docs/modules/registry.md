...
| `commerce` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-commerce/docs/implementation-plan.md` (admin FFA transport module split + FBA-readiness gate: crate root only wires/re-exports `ui::CommerceAdmin`; `admin/src/core/` owns form/command/view-model policy and `admin/src/transport/` is split into shipping_profile/promotion/order_change submodules over the existing native/GraphQL-capable api layer; native-first post-order operator keeps module-owned transport + GraphQL fallback) |
| `workflow` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-workflow/docs/implementation-plan.md` (slice: admin core helpers владеют status/row/template presentation, module route toggle, transport request context, error presentation и template create command policy; тонкий `transport/` facade принимает core-owned context/command DTO + явный `ui/leptos` adapter; внешний GraphQL contract не изменялся) |
