...
| `search` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-search/docs/implementation-plan.md` (slice #30: storefront results adapter renders core-owned results view-models; GraphQL fallback unchanged) |
...
| `commerce` | admin + storefront | `in_progress` | `in_progress` | `docs_boundary` | `crates/rustok-commerce/docs/implementation-plan.md` (slice 10.4: admin REST/GraphQL return decision-tree transport plus native-first module-owned post-order order-change operator over `OrderService`, GraphQL fallback, and resolution summary cards) |
...
| `inventory` | admin | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-inventory/docs/implementation-plan.md` (Wave 5: backend `AdminInventoryReadService`/DTO + primary native server-function read path + первый native write endpoint set-quantity + UI targeted set-quantity control + admin core/read/write facade + dedicated `native.rs` server-function transport + package-private native-unavailable transitional commerce GraphQL read fallback + explicit `ui/leptos.rs` adapter, unit/compatibility tests на inventory read model/normalized variables/facade request builders/error mapping + boundary tests на GraphQL isolation/separate native read + native-only write facade/UI markers) |
