...
| `blog` | admin + storefront | `in_progress` | `boundary_ready` | `core_transport_ui` | [Live plan](../../crates/rustok-blog/docs/implementation-plan.md); `crates/rustok-blog/contracts/blog-fba-registry.json`, `crates/rustok-blog/contracts/evidence/blog-comments-runtime-fallback-smoke.json`, and `crates/rustok-blog/contracts/evidence/blog-comments-consumer-runtime-order-smoke.json` lock the `CommentsThreadPort` consumer boundary and the static `comment.created`/`comment.deleted` reply-count projection. Next and Leptos article authoring now use the shared framed editor and canonical document contract; native `#[server]`, storefront/storage cutover, and live comments fallback/error, delivery, retry, and recovery execution remain required for `transport_verified`. |
...
| `modules` | none | `not_started` | `boundary_ready` | `no_ui_boundary` | [Live plan](../../crates/rustok-modules/docs/implementation-plan.md); lifecycle/recovery and digest-pinned installation/runtime foundations are owner-owned. Native and GraphQL marketplace reads share a host-composed owner catalog, registry lifecycle derivation is owner-owned, and the admin host no longer contains direct registry SQL or workspace/Cargo catalog scanning and build planning. Command and HTTP artifact dispatch use owner-typed request envelopes that bind release, admitted bindings, installation target, tenant, payload, and execution context. The artifact-aware definition catalog/dispatcher, CAS, composition/governance, and broader transport cutover remain in progress; the operator-only external-prebuilt staging endpoint preserves authenticated actor/quarantine authority rather than accepting caller-supplied principals. |
...
| `translation` | Leptos admin + Next admin | `in_progress` | `in_progress` | `core_transport_ui` | [Live plan](../../crates/rustok-translation/docs/implementation-plan.md); module core, neutral provider dependency, RBAC surface, CAS-guarded inventory/change checkpoints, atomic bounded full-rescan, tenant-scoped workflow tables, owner-validated workflow, required-target policy, deterministic QA, durable apply/recovery, versioned tenant glossaries with immutable job bindings, approved-only Translation Memory with deterministic exact/contextual lookup and revisioned retention/tombstone/purge, bounded owner-aware interchange, and manifest-composed GraphQL exist. Interchange export includes only public or tenant-private non-excluded immutable owner snapshots with exact revision/hash evidence; atomic per-item import creates proposals only through owner validation and deterministic QA. Owner `Deleted` inventory observations atomically mark matching memory with content-free lifecycle evidence; a module-owned generic-work adapter tombstones expired or owner-deleted entries and purges after a 24-hour grace while excluding legal hold and machine-operation pins. `rustok-translation-admin` and `@rustok/translation-admin` own matching six-tab workbenches over a 38-operation native/GraphQL contract with bounded interchange controls, host locale/tenant, and URL-owned `tab`/`glossary_id`/`memory_entry_id`. Authenticated GraphQL runtime evidence covers successful import through canonical QA, invalid bounds, stale source rejection, cross-tenant isolation, and tenant-context mismatch. Registered native HTTP evidence covers policy, glossaries, bounded interchange, assignment, manual workflow/apply, QA rejection, cancellation, retry, unknown-outcome apply recovery, job progress, Translation Memory, inventory rebuild, provider/required-target progress, and machine generation/status/cancellation/recovery through a deterministic neutral provider, including degraded health, idempotency, and missing-runtime failure. Production application-router evidence wraps the registered read-policy function with tenant cache/resolution, locale negotiation, JWT/session/RBAC, channel, rate-limit, and security middleware and verifies cross-tenant token rejection. Authenticated Next browser evidence covers URL-owned tab state, semantic tabs, interchange controls, and the host module-disabled fallback. Compiled Leptos CSR browser evidence covers all six URL-owned tabs, roving arrow/Home/End focus, exact tab/panel and localized form-label relationships, and zero axe violations on every tab. Translation owns actor/idempotency-bound machine proposal, cancellation, stable-key status, and audited stuck-save recovery commands plus a content-free operation journal. Registered operations pin normalized memory entry identities, order, and scores until completion or cancellation; replay still reads tombstoned entries, while purge is blocked by the pin. AI status, result recovery, and cancellation resolve by stable owner/idempotency identity, including durable pre-registration cancellation, and Translation receipts preserve propagation/recovery evidence. Recovery revalidates the exact original request and resumes canonical proposal save without another billable execution. The generation command projects the immutable glossary revision and bounded memory suggestions, invokes the neutral port selected by the production-selected distribution bridge, and saves AI output only through owner validation and deterministic QA. Composed missing-keyring behavior is verified as optional and fail-closed. Deterministic composed AI evidence covers ordered fallback, fail-closed output-schema validation, sanitized failure, exact settlement, request conflict, in-flight cancellation with reservation release, quota rejection before provider execution, encrypted restart replay without another call or bill, and typed degraded health for configuration-level provider unavailability. Real separate-process file-backed AI evidence covers expired-attempt recovery, reservation preservation, immutable failure recording, and reclaim with a new lease. Live external-provider runtime outage/degradation/restart and production-database multi-replica concurrency evidence plus retention-worker multi-replica/restart evidence remain open. |
| `ai-translation` | none | `not_started` | `in_progress` | `no_ui_boundary` | [Live plan](../../crates/rustok-ai-translation/docs/implementation-plan.md); bridge contracts, typed schemas, exact registered-task descriptor, proposal-only validation, dependency guard, durable accounting/recovery, production-selected distribution-owned lazy factory composition, and fail-closed missing-keyring evidence exist. Deterministic composed runtime evidence covers fallback, output-schema validation, exact settlement, request conflict, in-flight cancellation, quota rejection, encrypted restart replay without duplicate calls or billing, and typed degraded health for configuration-level provider unavailability. An ignored operator-only durable structured-runtime probe is ready; retained live external-provider runtime failure/restart evidence remains open. |
...
| `ai` | admin + Next admin | `in_progress` | `boundary_ready` | `core_transport_ui` | [Live plan](../../crates/rustok-ai/docs/implementation-plan.md); structured tenant budget/provider policy reads and writes now have permission-checked native `#[server]` and parallel GraphQL contracts, while deployment-owned result keys remain outside tenant transport. The production-selected distribution bridge publishes the Translation-owned lazy runtime factory without server capability imports, and missing-keyring composition is optional and fail-closed. Deterministic composed evidence covers ordered fallback, fail-closed output-schema validation, sanitized failure, exact settlement, request conflict, in-flight cancellation with reservation release, quota rejection before provider execution, encrypted restart replay without another provider call or bill, and typed degraded health for configuration-level provider unavailability. Real separate-process file-backed evidence covers expired-attempt recovery, reservation preservation, immutable failure recording, and reclaim with a new lease. An ignored operator-only durable structured-runtime probe is ready; retained live external-provider runtime outage/degradation/restart and production-database multi-replica concurrency evidence remain open. |
...
| `pages` | `rustok-pages` | `content`, `page_builder` | Pages and page-builder surfaces; tenant module settings are read through the public `TenantService::find_tenant_module` contract; FBA consumer metadata synchronized with `crates/rustok-page-builder/contracts/page-builder-fba-registry.json`; Navigation owns menus |
| `navigation` | `rustok-navigation` | `channel` | Tenant navigation trees, channel-scoped menu composition, and route presentation |
...
| `media` | `rustok-media` | — | Media-owned asset/blob/rendition/upload-session persistence, restart-safe object lifecycle and reconciliation, upload API, and typed image descriptor contract `MediaImageDescriptor` for cross-module SEO/media consumers |
...
| `rustok-ai-translation` | Stateless support adapter from Translation-owned `MachineTranslationPort` to AI-owned `AiStructuredTaskPort`; owns `machine_translation`, prompt-policy/schema digests, bounded mapping, deterministic output validation, review-required evidence, and `scripts/verify/verify-ai-translation-boundary.mjs`. The production-selected distribution feature `ai-translation` publishes its Translation-owned lazy factory only when both owner modules are selected; missing-keyring composition is optional and fail-closed. Deterministic composed evidence covers fallback, output-schema validation, exact settlement, request conflict, in-flight cancellation, quota rejection, encrypted restart replay without duplicate calls or billing, and typed degraded health for configuration-level provider unavailability. An ignored operator-only durable probe is ready, while retained live external-provider runtime evidence remains gated. |
