...
| `commerce` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-commerce/docs/implementation-plan.md` (admin FFA display/form/command hardening + FBA-readiness gate: crate root only wires/re-exports `ui::CommerceAdmin`; shipping-profile form/draft policy, optional filters, cart-promotion/order-change command preparation, badge presentation, display fallbacks, cart-adjustment view-models and resolution summary mapping live in framework-agnostic core; native-first post-order operator keeps module-owned transport + GraphQL fallback) |
