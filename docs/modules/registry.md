...
| `customer` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-customer/docs/implementation-plan.md` (admin slice: `core` request policy + `transport/mod.rs` facade + native-only `transport/native_server_adapter.rs` server-function adapter + explicit `ui/leptos.rs` adapter; legacy `api.rs` removed and covered CRUD flows no longer call raw `api::*` from UI) |
