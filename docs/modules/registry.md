...
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (latest admin slices moved save-result apply/refresh/query-replace policy, selected-post load-result policy and transport failure issue mapping into Leptos-free core helpers; earlier slices moved issue banner presentation, table-row display/action state, archive/delete labels/visibility and save-command preparation into `admin/src/core.rs`; fast boundary guardrail `scripts/verify/verify-blog-admin-boundary.mjs`; explicit admin/storefront `ui/leptos.rs` adapters and native/GraphQL contract remain unchanged) |
