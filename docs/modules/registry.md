...
| `inventory` | admin | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-inventory/docs/implementation-plan.md` (Wave 5: backend `AdminInventoryReadService`/DTO + primary native server-function read path + первый native write endpoint set-quantity + UI targeted set-quantity control + admin core/read/write facade + dedicated `native.rs` server-function transport + package-private native-unavailable transitional commerce GraphQL read fallback + explicit `ui/leptos.rs` adapter, unit/compatibility tests на inventory read model/normalized variables/facade request builders/error mapping + boundary tests на GraphQL isolation/native-only write facade/UI markers) |
