...
| `inventory` | admin | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-inventory/docs/implementation-plan.md` (Wave 5: admin core/read facade + package-private transitional commerce GraphQL adapter + explicit `ui/leptos.rs` adapter, compatibility tests на минимальную inventory read model/normalized variables/facade request builders + boundary tests на GraphQL isolation) |
