...
| `cart` | storefront | `phase_b_ready` | `in_progress` | `core_transport_ui` | `crates/rustok-cart/docs/implementation-plan.md` (FBA provider registry `crates/rustok-cart/contracts/cart-fba-registry.json`, static contract evidence `crates/rustok-cart/contracts/evidence/cart-contract-test-static-matrix.json` and neutral `CartSnapshotReadPort` / `cart.checkout_snapshot.v1` are locked for commerce checkout snapshot consumers; runtime contract execution/fallback smoke remain pending before `boundary_ready`; cart-owned storefront UI stays in `rustok-cart/storefront` with `core/transport/ui` split and exports `CartCheckoutHandoffCard`/view-model consumed by commerce checkout orchestration) |
