...
|---|---|---|---|---|---|
| `channel` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-channel/docs/implementation-plan.md` (admin FFA structural split: crate root только подключает/реэкспортирует `ChannelAdmin`; `admin/src/core.rs` владеет Leptos-free policy очистки selected-channel route; `admin/src/transport/mod.rs` владеет facade/fallback policy, `admin/src/transport/native_server_adapter.rs` содержит native endpoints, `admin/src/transport/rest_adapter.rs` содержит REST fallback; `admin/src/ui/leptos.rs` является явным Leptos render adapter и больше не вызывает pre-FFA модуль `api`; fast boundary guardrail + fixture tests: `scripts/verify/verify-channel-admin-boundary.mjs`, `scripts/verify/verify-channel-admin-boundary.test.mjs`) |
| `pages` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-pages/docs/implementation-plan.md` (admin + storefront slices: `core` + thin `transport` facades + explicit `ui/leptos` adapters; native/GraphQL contract unchanged; fast boundary guardrail `scripts/verify/verify-pages-ui-boundary.mjs`) |
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (slice #76: admin save-command preparation (`prepare_blog_post_save_command`) moved into `admin/src/core.rs` with unit-test evidence for validation rejection plus create/update operation selection; Leptos adapter delegates validation/outcome/busy-key policy to core while dispatching through the module-owned transport facade; slice #77 adds fast boundary guardrail `scripts/verify/verify-blog-admin-boundary.mjs`; previous slice added explicit admin/storefront `ui/leptos.rs` render adapters and storefront adapters consume core-owned fetch request) |
| `outbox` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-outbox/docs/implementation-plan.md` (read-only admin FFA slice: `admin/src/core.rs` владеет Leptos-free DTO/view-model fallback policy, `admin/src/transport/` владеет native server-function facade, `admin/src/ui/leptos.rs` является явным Leptos render adapter; текущий admin surface зафиксирован как temporary native-only single-adapter state) |
| `index` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-index/docs/implementation-plan.md` (admin overview FFA slice: `admin/src/core.rs` владеет Leptos-free view-model/error formatting, `admin/src/transport/` владеет native server-function bootstrap facade, `admin/src/ui/leptos.rs` является явным Leptos render adapter; текущий overview зафиксирован как temporary native-only single-adapter state because no GraphQL/REST operator contract exists yet) |
| `rbac` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-rbac/docs/implementation-plan.md` (admin overview FFA slice: `admin/src/core.rs` владеет Leptos-free view-model/error formatting for live permissions/catalog, `admin/src/transport/` владеет native server-function bootstrap facade, `admin/src/ui/leptos.rs` является явным Leptos render adapter; текущий overview зафиксирован как temporary native-only single-adapter state because no GraphQL/REST operator contract exists yet) |
| `tenant` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-tenant/docs/implementation-plan.md` (admin FFA slice: `admin/src/core.rs` владеет Leptos-free tenant bootstrap view-model/copy/error policy, `admin/src/transport/` владеет native-only server-function facade, `admin/src/transport/native_server_adapter.rs` содержит native endpoint, `admin/src/ui/leptos.rs` является явным Leptos render adapter; current overview зафиксирован как temporary native-only single-adapter state because no legacy GraphQL/REST tenant bootstrap UI contract exists for this surface; fast guardrail: `scripts/verify/verify-tenant-admin-boundary.mjs`) |
...
| `cart` | storefront | `phase_b_ready` | `not_started` | `core_transport_ui` | `crates/rustok-cart/docs/implementation-plan.md` (Phase B closed: cart-owned storefront UI stays in `rustok-cart/storefront` with `core/transport/ui` split; no mechanical FFA slices without new cart-owned UI/transport surface; next work is parity/evidence hardening) |
| `commerce` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-commerce/docs/implementation-plan.md` (admin + storefront FFA transport module split + next-admin host registration: next-admin host registers `@rustok/commerce-admin` package with typed GraphQL adapter methods for shipping profiles, cart promotions, and order changes; crate root only wires/re-exports `ui::CommerceAdmin`; `admin/src/core/` owns form/command/view-model policy and `admin/src/transport/` is split into shipping_profile/promotion/order_change submodules over the existing native/GraphQL-capable api layer; `storefront/src/core/` owns aggregate checkout request structs plus commerce-owned context fallback view-models via `requests`/`presentation` submodules, cart totals/line-items/adjustments remain `rustok-cart` UI-owned, payment/order/fulfillment details remain owned by their split modules, commerce storefront is limited to orchestration handoff UI, and `storefront/src/transport/` is split into native_server_adapter/graphql_adapter with the single native-first + GraphQL fallback policy after removing duplicate combined fallback helpers from `storefront/src/api.rs`; native-first post-order operator keeps module-owned transport + GraphQL fallback) |
...
| `region` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-region/docs/implementation-plan.md` (slice #32: admin/storefront crate roots wire module layers and re-export explicit `ui/leptos.rs` render adapters; admin core now owns shell/list header view-models, list loading/error/empty/ready state mapping, selected-region route/query intent normalization and open/save/new writer updates, tenant-aware list subtitle fallback policy, list item view-models, selected-row CSS policy, detail panel empty/ready aggregation, detail meta/header/timestamp formatting, editor form-state snapshots/defaults/mode-copy/field-copy, submit command preparation with normalized payload, host-locale/required-field validation errors, create/update save-mode decision, detail policy-section rows/fallbacks and raw JSON section title/body mapping; fast guardrail `scripts/verify/verify-region-admin-boundary.mjs`; storefront keeps native/GraphQL transport split) |
...
| `pricing` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-pricing/docs/implementation-plan.md` (admin/storefront slices: storefront `core/transport/ui` split plus admin Leptos-free `core/` presentation/routing/request-policy helpers, detail-header view-model, API input sanitizers, `transport.rs` facade and explicit `ui/leptos.rs` adapter; native-first + GraphQL fallback unchanged) |
...
| `order` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-order/docs/implementation-plan.md` (admin slice: `core` helpers + `transport/mod.rs` facade + explicit `ui/leptos.rs` adapter; latest core helpers prepare mark-paid/ship/deliver/cancel command payloads before transport dispatch and own admin presentation policy for status labels/classes, captions, summaries, timeline/action hints, optional display fallback and selected-detail form-state/default/fallback mapping; GraphQL transport code now lives under `admin/src/transport/graphql_adapter.rs` behind the module-owned `admin/src/transport/mod.rs` facade; fast guardrail `scripts/verify/verify-order-admin-boundary.mjs` is wired into the aggregate FFA migration verification; return/refund/exchange/claim resolution validation stays module-owned) |
| `payment` | no module-owned UI | `in_progress` | `not_started` | `no_ui_boundary` | `crates/rustok-payment/docs/implementation-plan.md` |
| `fulfillment` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-fulfillment/docs/implementation-plan.md` (slice: admin core list/filter policy + transport facade + explicit `ui/leptos.rs` render adapter over existing GraphQL shipping-option transport; fast boundary guardrail `scripts/verify/verify-fulfillment-admin-boundary.mjs`; CRUD behavior unchanged) |
