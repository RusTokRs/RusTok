...
| `ai` | admin + Next admin | `in_progress` | `boundary_ready` | `core_transport_ui` | [Live plan](../../crates/rustok-ai/docs/implementation-plan.md); structured tenant budget/provider policy reads and writes now have permission-checked native `#[server]` and parallel GraphQL contracts, while deployment-owned result keys remain outside tenant transport. The optional distribution bridge publishes the Translation-owned lazy runtime factory without server capability imports; production-profile enablement and live multi-replica accounting/recovery evidence remain open. |
...
| `rustok-ai-translation` | Stateless support adapter from Translation-owned `MachineTranslationPort` to AI-owned `AiStructuredTaskPort`; owns `machine_translation`, prompt-policy/schema digests, bounded mapping, deterministic output validation, review-required evidence, and `scripts/verify/verify-ai-translation-boundary.mjs`. The explicit distribution feature `ai-translation` publishes its Translation-owned lazy factory only when both owner modules are selected; production-profile enablement and live evidence remain gated. |
