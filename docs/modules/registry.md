...
| `region` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-region/docs/implementation-plan.md` (slice #23: admin/storefront crate roots wire module layers and re-export explicit `ui/leptos.rs` render adapters; admin core now owns list item view-models, selected-row CSS policy, detail meta/header/timestamp formatting, editor form-state snapshots/defaults/mode-copy, detail policy-section rows/fallbacks and raw JSON section title/body mapping; storefront keeps native/GraphQL transport split) |
