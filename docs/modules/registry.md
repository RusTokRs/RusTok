...
| `commerce` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-commerce/docs/implementation-plan.md` (admin FFA root/core hardening + FBA-readiness gate: crate root only wires/re-exports `ui::CommerceAdmin`, order-change resolution summary mapping lives in framework-agnostic core, native-first post-order operator keeps module-owned transport + GraphQL fallback) |
