...
| `fulfillment` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-fulfillment/docs/implementation-plan.md` (admin core/list/filter + transport facade + explicit Leptos adapter; storefront `rustok-fulfillment/storefront` now owns shipping/fulfillment handoff plus seller-aware shipping selection DTO/core/UI consumed by commerce checkout orchestration; fast guards `verify-fulfillment-admin-boundary.mjs` and `verify-fulfillment-storefront-boundary.mjs`; selection transport move remains next cutover) |
