...
|---|---|---|---|---|---|
| `events` | Leptos admin + Next admin | `in_progress` | `in_progress` | `core_transport_ui` | [Live plan](../../crates/modules/rustok-events-module/docs/implementation-plan.md); `rustok-events-module` is the cycle-free runtime/manifest adapter and owns the UI packages. Its Next package exposes a client-safe registration entrypoint separately from server page exports. The UI exposes only durable `outbox` (default) and `outbox_iggy` profiles. |
| `cache` | Next admin | `in_progress` | `in_progress` | `core_transport_ui` | [Live plan](../../crates/modules/rustok-cache/docs/implementation-plan.md); the module-owned package separates client-safe shell registration from server page exports and renders cache health through the host GraphQL and locale providers. |
| `iggy_connector` | Leptos admin + Next admin | `in_progress` | `in_progress` | `core_transport_ui` | [Live plan](../../crates/modules/rustok-iggy-connector/docs/implementation-plan.md); owner persistence, native server functions, parallel GraphQL, secret references, and readiness validation are implemented. Packaging and live Iggy evidence remain open. |
| `email` | none | `not_started` | `transport_verified` | `no_ui_boundary` | [Live plan](../../crates/modules/rustok-email/docs/implementation-plan.md); `crates/modules/rustok-email/contracts/email-fba-registry.json` and `crates/modules/rustok-email/contracts/evidence/email-runtime-fallback-smoke.json`. |
| `flex` | none | `not_started` | `boundary_ready` | `no_ui_boundary` | [Live plan](../../crates/modules/flex/docs/implementation-plan.md); `node scripts/verify/verify-flex-multilingual-contract.mjs` locks the capability-only multilingual owner boundary. |
| `rustok-mcp` | admin + Next admin | `in_progress` | `boundary_ready` | `core_transport_ui` | [Live plan](../../crates/modules/rustok-mcp/docs/implementation-plan.md) |
| `channel` | admin | `in_progress` | `boundary_ready` | `core_transport_ui` | [Live plan](../../crates/modules/rustok-channel/docs/implementation-plan.md); `crates/modules/rustok-channel/contracts/channel-fba-registry.json`, `crates/modules/rustok-channel/contracts/evidence/channel-runtime-fallback-smoke.json`, `scripts/verify/verify-channel-admin-boundary.mjs`, and `npm run verify:channel:resolution-contract`. The built-in host fast-path keeps `explicit selectors -> built-in host slice -> typed policies -> explicit default -> unresolved`; `rustok-pages`, `rustok-blog`, `rustok-commerce`, and `rustok-forum` are locked by `verify:channel:proof-points`. |
| `page_builder` | no module-owned UI | `not_started` | `boundary_ready` | `no_ui_boundary` | [Live plan](../../crates/modules/rustok-page-builder/docs/implementation-plan.md) |
| `pages` | admin + storefront | `in_progress` | `boundary_ready` | `core_transport_ui` | [Live plan](../../crates/modules/rustok-pages/docs/implementation-plan.md); `scripts/verify/verify-pages-ui-boundary.mjs` locks the builder-first UI boundary. |
| `blog` | admin + storefront | `in_progress` | `boundary_ready` | `core_transport_ui` | [Live plan](../../crates/modules/rustok-blog/docs/implementation-plan.md); `crates/modules/rustok-blog/contracts/blog-fba-registry.json`, `crates/modules/rustok-blog/contracts/evidence/blog-comments-runtime-fallback-smoke.json`, and `crates/modules/rustok-blog/contracts/evidence/blog-comments-consumer-runtime-order-smoke.json` lock the `CommentsThreadPort` consumer boundary and the static `comment.created`/`comment.deleted` reply-count projection. Blog Category canonical copy is Taxonomy-owned through the same-ID Blog-to-Taxonomy binding and the registered `taxonomy/term` Translation target; Blog post copy remains a separate future Blog-owned editorial surface. Next and Leptos article authoring use the shared framed editor and canonical document contract. Both Blog storefronts compose a Comments-owned reusable composer with Blog-bound commands that enforce permission, tenant, module/channel, published status, and channel visibility; Leptos selects native/GraphQL transport and Next uses the parallel GraphQL command. The Leptos selected-article path builds an isolated authenticated comment WASM island and keeps full storefront hydration out of the form. The pre-release initial schema stores canonical article documents directly; corrective richtext migration/backfill paths are deleted and forbidden by the Blog source gate. A fresh PostgreSQL migration run and authenticated Next mounted comment submission verify canonical persistence with moderation-pending state. Mounted Leptos persistence/reload parity, Next rejection/reload, live comments errors, delivery, retry, and recovery remain required for `transport_verified`; canonical Category Translation concurrency/cursor recovery is Taxonomy-owned and tracked with the Taxonomy provider. |
...
verification is pending. Secret-instance scope/storage now uses independent
installation authority and tenant/owner/opaque-instance keys; its stateless
owner-isolation and full-request replay fixtures pass 9/9, with owner lib
308/308 and event-contract verification 14/14.
Host management/revocation fences and production secret-use policy remain open.
Maintenance commands share the canonical owner namespace facts/root-lock reader.
The structured copier requires transaction-backed policy and exact namespace
revisions. Frozen request/page admission is durable before writes; records,
admitted-schema/index/quota checks, target CAS, and immutable exact-response
receipts commit atomically in a separate owner transaction. Shared holds retain
both namespaces between pages. Reconciliation rechecks current policy/quota
using the original request, and receipts record the commit policy revision.
Revised file-backed SQLite runtime tests pass 11/11. Whole-operation source
retention, host maintenance/fleet fences, PostgreSQL
crash/race evidence, outbox composition, and full-snapshot serving CAS remain open.
...
Callers, fixtures, secret/MCP runtime evidence, migration-object publication,
...
Object migration now durably reserves a complete frozen request/inventory and
opaque copy keys before publishing SHA-256/size-verified create-only bytes. Exact
reconciliation preserves keys and revalidates current policy; pending holds block
purge, writes, and serving-reference changes through owner checks and schema
guards. File-backed runtime fixtures require fresh execution. Production
maintenance/fleet fences, PostgreSQL race/kill evidence, full-manifest serving CAS,
and safe orphan collection remain open; FFA/FBA statuses are unchanged.
...
Consumer FBA batch evidence: `blog` publishes `crates/modules/rustok-blog/contracts/evidence/blog-comments-consumer-runtime-order-smoke.json`; shared `scripts/verify/verify-consumer-fba-runtime-order.mjs` verifies Blog target validation/provider-call/typed mapping order. SEO retains a template-only Media consumer artifact because its target owners do not yet publish media asset UUIDs; it must not claim media-port invocation evidence before that owner contract and consumer composition exist. Both statuses remain `in_progress` until their respective consumer runtime execution.
...
|---|---|---|---|
| `modules` | `rustok-modules` | — | Mandatory artifact, marketplace, installation, lifecycle, build/publication orchestration, tenant-policy control plane, and browser-safe static-composition read contracts; delegates isolated execution to `rustok-sandbox` and isolated Rust compilation to the build-worker boundary |
...
| `product_relations` | `rustok-product-relations` | `product` | Product relations, merchandising associations (cross-sells, up-sells, related, accessories, alternatives) and reordering |
| `brand` | `rustok-brand` | `product` | Brand catalog, manufacturers, media presentation, localized translations, product-brand associations, and dedicated Leptos FFA admin UI |
| `product_bundles` | `rustok-product-bundles` | `product` | Product bundles, kits, configurable sets, package discounts, localized translations, and dedicated Leptos FFA admin UI |
...
| `blog` | `rustok-blog` | `content`, `taxonomy`, `outbox`, `channel` | Blog domain, posts, Category bindings, tags, transport/UI; canonical Category localized copy and Translation are Taxonomy-owned through the same-ID binding, while Blog retains Blog-specific membership/settings and post/editorial ownership. Comments and Profiles presentation are optional capabilities rather than static module dependencies. |
| `forum` | `rustok-forum` | `content`, `media`, `taxonomy` | Forum domain, topics, replies, moderation, attachment relations, transport/UI and page-builder widget consumer fallback contract |
...
| `page_builder` | `rustok-page-builder` | — | Standalone FBA reference module for visual builder capabilities (`preview/tree/properties/publish`); machine-readable FBA registry now includes contract versions, port call policies, typed error catalog/error codes, health states, degradation reasons and pilot SLO thresholds, while adapter seams publish `PageBuilderAdapterCallEvidence` / `PageBuilderAdapterTelemetry` for host persistence/rendering audit markers: `crates/modules/rustok-page-builder/contracts/page-builder-fba-registry.json` |
...
| `rustok-mcp` | MCP adapter/server tool surface; Alloy scaffold stage/review/apply responses carry a typed owner-issued SHA-256 source digest over the canonical logical crate root and generated source files |
| `rustok-ai` | Deployment-scoped, globally active AI host/orchestrator capability with a Rig 0.39 registry/engine, generic agent-principal and owner-workflow contracts, owner-owned provider profile migration and GraphQL roots/DTO, a tenant provider egress-classification allowlist enforced before structured execution registration and provider calls, registry-driven Leptos and Next admin surfaces, external secret resolution through `rustok-secrets`, neutral runtime-extension and durable-work registration, an optional deployment-owned encrypted structured-result keyring, domain direct handler registration adapters from `rustok-ai-product`/`rustok-ai-content`/`rustok-ai-order`/`rustok-ai-media`/`rustok-ai-alloy`, typed owner source-lineage receipts retained separately from redacted MCP output, server module-surface guard, admin guardrail `scripts/verify/verify-ai-admin-boundary.mjs`, Rig-only cutover guardrail `scripts/verify/verify-ai-rig-cutover.mjs`, and domain vertical ownership guardrail `scripts/verify/verify-ai-domain-verticals.mjs` |
...
| `rustok-ai-media` | Domain-owned AI support crate/adapter for media/image asset AI vertical registration and image size validation; its composed `rustok-ai` direct path persists provider output and localized metadata through the Media owner service. FBA support-consumer registry `crates/modules/rustok-ai-media/contracts/ai-media-fba-registry.json` and static matrix `crates/modules/rustok-ai-media/contracts/evidence/ai-media-consumer-static-matrix.json` plus runtime fallback source-smoke `crates/modules/rustok-ai-media/contracts/evidence/ai-media-runtime-fallback-smoke.json` lock the `ai_asset_descriptor` dependency on media `MediaAssetReadPort` / `media.asset_read.v1`, fallback profile `embedded_native` and degraded modes `skip_asset_enrichment` / `proxy_storage_relative_url` / `summarize_internal_binary` under `npm run verify:ai-media:fba` |
| `rustok-ai-alloy` | Domain-owned AI support crate/adapter for Alloy scripting vertical registration, runtime payload validation, script execution policy metadata with allowed operations, code-agent descriptors, and the `alloy_change_review` workflow; registry `crates/modules/rustok-ai-alloy/contracts/ai-alloy-policy-registry.json` plus static matrix `crates/modules/rustok-ai-alloy/contracts/evidence/ai-alloy-policy-static-matrix.json` lock `alloy_script_execution_policy` ownership under `scripts/verify/verify-ai-alloy-policy.mjs` |
...
- `seo`: `crates/modules/rustok-seo/contracts/seo-fba-registry.json`; `crates/modules/rustok-seo/contracts/evidence/seo-media-consumer-runtime-order-smoke.json`.
- Ecommerce FBA registries: `crates/modules/rustok-product/contracts/product-fba-registry.json`, `crates/modules/rustok-pricing/contracts/pricing-fba-registry.json`, `crates/modules/rustok-inventory/contracts/inventory-fba-registry.json`, `crates/modules/rustok-customer/contracts/customer-fba-registry.json`, `crates/modules/rustok-cart/contracts/cart-fba-registry.json`, `crates/modules/rustok-order/contracts/order-fba-registry.json`, `crates/modules/rustok-payment/contracts/payment-fba-registry.json`, `crates/modules/rustok-fulfillment/contracts/fulfillment-fba-registry.json`, and `crates/modules/rustok-commerce/contracts/commerce-fba-registry.json`.
- `pricing`: `scripts/verify/verify-pricing-admin-boundary.mjs`; `scripts/verify/verify-pricing-storefront-boundary.mjs`.
- `pricing`: `crates/modules/rustok-pricing/contracts/evidence/pricing-runtime-contract-smoke.json`.
...
- `blog`: `crates/modules/rustok-blog/contracts/blog-fba-registry.json`.
- `blog`: `crates/modules/rustok-blog/contracts/evidence/blog-comments-runtime-fallback-smoke.json`.
- `search`: `crates/modules/rustok-search/contracts/search-fba-registry.json`.
- `forum`: `scripts/verify/verify-forum-admin-boundary.mjs`; `scripts/verify/verify-forum-storefront-boundary.mjs`.
- `forum`: `crates/modules/rustok-page-builder/contracts/evidence/forum-wave1-rollout-evidence.json`.
- Ecommerce runtime contract smoke: `crates/modules/rustok-product/contracts/evidence/product-runtime-contract-smoke.json`, `crates/modules/rustok-inventory/contracts/evidence/inventory-runtime-contract-smoke.json`, `crates/modules/rustok-customer/contracts/evidence/customer-runtime-contract-smoke.json`, and `crates/modules/rustok-cart/contracts/evidence/cart-runtime-contract-smoke.json`.
- `product`: `crates/modules/rustok-product/contracts/evidence/product-runtime-fallback-smoke.json`.
