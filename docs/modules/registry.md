...
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (latest accepted slices moved admin save-result post-apply/refresh/query-replace policy and selected-post route/query push/replace/clear intent selection into Leptos-free `BlogPostSaveResultViewModel` + `BlogPostAdminRouteQueryIntent` in `admin/src/core.rs`; slice #88 SEO copy extraction was rolled back as over-extraction, and fast boundary guardrail `scripts/verify/verify-blog-admin-boundary.mjs` now rejects one-off SEO copy wrappers in core while still checking route/query intent ownership; explicit admin/storefront `ui/leptos.rs` adapters and native/GraphQL contract remain unchanged) |
