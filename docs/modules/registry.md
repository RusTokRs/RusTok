...
| `auth` | admin | `in_progress` | `boundary_ready` | `core_transport_ui` | `crates/rustok-auth/docs/implementation-plan.md` (admin UI extracted into `crates/rustok-auth/admin` using `core/transport/ui` structural shape; explicit `admin/src/ui/leptos.rs` aggregation adapter and module-owned password-reset transport facade are present; `AUTH_USER_PERMISSIONS`, admin request normalization, host-locale catalog consumption and actionable auth-error fallback are locked by FBA capability registry `crates/rustok-auth/contracts/auth-fba-registry.json`, static matrix `crates/rustok-auth/contracts/evidence/auth-capability-static-matrix.json` and runtime-verified source-smoke `crates/rustok-auth/contracts/evidence/auth-runtime-fallback-smoke.json` under `npm run verify:ai:fba-baseline`; `cargo test -p rustok-auth --lib` and `scripts/verify/verify-auth-admin-boundary.mjs` back the boundary evidence) |
