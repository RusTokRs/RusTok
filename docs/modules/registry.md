...
| `search` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-search/docs/implementation-plan.md` (slice #31: storefront transport split into native_server_adapter + graphql_adapter with native-first facade; GraphQL fallback unchanged) |
...
| `customer` | admin | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-customer/docs/implementation-plan.md` (admin `core.rs` request defaults + transport facade + explicit `ui/leptos.rs` adapter) |
| `pricing` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-pricing/docs/implementation-plan.md` (storefront native/GraphQL adapter split plus admin transport facade and explicit `ui/leptos.rs` adapter) |
| `inventory` | admin | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-inventory/docs/implementation-plan.md` (Wave 5: backend `AdminInventoryReadService`/DTO + primary native server-function read path + admin core/read facade + dedicated `native.rs` server-function transport + package-private native-unavailable transitional commerce GraphQL fallback + explicit `ui/leptos.rs` adapter, unit/compatibility tests на inventory read model/normalized variables/facade request builders/error mapping + boundary tests на GraphQL isolation) |
| `order` | admin | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-order/docs/implementation-plan.md` (admin `core.rs` list/filter policy + transport facade + explicit `ui/leptos.rs` adapter for covered order flows) |
