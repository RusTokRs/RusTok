...
| `translation` | Leptos admin + Next admin | `in_progress` | `in_progress` | `core_transport_ui` | [Live plan](../../crates/rustok-translation/docs/implementation-plan.md); module core, neutral provider dependency, RBAC surface, CAS-guarded inventory/change checkpoints, atomic bounded full-rescan, tenant-scoped workflow tables, owner-validated workflow, required-target policy, deterministic QA, durable apply/recovery, versioned tenant glossaries with immutable job bindings, approved-only Translation Memory with deterministic exact/contextual lookup and revisioned retention/tombstone/purge, and manifest-composed GraphQL exist. `rustok-translation-admin` and `@rustok/translation-admin` own matching six-tab workbenches over a 34-operation native/GraphQL contract with host locale/tenant and URL-owned `tab`/`glossary_id`/`memory_entry_id`. Translation owns actor/idempotency-bound machine proposal and cancellation commands plus a content-free operation journal. Registered operations pin normalized memory entry identities, order, and scores until completion or cancellation; replay still reads tombstoned entries, while purge is blocked by the pin. The command projects the immutable glossary revision and bounded memory suggestions, invokes the neutral port selected by the optional distribution bridge, and saves AI output only through owner validation and deterministic QA. Production-profile enablement plus live ledger/replay/budget/fallback/restart evidence, AI-runtime cancellation/status correlation, stuck-save recovery, browser/accessibility/module-disablement evidence, owner-deletion retention propagation, and exchange remain open. |
