...
| `pricing` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | [Live plan](../../crates/rustok-pricing/docs/implementation-plan.md); compiled in-process `PricingReadPort` SQLite tests cover variant-first resolution, active price-list projection and missing-deadline rejection. Consumer fallback/degraded execution remains open. |
