...
| `region` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-region/docs/implementation-plan.md` (slice #38: region boundary verifier now self-checks package wiring for `test:verify:region:admin-boundary` and fixture-test canonical/docs-sync cases; slice #37 added fast guardrail fixture tests for canonical/negative boundary evidence; fast guardrail `scripts/verify/verify-region-admin-boundary.mjs`; storefront keeps native/GraphQL transport split) |
