...
| `comments` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-comments/docs/implementation-plan.md` (admin slice: Leptos-free `core` status parsing + thread/detail/comment row view-model helpers + request/command DTOs + native-only `transport/` facade + explicit `native_server_adapter` + `ui/leptos.rs` render adapter; GraphQL/headless fallback remains a planned parity adapter) |
...
| `pricing` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-pricing/docs/implementation-plan.md` (admin/storefront slices: storefront `core/transport/ui` split plus admin Leptos-free `core.rs`, `transport.rs` facade and explicit `ui/leptos.rs` adapter; native-first + GraphQL fallback unchanged) |
