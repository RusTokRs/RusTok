...
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (slice #76: admin save-command preparation (`prepare_blog_post_save_command`) moved into `admin/src/core/` with unit-test evidence for validation rejection plus create/update operation selection; Leptos adapter delegates validation/outcome/busy-key policy to core while dispatching through the module-owned transport facade; slice #77 adds fast boundary guardrail `scripts/verify/verify-blog-admin-boundary.mjs`; previous slice added explicit admin/storefront `ui/leptos.rs` render adapters and storefront adapters consume core-owned fetch request) |
...
| `ai` | admin + Next admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-ai/docs/implementation-plan.md` (capability-owned AI admin FFA-срез: `admin/src/core/` владеет Leptos-free нормализацией request, direct-job payload builders и diagnostics summary policy; `admin/src/transport/` владеет native server-function facade; `admin/src/ui/leptos.rs` является explicit Leptos render adapter; Next.js GraphQL admin package остаётся параллельным headless/admin contract; fast guardrail: `scripts/verify/verify-ai-admin-boundary.mjs`) |
