...
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (latest slices moved admin edit-banner and body-format raw-warning presentation envelopes into Leptos-free `BlogPostAdminEditBannerViewModel` + `BlogPostAdminBodyFormatWarningViewModel` in `admin/src/core.rs`; slice #77 fast boundary guardrail `scripts/verify/verify-blog-admin-boundary.mjs` now checks route/query intent and warning view ownership; explicit admin/storefront `ui/leptos.rs` adapters and native/GraphQL contract remain unchanged) |
