...
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (latest slices moved admin selected-post route/query push/replace/clear intent selection and SEO panel copy into Leptos-free `BlogPostAdminRouteQueryIntent` + `BlogPostAdminSeoPanelCopy` in `admin/src/core.rs`; slice #77 fast boundary guardrail `scripts/verify/verify-blog-admin-boundary.mjs` now checks route/query intent ownership and SEO copy ownership; explicit admin/storefront `ui/leptos.rs` adapters and native/GraphQL contract remain unchanged) |
