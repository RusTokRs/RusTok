...
| `cart` | storefront | `phase_b_ready` | `boundary_ready` | `core_transport_ui` | [Live plan](../../crates/rustok-cart/docs/implementation-plan.md); `CartCheckoutPort` owns checkout snapshot and lifecycle writes, while `CartStorefrontPort` owns REST and GraphQL storefront cart reads, mutations, and repricing. Recovery, checkout adapter migration, fallback, and transport proof remain open. |
