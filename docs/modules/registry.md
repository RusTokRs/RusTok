...
| `commerce` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-commerce/docs/implementation-plan.md` (admin FFA command/view-model hardening + FBA-readiness gate: crate root only wires/re-exports `ui::CommerceAdmin`; cart-promotion/order-change command preparation, badge presentation and resolution summary mapping live in framework-agnostic core; native-first post-order operator keeps module-owned transport + GraphQL fallback) |
