...
| `media` | admin | `in_progress` | `boundary_ready` | `core_transport_ui` | [Live plan](../../crates/rustok-media/docs/implementation-plan.md); whole-module isolated-service pilot using `MediaAssetReadPort` and `MediaAssetWritePort`. Binary uploads remain Media-owned streaming REST or future presigned upload, while gRPC is reserved for metadata/control; `crates/rustok-media/contracts/media-fba-registry.json`, `crates/rustok-media/contracts/evidence/media-provider-runtime-order-smoke.json`, and `scripts/verify/verify-media-admin-boundary.mjs`. |
