...
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (slice #80: admin table-row archive/delete labels and archive visibility moved into Leptos-free `BlogPostAdminTableRowViewModel` in `admin/src/core.rs`; slice #79 moved row display/action state into core; slice #78 moved editor form-state mapping/reset defaults into core; slice #76 moved save-command preparation into core; slice #77 added fast boundary guardrail `scripts/verify/verify-blog-admin-boundary.mjs`; explicit admin/storefront `ui/leptos.rs` adapters and native/GraphQL contract remain unchanged) |
...
| `pricing` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-pricing/docs/implementation-plan.md` (admin/storefront slices: storefront `core/transport/ui` split plus admin Leptos-free `core/` presentation/routing/request-policy helpers, detail-header and variant-card view-models, API input sanitizers, `transport.rs` facade and explicit `ui/leptos.rs` adapter; native-first + GraphQL fallback unchanged) |
