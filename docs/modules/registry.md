...
|---|---|---|---|---|---|
| `channel` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-channel/docs/implementation-plan.md` (admin FFA structural split: crate root только подключает/реэкспортирует `ChannelAdmin`; `admin/src/core.rs` владеет Leptos-free policy очистки selected-channel route; `admin/src/transport.rs` владеет смешанным native/REST facade; `admin/src/ui/leptos.rs` является явным Leptos render adapter и больше не вызывает pre-FFA модуль `api`) |
