...
| `blog` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (slice #75: admin transport facade plus explicit admin/storefront `ui/leptos.rs` render adapters; storefront adapters consume core-owned fetch request) |
...
| `search` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-search/docs/implementation-plan.md` (slice #37: storefront results header renders core-owned header view-model; GraphQL fallback unchanged) |
...
| `commerce` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-commerce/docs/implementation-plan.md` (slice 10.4: native-first post-order order-change operator with module-owned transport, GraphQL fallback, and resolution summary cards) |
...
| `region` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-region/docs/implementation-plan.md` (slice #17: admin/storefront crate roots wire module layers and re-export explicit `ui/leptos.rs` render adapters; storefront keeps native/GraphQL transport split) |
| `product` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-product/docs/implementation-plan.md` (slice: storefront native/GraphQL + Leptos adapter split; product admin core helpers, `SelectedProductSummaryViewModel`, `ProductAdminListItemViewModel`, transport facade and `admin/src/ui/leptos.rs` adapter) |
| `customer` | admin | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-customer/docs/implementation-plan.md` (admin slice: core list request policy + native server-function transport facade + explicit `ui/leptos.rs` adapter) |
| `pricing` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-pricing/docs/implementation-plan.md` (storefront core/transport/ui split plus admin transport facade and explicit `ui/leptos.rs` adapter) |
| `inventory` | admin | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-inventory/docs/implementation-plan.md` (Wave 5: backend `AdminInventoryReadService`/DTO + primary native server-function read path + первые native write endpoints set/adjust/reserve/release quantity + native check-availability validation endpoint + UI targeted set-quantity/+/-1 adjustment/reserve/release/check-availability controls + admin core/read/write facade + dedicated `native.rs` server-function transport + package-private native-unavailable transitional commerce GraphQL read fallback + explicit `ui/leptos.rs` adapter, unit/compatibility tests на inventory read model/normalized variables/facade request builders/error mapping + boundary tests на GraphQL isolation/separate native read + read-only transitional adapter/removal criteria + native-only write facade/UI markers + reservation/release/availability typed result evidence + service-level non-negative reserve/availability request invariants + reservation-aware set-quantity stocked/available calculation + no-create failed reservation release guardrail + tracked reservation item release guardrail + reservation-aware admin read availability from inventory levels) |
| `order` | admin | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-order/docs/implementation-plan.md` (admin slice: core list/filter request policy + transport facade + explicit `ui/leptos.rs` adapter; returns validation matrix remains in commerce track) |
