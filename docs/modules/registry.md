...
| `commerce` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | [Live plan](../../crates/rustok-commerce/docs/implementation-plan.md); `scripts/verify/verify-commerce-admin-boundary.mjs`; checkout now invokes the inventory-owned `InventoryReservationPort` with typed request context instead of the public inventory helper, but remains `in_progress` until its targeted provider-consumer integration execution is recorded. Commerce admin native order-change/cart-promotion adapter uses `HostRuntimeContext` DB plus typed `TransactionalEventBus`, without host-framework dependencies and without root GraphQL/state-machine aliases. |
