...
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (latest slices keep admin warning and posts-load presentation envelopes in Leptos-free `BlogPostAdminBodyFormatWarningViewModel` and `BlogPostAdminPostsLoadViewModel`; over-extracted shell-copy helper was removed under the anti-over-extraction gate; fast boundary guardrail `scripts/verify/verify-blog-admin-boundary.mjs` and fixture suite `scripts/verify/verify-blog-admin-boundary.test.mjs` check route/query intent, warning/posts-load ownership, non-module `api;` text tolerance, and that GraphQL does not swallow posts contract-unavailable errors before the Leptos parity branch; explicit admin/storefront `ui/leptos.rs` adapters and native/GraphQL contract remain unchanged) |
