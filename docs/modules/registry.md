...
| `index` | admin | `in_progress` | `in_progress` | `core_transport_ui` | [Live plan](../../crates/rustok-index/docs/implementation-plan.md); M2 accepted JSONB and archived same-commit evidence. M3 now registers the generic schema/entity/link/inbox/job/checkpoint/consistency migration foundation; runtime mutation/query adapters and provider-consumer evidence remain pending, so FBA stays `in_progress`. |
...
| `translation` | planned admin | `not_started` | `in_progress` | `core_no_ui` | [Live plan](../../crates/rustok-translation/docs/implementation-plan.md); module core, neutral provider dependency, RBAC surface, rebuildable inventory identity projection, and optimistic provider checkpoints exist. No transport or UI is published yet; Leptos native plus GraphQL and Next parity remain planned. |
