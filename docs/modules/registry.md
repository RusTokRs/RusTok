...
| `cart` | storefront | `phase_b_ready` | `in_progress` | `core_transport_ui` | [Live plan](../../crates/rustok-cart/docs/implementation-plan.md); `CartCheckoutPort` owns checkout snapshot and lifecycle write operations. Status remains `in_progress` until live provider-consumer execution is recorded. |
| `commerce` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | [Live plan](../../crates/rustok-commerce/docs/implementation-plan.md); `scripts/verify/verify-commerce-admin-boundary.mjs`; checkout invokes inventory through `InventoryReservationPort` and catalog/variant-first reads through `ProductCatalogReadPort`, with typed request context and without direct product entity access. It remains `in_progress` until targeted provider-consumer integration execution is recorded. Commerce admin native order-change/cart-promotion adapter uses `HostRuntimeContext` DB plus typed `TransactionalEventBus`, without host-framework dependencies and without root GraphQL/state-machine aliases. |
...
| `pricing` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | [Live plan](../../crates/rustok-pricing/docs/implementation-plan.md); pricing admin native handlers use `HostRuntimeContext` DB plus typed `TransactionalEventBus`, without host-framework dependencies. |
