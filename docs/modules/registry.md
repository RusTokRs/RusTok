...
| `workflow` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-workflow/docs/implementation-plan.md` (slice: admin core helpers владеют status/row/template presentation, module route toggle, error presentation и template create command policy; тонкий `transport/` facade + явный `ui/leptos` adapter; внешний GraphQL contract не изменялся) |
