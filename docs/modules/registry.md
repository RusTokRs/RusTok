...
| `blog` | admin + storefront | `in_progress` | `boundary_ready` | `core_transport_ui` | [Live plan](../../crates/rustok-blog/docs/implementation-plan.md); `crates/rustok-blog/contracts/blog-fba-registry.json`, `crates/rustok-blog/contracts/evidence/blog-comments-runtime-fallback-smoke.json`, and `crates/rustok-blog/contracts/evidence/blog-comments-consumer-runtime-order-smoke.json` lock the `CommentsThreadPort` consumer boundary and the static `comment.created`/`comment.deleted` reply-count projection. Registered `blog/category` Translation target exposes exact category copy through owner CAS, durable receipts, cursor evidence, and transactional Search reindex; Blog post copy and Taxonomy-owned tags remain outside the pilot. Next and Leptos article authoring use the shared framed editor and canonical document contract. Blog admin/storefront select native `#[server]` in SSR/hydrate and retain GraphQL for CSR/headless use; the target storage source cutover and fail-closed migration are implemented. Retained PostgreSQL migration, mounted browser parity, live comments fallback/error, delivery, retry, recovery, and Blog Translation concurrency/cursor recovery execution remain required for `transport_verified`. |
...
| `forum` | admin + storefront | `in_progress` | `boundary_ready` | `core_transport_ui` | [Live plan](../../crates/rustok-forum/docs/implementation-plan.md); topic/reply richtext is atomically canonical across storage, REST/GraphQL, Next, and Leptos. Shared React and Leptos frames submit one `RichTextDocument`; owner reads provide `RichTextView` and plain text. Forum admin category/topic operations use one GraphQL transport with no REST fallback. Native/WASM checks, 113 Forum unit tests, 36 Forum admin unit tests, canonical SQLite/PostgreSQL soft-delete/revision execution, package tests, Next typecheck, and the ownership verifier pass. Broader Forum product parity and live cross-module runtime evidence remain open. |
...
| `blog` | `rustok-blog` | `content`, `comments`, `taxonomy`, `outbox` | Blog domain, posts, categories, tags, transport/UI; registered exact `blog/category` Translation target with owner-local revisions, durable receipts, change cursor, and Search reindex |
