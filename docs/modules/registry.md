...
| `search` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-search/docs/implementation-plan.md` (slice #27: admin/storefront crate roots are wiring-only and explicit `ui/leptos.rs` adapters consume module-owned `core` + `transport`; GraphQL fallback unchanged) |
