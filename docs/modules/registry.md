...
|---|---|---|---|---|---|
| `pages` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-pages/docs/implementation-plan.md` (admin slice: `core` + thin `transport` facade + explicit `ui/leptos` adapter; GraphQL contract unchanged) |
...
| `comments` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-comments/docs/implementation-plan.md` (admin slice: Leptos-free `core` status filter parsing + native-only `transport.rs` facade + explicit `ui/leptos.rs` render adapter; GraphQL/headless fallback remains a planned parity adapter) |
...
| `pricing` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-pricing/docs/implementation-plan.md` (admin/storefront slices: storefront `core/transport/ui` split plus admin `transport.rs` facade and explicit `ui/leptos.rs` adapter; native-first + GraphQL fallback unchanged) |
