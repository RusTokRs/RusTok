...
| `cart` | storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-cart/docs/implementation-plan.md` (slice: storefront `core/request.rs` owns fetch/decrement/remove request objects consumed by `ui/leptos` and native/GraphQL transport adapters without dropping parity) |
