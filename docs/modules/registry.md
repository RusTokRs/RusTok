...
| `region` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-region/docs/implementation-plan.md` (slice #33: admin open-detail success/error outcome mapping moved into Leptos-free `RegionAdminOpenDetailViewModel`; prior slices own shell/list/detail/editor/submit route-query view-models and submit command preparation in core; fast guardrail `scripts/verify/verify-region-admin-boundary.mjs`; storefront keeps native/GraphQL transport split) |
