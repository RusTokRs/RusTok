...
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (latest slices moved admin warning, posts-load and shell-header presentation envelopes into Leptos-free `BlogPostAdminBodyFormatWarningViewModel`, `BlogPostAdminPostsLoadViewModel` and `BlogPostAdminShellViewModel` in `admin/src/core.rs`; fast boundary guardrail `scripts/verify/verify-blog-admin-boundary.mjs` checks route/query intent, warning/posts-load/shell view ownership; explicit admin/storefront `ui/leptos.rs` adapters and native/GraphQL contract remain unchanged) |
