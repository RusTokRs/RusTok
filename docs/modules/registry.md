...
| `rustok-ai-translation` | Stateless support adapter from Translation-owned `MachineTranslationPort` to AI-owned `AiStructuredTaskPort`; owns `machine_translation`, prompt-policy/schema digests, bounded mapping, deterministic output validation, review-required evidence, and `scripts/verify/verify-ai-translation-boundary.mjs`. It is not runtime-registered until the durable structured-execution gate is complete. |
