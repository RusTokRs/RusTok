...
| `ai-product` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-ai-product/docs/implementation-plan.md` (module-owned admin FFA split now has `admin/src/core.rs`, `admin/src/transport.rs`, and `admin/src/ui/leptos.rs`; transport exposes a native-server plus GraphQL fallback placeholder profile while concrete host rendering remains a follow-up; local plan carries a parseable FFA/FBA status block) |
| `auth` | admin | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-auth/docs/implementation-plan.md` (admin UI extracted into `crates/rustok-auth/admin` using `core/transport/ui` structural shape; explicit `admin/src/ui/leptos.rs` aggregation adapter and module-owned password-reset transport facade are present; i18n dynamic match catalog matching `UiRouteContext.locale` integrated; routing successfully registered in host `apps/admin`) |
...
| `rustok-ai-product` | Domain-owned AI support crate for product vertical registration (`product_copy`, `product_attributes`), handler adapter API, generated payload validation, and compile-free ownership evidence via `scripts/verify/verify-ai-domain-verticals.mjs` |
