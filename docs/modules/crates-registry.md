...
| `rustok-profiles` | Универсальный публичный профиль пользователя поверх platform `users`: handle/display-name/visibility/public summary contract для forum/blog/groups/social/commerce. | `ProfilesModule`, `ProfileService`, `dto::*`, `entities::*`. | Схлопывать `profiles` обратно в auth/user identity, в `rustok-customer` или в будущий seller-domain. |
