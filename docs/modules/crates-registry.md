...
| `rustok-web` | Axum HTTP boundary helpers used while replacing Loco controller APIs with RusToK-owned response and error mapping. | `HttpError`, `HttpResult`, `ErrorBody`. | Own domain errors or business rules, runtime composition, CLI contracts, FBA metadata or UI transport. |
