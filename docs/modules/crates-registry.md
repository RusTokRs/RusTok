...
| `rustok-installer-persistence` | SeaORM adapter for installer database readiness, schema application, durable sessions, locks and step receipts, using schema composed by `rustok-migrations`. | `SeaOrmInstallerPorts`, `InstallerPersistenceService`, `entities::{install_session, install_step_receipt}`. | Depend on `apps/server`, expose HTTP/CLI parsing, duplicate installer state contracts, or move SeaORM installer mappings back to an executable host. |
