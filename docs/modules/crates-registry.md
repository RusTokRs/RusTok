...
| `rustok-media` | Media lifecycle, storage-facing services и owner-owned transport-адаптеры, включая usage statistics. | `MediaService`, `load_media_usage_snapshot`, `graphql::{MediaQuery, MediaMutation, MediaUsageStats}`, `controllers::*`. | Держать media resolver/DTO, включая `mediaUsage`, или прямые media entity imports в `apps/server`. |
