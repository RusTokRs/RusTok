...
| `rustok-profiles` | Универсальный публичный профиль пользователя поверх platform `users`: handle/display-name/visibility/public summary contract для forum/blog/groups/social/commerce. | `ProfilesModule`, `ProfileService`, `ProfilesReader`, `graphql::*`, `dto::*`, `entities::*`. | Схлопывать `profiles` обратно в auth/user identity, в `rustok-customer` или в будущий seller-domain. |
...
| `rustok-blog` | Blog-домен поверх content и transport-адаптеры, включая author presentation через `rustok-profiles`. | `BlogModule`, `PostService`, `CommentService`, `graphql::*`, `controllers::*`. | Обходить blog-правила напрямую через `rustok-content` или SQL. |
