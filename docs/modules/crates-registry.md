...
| `rustok-installer` | Neutral installation policy contracts: plans, versioned topology, preflight, receipts, seed workflow, and executor ports. | `InstallPlan`, `InstallTopology`, `InstallProfile`, `InstallApplyOptions`, `InstallApplyOutput`, install state/receipt/preflight contracts. | Depend on `apps/server`, parse CLI arguments, compose Axum routes, run Cargo/deployment-provider commands, or duplicate install sequencing in a host. |
