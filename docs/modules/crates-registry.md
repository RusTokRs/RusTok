...
| `alloy` | Capability-слой script/runtime: script storage, execution, scheduler, bridge helper-ы, GraphQL/HTTP-поверхности и hook-oriented integration-контракты. | `create_default_engine`, `ScriptEngine`, `ScriptOrchestrator`, `Scheduler`, `ScriptRegistry`, `SeaOrmStorage`, `create_router`. | Регистрировать Alloy как tenant-toggled платформенный модуль или навешивать на него module lifecycle из `tenant_modules`. |
...
Alloy остаётся capability-слоем с permission-поверхностью `scripts:*`, но не входит в runtime-реестр.
