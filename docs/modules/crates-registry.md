...
| `rustok-ai` | Capability crate AI host/orchestrator слоя: multiprovider registry (`OpenAI-compatible`, `Anthropic`, `Gemini`), `AiRouter`, task profiles, hybrid direct/MCP execution model, persisted control-plane service layer для provider/task/tool profiles, sessions/runs/traces/approvals и capability-owned dual admin UI packages. | `ModelProvider`, `OpenAiCompatibleProvider`, `AnthropicProvider`, `GeminiProvider`, `AiRouter`, `AiRuntime`, `McpClientAdapter`, `ExecutionMode`, `ExecutionOverride`, `TaskProfile`, `ToolTrace`, `AiManagementService`. | Расширять `rustok-mcp` до model host; прятать AI authorization за `MCP_MANAGE` вместо RBAC-first permissions; делать MCP обязательной внутренней шиной для first-party AI workflows; дублировать AI business UI в `apps/admin` или `apps/next-admin` вместо capability-owned пакетов. |
