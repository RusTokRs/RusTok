...
The fresh recheck finds **9** recursive Page Builder FBA blocker nodes. The production inventory runner does not hard-code these paths; it derives them recursively from the canonical FBA registry on every execution. The current review snapshot is:
...
8. `/consumers/0/artifact_repair/executed_evidence`
9. `/consumers/0/cache_consumer/executed_evidence`

These are blocker **nodes**, not a claim that nine unrelated test suites must be invented. Several are umbrella/nested evidence states for the same repair lineage. The canonical registry remains the authority for when each node may truthfully stop saying `pending`.
...
With the current 9 FBA blocker nodes and the current Pages marker, an inventory run must remain `terminal_evidence_inventory_incomplete` and `owner_platform_review_ready=false`.
