...
Only an explicit allow-list of instrumented stable static leaf text nodes receives `contenteditable="plaintext-only"`. Provider-owned, nested, template-backed, runtime-bound, conditional and repeated targets remain read-only. The DOM is a temporary interaction buffer: a single bounded normalized plain-text request is emitted on bubbling `focusout`, and listener/attribute cleanup is deterministic.
...
- edit rich text, nested markup, provider components or runtime-owned content;
