...
- the current multilingual storage and runtime locale foundations are
  ...
  are implemented in `main`. Remaining ownership drift, broad-surface
  completeness, retained production-style PostgreSQL evidence, and live
...
- Flex declares `is_localized` field semantics and stores localized values
  outside language-neutral payloads. Flex schema copy, the attached
  `taxonomy.category` extension surface, and standalone localized values now
  expose neutral Translation providers. Attached and standalone fields use
  Flex-owned tenant/entity/field governance policy; missing/reset policy is
  fail-closed, explicit safe policy may admit AI export, and forbidden
  classifications remain denied.
...
| Exact locale vs fallback | Registered providers expose exact-locale state, but candidate owners still commonly expose fallback-oriented reads | Every additional provider must expose exact-locale availability separately from rendered fallback |
...
| Owner identity | Blog Category/Taxonomy and Forum Category/Taxonomy ownership are resolved; broad Product/Commerce Foundation, Pages/Navigation, and Content/SEO still contain ownership/completeness drift outside already registered narrow targets | Assign exactly one owner and remove superseded schema/entity paths before broader target registration; do not reintroduce a Blog- or Forum-local Category Translation owner |
...
| Flex exact-locale behavior | Attached `taxonomy.category` is registered and repository-hosted pilot-candidate evidence covers exact list/read, concurrent revision-safe/idempotent apply, aggregate progress, bounded changes, schema fan-out, hard-delete tombstones, ChangeCursor recovery, and reusable attached-field policy. Standalone `standalone_localized_value` now has equivalent registered exact-locale/revision/progress/change and governance contracts, but its focused retained PostgreSQL evidence is not green | Preserve fail-closed missing/reset policy and live export-policy reads; keep attached operator/live rollout evidence separate and repair/retain green standalone exact-head plus post-merge lifecycle/policy evidence before readiness promotion |
...
7. Flex attached Category extension fields: `flex/attached_localized_value` is
   ...
   production enablement remains an operator/live-rollout decision.
8. Flex standalone localized values: `flex/standalone_localized_value` is
   registered with durable exact-locale revision/change semantics, aggregate
   progress, bounded high-water ChangeCursor and fail-closed classification/
...
`34592656347`. No green standalone Flex retained lifecycle/policy run is claimed
by this evidence ledger.
...
bulk AI, broad Product catalog assumptions, or arbitrary settings JSON scanning
in this phase; the registered Settings pilot is limited to owner-admitted typed
fields.
...
- registered `seo/seo_copy` remains blocked until retained PostgreSQL
  CAS/replay/progress/ChangeCursor and reindex/event evidence is green; future
  owner-embedded SEO and technical canonical/robots/structured-data surfaces
  require their own owner classification rather than being folded into it;
...
- Fly/Page Builder localized properties through stable component/property
  identities;
- keep registered Flex `schema_copy`, attached `taxonomy.category`, and
  standalone localized-value providers on explicit schema-declared leaves;
  retain attached as a repository-hosted pilot candidate and keep standalone
  blocked until its focused lifecycle/policy PostgreSQL evidence is green;
- keep the implemented Flex field classification/export policy as the single
  governance path: fail closed on missing/reset, require explicit safe
...
Standalone Flex is intentionally absent from the green ledger: its current
focused retained lifecycle/policy PostgreSQL evidence is not green, so
`flex/standalone_localized_value` stays `blocked` despite registration.
