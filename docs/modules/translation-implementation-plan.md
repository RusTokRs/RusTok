...
This is the active cross-cutting implementation plan. As of 2026-09-09:
...
- Media, Taxonomy, Navigation menu, Pages metadata, Settings static fields,
  ...
  remains only an owner serialization/CAS mechanism. Attached dynamic leaves are
  not AI-exportable by default until Flex has explicit reusable schema-level
  classification/export policy. Provider status is registered while readiness
  remains blocked pending retained PostgreSQL migration, concurrency, schema
  fan-out, hard-delete tombstone, aggregate-progress, idempotent replay, and
  cursor-recovery evidence. Media's exact-locale CAS apply, stable receipt,
...
- Flex declares `is_localized` field semantics and stores localized values
  ...
  attached dynamic AI export stays fail-closed pending explicit schema policy.
...
| Flex exact-locale behavior | Attached `taxonomy.category` exact list/read, revision-safe/idempotent apply, aggregate progress and bounded changes are source-complete and registered; standalone localized-value Translation parity remains open | Collect retained PostgreSQL/concurrency/recovery evidence for the attached provider, keep AI export denied without explicit schema policy, and add equivalent durable exact-owner/change semantics before standalone localized-value provider activation |
...
audit and updated for the 2026-09-09 Translation evidence reconciliation. They
...
| Correct Flex exact semantics | Attached and standalone authoring reject invalid locales and prepare from an exact target-locale row; presentation fallback remains isolated to explicit read resolution. Attached `taxonomy.category` additionally has provider-facing exact source/target APIs, durable `attached:N` revision-safe/idempotent apply, aggregate progress and bounded ChangeCursor registration | Collect retained attached PostgreSQL/concurrency/recovery evidence; add explicit attached classification/export policy before AI export; complete equivalent exact owner revision/progress/change semantics for standalone localized values |
...
| Flex | schema copy and attached/standalone localized values | `flex/schema_copy` is registered. `flex/attached_localized_value` is registered for the `taxonomy.category` donor with exact list/read, validate/apply, aggregate progress, bounded ChangeCursor and one durable `attached:N` resource revision; readiness remains blocked until retained PostgreSQL/concurrency/recovery evidence. Only active localized text/textarea leaves are exposed, arbitrary payload JSON is never traversed, and attached dynamic AI export is denied until explicit reusable schema policy exists. Standalone localized-value Translation parity remains open |
...
7. Flex attached Category extension fields: `flex/attached_localized_value` is
   ...
   high-water ChangeCursor. Source-level activation is complete, but readiness
   remains blocked until retained PostgreSQL migration, concurrent CAS/replay,
   progress stability, schema fan-out, delete-tombstone and cursor-recovery
   evidence is collected. Dynamic attached fields deny AI export until an
   explicit reusable Flex schema classification/export policy exists.
...
- add explicit reusable Flex field classification/export policy before any
  dynamic attached leaf becomes AI-exportable;
...
Focused retained evidence as of 2026-09-09:
...
- Translation runtime composition post-merge `main@b16da9a8babe147b7ce75f871913b4bc4c34c506`:
  `33609559524`;
- Flex attached `taxonomy.category`: provider source-level activation is present,
  but no retained execution is claimed yet.
