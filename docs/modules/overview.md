...
Рекомендуемая структура внутри доменного crate:
...
- `adminNavItems` (или эквивалентный контракт admin-навигации, slot-компонентов и guard'ов).
- `frontendNavItems` (или эквивалентный контракт storefront-навигации/слотов).
...
| --- | --- | --- | --- | --- |
| `content` | отсутствует | отсутствует | ❌ Not ready | Создать `ui/admin` + `ui/frontend`; экспортировать `adminNavItems`/`frontendNavItems`; подключить через единый registry-контракт в `apps/admin` и `apps/next-admin`. |
| `commerce` | отсутствует | отсутствует | ❌ Not ready | Создать `ui/admin` + `ui/frontend`; определить контракты навигации/guard'ов; убрать хардкод экранов из приложений. |
| `blog` | `crates/rustok-blog/ui/admin` | `crates/rustok-blog/ui/frontend` | ✅ Ready (reference) | Это референсный образец UI-пакетов для optional-модулей; поддерживать совместимость export-контрактов (`adminNavItems`/`frontendNavItems`) для обоих runtime-стеков. |
| `forum` | частично (через `rustok-blog/ui/admin`) | отсутствует | ⚠️ Partial | Вынести в самостоятельные `ui/admin` и `ui/frontend` entry points (или зафиксировать composition-пакет), добавить `frontendNavItems`. |
| `pages` | отсутствует | отсутствует | ❌ Not ready | Создать модульные UI-пакеты и entry points для меню/редакторов/guard'ов. |
| `alloy-scripting` | отсутствует | отсутствует | ❌ Not ready | Определить UI surface (если требуется), добавить `ui/admin` entry point и контракт интеграции; для frontend зафиксировать N/A или реализовать пакет. |

> Референс-образец модульных UI-пакетов в репозитории: `crates/rustok-blog/ui/admin` и `crates/rustok-blog/ui/frontend`.
