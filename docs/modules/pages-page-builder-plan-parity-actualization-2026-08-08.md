...
- `docs/modules/pages-page-builder-provider-health-server-binding-actualization-2026-08-09.md`, which defines host/module authority, exact live identity revalidation, hot accepted/rejected packet replacement and a freshness lease while leaving live activation and capability-consumer binding pending.
...
- the authority revalidates accepted packet contract, exact live source/deployment identity, target/sample/query/freshness bounds and canonical Rust health/SLO policy on every rollout-status read;
- accepted packets are time-bounded by `evaluation.evaluated_at + deployment.freshness_seconds`; missing, rejected, malformed, identity-mismatched or expired packets return the default unobserved GraphQL shape;
...
-> server provider-health binding + hot revoke + freshness lease [source-ready / maintainer activation pending]
