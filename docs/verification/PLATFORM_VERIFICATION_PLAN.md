...
`core/rbac` remains in progress after PR #2747 merged the first trust correction.
PR #2837 added invalidation observability, PR #2842 made the access-token resolver
the single typed principal classifier, and PR #2846 added a source-ready incident
packet. Draft #2843 removes `RoleAssignmentStore` and resolver mutation delegation;
draft #2847 adds one sealed transactional artifact-permission event through the
canonical Outbox and still requires the repository-generated digest. Merged #2849
adds source-ready PostgreSQL concurrency, merged #2853 adds independent-process
watchdog recovery, merged #2856 adds a long-lived observer plus real Redis fast-
path/outage/restart source packet, and merged #2862 adds the full registered-CLI
repair propagation packet with two live DB-only observers. Draft #2863 corrects
the live Auth admin/GraphQL role path so exact canonical role or status replay does
not advance generation, fan out invalidation or redundantly revoke sessions, while
multiple or malformed assignments remain repairable. Historical draft #2859 was
closed as superseded by #2862. All recovery/concurrency and new regression packets
remain unexecuted. Formatting, API/Events/telemetry/all-feature owner/Admin/server/
CLI compilation, focused Rust and Node gates, live negative transports, retained
PostgreSQL/Redis/subprocess evidence, draft landing-order reconciliation and FFA/FBA
management evidence remain mandatory before RBAC can be completed or blocked.
