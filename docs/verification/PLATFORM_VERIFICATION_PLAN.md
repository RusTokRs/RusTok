...
job before RBAC Admin or `rustok-server`. The current cycle adds dedicated
invalidation observability in the canonical telemetry registry: durable and
applied generation, signed lag, watchdog state/restarts, database-read failures,
...
did not create jobs. Formatting, telemetry/all-feature owner/admin/server compile,
focused tests, the new source verifier, module validation, live negative transport
requests, PostgreSQL role mutation concurrency, one complete incident trace and
