...
context with a non-nil operator actor. The current operator-source branch deliberately
does not use tenant OAuth client ids because tenant administrators can rotate their
secrets; it instead validates a dedicated host-owned opaque token by SHA-256 digest for
native and HTTP GraphQL requests, keeps WebSocket denied, and uses tenant equality only
for Iggy secret ownership. Issue #2680 remains open until same-SHA source/unit/compile
and live denial, admission, audit, rotation/revocation and replica evidence exist.
Final-SHA formatting, source guards, compilation, targeted admin/commerce/events tests,
both PostgreSQL races, PostgreSQL fixture, real MySQL, Redis recovery and deployed/native
parity still gate completion. Source inspection and historical workflow runs are
