...
`core/tenant` remains in progress after nine fixed P1 findings and one fixed P2:
...
module policy for Tenant Admin badges; static public Tenant Admin native error envelopes
with correlation-aware private diagnostics; equivalent one-default-locale enforcement
on MySQL; and removal of the public low-level module-state writer, DTO/export and legacy
...
both PostgreSQL races, PostgreSQL fixture, real MySQL, tenant-admin/storefront/native
parity, Redis recovery, lifecycle and RBAC evidence still gate completion. GitHub
Actions jobs remain queued; source inspection is supporting evidence only.
