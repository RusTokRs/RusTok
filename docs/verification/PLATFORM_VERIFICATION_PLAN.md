...
`core/rbac` remains in progress after PR #2747 merged one P0 and one P1 trust
correction as `75b67f877eb405abe4e6761a16d6b7ece98bc103`. GraphQL role
reads/writes, REST artifact-role permission writes and native RBAC Admin now
consume an explicit typed principal context before `settings:read`,
`users:manage` or `modules:manage`. The access-token resolver classifies validated
claims once as direct user, OAuth delegated user or service principal and stores
the typed result on `CurrentUser`; HTTP/native middleware and GraphQL HTTP/
WebSocket composition only propagate it. Inconsistent combinations fail closed
before request context construction. `RbacControlPlanePrincipal` carries only
typed kind plus tenant id and cannot fall back to client, grant or session
metadata. Mutation audit identity remains `AuthContext.user_id`. Exact head
`3cf4b3a44980ca257f7f53849e905673141db289` compiled default `rustok-rbac`
in Rust-host workflow `30650883159`, but issue #2740 stopped the job before
RBAC Admin or `rustok-server`. The cycle also adds dedicated invalidation
observability in the canonical telemetry registry: durable and applied
generation, signed lag, watchdog state/restarts, database-read failures,
recovery/full-clear counters, bounded alert thresholds and Redis/missed-event/
regression/repair procedures. It preserves the database generation as authority
and the existing watchdog as the only recovery path. Standard CI and Hardening
did not create jobs. Formatting, rustok-api default/server, telemetry, all-feature
owner/admin/server compile, focused tests, the new source verifiers, module
validation, live negative transport requests, PostgreSQL role mutation
concurrency, one complete incident trace and multi-replica Redis recovery remain
mandatory before RBAC can be completed or blocked.
