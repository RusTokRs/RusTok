...
`users:manage` or `modules:manage`. Authentication classifies validated facts
once as direct user, OAuth delegated user or service principal at HTTP/native and
GraphQL HTTP/WebSocket composition; inconsistent combinations fail closed.
`RbacControlPlanePrincipal` carries only typed kind plus tenant id and cannot
fall back to client, grant or session metadata. Mutation audit identity remains
`AuthContext.user_id`. Exact head `3cf4b3a44980ca257f7f53849e905673141db289`
compiled default `rustok-rbac` in Rust-host workflow `30650883159`, but issue
#2740 stopped the job before RBAC Admin or `rustok-server`. The cycle also adds
dedicated invalidation observability in the canonical telemetry registry:
durable and applied generation, signed lag, watchdog state/restarts,
database-read failures, recovery/full-clear counters, bounded alert thresholds
and Redis/missed-event/regression/repair procedures. It preserves the database
generation as authority and the existing watchdog as the only recovery path.
Standard CI and Hardening did not create jobs. Formatting, rustok-api default/
server, telemetry, all-feature owner/admin/server compile, focused tests, the
new source verifiers, module validation, live negative transport requests,
PostgreSQL role mutation concurrency, one complete incident trace and
multi-replica Redis recovery remain mandatory before RBAC can be completed or
blocked.
