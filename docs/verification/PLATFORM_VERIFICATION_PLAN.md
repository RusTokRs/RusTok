...
correction as `75b67f877eb405abe4e6761a16d6b7ece98bc103`. REST artifact-role
permission writes and native RBAC Admin metadata now share the GraphQL owner
policy: direct grant, non-nil session and authenticated/resolved tenant equality
must pass before `modules:manage` or `settings:read`, and mutation audit identity
comes only from `AuthContext.user_id`. The active branch
`verification/cycle-001-rbac-read-only-resolver` fixes an additional P1:
`PermissionResolver` previously exposed role writes that the server delegated to
direct persistence helpers plus local-only cache invalidation, bypassing the
committed transaction owner, continuity locks, durable generation reservation
and cross-replica recovery. The branch makes the resolver read-only, removes the
obsolete `RoleAssignmentStore` export and `ServerRoleAssignmentStore` adapter
instead of retaining a compatibility wrapper, and adds
`rbac_permission_resolver_read_only_guard`. Exact head
`3cf4b3a44980ca257f7f53849e905673141db289` compiled default
`rustok-rbac` in Rust-host workflow `30650883159`, but issue #2740 stopped the
job before RBAC Admin or `rustok-server`; that historical run does not validate
the active branch. Standard CI and Hardening did not create jobs. Exact-branch
formatting, all-feature owner/admin/server compile, focused tests, verifiers,
module validation, live negative transport requests, PostgreSQL role mutation
concurrency and multi-replica Redis recovery remain mandatory before RBAC can be
