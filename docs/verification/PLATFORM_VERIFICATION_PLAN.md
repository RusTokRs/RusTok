...
correction as `75b67f877eb405abe4e6761a16d6b7ece98bc103`. PR #2837 added
bounded invalidation observability, PR #2842 made the access-token resolver the
single typed principal classifier, and PR #2846 added a source-ready incident
packet. Draft #2843 removes `RoleAssignmentStore` and resolver mutation
delegation; draft #2847 adds one sealed transactional artifact-permission event
through the canonical Outbox and still requires the repository-generated digest.
Merged #2849 adds source-ready PostgreSQL concurrency scenarios, and merged
#2853 adds independent observer/mutator watchdog recovery with Redis intentionally
absent. Draft #2857 adds the complementary Redis available and stop/restart source
packet with a real isolated Redis process, canonical success/failure counters,
subscriber readiness and durable recovery in the supervised ready callback. It
intentionally omits the watchdog so the mechanism cannot be satisfied by #2853's
fallback. All three recovery/concurrency packets remain unexecuted. Formatting,
API/Events/telemetry/all-feature owner/Admin/server compilation, focused Rust and
Node gates, live negative transports, retained PostgreSQL/Redis/subprocess evidence,
CLI repair propagation, draft landing-order reconciliation and FFA/FBA management
evidence remain mandatory before RBAC can be completed or blocked.
