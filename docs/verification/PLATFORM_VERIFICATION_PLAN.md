...
watchdog recovery, and merged #2856 adds a long-lived observer plus real Redis
fast-path/outage/restart source packet. Draft #2859 adds the remaining live CLI
repair path: two observers warm stale Manager authorization, a third process runs
the production `repair-system-roles` command provider, and both observers must
recover through the committed generation without restart. All recovery/concurrency
packets remain unexecuted. Formatting, API/Events/telemetry/all-feature owner/Admin/
server/CLI compilation, focused Rust and Node gates, live negative transports,
retained PostgreSQL/Redis/subprocess evidence, draft landing-order reconciliation
and FFA/FBA management evidence remain mandatory before RBAC can be completed or
blocked.
