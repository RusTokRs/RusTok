...
`core/rbac` remains in progress. PR #2747 merged one P0 and one P1 trust correction as
`75b67f877eb405abe4e6761a16d6b7ece98bc103`; PR #2867 merged the canonical
built-in role mutation policy and typed mutation events. Fresh draft PR #2870
supersedes closed #2866 and continues only on branch
`agent/core-rbac-cycle-001-20260803`; technical PR #2879 merged current `main` into
that branch as `a66d5d5c308a05fd5cc4c8ae941e25b68bde358c` without force-push or a
write to `main`. The draft makes permission resolvers read-only, keeps existing-user
role mutation on explicit caller transactions, preserves exact role and status replay
as authorization no-ops, and publishes artifact role-permission changes through the
canonical Outbox in the same RBAC owner transaction. Static inspection fixed five P1
source defects: localized label/description copy was stored in the authorization
identity row; locale keys were not normalized and allowed semantic duplicates;
trigger-only existence checks did not protect parents against concurrent update/delete;
a direct DB write could bind one tenant to another tenant's permission definition; and
admitted definition scope/installation/module/release/key identity remained mutable.
#2870 now uses one immutable language-neutral artifact permission definition plus owner
translations with canonical `rustok_api::normalize_locale_tag` and `VARCHAR(32)` locale,
binds grants and receipts to exact `artifact_permission_id` plus admitted
`permission_scope_key`, adds tenant-composite role/user and exact-scope permission
foreign keys/checks, propagates exact identity through authorization and the sealed
event, removes the superseded corrective migration, rejects nil tenant scope, and adds
SQLite and source regressions. Current account deletion is documented and verified as
deactivation/redaction; unsupported hard parent deletion fails closed through
`RESTRICT` until an owner-coordinated teardown workflow is introduced. The pass also
removes compatibility wording from the sole new-user role initialization entrypoint and
replaces a broad Events lint allowance with one narrow reasoned expectation. The branch
remains source-only: generated digest, formatting, Events/RBAC/Admin/server compilation,
focused tests, verifier/module gates, real PostgreSQL apply/integrity/canonical-locale/
scope/parent-delete concurrency/rollback, live negative transports, incident evidence,
watchdog/CLI packets and multi-replica Redis recovery remain mandatory. `core/rbac`
must not advance or merge until required evidence is retained and current workflow
