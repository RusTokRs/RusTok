...
correction as `75b67f877eb405abe4e6761a16d6b7ece98bc103` and PR #2837 merged
bounded invalidation observability as `1b7dc0fa749c792c35c74e0ca55d9510c64453cd`.
REST artifact-role permission writes, GraphQL role operations and native RBAC Admin
metadata share one owner policy. The current actor-kind slice publishes
`AuthPrincipalKind::{DirectUser, DelegatedUser, Service}`, centralizes legacy
fact validation in one fail-closed shared classifier and removes grant/client
interpretation from the RBAC owner and all three transport adapters. Direct
population at token resolution and deletion of the migration classifier remain
open. Draft #2799 removes the obsolete resolver mutation path; draft #2782 adds
the missing transactional artifact-permission owner event. Both are source-only
and need reconciliation with this branch. Formatting, rustok-api/telemetry/
all-feature owner/admin/server compile, focused tests/verifiers, event digest,
