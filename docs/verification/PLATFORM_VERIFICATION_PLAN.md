...
`core/tenant` remains in progress after seven fixed P1 findings and one fixed P2:
...
`TenantContext` scope for native storefront module-state reads; equivalent
one-default-locale enforcement on MySQL; and removal of the public low-level
...
Same-SHA Rust, both PostgreSQL races, PostgreSQL fixture, real MySQL, storefront/native
