...
`agent/core-rbac-cycle-001-20260803`. It makes permission resolvers read-only, keeps
existing-user role mutation on explicit caller transactions, preserves exact role and
status replay as authorization no-ops, and publishes artifact role-permission changes
through the canonical Outbox in the same RBAC owner transaction. Static inspection in
this handoff found two additional P1 source defects: localized label/description copy
was stored in the authorization identity row, and trigger-only existence checks did not
lock referenced parents, allowing a concurrent role/user/catalog delete or tenant
change to commit orphan authorization rows. #2870 now uses one language-neutral
artifact permission definition plus owner translations with `VARCHAR(32)` locale,
binds grants and receipts to exact `artifact_permission_id`, adds tenant-composite
role/user and permission foreign keys, propagates exact identity through authorization
and the sealed event, removes the superseded corrective migration, and adds SQLite and
source regressions. It also removes compatibility wording from the sole new-user role
initialization entrypoint and replaces a broad Events lint allowance with one narrow
reasoned expectation. The branch remains source-only: generated digest, formatting,
Events/RBAC/Admin/server compilation, focused tests, verifier/module gates, real
PostgreSQL apply/integrity/parent-delete concurrency/rollback, live negative transports,
incident evidence, watchdog/CLI packets and multi-replica Redis recovery remain
mandatory. Deliberate teardown ordering for referenced artifact grants and receipts is
an explicit P1 lifecycle decision. `core/rbac` must not advance or merge until required
evidence is retained and current workflow product failures are resolved.
