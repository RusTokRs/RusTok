...
- 2026-06-17, `blog`, slice #95: admin status-badge presentation moved into Leptos-free `BlogPostStatusBadgeViewModel`, raw-body warning rendering now uses an explicit memoized core payload, and form title/submit state reuses one memoized `BlogPostAdminFormViewModel`; guardrail markers and fixtures were updated for the new boundary. Evidence: `node scripts/verify/verify-blog-admin-boundary.mjs`; `node --test scripts/verify/verify-blog-admin-boundary.test.mjs`; Cargo compilation was intentionally not run by request.
