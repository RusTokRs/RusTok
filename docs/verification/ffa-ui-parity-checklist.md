...
- 2026-06-14, `blog`, slice #88 follow-up: the SEO panel copy-only core extraction was rolled back as over-extraction under the FFA anti-over-extraction rule; one-off SEO i18n labels remain in the Leptos adapter, and `verify-blog-admin-boundary.mjs` now rejects `BlogPostAdminSeoPanelCopy`/`blog_post_admin_seo_panel_copy` in core. `node scripts/verify/verify-blog-admin-boundary.mjs` and `node scripts/verify/verify-blog-admin-boundary.test.mjs` passed; native/GraphQL transport surfaces were not changed.
