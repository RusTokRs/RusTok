...
- 2026-06-14, `blog`, slice #88: admin GraphQL adapter moved from legacy root `admin/src/api.rs` into transport-owned `admin/src/transport/graphql_adapter.rs`; crate root no longer wires `mod api`, and `verify-blog-admin-boundary.mjs` rejects legacy api module/file fixtures while confirming `transport/mod.rs` owns the GraphQL adapter. `node scripts/verify/verify-blog-admin-boundary.mjs` and `node scripts/verify/verify-blog-admin-boundary.test.mjs` passed; native/GraphQL transport contract text was moved, not changed.
- 2026-06-14, `blog`, slices #86-#87: admin save-result policy moved into Leptos-free `BlogPostSaveResultViewModel`, then selected-post route/query push/replace/clear intent selection moved into Leptos-free `BlogPostAdminRouteQueryIntent`; create/update/delete/open flows now receive core-owned query intents and the Leptos adapter only applies them through the host query writer; `node scripts/verify/verify-blog-admin-boundary.mjs` passed after extending the guardrail for save-result and route/query intent helpers; Cargo compilation was intentionally avoided by request except for short non-compiling checks; native/GraphQL transport surfaces were not changed.
