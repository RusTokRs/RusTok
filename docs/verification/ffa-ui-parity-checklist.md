...
- 2026-06-14, `blog`, slice #88: admin SEO panel title/subtitle/empty-message copy moved into Leptos-free `BlogPostAdminSeoPanelCopy`; `BlogAdmin` now passes prepared copy into the owner-side `SeoEntityPanel`, while native/GraphQL transport surfaces remain unchanged. `node scripts/verify/verify-blog-admin-boundary.mjs` passed after extending the guardrail for SEO copy ownership; Cargo compilation was intentionally avoided by request.
