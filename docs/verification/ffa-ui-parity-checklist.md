...
- 2026-06-18, `region`, slice #40: added a dedicated storefront boundary verifier and fixture suite for native→GraphQL fallback order, adapter isolation, Leptos-free core and stable error DOM evidence; both scripts are wired into aggregate FFA verify/test pipelines. Runtime integration evidence remains pending.
- 2026-06-18, `channel`: the 2.7k-line flat Leptos adapter was split into `ui/leptos/` with `mod.rs`, `runtime_context.rs`, `policy_workbench.rs`, `policy_set_card.rs` and `channel_card.rs`; `ChannelAdmin` remains the public entry point and transport/core ownership is unchanged. Evidence: `node scripts/verify/verify-channel-admin-boundary.mjs`; `node --test scripts/verify/verify-channel-admin-boundary.test.mjs`; aggregate `npm run verify:ffa:ui:migration`.
- 2026-06-18, `channel`: policy-rule create/full-update/active-toggle transport payload preparation moved into Leptos-free `PolicyRuleFormState` methods and `policy_rule_active_update_payload`; Leptos snapshots signal values and dispatches prepared DTOs through the unchanged transport facade. Evidence: `node scripts/verify/verify-channel-admin-boundary.mjs`; `node --test scripts/verify/verify-channel-admin-boundary.test.mjs`.
- 2026-06-18, `channel`: policy-rule reorder bounds and swap policy moved into Leptos-free `reorder_policy_rule_ids`; move-up/down handlers only dispatch validated ordered IDs through the existing transport facade. Evidence: `node scripts/verify/verify-channel-admin-boundary.mjs`; `node --test scripts/verify/verify-channel-admin-boundary.test.mjs`.
- 2026-06-18, `channel`: policy-rule create defaults and edit DTO-to-form mapping moved into Leptos-free `PolicyRuleFormState`, `policy_rule_create_form_state` and `policy_rule_edit_form_state`; Leptos retains only signal application and event binding. Transport paths were not changed. Evidence: `node scripts/verify/verify-channel-admin-boundary.mjs`; `node --test scripts/verify/verify-channel-admin-boundary.test.mjs`.
- 2026-06-18, `channel`: policy-set/rule URL selection normalization moved into Leptos-free `ChannelPolicySelectionCleanup` / `channel_policy_selection_cleanup`; Leptos now only applies prepared clear-rule or clear-set-and-rule outcomes, while native-first/REST fallback transport remains unchanged. Evidence: `node scripts/verify/verify-channel-admin-boundary.mjs`; `node --test scripts/verify/verify-channel-admin-boundary.test.mjs`.
- 2026-06-18, `product`: shipping-profile loading/error/ready classification, select options and registry panel state now come from one Leptos-free `ProductAdminShippingProfilesLoadViewModel`; both Leptos consumers use `product_admin_shipping_profiles_load_view_from_result`, eliminating duplicate async-result branches without changing GraphQL transport. Evidence: `node scripts/verify/verify-product-admin-boundary.mjs`; `node --test scripts/verify/verify-product-admin-boundary.test.mjs`.
- 2026-06-18, `product`: admin products-list loading/error/empty/ready normalization moved into Leptos-free `ProductAdminProductsLoadViewModel` / `product_admin_products_load_view_from_result`; the Leptos adapter no longer unpacks `ProductList` or owns empty-result classification. Native/GraphQL transport surfaces were not changed. Evidence: `node scripts/verify/verify-product-admin-boundary.mjs`; `node --test scripts/verify/verify-product-admin-boundary.test.mjs`.
