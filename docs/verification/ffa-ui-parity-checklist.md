...
- 2026-06-14, `blog`, slice #88: admin GraphQL fallback parity guardrail now requires `BlogAdmin` to classify missing posts contract through `transport::is_posts_contract_unavailable`, and requires the GraphQL adapter to retain legacy `PostsFilter`/`posts` missing-contract markers; `node scripts/verify/verify-blog-admin-boundary.mjs` and `node scripts/verify/verify-blog-admin-boundary.test.mjs` passed; native/GraphQL transport surfaces were not changed.
