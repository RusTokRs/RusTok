...
- 2026-06-14, `blog`, slices #86-#87: admin save-result policy moved into Leptos-free `BlogPostSaveResultViewModel`, then selected-post load-result policy and transport failure issue mapping moved into Leptos-free `BlogPostLoadResultViewModel` / `blog_post_transport_failure_issue`; create/update success, edit/load missing-post handling and write/load transport errors now receive core-owned instructions/issues; `node scripts/verify/verify-blog-admin-boundary.mjs` passed after extending the guardrail for the save/load/error helpers; Cargo compilation was intentionally not run by request; native/GraphQL transport surfaces were not changed.
