...
**Монолитный режим** (как WordPress) — один бинарник, один процесс, один порт:
- Leptos-админка и storefront(ы) компилируются в тот же бинарник через SSR
...
- Фронтенд на другом порту/домене/стеке (Next.js, Nuxt, Flutter, мобилка)
...
- Leptos-админка встроена (сессии), при этом storefront — отдельный SPA или мобилка (OAuth)
...
Принцип: **OAuth — для внешних клиентов. Встроенные Leptos-приложения всегда работают через сессии, независимо от `oauth_enabled`.**
