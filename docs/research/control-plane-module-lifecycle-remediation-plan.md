...
- [ ] stale revision не создаёт build job;
- [ ] ошибка enqueue/build insert не меняет `platform_state.revision`;
- [ ] GraphQL/Leptos SSR parity по `manifest_ref`, revision и error mapping.
...
- [ ] parity ошибок GraphQL/Leptos SSR (`unknown/core/dependency/dependent`);
- [ ] journal (`module_operations`) пишется всегда при смене effective state;
