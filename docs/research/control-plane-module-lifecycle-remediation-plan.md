...
### Актуализация 2026-05-28 (итерация 26)

- В `ModuleLifecycleService` добавлен read-side recovery contract для failed `module_operations`: `ModuleOperationRecoveryPlan` классифицирует post-hook/pre-hook/other failures, показывает retryability, рекомендуемое действие, actor/correlation metadata и текущий journal context без локального SQL-дублирования.
- Добавлены canonical server-side операции `retry_failed_post_hook_operation(...)` и `compensate_failed_operation(...)`: retry повторяет только post-hook для уже committed target-state и создаёт новый journal attempt с новым `correlation_id`, а compensation идёт через обычный lifecycle toggle к `previous_effective_enabled`.
- Integration coverage расширено transient post-hook сценарием: первый `post_enable` падает, explicit retry создаёт committed recovery attempt без duplicate side effects; pre-hook failure отдельно проверен как non-retryable для post-hook retry API.
- На пути проверки remediation bundle устранены compile-drifts в server test-surface: актуализированы imports/typed error contracts для GraphQL mutations, channel controller, app lifecycle и platform composition build service tests под текущие сигнатуры.
...
- [x] GraphQL/Leptos SSR parity for `manifest_ref`, revision и error mapping (закрыто guard-набором: GraphQL mutations возвращают `manifestRef`/`manifestHash`/`manifestRevision`, admin helpers не парсят manifest contract локально, server build tests фиксируют `platform_state:<revision>`/revision/hash parity, Leptos SSR adapter tests фиксируют passthrough composition taxonomy/manifest fragments).
...
- [x] parity ошибок GraphQL/Leptos SSR (`unknown/core/dependency/dependent`): GraphQL mapper matrix, Leptos SSR adapter passthrough matrix, GraphQL-only admin helper guard и lifecycle runtime/journal parity guard фиксируют server-owned taxonomy без локального remap;
- [x] журнал (`module_operations`) всегда записывается при смене effective state (server integration tests покрывают success/failure actor metadata, `requested_by`/`correlation_id`/cardinality, post-hook no-rollback, predvalidation/no-op no-journal rules; Leptos SSR adapter и admin recovery surface фиксируют passthrough journal metadata без локального parsing/remap).
...
- [x] Ввести lifecycle status model: `validated -> running -> committed -> failed` (runtime journal использует typed модель, GraphQL recovery plan публикует `status`, admin recovery surface потребляет read-side contract, а guard фиксирует explicit status transitions).
- [x] Писать `module_operations` в статусе `running` до state mutation (с correlation id) (pipeline использует `validated -> running` до pre-hook/state commit, read-side GraphQL recovery plan публикует `status`/`correlation_id`, guard фиксирует transition + public fields).
...
- [x] Не откатывать committed state при post-hook failure: фиксировать retryable issue (состояние больше не откатывается; отдельный retryable-issue read/write contract опубликован через GraphQL и потребляется Leptos admin recovery block).
- [x] Добавить recovery механизм (retry/compensating operation) через journal (server service + GraphQL read/write recovery surface).

**Обязательные тесты закрытия:**

- [x] pre-hook failure не меняет effective state;
- [x] post-hook failure не откатывает committed state и создаёт retryable issue (state/no-rollback покрыт тестами; отдельный retryable-issue contract/read-side закрыт GraphQL recovery plan surface и Leptos admin consumption).
...
- [x] Убрать drift: GraphQL, BuildService и Leptos SSR используют один canonical hashing contract (Leptos SSR и server composition используют shared `rustok_api::manifest_hash`, GraphQL build payload сохраняет `manifestRef`/`manifestHash`/`manifestRevision`, server integration tests фиксируют snapshot/build hash+snapshot parity и стабильность hash/snapshot across revisions, admin guard фиксирует cross-surface contract).
...
- [x] Unified parity tests: GraphQL vs Leptos SSR (закрыто combined guard/matrix: server mapper tests, Leptos SSR adapter passthrough tests, GraphQL-only admin helper, recovery UI consumption и journal metadata source guards).

**Негативные сценарии, обязательные до merge.**

- [x] Unknown module, core module, missing dependency, dependent modules: одинаковые ошибки в обоих surfaces (server mapper matrix + Leptos SSR adapter passthrough matrix + admin GraphQL-only toggle guard фиксируют runtime taxonomy без local remap).
- [x] При успешном toggle всегда пишется journal (`module_operations`) и actor/correlation metadata (server integration tests + Leptos SSR passthrough + admin recovery consumption фиксируют journal metadata parity).

### Итерация C — закрыть P0.3 (hook semantics + recovery)

**Scope.**

- [x] Реализовать lifecycle state-machine (`validated/running/committed/failed`) (pipeline, GraphQL recovery read-side и admin consumption зафиксированы guard/docs).
- [x] Развести pre/post hooks по моменту исполнения и гарантиям.
- [x] Внедрить retryable post-hook issues без отката committed state (server-side recovery plan, explicit post-hook retry и GraphQL read/write recovery surface реализованы; Leptos admin обязан потреблять canonical GraphQL contract без локальной taxonomy).

**Deliverables.**

- [x] Compat-layer для legacy `on_enable`/`on_disable` с явно задокументированным поведением.
- [x] Journal schema/fields достаточны для manual и automated retry (поля покрывают recovery plan/retry attempt и опубликованы через GraphQL recovery plan surface).
...
- [x] Post-hook failure: effective state сохранён, operation помечен как retryable issue (no-rollback, failed journal, explicit retry и external GraphQL recovery contract закрыты).
...
### Актуализация 2026-05-29 (итерация 73)
...
### Актуализация 2026-05-30 (итерация 75)
...
### Актуализация 2026-05-30 (итерация 78)

- Закрыт lifecycle status/read-side хвост: `lifecycle_bypass_guard` теперь фиксирует explicit `validated/running/committed/failed` status model, transition writes (`running` до commit, `committed`/`failed` terminal states), GraphQL recovery plan fields (`status`, `issue`, `retryable`, `recommended_action`, `correlation_id`, `requested_by`, `error_message`) и admin GraphQL consumption.
- Чекбоксы status model / running-before-mutation / lifecycle state-machine переведены в `[x]`, потому что contract теперь покрыт runtime pipeline + GraphQL read-side + admin consumption + source guard без запуска долгой компиляции.
- Текущий snapshot после итерации: `pending=1`, а открытый хвост сузился до broad GraphQL/Leptos parity, journal metadata parity, manifest hash/ref parity, docs sync и полного minimal verification bundle.

### Актуализация 2026-05-30 (итерация 79)

- Закрыт manifest hash/ref/revision parity хвост: admin guard `manifest_hash_ref_revision_contract_is_shared_across_surfaces` связывает Leptos SSR runtime hash helper, server `PlatformCompositionService::manifest_hash`, GraphQL build payload (`manifestRef`/`manifestHash`/`manifestRevision`), server build-service parity tests и Leptos SSR adapter composition taxonomy/manifest-fragment passthrough tests.
- Чекбоксы `GraphQL/Leptos SSR parity for manifest_ref/revision/error mapping` и `GraphQL, BuildService и Leptos SSR используют один canonical hashing contract` переведены в `[x]` как закрытые lightweight guard cutover без долгой компиляции.
- Текущий snapshot после итерации: `pending=1`, открытыми остаются broad lifecycle/journal GraphQL↔Leptos parity пункты, docs sync и полный minimal verification bundle.

### Актуализация 2026-05-30 (итерация 80)
