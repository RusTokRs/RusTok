...
- [~] GraphQL/Leptos SSR parity по `manifest_ref`, revision и error mapping (GraphQL error mapping tests добавлены; Leptos SSR parity coverage остаётся незакрытым).
...
- [~] parity ошибок GraphQL/Leptos SSR (`unknown/core/dependency/dependent`): GraphQL error mapping вынесен в `map_toggle_module_error` и покрыт unit tests (unknown/core/dependency/dependent/hook_failed + internal mapping для database/policy); в `apps/admin` добавлен unit test для fallback error contract (`native path failed ...; graphql path failed ...`) при `toggle_module_native` disable-mode; остаётся закрыть full parity tests Leptos SSR vs GraphQL для runtime error taxonomy;
- [~] journal (`module_operations`) пишется всегда при смене effective state (добавлены server integration tests `successful_toggle_writes_done_module_operation`, `hook_failure_rolls_back_state` (failed journal), `successful_toggle_with_actor_persists_requested_by`, `toggle_without_actor_records_null_requested_by`, `hook_failure_with_actor_records_failed_operation_with_actor`, `hook_failure_without_actor_records_failed_operation_with_null_actor`, проверки что idempotent retry не создаёт duplicate journal rows и что pre-validation failures (`UnknownModule`/`CoreModuleCannotBeDisabled`/`MissingDependencies`/`HasDependents`) и no-op disable/enable transitions не создают лишние journal rows; остаётся parity-покрытие GraphQL/Leptos SSR);
- [~] прямой toggle bypass отсутствует в production code paths (repo-guard расширен на `apps/server` + `apps/admin`; остаётся закрыть parity evidence из GraphQL/Leptos SSR suite).

#### 1.3 Hook semantics без частичного rollback

- [ ] Ввести lifecycle status model: `validated -> running -> committed -> failed`.
- [ ] Писать `module_operations` в статусе `running` до state mutation (с correlation id).
...
- [ ] Убрать drift: GraphQL, Leptos SSR и BuildService используют общий hash builder.

**Обязательные тесты закрытия:**

- [ ] один и тот же manifest даёт одинаковый hash/ref/snapshot во всех surfaces;
- [ ] изменения в значимых полях гарантированно меняют hash;
- [ ] length/format hash соответствует DB контракту.
...
- [ ] Обновить `docs/verification/platform-quality-operations-verification-plan.md` (явно зафиксировать gates).
- [ ] Добавить script/test для проверки существования всех Dependabot directories.
- [ ] Вынести coverage threshold в единый источник (`env`/constant), чтобы docs/workflow не расходились.

**Обязательные тесты закрытия:**

- [ ] script падает на несуществующем dependabot directory;
- [ ] CI использует общий threshold source;
- [ ] docs описывают актуальные gates без расхождения с workflow.

### Этап 3 (P2): документация, ADR, финальная синхронизация

- [ ] Обновить central docs (`docs/architecture/modules.md`, `docs/modules/manifest.md`) по фактическому runtime contract.
- [ ] Обновить локальные docs server/admin по новым control-plane и lifecycle flows.
...
- [ ] В `deep-research-report (4).md` оставить ссылку на этот план как на актуальный remediation backlog.
...
- [ ] `deep-research-report (4).md` помечен как historical input + ссылка на этот remediation backlog.
