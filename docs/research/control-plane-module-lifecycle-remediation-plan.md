...
- [~] parity ошибок GraphQL/Leptos SSR (`unknown/core/dependency/dependent`): GraphQL error mapping вынесен в `map_toggle_module_error` и покрыт unit tests (unknown/core/dependency/dependent/hook_failed); Leptos SSR parity tests ещё открыты;
- [~] journal (`module_operations`) пишется всегда при смене effective state (добавлены server integration tests `successful_toggle_writes_done_module_operation`, `hook_failure_rolls_back_state` (failed journal) и `successful_toggle_with_actor_persists_requested_by`; остаётся parity-покрытие GraphQL/Leptos SSR);
- [ ] прямой toggle bypass отсутствует в production code paths.
