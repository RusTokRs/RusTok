...
- [~] Ввести lifecycle status model: `validated -> running -> committed -> failed` (частично: runtime journal уже использует `running/committed/failed`; остаётся добавить отдельный `validated` этап и связанный contract).
