...
| `pricing` | provider read-projection для checkout/product price consumers | `in_progress` | Закрыть contract tests для `PricingReadPort` и заменить embedded/GraphQL compatibility evidence фактическими parity snapshots перед повышением статуса | `crates/rustok-pricing/contracts/pricing-fba-registry.json`, `crates/rustok-pricing/docs/implementation-plan.md` |
| `order` | provider checkout completion/result для commerce orchestration | `in_progress` | Закрыть contract tests для `CheckoutCompletionPort`, зафиксировать идемпотентность completion и remote-fallback smoke до `boundary_ready` | `crates/rustok-order/contracts/order-fba-registry.json`, `crates/rustok-order/docs/implementation-plan.md` |
| `payment` | provider payment collection create/reuse для commerce checkout | `in_progress` | Закрыть contract tests для `PaymentCollectionPort` и provider SPI mapping без переноса gateway rules в commerce transport | `crates/rustok-payment/contracts/payment-fba-registry.json`, `crates/rustok-payment/docs/implementation-plan.md` |
| `fulfillment` | provider seller-aware shipping selection для commerce checkout | `in_progress` | Закрыть contract tests для `ShippingSelectionPort`, сохранить fallback/degraded profiles и перенести оставшийся selection transport из commerce compatibility path | `crates/rustok-fulfillment/contracts/fulfillment-fba-registry.json`, `crates/rustok-fulfillment/docs/implementation-plan.md` |
