...
| `payment` | provider payment collection create/reuse для commerce checkout | `in_progress` | Закрыть contract tests для `PaymentCollectionPort` и provider SPI mapping без переноса gateway rules в commerce transport | `crates/rustok-payment/src/ports.rs`, `crates/rustok-payment/contracts/payment-fba-registry.json`, `crates/rustok-payment/docs/implementation-plan.md` |
