...
| `cart` | provider checkout snapshot for commerce checkout consumers | `in_progress` | Close contract tests/fallback smoke for `CartSnapshotReadPort`, confirm embedded/GraphQL checkout/storefront compatibility snapshots before status promotion | `crates/rustok-cart/src/ports.rs`, `crates/rustok-cart/contracts/cart-fba-registry.json`, `crates/rustok-cart/docs/implementation-plan.md` |
...
| `comments` | provider generic comment thread boundary for blog/commentable-surface consumers | `in_progress` | Close runtime contract tests/fallback smoke for `CommentsThreadPort`, confirm blog embedded/native compatibility snapshots before status promotion | `crates/rustok-comments/src/ports.rs`, `crates/rustok-comments/contracts/comments-fba-registry.json`, `crates/rustok-comments/docs/implementation-plan.md` |
| `blog` | consumer generic comment thread boundary from `comments` | `in_progress` | Close real runtime consumer contract execution for `blog_post_comments`; no-compile source-smoke already confirms source markers for degraded modes `hide_comment_form`/`show_cached_thread_snapshot` without status promotion | `crates/rustok-blog/contracts/blog-fba-registry.json`, `crates/rustok-blog/contracts/evidence/blog-comments-consumer-static-matrix.json`, `crates/rustok-blog/contracts/evidence/blog-comments-runtime-fallback-smoke.json`, `crates/rustok-blog/docs/implementation-plan.md` |
...
| `seo` | consumer image descriptor boundary from `media` | `in_progress` | Close runtime consumer contract execution/fallback smoke for `seo_image_descriptor`; static matrix already `source_locked_pending_consumer_runtime` and confirms provider fallback-smoke source/degraded modes `omit_image_metadata`/`keep_existing_seo_image`/`proxy_storage_relative_url`, but does not promote status without runtime evidence | `crates/rustok-seo/contracts/seo-fba-registry.json`, `crates/rustok-seo/contracts/evidence/seo-media-consumer-static-matrix.json`, `crates/rustok-seo/docs/implementation-plan.md` |
...
As of 2026-06-18 `cart` added to the ecommerce provider track as a checkout snapshot provider: `CartSnapshotReadPort`/`cart.checkout_snapshot.v1`, registry `crates/rustok-cart/contracts/cart-fba-registry.json` and static evidence `crates/rustok-cart/contracts/evidence/cart-contract-test-static-matrix.json` are included in `npm run verify:ecommerce:fba` together with cart dependency in the commerce consumer registry.
...
As of 2026-06-30 the consumer wave added a unified no-compile runtime-order gate for `blog` and `seo`: `contracts/evidence/*-consumer-runtime-order-smoke.json` and `scripts/verify/verify-consumer-fba-runtime-order.mjs` capture blog -> comments target validation/provider-call/typed mapping order, SEO -> media read-policy/tenant parse/descriptor construction order, independent `SeoTargetImageRecord` mapping and fallback degraded metadata parity. Statuses remain `in_progress` until live consumer runtime execution.
...
As of 2026-06-19 `media` added as a provider track for asset read/image descriptor boundary: `MediaAssetReadPort`/`media.asset_read.v1`, registry `crates/rustok-media/contracts/media-fba-registry.json` and static evidence `crates/rustok-media/contracts/evidence/media-contract-test-static-matrix.json` and source-locked fallback smoke `crates/rustok-media/contracts/evidence/media-runtime-fallback-smoke.json` are checked by the fast gate `npm run verify:media:fba` without promotion to `boundary_ready` until executable runtime execution/fallback smoke.
