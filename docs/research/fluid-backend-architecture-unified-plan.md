...
| `blog` | consumer generic comment thread boundary от `comments` | `in_progress` | Закрыть runtime consumer contract execution/fallback smoke для `blog_post_comments`, подтвердить degraded modes `hide_comment_form`/`show_cached_thread_snapshot` перед повышением статуса | `crates/rustok-blog/contracts/blog-fba-registry.json`, `crates/rustok-blog/contracts/evidence/blog-comments-consumer-static-matrix.json`, `crates/rustok-blog/docs/implementation-plan.md` |
| `media` | provider asset read/image descriptor boundary для SEO/AI media consumers | `in_progress` | Закрыть runtime contract tests/fallback smoke для `MediaAssetReadPort`, подтвердить SEO image descriptor degraded modes перед повышением статуса | `crates/rustok-media/src/ports.rs`, `crates/rustok-media/contracts/media-fba-registry.json`, `crates/rustok-media/docs/implementation-plan.md` |
| `seo` | consumer image descriptor boundary от `media` | `in_progress` | Закрыть runtime consumer contract execution/fallback smoke для `seo_image_descriptor`, подтвердить degraded modes `omit_image_metadata`/`keep_existing_seo_image` перед повышением статуса | `crates/rustok-seo/contracts/seo-fba-registry.json`, `crates/rustok-seo/contracts/evidence/seo-media-consumer-static-matrix.json`, `crates/rustok-seo/docs/implementation-plan.md` |
| `ai-media` | support consumer image asset descriptor boundary от `media` | `in_progress` | Закрыть runtime support-consumer contract execution/fallback smoke для `ai_asset_descriptor`, подтвердить degraded mode `skip_asset_enrichment` перед повышением статуса | `crates/rustok-ai-media/contracts/ai-media-fba-registry.json`, `crates/rustok-ai-media/contracts/evidence/ai-media-consumer-static-matrix.json`, `crates/rustok-ai-media/docs/implementation-plan.md` |
...
На 2026-06-19 `blog` добавлен как consumer track для `comments.thread.v1`: registry `crates/rustok-blog/contracts/blog-fba-registry.json`, static evidence `crates/rustok-blog/contracts/evidence/blog-comments-consumer-static-matrix.json`, manifest `[fba.consumer]` и быстрый gate `npm run verify:blog:fba` фиксируют `CommentsThreadPort` dependency, fallback profiles и degraded modes без повышения до `boundary_ready` до runtime execution/fallback smoke.

На 2026-06-19 `media` добавлен как provider track для asset read/image descriptor boundary: `MediaAssetReadPort`/`media.asset_read.v1`, registry `crates/rustok-media/contracts/media-fba-registry.json` и static evidence `crates/rustok-media/contracts/evidence/media-contract-test-static-matrix.json` проверяются быстрым gate `npm run verify:media:fba` без повышения до `boundary_ready` до runtime execution/fallback smoke.

На 2026-06-19 `seo` добавлен как consumer track для `media.asset_read.v1`: registry `crates/rustok-seo/contracts/seo-fba-registry.json`, static evidence `crates/rustok-seo/contracts/evidence/seo-media-consumer-static-matrix.json`, manifest `[fba.consumer]` и быстрый gate `npm run verify:seo:fba` фиксируют `MediaAssetReadPort` dependency, fallback profiles и degraded modes без повышения до `boundary_ready` до runtime execution/fallback smoke.

На 2026-06-19 `ai-media` добавлен как support-consumer track для `media.asset_read.v1`: registry `crates/rustok-ai-media/contracts/ai-media-fba-registry.json`, static evidence `crates/rustok-ai-media/contracts/evidence/ai-media-consumer-static-matrix.json` и быстрый gate `npm run verify:ai-media:fba` фиксируют `MediaAssetReadPort` dependency, adapter source markers, fallback profile и degraded mode без повышения до `boundary_ready` до runtime execution/fallback smoke.
