...
| `order` | provider checkout completion/result для commerce orchestration | `boundary_ready` | Owner checkout-completion path has `runtime_evidence.checkout_completion_owner_path.status = "runtime_verified"` and is gated by `npm run verify:ecommerce:fba`; remote/base fallback smoke remains follow-up before `transport_verified` | `crates/rustok-order/src/ports.rs`, `crates/rustok-order/contracts/order-fba-registry.json`, `crates/rustok-order/docs/implementation-plan.md` |
| `payment` | provider payment collection create/reuse для commerce checkout | `boundary_ready` | Provider SPI live-adapter executed evidence уже зафиксирован (`payment-provider-spi-live-adapter-evidence.json`) и gated by `npm run verify:ecommerce:fba`; base `PaymentCollectionPort` runtime fallback остаётся follow-up перед `transport_verified` | `crates/rustok-payment/src/ports.rs`, `crates/rustok-payment/contracts/payment-fba-registry.json`, `crates/rustok-payment/docs/implementation-plan.md` |
...
| `media` | provider asset read/image descriptor boundary для SEO/AI media consumers | `in_progress` | Запустить executable runtime contract tests/fallback smoke для `MediaAssetReadPort`; source-locked degraded modes and public URL proxy policy are fixed in `media-runtime-fallback-smoke.json`, но статус не повышается без runtime execution | `crates/rustok-media/src/ports.rs`, `crates/rustok-media/contracts/media-fba-registry.json`, `crates/rustok-media/docs/implementation-plan.md` |
| `seo` | consumer image descriptor boundary от `media` | `in_progress` | Закрыть runtime consumer contract execution/fallback smoke для `seo_image_descriptor`; static matrix уже `source_locked_pending_consumer_runtime` и подтверждает provider fallback-smoke source/degraded modes `omit_image_metadata`/`keep_existing_seo_image`/`proxy_storage_relative_url`, но не повышает статус без runtime evidence | `crates/rustok-seo/contracts/seo-fba-registry.json`, `crates/rustok-seo/contracts/evidence/seo-media-consumer-static-matrix.json`, `crates/rustok-seo/docs/implementation-plan.md` |
| `ai-media` | support consumer image asset descriptor boundary от `media` | `boundary_ready` | Следующий шаг: provider-side media runtime execution evidence; support-adapter fallback smoke already verified by `cargo test -p rustok-ai-media --lib` | `crates/rustok-ai-media/contracts/ai-media-fba-registry.json`, `crates/rustok-ai-media/contracts/evidence/ai-media-consumer-static-matrix.json`, `crates/rustok-ai-media/contracts/evidence/ai-media-runtime-fallback-smoke.json`, `crates/rustok-ai-media/docs/implementation-plan.md` |
| `ai-alloy` | support adapter script execution policy boundary для Alloy vertical | `in_progress` | Source-level policy registry фиксирует `alloy_script_execution_policy`, `allowed_operations` и descriptor `runtime_operation`; следующий шаг — targeted Rust tests при разрешённых компиляциях | `crates/rustok-ai-alloy/contracts/ai-alloy-policy-registry.json`, `crates/rustok-ai-alloy/contracts/evidence/ai-alloy-policy-static-matrix.json`, `crates/rustok-ai-alloy/docs/implementation-plan.md` |
| `search` | provider search query/suggestions boundary для storefront/admin consumers | `boundary_ready` | Executable no-compile runtime fallback/contract/invocation evidence уже зафиксирован; следующий шаг перед `transport_verified` — live runtime contract execution с реальным provider invocation | `crates/rustok-search/src/ports.rs`, `crates/rustok-search/contracts/search-fba-registry.json`, `crates/rustok-search/docs/implementation-plan.md` |
| `fulfillment` | provider seller-aware shipping selection для commerce checkout | `boundary_ready` | Provider SPI live-adapter executed evidence уже зафиксирован (`fulfillment-provider-spi-live-adapter-evidence.json`) и gated by `npm run verify:ecommerce:fba`; base `ShippingSelectionPort` runtime fallback остаётся follow-up перед `transport_verified` | `crates/rustok-fulfillment/src/ports.rs`, `crates/rustok-fulfillment/contracts/fulfillment-fba-registry.json`, `crates/rustok-fulfillment/docs/implementation-plan.md` |
...
На 2026-06-29 `payment` и `fulfillment` подняты до `boundary_ready` по provider SPI live-adapter executed evidence: `crates/rustok-payment/contracts/evidence/payment-provider-spi-live-adapter-evidence.json` и `crates/rustok-fulfillment/contracts/evidence/fulfillment-provider-spi-live-adapter-evidence.json` фиксируют concrete external adapter contract execution для guarded invocation, typed provider-error mapping, degraded fallback, unavailable-mode blocking и webhook/tracking replay delegation. Base port contract/fallback smoke для `PaymentCollectionPort` и `ShippingSelectionPort` остаётся follow-up перед `transport_verified`.
...
На 2026-06-29 `order` поднят до `boundary_ready`: `crates/rustok-order/contracts/order-fba-registry.json` содержит `runtime_evidence.checkout_completion_owner_path.status = "runtime_verified"`, а `npm run verify:ecommerce:fba` проверяет owner `OrderService` implementation для write/read policy, lifecycle delegation, locale-aware snapshot reload и typed unavailable result projection. Remote/base fallback smoke остаётся условием перед `transport_verified`.
...
На 2026-06-30 пакетная commerce-domain волна добавила no-compile runtime-order evidence для `product`, `pricing`, `inventory`, `customer`, `cart` и `tax`: module-owned `contracts/evidence/*-runtime-contract-smoke.json` проверяются общим `scripts/verify/verify-commerce-domain-fba-runtime-smoke.mjs` с fixture regressions. Gate фиксирует порядок shared policy/idempotency -> owner provider -> typed error mapping и parity fallback/degraded metadata, но намеренно не повышает модули выше `in_progress` до live provider execution.

На 2026-06-30 owner-provider волна добавила единый no-compile runtime-order gate для `comments`, `rbac`, `workflow`, `region`, `media` и `outbox`: `contracts/evidence/*-provider-runtime-order-smoke.json` и `scripts/verify/verify-owner-fba-runtime-order.mjs` фиксируют shared policy, idempotency, tenant/request validation, owner invocation/evaluation и typed error mapping. `comments` использует canonical `PortCallPolicy::write()`, который уже применяет write semantics внутри shared primitive. На 2026-07-01 `outbox` registry/manifest переведены на единственный `rustok_api::ports` contract, а outbox-specific Loco adapter перенесён в owner crate, что устранило обратную runtime-зависимость из `rustok-api`. Статусы не повышаются без live execution.

На 2026-06-30 orchestrator/capability волна добавила единый no-compile runtime-order gate для `ai` и `page_builder`: `contracts/evidence/*-orchestrator-runtime-order-smoke.json` и `scripts/verify/verify-orchestrator-fba-runtime-order.mjs` фиксируют AI support-adapter registry parity, direct runtime registration APIs, router fallback diagnostics markers, native/GraphQL admin transport boundary, а также page-builder capability flag -> `PortCallPolicy` -> owner service call и authorization -> service call order. Статусы остаются `in_progress` до live runtime/tenant evidence.

На 2026-06-30 consumer волна добавила единый no-compile runtime-order gate для `blog` и `seo`: `contracts/evidence/*-consumer-runtime-order-smoke.json` и `scripts/verify/verify-consumer-fba-runtime-order.mjs` фиксируют blog -> comments target validation/provider-call/typed mapping order, SEO -> media read-policy/tenant parse/descriptor construction order, независимый `SeoTargetImageRecord` mapping и fallback degraded metadata parity. Статусы остаются `in_progress` до live consumer runtime execution.
...
На 2026-06-20 `ai-media` имеет support-consumer track для `media.asset_read.v1`: registry `crates/rustok-ai-media/contracts/ai-media-fba-registry.json`, static evidence `crates/rustok-ai-media/contracts/evidence/ai-media-consumer-static-matrix.json`, runtime fallback smoke `crates/rustok-ai-media/contracts/evidence/ai-media-runtime-fallback-smoke.json` и gate `npm run verify:ai-media:fba` фиксируют `MediaAssetReadPort` dependency, adapter source markers, fallback profile и degraded mode; на 2026-06-29 support-adapter smoke подтверждён `cargo test -p rustok-ai-media --lib` и статус поднят до `boundary_ready`. `ai-alloy` добавлен как source-level policy track: `crates/rustok-ai-alloy/contracts/ai-alloy-policy-registry.json` и `alloy_script_execution_policy` фиксируют ownership of script runtime payload policy, allowed operations and descriptor runtime operation while runtime composition stays in `rustok-ai`.
...
На 2026-06-20 `tenant` добавлен как provider track для tenant read-projection boundary: `TenantReadPort`/`tenant.read_projection.v1`, registry `crates/rustok-tenant/contracts/tenant-fba-registry.json` и evidence `crates/rustok-tenant/contracts/evidence/tenant-contract-test-static-matrix.json` проверяются gate `npm run verify:tenant:fba`; на 2026-06-29 runtime contract/fallback smoke выполнен через `cargo test -p rustok-tenant --lib --tests`, evidence помечен `runtime_verified`, и статус поднят до `boundary_ready`; текущий no-compile пакет `scripts/verify/verify-foundation-fba-runtime-smoke.mjs` добавляет source-locked fallback smoke `crates/rustok-tenant/contracts/evidence/tenant-runtime-fallback-smoke.json` без запуска компиляции.
...
На 2026-06-20 `email` добавлен как provider track для transactional delivery boundary: `EmailDeliveryPort`/`email.delivery.v1`, registry `crates/rustok-email/contracts/email-fba-registry.json` и evidence `crates/rustok-email/contracts/evidence/email-contract-test-static-matrix.json` проверяются gate `npm run verify:email:fba`; на 2026-06-29 targeted delivery-port tests выполнены через `cargo test -p rustok-email --lib`, disabled-provider noop fallback помечен `runtime_verified`, и статус поднят до `boundary_ready`; текущий no-compile пакет `scripts/verify/verify-foundation-fba-runtime-smoke.mjs` добавляет source-locked fallback smoke `crates/rustok-email/contracts/evidence/email-runtime-fallback-smoke.json` для disabled noop/SMTP delivery profiles без запуска компиляции.
