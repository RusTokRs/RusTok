...
The provider implementation is now centered on Rig 0.39 rather than a local provider abstraction. `ProviderSlug` and the registry preserve the product-level capability matrix while `InferenceEngine` and `RigAgentDriver` own provider dispatch, streaming, structured generation, image generation, embeddings and reranking. Policy remains a RusToK responsibility: routing, tenancy, approvals, RBAC and domain operations are not delegated to Rig.
