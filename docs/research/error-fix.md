...
- The next small Wave 5 reserve-transport slice extracted reservation write path into inventory-owned native/API facade: `InventoryService::reserve` returns typed `InventoryReservationWriteResult { reserved_quantity, available_quantity, in_stock }`, endpoint `inventory/variant/reserve-quantity` passes tenant/permission checks without GraphQL selected path, and backend/admin serde evidence captures reservation wire shape.
...
| VF-06 | Inventory admin currently reads through commerce GraphQL read-side | ✅ resolved in current branch | `crates/modules/rustok-inventory/admin` now uses native `AdminInventoryReadService` facade without `src/transport.rs`, `rustok-graphql`, token/tenant-slug fallback inputs and `CommerceGraphqlInventoryReadAdapter`; boundary/verifier checks forbid reintroduction of GraphQL selected-path markers. | Inventory admin read-side no longer describes commerce GraphQL as a contract; remaining Wave 5 tail — non-admin/umbrella mutation parity beyond current native set/adjust/reserve/release/check-availability flows. |
| VF-07 | Migration aggregation is centralized in the platform migrator | ✅ confirmed | `crates/utils/rustok-migrations/src/lib.rs` manually `extend`s migrations from domain crates. | Reduce the manual hotspot through module-owned descriptors/exporters and smoke order checking. |
...
8. Update `crates/modules/rustok-inventory/admin/README.md` and module docs.
