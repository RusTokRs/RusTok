...
- Wave 5 admin scope доведён до native-only read/write boundary: inventory admin read-side остаётся на inventory-owned `core` + facade `api` + dedicated `native.rs` server functions + explicit `ui/leptos.rs` adapter, а текущие native endpoints `inventory/variant/set-quantity`, `inventory/variant/adjust-quantity`, `inventory/variant/reserve-quantity`, `inventory/variant/release-reservation` и `inventory/variant/check-availability` идут через inventory-owned API/native facade без GraphQL fallback.
- Открытым остатком Wave 4 остаётся наблюдать первый CI `migration-smoke` прогон на реальном GitHub Actions окружении и исправить возможные environment-specific failures; текущий Wave 5 admin scope закрыт для существующих UI stock operations, а дальнейшая работа уходит в non-admin/channel-aware availability semantics и CI evidence.
- Следующий малый Wave 5 ownership slice вынес storefront product inventory projection в inventory-owned public-channel facade: `rustok-inventory` теперь возвращает `PublicChannelInventoryProjection { available_quantity, in_stock }`, а commerce storefront DTO adapter только переносит готовую projection в `ProductResponse`, не дублируя backorder policy branching и не вызывая availability loaders напрямую; быстрый verifier закрепляет этот boundary без долгой компиляции. Follow-up добавил pure projection-map regression test для missing inventory levels + backorder `continue` semantics, чтобы bulk storefront projection не расходилась с single-variant availability facade.
- Завершающий admin-copy cleanup slice убрал stale copy про оставшиеся inventory admin mutations из Leptos fallback strings и locale bundles: текущие set/adjust/reserve/release/check-availability UI operations описаны как native inventory facade, а будущие операции должны добавляться только через module-owned API/native boundary. Следующий verification-evidence slice расширил быстрый inventory boundary verifier на implementation-plan evidence и закрыл локальный Quality backlog по тестовому покрытию/docs для текущего Wave 5 состояния.
...
| VF-06 | Inventory admin сейчас читает commerce GraphQL read-side | ✅ устранено в текущей ветке | `crates/rustok-inventory/admin` теперь использует native `AdminInventoryReadService` facade без `src/transport.rs`, `leptos-graphql`, token/tenant-slug fallback inputs и `CommerceGraphqlInventoryReadAdapter`; boundary/verifier checks запрещают возврат GraphQL fallback markers, а текущие UI stock operations идут через native/API set/adjust/reserve/release/check-availability facade. | Inventory admin read/write surface больше не описывает commerce GraphQL как contract; дальнейший хвост относится к non-admin/channel-aware availability semantics и CI evidence. |
...
**Статус на 2026-06-07:** inventory admin package переведён на native-only read/write boundary: `AdminInventoryReadService`/DTO остаётся inventory-owned backend read-side source, `admin/src/native.rs` содержит dedicated server-function read endpoints и write/validation endpoints `inventory/variant/set-quantity`, `inventory/variant/adjust-quantity`, `inventory/variant/reserve-quantity`, `inventory/variant/release-reservation`, `inventory/variant/check-availability`, а `admin/src/api.rs`/`admin/src/ui/leptos.rs` используют только module-owned facade. Transitional commerce GraphQL fallback удалён из package (`src/transport.rs`, `leptos-graphql`, token/tenant-slug fallback inputs, `CommerceGraphqlInventoryReadAdapter` и GraphQL runtime markers отсутствуют), а boundary tests/verifier запрещают его возврат. Set/adjust/reserve/release/check-availability возвращают typed results (`InventoryQuantityWriteResult`, `InventoryReservationWriteResult`, `InventoryReservationReleaseWriteResult`, `InventoryAvailabilityCheckResult`), сохраняют reservation-aware available quantity semantics, non-negative request guardrails, policy-aware `in_stock` для `continue`/`CONTINUE`, no-create failed release guardrail и tracked reservation item release guardrail. Public-channel storefront/checkout compatibility тоже потребляет inventory-owned availability/projection facades: commerce callers не вызывают channel-visible inventory loaders и не дублируют backorder branching. Текущий Wave 5 admin scope закрыт для существующих UI stock operations; будущие inventory admin operations должны добавляться только через inventory-owned API/native facade с targeted parity tests. Оставшийся хвост вне admin scope — развивать stock locations/reservations/channel-aware availability semantics и наблюдать CI/migration smoke environment-specific failures.
